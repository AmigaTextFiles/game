...
static GAMESTATE *init_all(SDL_Surface *screen)
{
	...
	i = gs->lifepig = pig_sprites(gs->pe, "gfx/lifepig.png", 0, 0);
	i |= gs->scorefont = pig_sprites(gs->pe, "gfx/font.png", 44, 56);
	i |= gs->glassfont = pig_sprites(gs->pe, "gfx/glassfont.png", 60, 60);
	i |= gs->icons = pig_sprites(gs->pe, "gfx/icons.png", 48, 48);
	i |= gs->stars = pig_sprites(gs->pe, "gfx/stars.png", 32, 32);
	i |= gs->pigframes = pig_sprites(gs->pe, "gfx/pigframes.png", 64, 48);
	i |= gs->evil = pig_sprites(gs->pe, "gfx/evil.png", 48, 48);
	i |= gs->slime = pig_sprites(gs->pe, "gfx/slime.png", 48, 48);
#else
...
#if defined(__MORPHOS__)
	if(pig_map_tiles(pm, "gfx/tiles.png", TILE_W, TILE_H) < 0)
#else
