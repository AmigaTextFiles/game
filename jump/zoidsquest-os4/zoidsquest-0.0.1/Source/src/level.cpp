...
void Level::FindCollisions(int j){
  double dx, dy;

  levelobjects[j]->SaveOldPosition();  
  levelobjects[j]->GetVelocity(&dx, &dy);
  dx = dx / Detect_Div;
  dy = dy / Detect_Div;

  if(fabs(dx) > 0.0001){
    levelobjects[j]->ChangePosition(dx, 0);
    if(Terrain.CheckObjectSpace(levelobjects[j], Check_Horizontal) == True || FindObjectCollisions(levelobjects[j], j, Check_Horizontal))
      levelobjects[j]->ChangePosition(-dx, 0);
  }
  if(fabs(dy) > 0.0001){
    levelobjects[j]->ChangePosition(0, dy);
    if(Terrain.CheckObjectSpace(levelobjects[j], Check_Vertical) == True || FindObjectCollisions(levelobjects[j], j, Check_Vertical))
      levelobjects[j]->ChangePosition(0, -dy);
  }
  if(levelobjects[j]->Properties.HasProperty("magic")){
    vector<RuneObject *> runes = ((MagicObject *)levelobjects[j])->Magic.GetUsedRunes();
    for(unsigned int i = 0; i < runes.size(); i++){
      FindObjectCollisions(runes[i]);
    }
  }
  levelobjects[j]->RestoreOldPosition();
}


int Level::FindObjectCollisions(BaseObject *object_to_check, unsigned int offset, int mode){
  //BaseObject *object_to_check = levelobjects[offset];
