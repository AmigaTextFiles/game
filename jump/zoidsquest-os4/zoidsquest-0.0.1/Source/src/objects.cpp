...
void BaseObject::SolidHandler(struct CollisionInfo *collision_info){
  if(collision_info->side == BottomSide || collision_info->side == TopSide){
    y = collision_info->bcy;
    vy = 0;
  }
  else if(collision_info->side == RightSide || collision_info->side == LeftSide){
    x = collision_info->bcx;
    vx = 0;
  }
}
...
void BaseObject::UseTemplate(BaseObject *object_template){
  ...
  for(unsigned int i = 0; i < object_template->Properties.Number(); i++)
    Properties.SetProperty(object_template->Properties[i]);
...
void PlayerObject::PreUpdate(){
  ...
  if(invulnerable == 0){
    Properties.DeleteProperty("invulnerable");
  }
  SpriteAnimation.Update();
...
void PlayerObject::SolidHandler(struct CollisionInfo *collision_info){
  double xtmp, ytmp;
  CommonSolidHandler(collision_info);
  ...
  if(collision_info->side == BottomSide){
    ...
    i_can_jump = True;
    if(Properties.HasProperty("stoppable") && collision_info->Object->Properties.HasProperty("unstoppable")){
      collision_info->Object->GetVelocity(&xtmp, &ytmp);
      if(ytmp * oldvy <= 0) oldvy = ytmp;
      else if(fabs(ytmp) >= fabs(oldvy)) oldvy = ytmp;
      else oldvy = 0;
      if(collision_info->Object->Properties.HasProperty("sticky")) ivx = xtmp;
    }
    else oldvy = 0;
  }
    ...
    if(Properties.HasProperty("stoppable") && collision_info->Object->Properties.HasProperty("unstoppable")){
      collision_info->Object->GetVelocity(&xtmp, &ytmp);
      if(xtmp * oldvx <= 0) oldvx = xtmp;
      else if(fabs(xtmp) >= fabs(oldvx)) oldvx = xtmp;
      else oldvx = 0;
    }
    else oldvx = 0;
  }
...
void MovingObject::Update(){
  ...
  //velocity modification stuff must go here
...
void MovingGroundObject::SolidHandler(struct CollisionInfo *collision_info){
  ...
  if(collision_info->side == RightSide){
    hit_right = True;
    if(!(Properties.HasProperty("unstoppable") && collision_info->Object->Properties.HasProperty("stoppable"))){
...
void FallingBlock::SolidHandler(struct CollisionInfo *collision_info){
  ...
  else if(collision_info->side == BottomSide){
    if(oldvy > 0) oldvy = 0;
  }
}
...
int BaseObject::CommonSolidHandler(struct CollisionInfo *collision_info){
  switch(collision_info->side){
    case BottomSide:
      if(vy > 0){
        if(collision_info->was_i_moving == True) y = collision_info->bcy;
        else y = collision_info->cy;
        vy = 0;
      }
      return BottomSide;
    ...
    case RightSide:
      if(vx > 0){
        if(collision_info->was_i_moving == True) x = collision_info->bcx;
        else x = collision_info->cx;
        vx = 0;
      }
      return RightSide;
