...
int GameWrapper::LoadGlobalTemplates(char *gamefile){
  ...
  while(GlobalParser.FindElement("template") == Ok){
    ...
    if(GlobalParser.FindElement("boundary") == Error){
...
int GameWrapper::UpdateMenu(){
  ...
  if(change_key != NULL){
    ...
    if(UserInput.IsKeyDown(SDLK_SPACE) == True) Menu.CurrentPage()->ExecuteEntry();
    if(UserInput.IsKeyDown('w') == True) Menu.CurrentPage()->LastEntry();
    if(UserInput.IsKeyDown('s') == True) Menu.CurrentPage()->NextEntry();
  }
