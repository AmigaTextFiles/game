...
//		glAreTexturesResident(nparts,tex,res);
		for(i=0;i<nparts && !reload && textures_loaded==reload_textures;i++) {
...
#endif
			if (res==GL_FALSE) {
//				printf("Texture not loaded: %i, tex %i, res %i (GL_TRUE = %i, GL_FALSE = %i)\n",i,tex[i],res,GL_TRUE,GL_FALSE);
				reload=true;
#ifdef __DEBUG_MESSAGES
				output_debug_message("Out of texture memory! reloading textures...\n");
#endif
			} // if
		} // for

		if (reload || textures_loaded!=reload_textures) load_textures();
	}
...
{
	if (textures_loaded!=reload_textures) load_textures();
	return tex[i];
} /* GLTile::get_texture */ 
