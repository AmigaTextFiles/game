...
{
	m_class=new Symbol("GO_exitdoor");

	m_state_cycle=0;
} /* GO_exitdoor::GO_exitdoor */ 


bool GO_exitdoor::cycle(VirtualController *k,GMap *map,int layer,GLTManager *GLTM,SFXManager *SFXM)
{
	if (m_state==0) {
		GO_character *c=(GO_character *)map->get_object("GO_character");
		if (c->get_rescued_goonies()==7) m_state=1;
	} else {		
	} // if 
...
{
	if (m_state==0) m_last_tile_used=GLTM->get("ob_exitdoor");
			   else m_last_tile_used=GLTM->get("ob_entrydoor");
...
{
	if (c->cmp("GO_exitdoor")) return true;
