...
{
	m_class=new Symbol("GO_closingwall");
...
bool GO_closingwall::cycle(VirtualController *k,GMap *map,int layer,GLTManager *GLTM,SFXManager *SFXM)
{
	...
	case 1: m_state_cycle++;
			...
			case 0:
				...
				if (m_state_cycle % 20 == 0) {
					Sound_play(SFXM->get("sfx/closingwall"),m_sfx_volume);					
				}
				break;
			case 1:
				if (m_state_cycle==20) {
			...
			if (m_state_cycle==100) return false;
			break;
	} // switch
...
{
	if (c->cmp("GO_closingwall")) return true;
