...
void font_extract(char *name,char *graphic_file,int nc,char *characters)
{
	...
	if (sfc!=0) {
		...
		for(i=0;i<sfc->h*sfc->w;i++) visited[i]=false;
					...
					while(!stack.EmptyP()) {
						...
						for(k=0;k<4;k++) {
							...
							if (x>=0 && y>=0 && x<sfc->w && y<sfc->h &&
								...
								!visited[offs2] && !stack.MemberP(&offs2)) stack.Add(new int(offs2));
						} // for
						visited[*offs]=true;
						delete offs;
					} // while 
...
//		assert(n==nc);
		...
		// according to their position in the image:
		{
			...
			do{
				...
				for(i=0;i<n-1;i++) {
					...
					if (*(l_maxy[i])>*(l_miny[i+1]) && *(l_minx[i])>*(l_maxx[i+1])) change=true;
		...
		// Automatically identify the rows of characters:
		{
			...
			for(i=0;i<n;i++) {
				if (first) {
					...
					if (*(l_miny[i])<prev_y2 && *(l_maxy[i])>prev_y1) {
						if (*(l_miny[i])<prev_y1) prev_y1=*(l_miny[i]);
						if (*(l_maxy[i])<prev_y2) prev_y2=*(l_maxy[i]);
					} else {
						// end of row:
		...
		// Create a vector of 256 empty characters, and then fill it with the characters found in the image:
		{
			...
			l2=new List<GLTile>;
			for(i=0;i<256;i++) l2->Add(0);

			for(i=0;i<n;i++) l2->SetObj(characters[i],l[i]);
			l.ExtractAll();
...
void font_print(char *prefix,char *text,int x_offset)
{
	int k=font_name.Position(prefix);
	List<GLTile> *l=font_characters[k];
	GLTile *t;
...
void font_print_c(char *prefix,char *text,int x_offset)
{
	int k=font_name.Position(prefix);
	List<GLTile> *l=font_characters[k];
	GLTile *t;
...
void font_print_c(int x,int y,int z,float angle,float scale,char *font,char *text,int x_offset)
{
	...
	font_print_c(font,text,x_offset);
