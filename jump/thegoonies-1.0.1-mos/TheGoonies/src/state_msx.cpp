...
void TheGooniesApp::msx_draw(SFXManager *SFXM)
{
	...
	if (m_state_cycle>100) {
		if (m_state_cycle<150) {
			...
			// MSX sign appearing:
			m_GLTM->get("msx")->draw(110,274-160*((m_state_cycle-100)/50.0F),0,0,1);
			...
			m_GLTM->get("msx")->draw(110,114,0,0,1);	
		} // if  
		if (m_state_cycle>150) {
			if (m_state_cycle<150+25) {
				...
				// MSX sign appearing:
				m_GLTM->get("msx-text1")->draw(160,282,0,0,1);
				if (!scissor_state) glDisable(GL_SCISSOR_TEST);
				if (scissor_state) glScissor(scissor_window[0],scissor_window[1],scissor_window[2],scissor_window[3]);
			} else {
				m_GLTM->get("msx-text1")->draw(160,282,0,0,1);
		...
		if (m_state_cycle>163) {
			if (m_state_cycle<163+25) {
				...
				// MSX sign appearing:
				m_GLTM->get("msx-text2")->draw(160,302,0,0,1);
				if (!scissor_state) glDisable(GL_SCISSOR_TEST);
				if (scissor_state) glScissor(scissor_window[0],scissor_window[1],scissor_window[2],scissor_window[3]);
			} else {
				m_GLTM->get("msx-text2")->draw(160,302,0,0,1);
	...
	if (m_state_cycle == 350) {
		Sound_play(SFXM->get("sfx/msxbeep"),m_sfx_volume);
	}
