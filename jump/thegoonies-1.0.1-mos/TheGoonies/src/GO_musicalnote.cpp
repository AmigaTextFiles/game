...
{
	m_class=new Symbol("GO_musicalnote");
...
bool GO_musicalnote::cycle(VirtualController *k,GMap *map,int layer,GLTManager *GLTM,SFXManager *SFXM)
{
	m_x+=m_speedx;
	m_y+=m_speedy;
	m_state_cycle++;
	...
	if (m_x<0 || m_x>map->get_dx() ||
		m_y<0 || m_y>map->get_dy()) return false;
...
{
	m_last_tile_used=GLTM->get("ob_musicalnote");
...
{
	if (c->cmp("GO_musicalnote")) return true;
