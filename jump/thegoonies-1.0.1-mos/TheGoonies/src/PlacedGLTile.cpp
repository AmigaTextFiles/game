...
{
	if (different_color) tile->draw(r,g,b,a,x,y,z,angle,zoom);
					else tile->draw(x,y,z,angle,zoom);
} /* CPlacedGLTile::draw */ 
...
{
	if (tile!=0) {
		if (different_color) tile->draw(r,g,b,a,x+dx,y+dy,z+dz,angle+pangle,zoom*pzoom);
						else tile->draw(x+dx,y+dy,z+dz,angle+pangle,zoom*pzoom);
	} /* if */ 
} /* CPlacedGLTile::draw */ 
