...
bool GO_bullet::cycle(VirtualController *k,GMap *map,int layer,GLTManager *GLTM,SFXManager *SFXM)
{
	if (m_facing==0) m_x-=BULLET_SPEED;
			    else m_x+=BULLET_SPEED;
	m_state_cycle++;
...
{
	if (m_facing==0) m_last_tile_used=GLTM->get("ob_bullet-l");
				else m_last_tile_used=GLTM->get("ob_bullet-r");
...
{
	if (c->cmp("GO_bullet")) return true;
