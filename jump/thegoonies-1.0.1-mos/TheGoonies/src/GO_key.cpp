...
{
	m_class=new Symbol("GO_key");

	m_state_cycle=0;
} /* GObject::GObject */ 


bool GO_key::cycle(VirtualController *k,GMap *map,int layer,GLTManager *GLTM,SFXManager *SFXM)
{
	m_state_cycle++;
	if (m_state==1) {
		m_state=2;
...
void GO_key::draw(GLTManager *GLTM)
{
	...
	if (f>1) f=1;
	if (f<0) f=0;
	m_last_tile_used=GLTM->get("ob_key");
