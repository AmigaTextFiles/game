...
//	m_state=THEGOONIES_STATE_ENDSEQUENCE;
	m_state=THEGOONIES_STATE_SPLASH;
//	m_state=THEGOONIES_STATE_MSX;
//	m_state=THEGOONIES_STATE_KONAMI;
//	m_state=THEGOONIES_STATE_TITLE;
//	m_state=THEGOONIES_STATE_GAMESTART;
	m_state_cycle=0;
	m_title_state=0;
	...
	m_title_music_current_volume=0;
...
//	m_music_title=Sound_create_sound("sound/goonies_menu");
...
TheGooniesApp::~TheGooniesApp()
{
//	Sound_delete_sound(m_music_title);
//	Sound_delete_sound(m_sfx_menu_select);
//	Sound_delete_sound(m_sfx_menu_move);
...
bool TheGooniesApp::cycle(KEYBOARDSTATE *k)
{
	...
	switch(m_state) {
	case THEGOONIES_STATE_SPLASH:m_state=splash_cycle(k);
								 break;
	case THEGOONIES_STATE_DISCLAIMER:m_state=disclaimer_cycle(k);
									 break;
	case THEGOONIES_STATE_MSX:m_state=msx_cycle(k);
							  break;
	case THEGOONIES_STATE_KONAMI:m_state=konami_cycle(k);
								 break;
	case THEGOONIES_STATE_TITLE:m_state=title_cycle(k);
								break;
	case THEGOONIES_STATE_TITLEANIMATION:m_state=titleanimation_cycle(k);
								break;
	case THEGOONIES_STATE_GAMESTART:m_state=gamestart_cycle(k);
									break;
	case THEGOONIES_STATE_GAME:m_state=game_cycle(k);
							   break;
	case THEGOONIES_STATE_GAMEOVER:m_state=gameover_cycle(k);
								   break;
	case THEGOONIES_STATE_ENDSEQUENCE:m_state=endsequence_cycle(k);
								      break;
	case THEGOONIES_STATE_INTERLEVEL:m_state=interlevel_cycle(k);
									 break;
	case THEGOONIES_STATE_HOWTOPLAY:m_state=howtoplay_cycle(k);
									break;
	case THEGOONIES_STATE_CREDITS  :m_state=credits_cycle(k);
									break;
	default:return false;
	} /* switch */ 

	if (m_game!=0 && m_game->get_hiscore()>m_hiscore) set_hiscore(m_game->get_hiscore());
...
void TheGooniesApp::draw(int SCREEN_X,int SCREEN_Y)
{
...
*/
	...
	/* 3d view: */ 
//  gluPerspective( 30.0, ratio, 1.0, 2024.0 );
//	gluLookAt(320,800,-150.0F ,320,240,0, 0,-1,0);
	...
	switch(m_state) {
	case THEGOONIES_STATE_SPLASH:splash_draw();
								 break;
	case THEGOONIES_STATE_DISCLAIMER:disclaimer_draw();
									 break;
	case THEGOONIES_STATE_MSX:msx_draw(m_SFXM);
							  break;
	case THEGOONIES_STATE_KONAMI:konami_draw();
								 break;
	case THEGOONIES_STATE_TITLE:title_draw();
								break;
	case THEGOONIES_STATE_TITLEANIMATION:titleanimation_draw();
								break;
	case THEGOONIES_STATE_GAMESTART:gamestart_draw();
									break;
	case THEGOONIES_STATE_GAME:game_draw();
							   break;
	case THEGOONIES_STATE_GAMEOVER:gameover_draw();
								   break;
	case THEGOONIES_STATE_ENDSEQUENCE:endsequence_draw();
								      break;
	case THEGOONIES_STATE_INTERLEVEL:interlevel_draw();
									 break;
	case THEGOONIES_STATE_HOWTOPLAY: howtoplay_draw();
									 break;
	case THEGOONIES_STATE_CREDITS:   credits_draw();
									 break;
	} // switch 
...
void TheGooniesApp::save_configuration(void)
{
	...
	fprintf(fp,"%i\n",difficulty);
	for(i=0;i<7;i++) fprintf(fp,"%i ",m_keys_configuration[i]);
	fprintf(fp,"\n");
	fclose(fp);
} /* TheGooniesApp::save_configuration */ 
...
void TheGooniesApp::load_configuration(void)
{
	...
	if (fp==0) {
		...
		// keyboard
		for(i=0;i<7;i++) fscanf(fp,"%i",&(m_keys_configuration[i]));

		fclose(fp);
	} // if 
} /* TheGooniesApp::load_configuration */ 
