...
bool GO_ghost::cycle(VirtualController *k,GMap *map,int layer,GLTManager *GLTM,SFXManager *SFXM)
{
	switch(m_state) {
	case 0:	// waiting for the player
			{	
				...
				if (layer!=3) {
					if (map!=0)	map->remove_object(this,layer);
					if (map!=0) map->add_object(this,3);
				} // if
				if (c!=0) {
	...
	case 2: // moving
			{
				...
				if (m_radius<60) m_radius+=0.33f;
				{
					GO_character *c=(GO_character *)map->get_object("GO_character");
					if (c!=0) {
						if (c->get_x()-10<m_destination_x) m_destination_x-=0.5;
						if (c->get_x()-10>m_destination_x) m_destination_x+=0.5;
						if (c->get_y()-30<m_destination_y) m_destination_y-=0.5;
						if (c->get_y()-30>m_destination_y) m_destination_y+=0.5;
					} // if
				}
	...
	if (m_hit_timmer>0) m_hit_timmer--;
	return true;
} /* GO_ghost::cycle */ 
...
void GO_ghost::draw(GLTManager *GLTM)
{
	...
	case 2:
			if (s==0) m_last_tile_used=GLTM->get("ob_ghost1");
			     else m_last_tile_used=GLTM->get("ob_ghost2");
			if (m_hit_timmer>0) f=float(0.5F+0.5F*sin(m_hit_timmer*0.6f));
			break;
	case 3: m_last_tile_used=GLTM->get("ob_ghost-death");
			break;
	} // switch 

	if (m_last_tile_used!=0) m_last_tile_used->draw(1,1,1,f,m_x,m_y,0,0,1);
} /* GO_ghost::draw */ 


bool GO_ghost::is_a(Symbol *c)
{
	if (c->cmp("GO_ghost")) return true;
