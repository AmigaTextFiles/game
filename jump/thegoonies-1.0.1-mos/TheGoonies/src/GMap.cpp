...
void GMap::reset(int sfx_volume)
{
	...
	m_layers.Rewind();
	while(m_layers.Iterate(l)) l->reset(sfx_volume);
} /* GMap::reset */ 
...
void GMap::reset_first(int sfx_volume)
{
	...
	m_layers.Rewind();
	while(m_layers.Iterate(l)) l->reset_first(sfx_volume,this);
} /* GMap::reset_first */ 


void GMap::cycle(VirtualController *v,GLTManager *GLTM,SFXManager *SFXM)
{
	int i=0;
	GMapLayer *l;

	if (m_shake_timmer>0) m_shake_timmer--;


//	output_debug_message("map\n");

	// look for bridges and other tiles that have non standard collisions:
	if (!m_searched_for_bridges) search_for_bridges(GLTM);

	m_layers.Rewind();
	while(m_layers.Iterate(l)) l->cycle(v,this,i++,GLTM,SFXM);
} /* GMap::cycle */ 


void GMap::cycle_clock(VirtualController *v,GLTManager *GLTM,SFXManager *SFXM)
{
	int i=0;
	GMapLayer *l;
...
//	output_debug_message("map\n");
	...
	m_layers.Rewind();
	while(m_layers.Iterate(l)) l->cycle_clock(v,this,i++,GLTM,SFXM);
} /* GMap::cycle_clock */ 
		...
		while(m_layers.Iterate(l)) {
			...
			while(l->m_tiles.Iterate(pt)) {
				if (pt->m_tile_name->cmp("gr_tilegreen-bridge-l") ||
					...
					pc=new GMapCMCPlace();
					pc->m_cmc=GLTM->get(pt->m_tile_name)->get_cmc();
					pc->m_x=pt->m_x;
					pc->m_y=pt->m_y;
...
void GMap::draw(GLTManager *GLTM,bool water)
{
	...
	if (water &&
		...
		for(i=0;i<4;i++) m_layers[i]->draw(GLTM);
		get_water_info();
		glClear(GL_COLOR_BUFFER_BIT);
		glViewport(vp[0],vp[1],vp[2],vp[3]);

		m_layers.Rewind();
		while(m_layers.Iterate(l)) l->draw(GLTM);
	} else {
		m_layers.Rewind();
		while(m_layers.Iterate(l)) l->draw(GLTM);
	} // if 
...
{
	if (layer>=0 && layer<m_layers.Length()) m_layers[layer]->draw(GLTM);
} /* GMap::draw */ 


void GMap::add_object(GObject *o,int layer)
{
	if (layer>=0 && layer<m_layers.Length()) m_layers[layer]->add_object(o);
} /* GMap::add_object */ 


void GMap::add_auxiliar_object(GObject *o,int layer)
{
	if (layer>=0 && layer<m_layers.Length()) m_layers[layer]->add_auxiliar_object(o);
} /* GMap::add_auxiliar_object */ 


bool GMap::remove_object(GObject *o,int layer)
{
	if (layer>=0 && layer<m_layers.Length()) return m_layers[layer]->remove_object(o);
	return false;
} /* GMap::add_object */ 
...
bool GMap::collision(GObject *o,GLTManager *GLTM)
{
	...
	if (m_layers[2]->collision(o,GLTM) ||
		m_layers[3]->collision(o,GLTM)) return true;
...
bool GMap::collision_with_background(GObject *o,GLTManager *GLTM)
{
	...
	if (m_layers[2]->collision_with_background(o,GLTM) ||
		m_layers[3]->collision_with_background(o,GLTM)) return true;
...
#endif
		if (o->collision(c->m_cmc,c->m_x,c->m_y)) return true;
	} // while
	...
	return false;
} /* GMap::collision_with_background */ 
...
bool GMap::collision_with_background(GObject *o,int x_offs,int y_offs,GLTManager *GLTM)
{
	...
	if (m_layers[2]->collision_with_background(o,x_offs,y_offs,GLTM) ||
		m_layers[3]->collision_with_background(o,x_offs,y_offs,GLTM)) return true;

	GMapCMCPlace *c;

	m_extra_collisions.Rewind();
	while(m_extra_collisions.Iterate(c)) {
		if (o->collision(c->m_cmc,c->m_x-x_offs,c->m_y-y_offs)) return true;
	} // while
...
bool GMap::collision_with_background(GLTile *t,int x,int y,GLTManager *GLTM)
{
	...
	if (m_layers[2]->collision_with_background(t,x,y,GLTM) ||
		m_layers[3]->collision_with_background(t,x,y,GLTM)) return true;

	GMapCMCPlace *c;
	m_extra_collisions.Rewind();
	while(m_extra_collisions.Iterate(c)) {
		if (t->get_cmc()->collision(float(x),float(y),0,c->m_cmc,float(c->m_x),float(c->m_y),0)) return true;
	} // while
...
bool GMap::collision_with_background_wo_bridges(GObject *o,GLTManager *GLTM)
{
	...
	if (m_layers[2]->collision_with_background_wo_bigrocks(o,0,0,GLTM) ||
		m_layers[3]->collision_with_background_wo_bigrocks(o,0,0,GLTM)) return true;
	return false;
} /* GMap::collision_with_background */ 
...
bool GMap::collision_with_background_wo_bridges(GObject *o,int x_offs,int y_offs,GLTManager *GLTM)
{
	...
	if (m_layers[2]->collision_with_background_wo_bigrocks(o,x_offs,y_offs,GLTM) ||
		m_layers[3]->collision_with_background_wo_bigrocks(o,x_offs,y_offs,GLTM)) return true;
	return false;
} /* GMap::collision_with_background */ 
...
void GMap::pause_continuous_sfx(void)
{
	GMapLayer *l;

	m_layers.Rewind();
	while(m_layers.Iterate(l)) l->pause_continuous_sfx();
} /* GMap::pause_continuous_sfx */ 


void GMap::stop_continuous_sfx(void)
{
	GMapLayer *l;

	m_layers.Rewind();
	while(m_layers.Iterate(l)) l->stop_continuous_sfx();

} /* GMap::stop_continuous_sfx */ 


void GMap::resume_continuous_sfx(void)
{
	GMapLayer *l;

	m_layers.Rewind();
	while(m_layers.Iterate(l)) l->resume_continuous_sfx();
} /* GMap::resume_continuous_sfx */ 
...
#endif
//		glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA8,256,256,0,GL_RGB,GL_UNSIGNED_BYTE,m_water_info);
...
int GMap::get_water_level(void)
{
	if (m_water_level==-1) {
		...
		while(m_layers[4]->m_objects.Iterate(op)) {
			if (op->m_object_name->cmp("GO_water")) {
				if (op->m_y<m_water_level) m_water_level=op->m_y;
			} // if 
		} // while 
	...
	if (m_lava_level==-1) {
		...
		while(m_layers[4]->m_objects.Iterate(op)) {
			if (op->m_object_name->cmp("GO_lava")) {
				if (op->m_y<m_lava_level) m_lava_level=op->m_y;
			} // if 
		} // while 
