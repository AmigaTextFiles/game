...
bool GO_item::cycle(VirtualController *k,GMap *map,int layer,GLTManager *GLTM,SFXManager *SFXM)
{
	m_state_cycle++;
	if (m_state==1) {
		m_state=2;
	...
	if (m_state==2) {
		if (m_state_cycle>8) return false;
	} // if 
	return true;
} /* GO_item::cycle */ 
...
void GO_item::draw(GLTManager *GLTM)
{
	...
	switch(m_type) {
	case 0: if (s == 0) m_last_tile_used=GLTM->get("it_goonie1");
				else m_last_tile_used=GLTM->get("it_goonie2");
			break;
	case 1: if (s == 0) m_last_tile_used=GLTM->get("it_greenbottle1");
			if (s == 1) m_last_tile_used=GLTM->get("it_greenbottle2");
			if (s == 2) m_last_tile_used=GLTM->get("it_greenbottle3");
			break;
	case 2: // m_last_tile_used=GLTM->get("it_yellowhelmet");
			m_last_tile_used=GLTM->get("it_bag");
			break;
	...
	case 6: //m_last_tile_used=GLTM->get("it_hammer");
			m_last_tile_used=GLTM->get("it_bag");
			break;
	case 7: m_last_tile_used=GLTM->get("it_bag");
			break;
	...
	case 15://m_last_tile_used=GLTM->get("it_yellowshield");
			m_last_tile_used=GLTM->get("it_bag");
			break;
	case 16:m_last_tile_used=GLTM->get("it_bag");
			break;
	...
	case 26://m_last_tile_used=GLTM->get("it_blueanti");
			...
			break;
	default:
			m_last_tile_used=GLTM->get("it_goonie");
			break;
	} // switch 
	float f;
