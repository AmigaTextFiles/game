...
void GooniesScript::execute(GO_character *character,GMap *map,GLTManager *GLTM,SFXManager *SFXM,int m_sfx_volume)
{
	...
	while(m_scripts.Iterate(c)) {
		if (!c->execute(character,map,GLTM,SFXM,m_sfx_volume)) to_delete.Add(c);
	} // while 
	...
	case GSCRIPT_ACTION_NEW:
		{
			...
			m_int_parameters.Next();
			while(m_int_parameters.Iterate(i)) l.Add(new int(*i));
...
bool GScriptCondition::execute(GO_character *character,GMap *map,GLTManager *GLTM,SFXManager *SFXM,int m_sfx_volume)
{
	...
	case GSCRIPT_CONDITION_SEQUENCE:
		if (m_timer==0) m_cond_parameters.Rewind();
	...
	case GSCRIPT_CONDITION_AND:
		{
			...
			while(m_cond_parameters.Iterate(c)) if (!c->execute(character,map,GLTM,SFXM,m_sfx_volume)) return false;
			return true;
		}
		break;
	case GSCRIPT_CONDITION_OR:
		{
			GScriptCondition *c;
			m_cond_parameters.Rewind();
			while(m_cond_parameters.Iterate(c)) if (c->execute(character,map,GLTM,SFXM,m_sfx_volume)) return true;
			return false;
		}
		break;
	case GSCRIPT_CONDITION_NOT:
		if (!m_cond_parameters[0]->execute(character,map,GLTM,SFXM,m_sfx_volume)) return true;
		break;
	case GSCRIPT_CONDITION_POSITION:
		if (character->get_x()>=(*(m_int_parameters[0]))*20 &&
			...
			character->get_y()<(*(m_int_parameters[3]))*20) return true;
		break;
	case GSCRIPT_CONDITION_PICKS:
		if (character->get_last_pick()!=0 &&
			character->get_last_pick()->cmp(m_string_parameters[0])) return true;
		break;
	case GSCRIPT_CONDITION_HAS:
		if (character->player_has(m_string_parameters[0])) return true;
		break;
	case GSCRIPT_CONDITION_JUMPS:
		if (character->get_state()!=character->get_last_state() &&
			(character->get_state()==CSTATE_JUMPING_LEFT ||
			 ...
			 character->get_state()==CSTATE_JUMPING_RIGHT_RIGHT)) return true;
		break;
		...
		if (character->get_state()!=character->get_last_state() &&
			...
			if (m_string_parameters.EmptyP()) {
				...
				if (character->get_last_hit()!=0 &&
					character->get_last_hit()->cmp(m_string_parameters[0])) return true;
			} // if 
		} // if 
		break;
	case GSCRIPT_CONDITION_SAVEDGOONIES:
		if (character->get_rescued_goonies()==7) return true;
		break;
	...
	case GSCRIPT_CONDITION_HITBY:
		if (character->get_last_hit_by()!=0 &&
			character->get_last_hit_by()->cmp(m_string_parameters[0])) return true;
		break;
	case GSCRIPT_CONDITION_CAMEFROM:
		if (character->get_camefrom()!=0 &&
			character->get_camefrom()->cmp(m_string_parameters[0])) return true;
		break;
	case GSCRIPT_CONDITION_EXISTS:
		if (map->get_object(m_string_parameters[0])!=0) return true;
		break;
	} // switch 
	return false;
	...
	if (token_type==TOKEN_LEFT_PAR) {
		...
		if (token_type==TOKEN_SYMBOL) {
			if (strcmp(token,"and")==0 ||
				...
				if (strcmp(token,"and")==0) type=GSCRIPT_CONDITION_AND;
				if (strcmp(token,"or")==0) type=GSCRIPT_CONDITION_OR;
				if (strcmp(token,"not")==0) type=GSCRIPT_CONDITION_NOT; 
				if (strcmp(token,"sequence")==0) type=GSCRIPT_CONDITION_SEQUENCE;
				...
				if (type!=CSCRIPT_WRONG) {
					...
					if (token_type==TOKEN_LEFT_PAR) {
						...
						if (token_type==TOKEN_RIGHT_PAR) {
							...
							c->m_type=type;
							c->m_timer=0;
							while(!m_cond.EmptyP()) c->m_cond_parameters.Add(m_cond.ExtractIni());
							return c;
						} // if 
					} // if
				} // if 
				...
				int type=CSCRIPT_WRONG;
				if (strcmp(token,"position")==0) type=GSCRIPT_CONDITION_POSITION;
				if (strcmp(token,"picks")==0) type=GSCRIPT_CONDITION_PICKS;
				if (strcmp(token,"has")==0) type=GSCRIPT_CONDITION_HAS;
				if (strcmp(token,"jumps")==0) type=GSCRIPT_CONDITION_JUMPS;
				if (strcmp(token,"punches")==0) type=GSCRIPT_CONDITION_PUNCHES;
				if (strcmp(token,"allgooniessaved")==0) type=GSCRIPT_CONDITION_SAVEDGOONIES;
				if (strcmp(token,"allenemieskilled")==0) type=GSCRIPT_CONDITION_KILLEDENEMIES;
				if (strcmp(token,"hitby")==0) type=GSCRIPT_CONDITION_HITBY;
				if (strcmp(token,"camefrom")==0) type=GSCRIPT_CONDITION_CAMEFROM;
				if (strcmp(token,"exists")==0) type=GSCRIPT_CONDITION_EXISTS;
				if (strcmp(token,"hit")==0) type=GSCRIPT_CONDITION_HIT;
				...
				if (type!=CSCRIPT_WRONG) {
					...
					if (token_type==TOKEN_LEFT_PAR) {
						...
						if (token_type==TOKEN_RIGHT_PAR) {
							...
							c->m_type=type;
							c->m_timer=0;
							while(!m_int.EmptyP()) c->m_int_parameters.Add(m_int.ExtractIni());
							while(!m_string.EmptyP()) c->m_string_parameters.Add(m_string.ExtractIni());
							return c;
						} // if 
					} // if 
				} // if
	...
	if (strcmp(token,"new")==0) {
		...
		if (token_type==TOKEN_LEFT_PAR) {
			...
			if (token_type==TOKEN_RIGHT_PAR) {
				...
				while(!m_int.EmptyP()) a->m_int_parameters.Add(m_int.ExtractIni());
				while(!m_string.EmptyP()) a->m_string_parameters.Add(m_string.ExtractIni());
				return a;
			} // if 
		} // if
	} // if 
