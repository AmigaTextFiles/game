...
{
	m_class=new Symbol("GO_entrydoor");

	m_state_cycle=0;
} /* GObject::GObject */ 


bool GO_entrydoor::cycle(VirtualController *k,GMap *map,int layer,GLTManager *GLTM,SFXManager *SFXM)
{
	m_state_cycle++;

	if (m_state_cycle>64) return false;
	return true;
} /* GO_entrydoor::cycle */ 
...
void GO_entrydoor::draw(GLTManager *GLTM)
{
	...
	if (f<0) f=0;
	if (f>1) f=1;
	m_last_tile_used=GLTM->get("ob_entrydoor");
...
{
	if (c->cmp("GO_entrydoor")) return true;
