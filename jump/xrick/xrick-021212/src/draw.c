...
#ifdef ENABLE_CHEATS
#define LOOP(N, C0, C1) d = sprites_data[number][g + N]; for (c = C0; c >= C1; c--, i--, d >>= 4, im--) { if (im == 0) { flg = map_eflg[map_map[(y + r) >> 3][(x + c) >> 3]]; im = 8;}if (c >= w || x + c < x0) continue; if (!front && !game_cheat3 && (flg & MAP_EFLG_FGND)) continue; if (d & 0x0F) fb[i] = (fb[i] & 0xF0) | (d & 0x0F);if (game_cheat3) fb[i] |= 0x10; }
#else
#define LOOP(N, C0, C1) d = sprites_data[number][g + N]; for (c = C0; c >= C1; c--, i--, d >>= 4, im--) { if (im == 0) { flg = map_eflg[map_map[(y + r) >> 3][(x + c) >> 3]]; im = 8; } if (!front && (flg & MAP_EFLG_FGND)) continue; if (c >= w || x + c < x0) continue; if (d & 0x0F) fb[i] = (fb[i] & 0xF0) | (d & 0x0F);}
#endif
    LOOP(3, 0x1f, 0x18);
    LOOP(2, 0x17, 0x10);
    LOOP(1, 0x0f, 0x08);
