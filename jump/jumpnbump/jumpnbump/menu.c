...
int menu_init(void)
{
    ...
    if ((handle = (char *)dat_open("menu.pcx")) == 0) {
        strcpy(main_info.error_str, "Error loading 'menu.pcx', aborting...\n");
        return 1;
    }
    if (read_pcx((unsigned char *)handle, background_pic, JNB_WIDTH*JNB_HEIGHT, menu_pal) != 0) {
        strcpy(main_info.error_str, "Error loading 'menu.pcx', aborting...\n");
        return 1;
    }
    if ((handle = (char *)dat_open("menumask.pcx")) == 0) {
        strcpy(main_info.error_str, "Error loading 'menumask.pcx', aborting...\n");
        return 1;
    }
    if (read_pcx((unsigned char *)handle, mask_pic, JNB_WIDTH*JNB_HEIGHT, 0) != 0) {
        strcpy(main_info.error_str, "Error loading 'menumask.pcx', aborting...\n");
        return 1;
    }
