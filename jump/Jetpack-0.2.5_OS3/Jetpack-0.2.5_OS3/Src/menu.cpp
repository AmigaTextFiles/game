...
SDL_SetColorKey(Load_Message,
	(SDL_SRCCOLORKEY|SDL_RLEACCEL),
	SDL_MapRGB(Load_Message->format,255,255,255));
SDL_Surface * Pointer = LoadBMP("pixmaps/jetmanb.bmp");
SDL_SetColorKey(Pointer,
	(SDL_SRCCOLORKEY|SDL_RLEACCEL),
	SDL_MapRGB(Pointer->format,255,255,255));
...
case 2: //Level code input screen
	...
	if (key_store == "\r") {
		...
		Code_Match = 2;
		for (i=0;i<5;i++) {
			if (LevelCodeStore[i] != Level2Code[i]) Code_Match = 0;
		}
		if (Code_Match == 2) { Initial_Stage = 6; return; }
		
		Code_Match = 3;
		for (i=0;i<5;i++) {
			if (LevelCodeStore[i] != Level3Code[i]) Code_Match = 0;
		}
		if (Code_Match == 3) { Initial_Stage = 4; return; }
...
case 1: //Info page
	...
	if (keys[SDLK_SPACE] || SDL_JoystickGetButton(Joystick, 0) || keys[SDLK_RETURN] || keys[SDLK_TAB] || keys[SDLK_ESCAPE]) state = 0;
	}
	break;
...
case 0: //Main menu screen
	...
	switch(choice) {
		case 0:
			...
			if (Pointer_Ypos>200) Pointer_Ypos--;
			if (Pointer_Ypos<200) Pointer_Ypos++;
		...
		case 1:
			...
			SDL_Delay(8); //Use timer to slow things down a little.
			if (Pointer_Ypos>260) Pointer_Ypos--;
			if (Pointer_Ypos<260) Pointer_Ypos++;
			Erase(180,Pointer_Ypos,Pointer->w,Pointer->h,screen,Menu_Background);
			Draw(180, Pointer_Ypos, Pointer->w,Pointer->h,Pointer,screen);
			}
		...
		case 2:
			...
			SDL_Delay(8); //Use timer to slow things down a little.
			if (Pointer_Ypos>320) Pointer_Ypos--;
			if (Pointer_Ypos<320) Pointer_Ypos++;
			Erase(180,Pointer_Ypos,Pointer->w,Pointer->h,screen,Menu_Background);
			Draw(180, Pointer_Ypos, Pointer->w,Pointer->h,Pointer,screen);
			}
		...
		case 3:
			...
			SDL_Delay(8);
			if (Pointer_Ypos>380) Pointer_Ypos--;
			if (Pointer_Ypos<380) Pointer_Ypos++;
			Erase(180,Pointer_Ypos,Pointer->w,Pointer->h,screen,Menu_Background);
			Draw(180, Pointer_Ypos, Pointer->w,Pointer->h,Pointer,screen);
			}
