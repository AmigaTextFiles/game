...
extern void Draw(unsigned int Xpos, unsigned int Ypos, unsigned int Width, unsigned int
	Height, SDL_Surface * image, SDL_Surface * screen);

extern void Erase(unsigned int Xpos, unsigned int Ypos, unsigned int Width, unsigned int
	Height, SDL_Surface * screen, SDL_Surface * background);

extern void Update(unsigned int Xpos, unsigned int Ypos, unsigned int Width, unsigned int
	Height, SDL_Surface * screen);
...
if (rand()%500 != 2) return;
if (!Level.Get_Rocket()->Is_Complete()) return; //Don't drop fuel for an unfinished rocket.
...
for (n=0;n<Level.Get_Num_Booty();n++)
{ if (Level.Get_Booty(n)->Is_Alive()==false) Spare_Slot=n; }
...
{
	if ( (Object1->Get_Ypos() >= (Object2->Get_Ypos()+
				Object2->Get_Image(Object2->Get_Anim_Frame())->h)) ||
	     (Object1->Get_Xpos() >= (Object2->Get_Xpos()+
	     			Object2->Get_Image(Object2->Get_Anim_Frame())->w)) ||
	     (Object2->Get_Ypos() >= (Object1->Get_Ypos()+
				Object1->Get_Image(Object1->Get_Anim_Frame())->h)) ||
	     (Object2->Get_Xpos() >= (Object1->Get_Xpos()+ 
	     			Object1->Get_Image(Object1->Get_Anim_Frame())->w)) ) {
		return(0);
	}
	return(1);
...
for (n=0;n<Level.Get_Num_Booty();n++)
{
	...
	if (Collide(Level.Get_Booty(n),
	Level.Get_Rocket()->Get_Rocket_Segment(Level.Get_Rocket()->Get_Num_Segments()-1))) {
	...
	if (Level.Get_Booty(n)->Get_Ypos()+Level.Get_Booty(n)->Get_Image(0)->h >=
		(unsigned int)Level.Get_Background()->h-2) {
		Level.Get_Booty(n)->Set_State(UNTOUCHED);
		}
...
{
	if ( (Object1->Get_Ypos() >= (Object2->Get_Ypos()+
				Object2->Get_Image(Object2->Get_Anim_Frame())->h-20)) ||
	     (Object1->Get_Xpos() >= (Object2->Get_Xpos()+
	     			Object2->Get_Image(Object2->Get_Anim_Frame())->w-25)) ||
	     (Object2->Get_Ypos() >= (Object1->Get_Ypos()+
				Object1->Get_Image(Object1->Get_Anim_Frame())->h-20)) ||
	     (Object2->Get_Xpos() >= (Object1->Get_Xpos()+ 
	     			Object1->Get_Image(Object1->Get_Anim_Frame())->w-25)) ) {
		return(0);
	}
	return(1);
...
{
	if ( (Object1->Get_Ypos() >= (The_Bullet->Get_Ypos()+
			The_Bullet->Get_Image(The_Bullet->Get_Anim_Frame())->h-20)) ||
	     (Object1->Get_Xpos() >= (The_Bullet->Get_Xpos()+
	     			The_Bullet->Get_Image(The_Bullet->Get_Anim_Frame())->w-25)) ||
	     (The_Bullet->Get_Ypos() >= (Object1->Get_Ypos()+
				Object1->Get_Image(Object1->Get_Anim_Frame())->h-20)) ||
	     (The_Bullet->Get_Xpos() >= (Object1->Get_Xpos()+ 
	     			Object1->Get_Image(Object1->Get_Anim_Frame())->w-25)) ) {
		return(0);
	}
	//Confirmed hit
...
if (!Player.Get_In_Rocket()) return;	//Don't do this unless player is inside rocket.
...
if (Level.Get_Rocket()->Get_Rocket_Segment(n)->Get_Ypos()>=2) {
	Level.Get_Rocket()->Get_Rocket_Segment(n)->
		Set_Ypos(Level.Get_Rocket()->Get_Rocket_Segment(n)->Get_Ypos()-1); }
else {
	Player.Add_Score(50);
...
if (Player.Get_In_Rocket()) return 0;	//Exit if the player is already in the rocket.
...
if (Player.Get_Carrying()) return false;//Can't collect stuff if already
					//carrying.				
...
if (!Player.Get_Carrying()) return; //Can't drop anything if not carrying
...
if (C_Segment->Get_State()==FALLING) {
	if (C_Segment->Get_Ypos()<= Level.Get_Rocket()->Get_Rocket_Segment(0)->Get_Ypos()-(50*n)) {
		...
		Update(C_Segment->Get_Xpos(), C_Segment->Get_Ypos()-1,
			C_Segment->Get_Image(0)->w,1,screen);
		C_Segment->Set_Ypos(C_Segment->Get_Ypos()+1);
		C_Segment->Draw(screen);
		}
	else	{C_Segment->Set_State(ON_ROCKET);
...
if (Level.Monsters_Frozen()) return; //don't move monsters when frozen.
...
if (Player.Get_Carrying()) return;	//cant fire if carrying stuff.
...
if (Player.Get_Xpos()<=50 && Player.Get_Direction()==LEFT) return;
if (Player.Get_Xpos()>=(unsigned int)Level.Get_Background()->w-50 && Player.Get_Direction() ==
	RIGHT)	return;
...
if (Player.Get_Direction() == LEFT) {
		...
		else	{ Player.Set_Anim_Frame(5); }
		}
if (Player.Get_Direction() == RIGHT){
		if (!Player.Get_Carrying())
			{ Player.Set_Anim_Frame(0); }
		else	{ Player.Set_Anim_Frame(4); }
		}
...
if ((keys[SDLK_DOWN] || SDL_JoystickGetAxis(Joystick, 1) > 3000) && (Player.Get_Ypos() < 430) && !Player.Get_Falling()) {
...
if ((keys[SDLK_RIGHT] || SDL_JoystickGetAxis(Joystick, 0) > 3000) && (Player.Get_Xpos() < 590) && !Player.Get_Falling()) {
...
if ((keys[SDLK_LEFT] || SDL_JoystickGetAxis(Joystick, 0) < -3000) && (Player.Get_Xpos() > 0) && !Player.Get_Falling()) {
...
if ((keys[SDLK_UP] || SDL_JoystickGetAxis(Joystick, 1) < -3000) && (Player.Get_Ypos()>20) && !Player.Get_Falling()) {
	//Do rocket pack.
	if (Player.Get_Direction() == LEFT) {
		if (!Player.Get_Carrying()) 
			{ Player.Set_Anim_Frame(3); }
		else 	{ Player.Set_Anim_Frame(7); }
		}
	if (Player.Get_Direction() == RIGHT){
		if (!Player.Get_Carrying()) 
			{ Player.Set_Anim_Frame(2); }
		else	{ Player.Set_Anim_Frame(6); }
		}
...
if (Player.Get_Falling()) Player.Set_Anim_Frame(8);	
...
if (stage == 1) return;		//Don't tell them that it is stage 1.
...
while (1) {
	...
	if (!keys[SDLK_SPACE] || !keys[SDLK_TAB] || !keys[SDLK_ESCAPE]) return;
	}
...
Uint8 iconmask[128]={0x00, 0x7f, 0x80, 0x00, 
				0x00, 0xff, 0xe0, 0x00,
				0x01, 0xff, 0xf0, 0x00,
				0x03, 0xff, 0xf8, 0x00,
				0x03, 0xff, 0xfc, 0x00,
