...
#ifdef __MORPHOS__
		nf = sumStringTabs_Cat(&fullpaths, nf, buf, n, gblOps.repDirs[i], FALSE);
#else
...
int chooseLangMenu(data_t* gfx)
{
	...
	for (i=0; i < gblOps.nlfolders; i++) {
		...
		sumStringTabs(&dirs, nf, buf, n);
#ifdef __MORPHOS__
		nf = sumStringTabs_Cat(&fullpaths, nf, buf, n, gblOps.langDirs[i], FALSE);
#else
...
void chooseThemeMenu(data_t* gfx)
{
	...
	for (i=0; i < gblOps.ntfolders; i++) {
		...
		sumStringTabs(&dirs, nf, buf, n);
#ifdef __MORPHOS__
		nf = sumStringTabs_Cat(&fullpaths, nf, buf, n, gblOps.themeDirs[i], TRUE);
#else
