...
{   
...
#ifdef __MORPHOS__
	gblOps.dataDir = malloc(strlen(DATA_PREFIX) + strlen("skins/") + strlen(DEFTHEME) + 1);
	sprintf(gblOps.dataDir, "%sskins/%s", DATA_PREFIX, DEFTHEME);
	
	gblOps.langFile = malloc(strlen(DATA_PREFIX) + strlen(DEFLANG) + 1);
	sprintf(gblOps.langFile, "%s%s", DATA_PREFIX, DEFLANG);
	
	gblOps.repDir = malloc(strlen(DATA_PREFIX) + strlen("replays/") + 1);
	sprintf(gblOps.repDir, "%sreplays/", DATA_PREFIX);
#else
#ifdef WIN32
	...
	gblOps.themeDirs[0] = malloc(strlen(DATA_PREFIX) + strlen("skins/") + 1);
	sprintf(gblOps.themeDirs[0], "%sskins/", DATA_PREFIX);
	
	gblOps.langDirs[0] = malloc(strlen(DATA_PREFIX) + strlen("lang/") + 1);
	sprintf(gblOps.langDirs[0], "%slang/", DATA_PREFIX);
	
	gblOps.repDirs[0] = malloc(strlen(DATA_PREFIX) + strlen("replays/") + 1);
	sprintf(gblOps.repDirs[0], "%sreplays/", DATA_PREFIX);
#else
#ifdef WIN32
	gblOps.ntfolders = 1;
    gblOps.nlfolders = 1;
...
{
	if (fullscreen == 1) {
		screen = SDL_SetVideoMode(w,h,bpp, SDL_ANYFORMAT | SDL_FULLSCREEN | SDL_SWSURFACE);
		if (screen == NULL) {
			printf("ERROR: The screen wasn't initialized because: %s\n", SDL_GetError());
			fullscreen = 0;
		}
	}
	if (fullscreen == 0) { //Fullscreen OFF
#ifdef __MORPHOS__
		screen = SDL_SetVideoMode(w,h,bpp,SDL_SWSURFACE | SDL_DOUBLEBUF);
#else
