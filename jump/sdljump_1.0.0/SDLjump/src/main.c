...
#ifdef __MORPHOS__
char version_tag[] = "$VER: SDLjump 1.0.0 (09.10.2006)";
#endif
...
#ifdef __MORPHOS__
	cfgFile = malloc(strlen(DATA_PREFIX) + strlen(CFGFILE) + 1);
	sprintf(cfgFile, "%s", DATA_PREFIX);
	
	hscFile = malloc(strlen(DATA_PREFIX) + strlen(HSCFILE) + 1);
	sprintf(hscFile, "%s", DATA_PREFIX);
#else
#ifndef WIN32 
    homeDir = getenv("HOME");
