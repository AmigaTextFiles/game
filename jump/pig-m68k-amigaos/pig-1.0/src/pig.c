...
static GAMESTATE *init_all(SDL_Surface *screen)
{
	...
	if(!gs->pe)
	{
		printf("Could not open the Pig Engine!\n");
		free(gs);
		return NULL;
	}
	gs->pe->userdata = gs;
	...
	if(i < 0)
	{
		printf("Could not load graphics!\n");
		pig_close(gs->pe);
		free(gs);
		return NULL;
	}
	...
	{
		printf( "Could not load background graphics!\n");
		pig_close(gs->pe);
		free(gs);
		return NULL;
	}
...
static void handle_input(GAMESTATE *gs, SDL_Event *ev)
{
	...
	else {screen = SDL_SetVideoMode(SCREEN_W, SCREEN_H, bpp, flags | SDL_FULLSCREEN);
	flags &= ~SDL_DOUBLEBUF;}
...
music = Mix_LoadMUS(DATAFILE("memories.mod"));

if(!music) {
    printf("Mix_LoadMUS(\"music.wav\"): %s\n", Mix_GetError());
	...
	screen = SDL_SetVideoMode(SCREEN_W, SCREEN_H, bpp, flags | SDL_HWSURFACE);
	if(!screen)
	{
		printf( "Failed to open screen!\n");
		return 1;
