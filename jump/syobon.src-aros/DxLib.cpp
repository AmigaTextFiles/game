...
#ifdef __AROS__
			if (scale_screen = SDL_SetVideoMode(scale_w, scale_h, 32, SDL_SWSURFACE | SDL_FULLSCREEN))
#else
...
void SetFontSize(byte size)
{
    ...
    if (font[size] == NULL) {
#ifdef __AROS__
	font[size] = TTF_OpenFont("res/DejaVuSans.ttf"/*"res/sazanami-gothic.ttf"*/, size);
#else
