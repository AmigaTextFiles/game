...
enum
{
ID_MENU_Autoplay_OID			= 1,
ID_MENU_Supervisor_OID,
ID_MENU_BookOff_OID,
ID_MENU_BookOn_OID,
ID_MENU_BookBest_OID,
ID_MENU_BookWorst_OID,
ID_MENU_BookRandom_OID,
ID_MENU_Depth0_OID,
ID_MENU_Depth1_OID,
ID_MENU_Depth2_OID,
ID_MENU_Depth3_OID,
...
ID_MENU_Depth9_OID,
ID_MENU_Depth10_OID,
ID_MENU_ShowThinking_OID,
ID_MENU_NullMove_OID,
ID_MENU_UseHash_OID,
ID_MENU_ReverseBoard_OID,
ID_MENU_Voice_OID,
ID_MENU_HideMoves_OID,
ID_MENU_BoardSmall_OID,
ID_MENU_BoardMedium_OID,
ID_MENU_BoardLarge_OID,
ID_timem_OID,
ID_timet_OID,
ID_times_OID,
};
...
{NM_SUB,NM(MSG_MENU_BOARDSMALL),0,CHECKIT,~0x01,(APTR)ID_MENU_BoardSmall},
{NM_SUB,NM(MSG_MENU_BOARDMEDIUM),0,CHECKIT,~0x02,(APTR)ID_MENU_BoardMedium},
{NM_SUB,NM(MSG_MENU_BOARDLARGE),0,CHECKIT,~0x04,(APTR)ID_MENU_BoardLarge},
...
{NM_ITEM,NM(MSG_MENU_HIDEMOVES),0,CHECKIT|MENUTOGGLE,0,(APTR)ID_MENU_HideMoves},
...
obj=DoSuperNew(cl,obj,
        ...
        MUIA_Application_Copyright,"Based on GnuChess 5.06",
				MUIA_Application_Version,"$VER: AmiChess 2.3 (1.11.2006)",
        ...
        SubWindow,win=WindowObject,
								MUIA_Window_Title,"AmiChess 2.3",
        ...
        SubWindow,editwin=WindowObject,
								MUIA_Window_Title,getstr(MSG_EDITBOARD), //"Edit board",
                ...
                WindowContents,VGroup,
                        ...
                        Child,bt_edit_clear=TextObject,
                                ...
                                MUIA_Text_PreParse,MUIX_C,
																MUIA_Text_Contents,getstr(MSG_EDITBOARD_CLEAR), //"_Clear board",
                        End,
                        Child,HVSpace,
                        Child,bt_edit_ok=TextObject,
                                ...
                                MUIA_Text_HiChar,'o',
                                MUIA_Text_PreParse,MUIX_C,
																MUIA_Text_Contents,getstr(MSG_OK), //"_Ok",
                        End,
                End,
        End,
	...
	if(obj)
	{
		...
		/* initialize board size in menu */
		if (pix_x==45)
			SetAttrs(find(ID_MENU_BoardSmall),MUIA_Menuitem_Checked,1);
		else if (pix_x==56)
			SetAttrs(find(ID_MENU_BoardMedium),MUIA_Menuitem_Checked,1);
		else
			SetAttrs(find(ID_MENU_BoardLarge),MUIA_Menuitem_Checked,1);
		...
		DoMethod(find(ID_MENU_HideMoves),MUIM_Notify,MUIA_Menuitem_Checked,MUIV_EveryTime,obj,2,MUIM_Chess_HideMoves,MUIV_TriggerValue);
		...
		DoMethod(find(ID_MENU_BoardSmall),MUIM_Notify,MUIA_Menuitem_Checked,MUIV_EveryTime,obj,1,MUIM_Chess_BoardSmall);
		DoMethod(find(ID_MENU_BoardMedium),MUIM_Notify,MUIA_Menuitem_Checked,MUIV_EveryTime,obj,1,MUIM_Chess_BoardMedium);
		DoMethod(find(ID_MENU_BoardLarge),MUIM_Notify,MUIA_Menuitem_Checked,MUIV_EveryTime,obj,1,MUIM_Chess_BoardLarge);
		...
		/* set object ids for loading/saving prefs */
		SetAttrs(find(ID_MENU_Autoplay),MUIA_ObjectID,ID_MENU_Autoplay_OID);
		SetAttrs(find(ID_MENU_Supervisor),MUIA_ObjectID,ID_MENU_Supervisor_OID);
		SetAttrs(find(ID_MENU_BookOff),MUIA_ObjectID,ID_MENU_BookOff_OID);
		SetAttrs(find(ID_MENU_BookOn),MUIA_ObjectID,ID_MENU_BookOn_OID);
		SetAttrs(find(ID_MENU_BookBest),MUIA_ObjectID,ID_MENU_BookBest_OID);
		SetAttrs(find(ID_MENU_BookWorst),MUIA_ObjectID,ID_MENU_BookWorst_OID);
		SetAttrs(find(ID_MENU_BookRandom),MUIA_ObjectID,ID_MENU_BookRandom_OID);
		SetAttrs(find(ID_MENU_Depth0),MUIA_ObjectID,ID_MENU_Depth0_OID);
		SetAttrs(find(ID_MENU_Depth1),MUIA_ObjectID,ID_MENU_Depth1_OID);
		SetAttrs(find(ID_MENU_Depth2),MUIA_ObjectID,ID_MENU_Depth2_OID);
		SetAttrs(find(ID_MENU_Depth3),MUIA_ObjectID,ID_MENU_Depth3_OID);
		SetAttrs(find(ID_MENU_Depth4),MUIA_ObjectID,ID_MENU_Depth4_OID);
		SetAttrs(find(ID_MENU_Depth5),MUIA_ObjectID,ID_MENU_Depth5_OID);
		SetAttrs(find(ID_MENU_Depth6),MUIA_ObjectID,ID_MENU_Depth6_OID);
		SetAttrs(find(ID_MENU_Depth7),MUIA_ObjectID,ID_MENU_Depth7_OID);
		SetAttrs(find(ID_MENU_Depth8),MUIA_ObjectID,ID_MENU_Depth8_OID);
		SetAttrs(find(ID_MENU_Depth9),MUIA_ObjectID,ID_MENU_Depth9_OID);
		SetAttrs(find(ID_MENU_Depth10),MUIA_ObjectID,ID_MENU_Depth10_OID);
		SetAttrs(find(ID_MENU_ShowThinking),MUIA_ObjectID,ID_MENU_ShowThinking_OID);
		SetAttrs(find(ID_MENU_NullMove),MUIA_ObjectID,ID_MENU_NullMove_OID);
		SetAttrs(find(ID_MENU_UseHash),MUIA_ObjectID,ID_MENU_UseHash_OID);
		SetAttrs(find(ID_MENU_ReverseBoard),MUIA_ObjectID,ID_MENU_ReverseBoard_OID);
		SetAttrs(find(ID_MENU_BoardSmall),MUIA_ObjectID,ID_MENU_BoardSmall_OID);
		SetAttrs(find(ID_MENU_BoardMedium),MUIA_ObjectID,ID_MENU_BoardMedium_OID);
		SetAttrs(find(ID_MENU_BoardLarge),MUIA_ObjectID,ID_MENU_BoardLarge_OID);
		SetAttrs(find(ID_MENU_Voice),MUIA_ObjectID,ID_MENU_Voice_OID);
		SetAttrs(find(ID_MENU_HideMoves),MUIA_ObjectID,ID_MENU_HideMoves_OID);
    ...
    MUIA_Group_Child, MUI_NewObject(MUIC_Group,
     ...
     MUIA_Group_Child, MUI_NewObject(MUIC_Text,
      MUIA_Font, MUIV_Font_Big,
			...
			MUIA_Text_Contents, (ULONG)"\33cAmiChess 2.3 - AmigaOS4 version",
#elif defined(__MorphOS__)
			MUIA_Text_Contents, (ULONG)"\33cAmiChess 2.3 - MorphOS version",
#else
			MUIA_Text_Contents, (ULONG)"\33cAmiChess 2.3",
#endif
     TAG_END),
     MUIA_Group_Child, MUI_NewObject(MUIC_Text,
#if defined(__AMIGAOS4__)
			MUIA_Text_Contents, (ULONG)"\33c\nwritten by\nAchim Stegemann\n\nAmigaOS4 port by\nMichael Merkel\n\33i<Michael.Merkel@gmx.net> \n",
#else
...
switch(msg->flag)
        {
        case 0:
								get(data->timem, MUIA_String_Integer, &val);
                TCMove=val;
                break;
        case 1:
								get(data->timet, MUIA_String_Integer, &val);
                TCTime=val;
                break;
        default:
								get(data->times, MUIA_String_Integer, &val);
                if(!val) val=5;
                SearchTime=val;
        }
if(!TCMove&&!TCTime)
...
static void mChessWindowRefresh(struct IClass *cl,Object *obj)
{
	struct Data *data=(struct Data *)INST_DATA(cl,obj);
...
static void mChessHideMoves(struct IClass *cl, Object *obj, struct MUIP_Chess_HideMoves *msg)
{
	struct Data *data=(struct Data *)INST_DATA(cl,obj);
	SetAttrs(data->lv_white,MUIA_ShowMe,!(msg->hide),TAG_END);
	SetAttrs(data->lv_black,MUIA_ShowMe,!(msg->hide),TAG_END);
	mChessWindowRefresh(cl, obj);
//	DoMethod(obj,MUIM_Chess_ShowBoard);
}
...
static void RestoreBoard(void)
{
	...
	get(menu, MUIA_Family_List, &list);
	node=list->lh_Head;
	
	while((sub=NextObject((APTR)&node)))
	{
	  ULONG checked;
		get(sub, MUIA_Menuitem_Checked, &checked);
	  if(checked)
		{
			char *t;
			get(sub, MUIA_Menuitem_Title, &t);
			LoadBoard(t);
			break;
		}
	}
...
{
	char *name;
	get(msg->menu, MUIA_Menuitem_Title, &name);
	LoadBoard(name);
	DoMethod(obj,MUIM_Chess_ShowBoard);
}
...
DISPATCHERPROTO(Chess_Dispatcher)
{
	ULONG retval = 0;
	switch(msg->MethodID)
        {
        ...
        case MUIM_Chess_ReverseBoard:
                ...
                break;
				case MUIM_Chess_HideMoves:
								mChessHideMoves(cl,obj,(struct MUIP_Chess_HideMoves *)msg);
                break;
...
{
	if (!(MUI_Chess_Class=MUI_CreateCustomClass(0,MUIC_Application,0,sizeof(struct Data),ENTRY(Chess_Dispatcher)))) exit(20);
}
