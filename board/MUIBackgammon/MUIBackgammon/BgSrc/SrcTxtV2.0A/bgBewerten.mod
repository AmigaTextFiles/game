...
PROCEDURE WeissHeimfeld ( brett : mui.Brett ) : LONGINT;
...
PROCEDURE SchwarzHeimfeld ( brett : mui.Brett ) : LONGINT;
...
PROCEDURE Endspiel (bh : mui.BgHandle) : BOOLEAN;
...
BEGIN (* Endspiel *)
  ...
  END; (* WHILE *)
  RETURN NOT (white OR (bh.brett[mui.schwarzBar] <> 0) OR (bh.brett[mui.weissBar] <> 0))

END Endspiel;

(*---------------------------------------------------------------------------*)

PROCEDURE FreeBlack (bh : mui.BgHandle) : REAL;
...
PROCEDURE FreeWhite (bh : mui.BgHandle) : REAL;
...
PROCEDURE SchlagWkt (brett : mui.Brett; x, angreifer : INTEGER) : REAL;
...
PROCEDURE Suche (brett : mui.Brett; pos, farbe : INTEGER) : INTEGER;
...
PROCEDURE Bewerten ( bh : mui.BgHandle ) : LONGINT;
(****** bgBewerten/Bewerten ************************************************
*
*       NAME
*           Bewerten
...
BEGIN (* Bewerten *)
  ...
  IF bh.brett[mui.weissZiel] = 15 THEN RETURN Max            (* weiss gewonnen *)
  ELSIF bh.brett[mui.schwarzZiel] = 15 THEN RETURN Min      (* schwarz gewonnen *)
  END; (* IF *)

  wert := 0;

  (* Steine auf der Bar *)
  DEC (wert, bh.brett[mui.weissBar] * set.kiSetting[bh.farbe,1,set.bar]);
  INC (wert, ABS (bh.brett[mui.schwarzBar]) * set.kiSetting[bh.farbe,2,set.bar]);

  (* Steine im Ziel *)
  INC (wert, bh.brett[mui.weissZiel] * set.kiSetting[bh.farbe,1,set.target]);
  DEC (wert, ABS (bh.brett[mui.schwarzZiel]) * set.kiSetting[bh.farbe,2,set.target]);
  ...
  FOR x := 1 TO 24 DO
    IF bh.brett[x] = 1 THEN           (* einzelner weisser Stein *)
      FOR i := x-1 TO 1 BY -1 DO      (* rechts davon suchen *)
        ...
        END (* IF *)
      END; (* FOR *)
      IF bh.brett[mui.schwarzBar] <> 0 THEN
        IF set.set.level >= 3 THEN
          DEC (wert, TRUNC (FLOAT (set.kiSetting[bh.farbe,1,set.singleProb]) *
               SchlagWkt (bh.brett, x, i) + 0.5))
        ELSE
    ...
    ELSIF bh.brett[x] = -1 THEN       (* einzelner schwarzer Stein *)
      FOR i := x+1 TO 24 DO           (* links davon suchen *)
        ...
        END (* IF *)
      END; (* FOR *)
      IF bh.brett[mui.weissBar] <> 0 THEN
        IF set.set.level >= 3 THEN
          INC (wert, TRUNC (FLOAT (set.kiSetting[bh.farbe,2,set.singleProb]) *
               SchlagWkt (bh.brett, x, i) + 0.5))
        ELSE
  ...
  FOR x := 1 TO 24 DO
    ...
    END (* IF *)
  END; (* FOR *)
  INC (whiteSum, 24 * bh.brett[mui.weissBar]);
  INC (blackSum, 24 * ABS (bh.brett[mui.schwarzBar]));
        ...
        IF blackBlock THEN
          ...
          IF blackLen > 1 THEN
            IF set.set.level >= 3 THEN
              ...
              blackLen := blackLen * Suche (bh.brett, x, mui.weiss)
            END; (* IF *)
        ...
        IF whiteBlock THEN
          ...
          IF whiteLen > 1 THEN
            IF set.set.level >= 3 THEN
              ...
              whiteLen := whiteLen * Suche (bh.brett, x-whiteLen-1, mui.schwarz)
            END; (* IF *)
