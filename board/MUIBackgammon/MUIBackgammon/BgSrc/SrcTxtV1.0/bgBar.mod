...
*
*       FUNCTION
*           Stellt das Objekt für eine Backgammon Bar
*
*       AUTHOR
*           Marc Ewert
*
...
TYPE BarDataPtr = POINTER TO BarData;
     BarData = RECORD
                 penSpec      : MD.mPenSpec;
...
BEGIN (* mNew *)
  ...
  IF obj <> NIL THEN
    ...
    WHILE tag <> NIL DO
      CASE tag^.tag OF
        brPen : IF tag^.data <> 0 THEN
                  data^.penSpec := y.CAST (MD.mPenSpecPtr, tag^.data)^
                END (* IF *)
      | brStonePen : IF tag^.data <> 0 THEN
                       data^.stonePenSpec := y.CAST (MD.mPenSpecPtr, tag^.data)^
                     END (* IF *)
      | brBottom : data^.bottom := tag^.data <> 0
      | brStones : data^.stones := tag^.data
      ELSE
      END; (* CASE *)
      tag := U.NextTagItem (tags);
    END (* WHILE *) ;
  ...
  IF tags <> NIL THEN
    REPEAT
      ...
      IF tag <> NIL THEN
        CASE tag^.tag OF
          brStones   : data^.stones := tag^.data
        | brPen      : data^.penSpec := y.CAST (MD.mPenSpecPtr, tag^.data)^;
                       data^.penChange := TRUE
        | brStonePen : data^.stonePenSpec := y.CAST (MD.mPenSpecPtr, tag^.data)^;
                       data^.penChange := TRUE
        ELSE
        END (* CASE *)
      END (* IF *)
...
BEGIN (* mGet *)
  ...
  CASE msg^.attrID OF
  brStones : store^ := data^.stones; RETURN LONGCARD(TRUE)
...
PROCEDURE BarDispatcher( cl : i.IClassPtr; obj : y.ADDRESS; msg : y.ADDRESS )
                         : y.ADDRESS;
...
BEGIN (* bgBar *)
  ...
  END; (* IF *)
  BarClass := I.MakeClass (NIL,NIL,SuperClass,SIZE (BarData),y.LONGSET {});
  IF BarClass = NIL THEN
    ML.moFreeClass (SuperClass);
    MS.fail (NULL, "Failed to create class!");
  END;
  MC.MakeDispatcher (BarDispatcher, BarClass);

CLOSE

  du := I.FreeClass (BarClass);             (* free our own class *)
  ML.moFreeClass (SuperClass)
