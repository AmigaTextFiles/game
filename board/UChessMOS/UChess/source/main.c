...
#else
#define mySetABPenDrMd(a,b,c,d) SetABPenDrMd(a,b,c,d);
#endif
...
struct SignalSemaphore mySemaphore; // should be all zeroed
...
static const char UCProcName[16]="UCTurbo Engine";
static const char InputPortName[16]="UC Port";
static const struct TagItem ProcTagList[8] = {
...
extern int RealThink;
int OpEntryRecvd=0;
struct MsgPort *InputReply=(struct MsgPort *)0L;
struct MsgPort *InThreadPort;
int InputThreadState=0;
struct myMsgStruct Global_Message;
char OpEntryStr[64];
...
char vstring[64]={AVSTR};

long piecechar[12]={'p','n','b','r','q','k',
                          'P','N','B','R','Q','K'};
char oldboard[8][8];
...
int doauto=0;
int teston=0;
int cmptr_sec,cmptr_min,hum_sec,hum_min;
struct TagItem myTagList[8] = {
...
ULONG globalsignalset=0L;

ULONG RTG_ModeID=0x0L;
STATIC CONST int SYSTEM_BOBS=1;
#ifndef __MORPHOS__
int procpri = 3;
#else
int procpri = 0;
...
int MenuStripSet = 0;
int GlobalTgtDepth=3;

CONST UWORD myPointer[]={
0,0,
31744,0,
4096,0,
4096,0,
...
struct Image BobImage;

struct TransImage *BobTransImage;
...
long OrigCol,OrigRow,DestCol,DestRow;
...
int USERBOX=168;
int COMPUTERBOX=60;
int BACKGNDTEXTCOLOR=0xe0; /* for 256 use 0xe0 */
int BOBHEIGHT=55; /* 55 for 480 screens */
int ROW8=24; /* 24 for 480 screens */
int MBLOCKPEN=251;
#endif

int TCadd = 0;
int thinking2=0; /* look for move now menu selection in elasped time when set */
int TIMEYCOORD1=100;
int TIMEYCOORD2=192;
int SYSBOXLEN=86;
int HEIGHT=400;
...
int SYSTEMBOX=233;
int COMPUTERBOX=50;
int BACKGNDTEXTCOLOR=15; /* for 256 use 0xe0 */
int BOBHEIGHT=46; /* 55 for 480 screens */
int ROW8=20; /* 24 for 480 screens */
int MBLOCKPEN=9;
...
#define BOBMEMASK 0
#define BOBHITMASK 0
int BOBPLANEPICK=0xf; /* if 16 colors should be 0xf if 256 should be 0xff */
...
UWORD *BlankImageData;

UWORD *WhiteImageData[6];
UWORD *BlackImageData[6];

void LoadBobImage(long);

long ColArray[8]={COLA,COLB,COLC,COLD,COLE,COLF,COLG,COLH};
long RowArray[8];
...
struct BitMap *WhiteBitMap,*BlackBitMap;
...
struct Screen *sC=0L;
struct Window *wG=0L;
struct Window *wGEdit;
unsigned char cookedchar[128]={'~',
			      '1',
			      '2',
			      '3',
			      '4',
...
static const struct TextAttr TOPAZ80 = {
	(STRPTR)"topaz.font",
	TOPAZ_EIGHTY,0,0
};

static const SHORT BorderVectors6[] = {
	0,0,
	44,0,
	44,19,
	0,19,
...
static const struct Border Border6 = {
...
static const struct IntuiText IText93 = {
...
static const struct Gadget Gadget9 = {
	NULL,	/* next gadget */
...
static const struct NewWindow NewWindowStructure5 = {
...
static const struct ExtNewScreen NewScreenStructure = {
...
static const struct ExtNewScreen NewScreenStructure2 = {
	0,0,	/* screen XY origin relative to View */
...
struct NewWindow NewWindowStructure2 = {
	...
	NULL,	/* custom CHECKMARK imagery */
#if defined(_M68040) || defined(__PPC__)
...
struct NewWindow pNewWindowStructure1 = {
...
#endif
#if !defined(_M68040) || !defined(__PPC__) /* small mem model */
#ifdef TINYCHESS
   rating -= 41;
#else
   rating -= 21;
...
int DoResign()
{
 ...
 ULONG iflags=0L;
 UBYTE	volname[4]={0,0,0,0};

 Delay(30L);
 DisplayBeep(0L);
 ResignOffered = -1;
/* return 0 if no resign accepted, yes if accepted */
 return(EasyRequest( wG, &resignES, &iflags, (ULONG)volname ));
}
...
static void tFreeBitMap(struct BitMap *);
static void sFreeBitMap(struct BitMap *);
static struct BitMap *tAllocBitMap(int,int,int,int,struct BitMap *);
static struct BitMap *sAllocBitMap(int,int,int,int,struct BitMap *);

static struct BitMap *tAllocBitMap(wid,ht,depth,flags,friend)
int wid,ht,depth,flags;
struct BitMap *friend;
{
  return AllocBitMap(wid,ht,depth,flags,friend);
}

static struct BitMap *sAllocBitMap(wid,ht,depth,flags,friend)
int wid,ht,depth,flags;
struct BitMap *friend;
{
 int i;
 ...
 tmp->pad = 32860;
 tt = planes*8;
 if (!(tmp->Planes[0] = AllocMem(tt,MEMF_ANY|MEMF_CLEAR)))
  {
...
static void tFreeBitMap(bmap)
struct BitMap *bmap;
{
  FreeBitMap(bmap);
}

static void sFreeBitMap(bmap)
struct BitMap *bmap;
{
 ...
 for(i=0;i<6;i++)
  {
   ...
   if (!(WhiteImageData[i] = 
        (UWORD *)AllocMem(BOBBLOCKSIZE,MEMF_ANY|MEMF_CLEAR)))
    {
     return(0);
    }
...
int InitmyGels()
{
 ...
 if (!(TheBob->SaveBuffer = (WORD *)
    AllocMem(sizeof(SHORT)*BOBWIDINW*BOBHEIGHT*BOBDEPTH,MEMF_ANY|MEMF_CLEAR)))
  {
   return(0);
  }
 ...
 if (!(TheBob->DBuffer = (struct DBufPacket *)AllocMem
        (sizeof(struct DBufPacket),MEMF_ANY|MEMF_CLEAR)))
    {
     return(0);
    }
...
#ifdef ALLOCFORREAL
	/* Allocate one plane */
	if (bm->Planes[0] = (PLANEPTR) AllocMem (rsize, MEMF_ANY | MEMF_CLEAR))
	{
...
AllocShadowRP (struct BitMap *bm)
{
    ...
    /* Allocate a RastPort */
    if ((rp = (struct RastPort *) AllocMem (msize, MEMF_ANY)))
    {
...
/* before unpacking the bitmap, save the titlebar area in a tmp bitmap */
 ...
 if (!(srcbuf = AllocMem(count,0L)))
  {
   sprintf(errstr,"Cannot allocate lharc %ld byte buffer",count);
   DisplayError(errstr);
   return(0);
  }
 if (!(LhBase = OpenLibrary(LH_NAME,LH_VERSION)))
...
#ifdef ttblsz
struct hashentry ttable[2][vttblsz + MAXrehash];
unsigned int ttblsize;
#endif
...
unsigned INTSIZE PV, SwagHt, Swag0, Swag1, Swag2, Swag3, Swag4;

#ifdef USE_SIDEBIT
unsigned INTSIZE sidebit;
#endif

#ifdef KILLT
INTSIZE killt[0x4000];
#endif
...
//      }
 ...
 else if (aargc > 1)
  {
   for(tmpargcnt=1;tmpargcnt<aargc;tmpargcnt++)
    {
     if (!(stricmp(aargv[tmpargcnt],"NOBOBS")))
      {
       SYSTEM_BOBS = 0;
      }
