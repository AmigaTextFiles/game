...
int thinkahead=0;
int ThinkInARow=0;
int ThinkAheadWorked=0;
int ThinkAheadDepth=0;
...
#else
  memset(QueenCheck, 0, MAXDEPTH*sizeof(short));
  memset(PrVar, 0, MAXDEPTH*sizeof(short));
  memset(Threat, 0, MAXDEPTH*sizeof(short));
  memset(ThreatSave, 0, MAXDEPTH*sizeof(short));
  memset(Pscore, 0, MAXDEPTH*sizeof(short));
  memset(Tscore, 0, MAXDEPTH*sizeof(short));
  memset(ChkFlag, 0, MAXDEPTH*sizeof(short));
  memset(CptrFlag, 0, MAXDEPTH*sizeof(short));
  memset(PawnThreat, 0, MAXDEPTH*sizeof(short));
#endif
...
#ifdef AMIGA
if (!func_num)
 sprintf(astr,"Mlst=%ldN/s",rate*100);
else if (func_num == 1)
 sprintf(astr,"Clst=%ldN/s",rate*100);
ShowMessage(astr);
#else
