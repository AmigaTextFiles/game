...
if(to<64)
        ...
        if(from<64)
                ...
                cmd[3]='1'+(to/8);
                cmd[4]=0;
                if (((side==white&&cmd[1]=='7'&&cmd[3]=='8') &&
                 (board.b[side][pawn]&BitPosArray[from]))
                 || ((cmd[1]=='2'&&cmd[3]=='1') && board.b[side][pawn]&BitPosArray[from]))
                        {
                        promotepiece=0;
...
struct EmulLibEntry MUIFieldGate = {TRAP_LIB, 0, (void(*)(void))Dispatcher};

void INIT_6_MUI_Field_Class(void)
{
if(!(MUI_Field_Class=MUI_CreateCustomClass(0,MUIC_Area,0,sizeof(struct Data),(APTR)&MUIFieldGate))) exit(20);
}
