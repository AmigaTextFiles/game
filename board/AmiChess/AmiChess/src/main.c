...
void InitTimer(void)
{
OpenDevice(TIMERNAME,UNIT_MICROHZ,&timerio.tr_node,0);
...
void LoadEdit(void)
{
if(piecesedit=NewDTObject("PROGDIR:Pieces/640/Default",DTA_GroupID,GID_PICTURE,PDTA_DestMode,PMODE_V43,PDTA_Remap,1,PDTA_Screen,wbscreen,PDTA_FreeSourceBitMap,1,PDTA_UseFriendBitMap,1,TAG_END))
	{
	if(DoDTMethod(piecesedit,0,0,DTM_PROCLAYOUT,0,1))
		{
		GetDTAttrs(piecesedit,PDTA_DestBitMap,&pieces_edit_bm,PDTA_MaskPlane,&pieces_edit_mask,TAG_END);
		}
	}
else exit(20);
}
...
if(locale=OpenLocale(0)) catalog=OpenCatalogA(locale,"AmiChess.catalog",0);
...
for(;;)
	{
	if(DoMethod(mui_app,MUIM_Application_NewInput,&signals)==MUIV_Application_ReturnID_Quit) break;
	if(flags&AUTOPLAY)
		{
