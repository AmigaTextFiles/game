...
obj=DoSuperNew(cl,obj,
	...
	MUIA_Application_Version,"$VER: AmiChess 2.0 (19.04.2003)",
	...
	SubWindow,win=WindowObject,
		MUIA_Window_Title,"AmiChess 2.0 by Achim Stegemann",
...
GetAttr(MUIA_Family_List,menu,&list);
node=list->lh_Head;
while(sub=NextObject(&node))
	{
	ULONG checked;
	GetAttr(MUIA_Menuitem_Checked,sub,&checked);
	if(checked)
		{
		char *t;
		GetAttr(MUIA_Menuitem_Title,sub,&t);
		LoadBoard(t);
		break;
		}
	}
...
{
char *name;
GetAttr(MUIA_Menuitem_Title,msg->menu,&name);
LoadBoard(name);
DoMethod(obj,MUIM_Chess_ShowBoard);
}
