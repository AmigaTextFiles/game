...
void Render(SDL_Surface *surface)
{
	...
	if (buffer_screen==NULL) {
		buffer_screen=new UBYTE[SCREEN_X*SCREEN_Y];
...
*/ 
	...
	// Transferir la imagen de 'buffer_screen' a la memoria de vídeo:
		...
		if (logic_dx[i]!=0) {
			...
			if (logic_dx[i]==physic_dx[i] &&
				...
				for(y=0;y<physic_dy[i];y++) {
					...
					dest=(UBYTE *)surface->pixels+physic_x[i]+(physic_y[i]+y)*surface->pitch;
