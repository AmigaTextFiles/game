...
/************************************/
#if defined (MAC_LINUX) || defined (__MORPHOS__)
// Version linux
...
void GetPath(char *Path)
{
  ...
  sprintf(Path,"Progdir:data/%s",Provi);
  if(FileExiste(Path)) return;
...
#ifdef __MORPHOS__
  char *PathPref="Progdir:ri-li.pref";
#endif
...
#ifdef __MORPHOS__
  SauveFichier("Progdir:ri-li.pref",(char*)&Pref,sizeof(sPreference));
#endif
