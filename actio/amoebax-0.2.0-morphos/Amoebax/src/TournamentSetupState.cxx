...
TournamentSetupState::loadGraphicResources (void)
{
            ...
            Surface::fromFile (File::getGraphicsFilePath ("menuBackground.png")));
    {
        ...
        for ( uint16_t currentCharacter = 0,
                       x = static_cast<uint16_t> (k_FaceStartX * originalScale);
              currentCharacter < 4 ;
              currentCharacter++,
                x += static_cast<uint16_t> (k_FaceSpan * originalScale) )
        {
            faces->blit ((currentCharacter % k_FacesPerRow) * faceWidth, 0,
                         faceWidth, faceHeight, x,
                         static_cast<uint16_t> (k_FemaleFacesRowY *
                                                originalScale),
                         m_Background->toSDLSurface());
        }
        for ( uint16_t currentCharacter = 4,
                       x = static_cast<uint16_t> (k_FaceStartX * originalScale);
              currentCharacter < 8 ;
              currentCharacter++,
                x += static_cast<uint16_t> (k_FaceSpan * originalScale) )
        ...
        for ( uint16_t currentCharacter = 1,
                       x = static_cast<uint16_t> (k_FaceStartX * originalScale);
              currentCharacter < 7 ;
              currentCharacter++,
                x += static_cast<uint16_t> (k_FaceSpan * originalScale) )
        {
            faces->blit ((currentCharacter % k_FacesPerRow) * faceWidth,
                         faceHeight, faceWidth, faceHeight, x,
