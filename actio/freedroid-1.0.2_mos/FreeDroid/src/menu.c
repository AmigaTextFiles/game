...
LevelEditor(void)
{
  ...
  while ( !Done )
      ...
      if (KeyIsPressedR (SDLK_KP5))
	{
	  ...
	  else CurLevel->map[BlockY][BlockX]=MAP_VOID;
	}
      ...
      while (!key)
	{
	  ...
	  switch (MenuPosition) 
		  ...
		  for ( i = 0 ; i < CurLevel->ylen ; i++ )
		    {
		      ...
		      // We don't want to fill the new area with junk, do we? So we set it MAP_VOID
		      CurLevel->map[ i ] [ CurLevel->xlen-1 ] = MAP_VOID;
		    }
		  InitiateMenu (FALSE);
		  MoveMenuPositionSound();
		}
	      ...
	      if ( RightPressedR()||MouseRightPressedR() )
		{
		  ...
		  // We don't want to fill the new area with junk, do we? So we set it MAP_VOID
		  memset( CurLevel->map[ CurLevel->ylen-1 ] , MAP_VOID , CurLevel->xlen );
		  InitiateMenu (FALSE);		      
		  MoveMenuPositionSound();
		}
