...
/*
 * optimize_map -- Tries to add hints in the map points that will hopefully
 *                 speed up the map drawing.
 */
void
optimize_map(void)
{
  ...
  int ecount, fcount;
  btype  blocktype;
  USHORT dflags, memflags;

  for (y = 0; y < World.Height; y++) {
    ecount = fcount = dflags = memflags = 0;
    for (x = World.Width-1; x >= 0; x--) {
      blocktype = World.map_points[y][x].blocktype;
      dflags    = World.map_points[y][x].draw_flags;

      if (blocktype == BLOCK_FILLED_ND || blocktype == BLOCK_EMPTY) {
        World.map_points[y][x].empty2Right = ecount;
        ecount++;        
      } else {
        World.map_points[y][x].empty2Right = ecount;
        ecount = 0;
      ...
      if (blocktype == BLOCK_FILLED) {
        /* We only want to check blocks with horizontal lines */
        if ( !((dflags & DRAW_LEFT) || (dflags & DRAW_RIGHT)) ) {
          if (dflags == memflags) {
            fcount++;
            memflags = dflags;
          } else {
            fcount = 0;
            memflags = dflags;
            World.map_points[y][x].filled2Right = 0;
          }
        } else {
          fcount = 0;
          memflags = 0;
      ...
      World.map_points[y][x].filled2Right = fcount;
    }
  }
}
      ...
      for ( x = start_x; x < end_x; x++ ) {
        switch (map_points[y][x].blocktype) {
          ...
          case BLOCK_FILLED:
            ...
            filled2Right = map_points[y][x].filled2Right;

            if (filled2Right > 0) {
              int fillCount, newbsz;
              /* 
               * Hehe, I wonder if this cryptic thingy really speeds things
               * up or slows them down :)
               *
               * What I check here is that we don't draw the 'filled2Right' blocks
               * out of the display area. Hopefully there is a better/faster way
               * to do this(?).
               */
              fillCount = (((filled2Right + x) < end_x) ? (filled2Right+1) : (end_x-x));
              newbsz = (MAP_BLOCKSIZE * fillCount) + 1;
#ifdef PURE_OS
              if (d_flags & DRAW_UP)    { HLINE(wRp, xbsz-bp_x, ybsz-bp_y, newbsz) }
              if (d_flags & DRAW_DOWN)  { HLINE(wRp, xbsz-bp_x, ybsz+bsz-bp_y, newbsz) }
#else
              if (d_flags & DRAW_UP)    { HorizontalLine(bpl1, xbsz-bp_x, ybsz-bp_y, newbsz, onoff); }
              if (d_flags & DRAW_DOWN)  { HorizontalLine(bpl1, xbsz-bp_x, ybsz+bsz-bp_y, newbsz, onoff); }
#endif
              x = x + filled2Right;
            } else {
#ifdef PURE_OS
        ...
        x = x + map_points[y][x].empty2Right;
      }
    }
  }
