...
char *version = "$VER: APilot_Opt is still beta (22.03.94)";
...
main(int argc, char *argv[])
{
  ...
  prefs.dpy_width = 639;	/* Default values (could really be anything) */
  prefs.dpy_height = 511;
  prefs.native_mode = 1;
  ...
  init_points();
  init_explosion();
  init_display(prefs.native_mode);
  init_font();
  init_kbdrepeat();
#ifndef PURE_OS
  ...
  init_ship(&dummyShip);
  dummyShip.shields = FALSE;
  dummyShip.local   = FALSE;
  dummyShip.next = World.players->next;
  World.players->next = &dummyShip;
...
/*---------------------------------------------------*/
    ...
    /* Only for testing */
    update_ship(&dummyShip, CurBuffer, w_framerate);
    ...
    /* And testing again */
    draw_ship(&dummyShip, &myRP[CurBuffer], CurBuffer);
...
cleanExit( LONG returnValue, char *fmt, ... )
{
  ...
  remove_VertBServer();
  if (deviceopen == 0) CloseDevice((struct IORequest *) inputReqBlk);
