...
void loadPreference() {
  ...
  if ( NULL == (fp = fopen(PREF_FILE,"rb")) ) {
	printf("Can't open prefs file.\n");
    initHiScore();
    return;
  }
  fscanf(fp, "%2d", &version);
  if ( version != VERSION_NUM ) {
	printf("Wrong prefs file version.\n");
    initHiScore();
    return;
  }
  for ( i=0 ; i<STAGE_NUM ; i++ ) {
    fscanf(fp, "%7s", tempstr);
	hiScore.stageScore[i] = atoi(tempstr);
    for ( j=0 ; j<SCENE_NUM ; j++ ) {
      fscanf(fp, "%6s", tempstr);
      hiScore.sceneScore[i][j] = atoi(tempstr);
    }
  }
  for ( i=0 ; i<ENDLESS_STAGE_NUM ; i++ ) {
    fscanf(fp, "%6s", tempstr);
    hiScore.stageScore[i+STAGE_NUM] = atoi(tempstr);
  }
  fscanf(fp, "%7s", tempstr);
  hiScore.stageScore[i+STAGE_NUM] = atoi(tempstr);
  fclose(fp);
...
void savePreference() {
  ...
  fprintf(fp, "%2d", VERSION_NUM);
  for ( i=0 ; i<STAGE_NUM ; i++ ) {
    fprintf(fp, "%7d", hiScore.stageScore[i]);
    for ( j=0 ; j<SCENE_NUM ; j++ ) {
      fprintf(fp, "%6d", hiScore.sceneScore[i][j]);
    }
  }
  for ( i=0 ; i<ENDLESS_STAGE_NUM ; i++ ) {
    fprintf(fp, "%7d", hiScore.stageScore[i+STAGE_NUM]);
  }
  fprintf(fp, "%7d", hiScore.stage);
  fclose(fp);
}
