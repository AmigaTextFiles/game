...
// 68k SDL/pdcurses conversion by Lorence Lombardo  28 May 2012
// "sh" 1st then gcc -m68020 -m68881 -noixemul aop.c -o aop -lpdcurses_ns `sdl-config2 --libs` -lm
...
start:	memset(field, ' ', 25*81);
	snprintf(tmp, 96, "PROGDIR:aop-level-%02d.txt", level);
