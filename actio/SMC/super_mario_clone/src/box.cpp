...
void cGoldBox :: PlayerCollision( int cdirection )
{
	if( cdirection == UP )// if player jumps from below
	{
		if( used == 0 && !bgp )
		{
			...
			if( !PosValid( (int)posx, (int)posy - rect.h, (int)(rect.w*0.8), 5 ) )
			{
				...
				{
					ActiveObjects[iCollisionNumber]->BoxCollision( DOWN, &rect );
				}
			}
...
void cGoldBox :: EnemyCollision( int cdirection )
{
	if( cdirection == RIGHT || cdirection == LEFT )
	{
		if( used == 0 && !bgp )
		{
			pAudio->PlaySound( SOUNDS_DIR "/goldpiece.ogg" );
...
void cGoldBox :: Update( void )
{
	if( bgp && bgp->visible )
	{
		...
		{
			AddAnimation( (int)bgp->posx,(int)bgp->posy, 1 );
...
void cBonusBox :: PlayerCollision( int cdirection )
{
	if( cdirection == UP )// if player jumps from below
	{
		if( !used )
		{
			if( type == TYPE_BONUSBOX_MUSHROOM_FIRE )
			{
				if( pPlayer->iSize == MARIO_SMALL || ( pPlayer->iSize == MARIO_FIRE && !Itembox->Item_id ) )
				{
...
void cBonusBox :: EnemyCollision( int cdirection )
{
	if( cdirection == RIGHT || cdirection == LEFT )
	{
		if( !used )
...
void cSpinBox :: PlayerCollision( int cdirection )
{
	if( cdirection == UP )	// if player jumps from below
	{
		if( posy == startposy )
...
void cSpinBox :: Update( void )
{
	if( spin )
	{
		...
		else 
		{
			...
			{
				if( RectIntersect( &pPlayer->rect, &rect ) )
				{
					spin_counter = 0;
				}
				else
