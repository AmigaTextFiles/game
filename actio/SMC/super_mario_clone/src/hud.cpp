...
void cItemBox :: Update( void )
{
	...
	if( !Item->PosValid( (int)Item->posx, (int)Item->posy ) && Item->iCollisionType == 4 ) // If colliding with the Player
	{
		...
		if( Item_id == TYPE_MUSHROOM_DEFAULT ) 
		{
			Item_id = 0;
			pPlayer->Get_Item( TYPE_MUSHROOM_DEFAULT, 1 );
		}
		else if( Item_id == TYPE_FIREPLANT ) 
		{
			Item_id = 0;
			pPlayer->Get_Item( TYPE_FIREPLANT, 1 );
		}
		else
		{
			Item_id = 0;
...
{
	if( !Game_debug || pPlayer->iSize == MARIO_DEAD ) 
	{
		return;
	}
	...
	if( strcmp( pLevel->Levelfile.c_str(), lvl_text_old ) != 0 ) 
	{
		...
		lvl_text.erase( 0, strlen( LEVEL_DIR ) + 1 );
	...
	// # Passive
	if( passcount != (unsigned int)PassiveCount ) 
	{
		char pass_text[100];
		sprintf( pass_text,"Passive Objects : %d", PassiveCount );
	...
	// # Massive
	if( masscount != (unsigned int)MassiveCount ) 
	{
		char mass_text[100];
		sprintf( mass_text,"Massive Objects : %d", MassiveCount );
	...
	// # Enemy
	if( enemcount != (unsigned int)EnemyCount ) 
	{
		char enem_text[100];
		sprintf( enem_text,"Enemy Objects : %d", EnemyCount );
	...
	// # Active
	if( activcount != (unsigned int)ActiveCount ) 
	{
		char activ_text[100];
		sprintf( activ_text,"Active Objects : %d", ActiveCount );
	...
	// # Active : HalfMassive
	if( achmasscount != (unsigned int)ActiveCount ) 
	{
	...
	// # Active : Cloud
	if( accloudcount != (unsigned int)ActiveCount ) 
	{
	...
	// # Active : Goldbox
	if( acgoldboxcount != (unsigned int)ActiveCount ) 
	{
	...
	// # Active : Bonusbox
	if( acbonusboxcount != (unsigned int)ActiveCount ) 
	{
	...
	if( acothercount != acothercount_old ) 
	{
		...
		sprintf( acother_text, "Other : %d", ActiveCount - acothercount );
...
void LoadHudObjects( void )
{
	if( !HUD_loaded && HUDCount > 0 )
	{
		UnloadHudObjects();
	}
	else if( HUD_loaded )
