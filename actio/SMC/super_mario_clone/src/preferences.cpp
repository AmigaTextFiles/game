...
void cPreferences :: Save( void )
{
	...
	sprintf( row, "### Preferences file V.%s for Super Mario Clone FX V.%s #### \n\n", PREF_VERSION, VERSION );
...
void cPreferences :: Update( void ) // Gets the settings from Current in-game Settings
{
	Music = pAudio->bMusic;
	Sounds = pAudio->bSounds;
}

void cPreferences :: Apply( void ) // Sets the settings to Current in-game Settings
{
	pAudio->bMusic = Music;
...
int cPreferences :: HandleMessage( string *parts, unsigned int count, unsigned int line )
{
	...
	{
		if( atoi( parts[1].c_str() ) < 0 || atoi( parts[1].c_str() ) > 1600)
		{
			//return 0;
		}
		else
		{
			Screen_H = atoi( parts[1].c_str() );
		}
	}
	else if( parts[0].compare( "Screen_W") == 0 )
	{
		...
		{
			Screen_W = atoi( parts[1].c_str() );
		}
	}
	else if( parts[0].compare( "Bpp" ) == 0 )
	{
		if( atoi( parts[1].c_str() ) < 0 || atoi( parts[1].c_str() ) > 32 )
		{
			//return 0;
		}
		else
		{
			Bpp = atoi( parts[1].c_str() );
		}
	}
	else if( parts[0].compare( "Key_up" ) == 0 )
	{
		if( atoi( parts[1].c_str() ) < 0 || atoi( parts[1].c_str() ) > 1000)
		{
			//return 0;
		}
		else
		{
			Key_up = (SDLKey)atoi( parts[1].c_str() );
		}
	}
	else if( parts[0].compare( "Key_down" ) == 0 )
	{
		...
		{
			Key_down = (SDLKey)atoi( parts[1].c_str() );
		}
	}
	else if( parts[0].compare( "Key_left" ) == 0 )
	{
		...
		{
			Key_left = (SDLKey)atoi( parts[1].c_str() );
		}
	}
	else if( parts[0].compare( "Key_right" ) == 0 )
	{
		...
		{
			Key_right = (SDLKey)atoi( parts[1].c_str() );
		}
	}
	else if( parts[0].compare( "Key_shoot" ) == 0 )
	{
		...
		{
			Key_shoot = (SDLKey)atoi( parts[1].c_str() );
		}
	}
	else if( parts[0].compare( "Joy_jump" ) == 0 )
	{
		if( atoi( parts[1].c_str() ) < 0 || atoi( parts[1].c_str() ) > 50 )
		{
			//return 0;
		}
		else
		{
			Joy_jump = atoi( parts[1].c_str() );
		}
	}
