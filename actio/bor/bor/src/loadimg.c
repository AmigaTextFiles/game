...
static int openbmp(char *filename, char *packfilename){
    ...
    bmp_header.bm = SDL_SwapLE16( bmp_header.bm );
    bmp_header.numplanes = SDL_SwapLE16( bmp_header.numplanes );
    bmp_header.bpp = SDL_SwapLE16( bmp_header.bpp );

    bmp_header.filesize = SDL_SwapLE32( bmp_header.filesize );
    bmp_header.reserved = SDL_SwapLE32( bmp_header.reserved );
    bmp_header.picstart = SDL_SwapLE32( bmp_header.picstart );
    bmp_header.headersize = SDL_SwapLE32( bmp_header.headersize );
    bmp_header.xsize = SDL_SwapLE32( bmp_header.xsize );
    bmp_header.ysize = SDL_SwapLE32( bmp_header.ysize );
    bmp_header.filesize = SDL_SwapLE32( bmp_header.filesize );

    bmp_header.compression = SDL_SwapLE32( bmp_header.compression );
    bmp_header.picsize = SDL_SwapLE32( bmp_header.picsize );
    bmp_header.hres = SDL_SwapLE32( bmp_header.hres );
    bmp_header.vres = SDL_SwapLE32( bmp_header.vres );
    bmp_header.numcolors_used = SDL_SwapLE32( bmp_header.numcolors_used );
    bmp_header.numcolors_important = SDL_SwapLE32( bmp_header.numcolors_important );

//    printf("BMP: %x %d %d\n", bmp_header.bm, bmp_header.bpp, bmp_header.compression );
...
//printf("GIF %c %c %c %d %d\n", gif_header.magic[0], gif_header.magic[1], gif_header.magic[2],
//	  SDL_SwapLE16(gif_header.screenwidth), SDL_SwapLE16(gif_header.screenheight) );
    ...
    gif_header.screenwidth = SDL_SwapLE16( gif_header.screenwidth );
    gif_header.screenheight = SDL_SwapLE16( gif_header.screenheight );
...
static int readgif(char *buf, char *pal, int maxwidth, int maxheight){
	...
	while(!done){
		...
		switch(c){
            ...
            iblock.left = SDL_SwapLE16(iblock.left);
            iblock.top = SDL_SwapLE16(iblock.top);
            iblock.width = SDL_SwapLE16(iblock.width);
            iblock.height = SDL_SwapLE16(iblock.height);
...
static int openpcx(char *filename, char *packname){
    ...
    pcx_header.xmin = SDL_SwapLE16( pcx_header.xmin );
    pcx_header.ymin = SDL_SwapLE16( pcx_header.ymin );
    pcx_header.xmax = SDL_SwapLE16( pcx_header.xmax );
    pcx_header.ymax = SDL_SwapLE16( pcx_header.ymax );
    pcx_header.hres = SDL_SwapLE16( pcx_header.hres );
    pcx_header.vres = SDL_SwapLE16( pcx_header.vres );
    pcx_header.bytesperline = SDL_SwapLE16( pcx_header.bytesperline );
    pcx_header.paltype = SDL_SwapLE16( pcx_header.paltype );
