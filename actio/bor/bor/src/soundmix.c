...
static int loadwave(char *filename, char *packname, samplestruct *buf, unsigned int maxsize){
    ...
    riffheader.riff = SDL_SwapLE32( riffheader.riff );
    riffheader.size = SDL_SwapLE32( riffheader.size );
    riffheader.type = SDL_SwapLE32( riffheader.type );
	...
	while(rifftag.tag!=HEX_fmt){
		if(readpackfile(handle, &rifftag, sizeof(rifftag))!=sizeof(rifftag)){
     	...
     	rifftag.tag = SDL_SwapLE32( rifftag.tag );
     	rifftag.size = SDL_SwapLE32( rifftag.size );
    ...
    fmt.format = SDL_SwapLE16( fmt.format );
    fmt.channels = SDL_SwapLE16( fmt.channels );
    fmt.unknown = SDL_SwapLE16( fmt.unknown );
    fmt.samplebits = SDL_SwapLE16( fmt.samplebits );
    fmt.samplerate = SDL_SwapLE32( fmt.samplerate );
    fmt.bps = SDL_SwapLE32( fmt.bps );
...
#if defined(SDL) || defined(PSP)
void update_sample(char *buf,int size)
{
	int i,u,todo = size;
	if (playbits==16) {
		todo >>=1;
...
int sound_open_music(char * filename, char * packname, int defaultvolume, int loop){
	...
	borhead.version = SDL_SwapLE32( borhead.version );
	borhead.frequency = SDL_SwapLE32( borhead.frequency );
	borhead.channels = SDL_SwapLE32( borhead.channels );
	borhead.datastart = SDL_SwapLE32( borhead.datastart );
