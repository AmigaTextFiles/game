...
//	printf("openpackfile(%s,%s)\n",filename,packfilename);
    ...
    if(read(handle,&magic,4)!=4 || magic!= SDL_SwapLE32( PACKMAGIC ) ){
	close(handle);
	return -1;
    }
    ...
    headerstart = SDL_SwapLE32( headerstart );
    ...
    while(read(handle,&pn,sizeof(pn))>12){
        ...
        pn.filesize = SDL_SwapLE32( pn.filesize );
        pn.filestart = SDL_SwapLE32( pn.filestart );
        pn.pns_len = SDL_SwapLE32( pn.pns_len );
