...
/* avoid unix stdc */
#define time bor_time
#define pause bor_pause
#define	kill	bor_kill
...
//#endif
	...
	savedata.usesound = 1;
	savedata.soundrate = DEFAULT_SOUNDRATE;
	savedata.soundvol = 14;
	savedata.usemusic = 1;
	savedata.musicvol = 128;
	savedata.effectvol = 48;
	savedata.soundbits = DEFAULT_SOUNDBITS;
	savedata.usejoy = 1;
//#if PSP
	savedata.soundstereo = 1;
//#endif
...
void load_background(char *filename, int createtables){
	...
	if(background==NULL) _shutdown("Error loading file '%s'", filename);
	pal[0] = pal[1] = pal[2] = 0;
	palette_set_corrected(pal, savedata.gamma,savedata.gamma,savedata.gamma, savedata.brightness,savedata.brightness,savedata.brightness);
	...
	if(createtables){
		...
		if(lut_mul==NULL) _shutdown("Failed to create colour conversion table! (Out of memory?)");
		lut_screen = palette_table_screen(pal);
		if(lut_screen==NULL) _shutdown("Failed to create colour conversion table! (Out of memory?)");
	}
}
...
void load_texture(char *filename){
	...
	if(texture==NULL) _shutdown("Error loading file '%s'", filename);
}
...
int loadfrontpanel(char *filename){
    ...
    //make_sprite( frontpanels[frontpanels_loaded], bitmap );
...
int loadsprite(char *filename, int ofsx, int ofsy){
	...
	if(sprites_loaded >= MAX_SPRITES) _shutdown("Too many sprites (max. %u)", MAX_SPRITES);
	bitmap = loadbitmap(filename, packfile);
	if(!bitmap) _shutdown("Unable to load file '%s' (may be out of memory)", filename);
	...
	if(!sprites[1][sprites_loaded]){
		freebitmap(bitmap);
		_shutdown("Out of memory!");
	}
	encodesprite(ofsx-clipl, ofsy-clipt, bitmap, sprites[1][sprites_loaded]);

    //make_sprite( sprites[1][sprites_loaded], bitmap );
    ...
    //make_sprite( sprites[0][sprites_loaded], bitmap );
...
s_model * nextplayermodel(void *current){
	...
	_shutdown("Fatal: can't find any player models!");
	return NULL;
}
...
void cache_model(const char *name, char *path){
	if(models_cached >= MAX_MODELS) _shutdown("Too many models, unable to cache '%s'", name);
	printf("Cacheing '%s'\n", name);

	model_cache[models_cached] = (s_modelcache *) malloc(sizeof(s_modelcache));
	if(model_cache[models_cached] == NULL) _shutdown("Out of memory cacheing '%s'", name);
...
void load_cached_model(char * name){
	...
	if(filename == NULL){
		_shutdown("Fatal: no cache entry for '%s'", name);
	}


	if(models_loaded >= MAX_MODELS){
		_shutdown("Cannot load model from '%s' - too many models", filename);
	}
	// printf("Loading model %s...\n", filename);

	if((handle=openpackfile(filename,packfile)) < 0) _shutdown("Unable to open file '%s'", filename);
	...
	if(buf==NULL){
		closepackfile(handle);
		_shutdown("Unable to create buffer for file '%s' (%i bytes)", filename, size);
	}
	if(readpackfile(handle, buf, size) != size){
		free(buf);
		closepackfile(handle);
		_shutdown("Read error accessing file '%s'", filename);
	}
	...
	if(newchar == NULL){
		free(buf);
		_shutdown("Out of memory loading model from '%s'", filename);
	}
	...
	while(pos<size){
		...
		if(command[0]){
			if(stricmp(command, "name")==0){
				...
				if(find_model(value)){
					free(buf);
					_shutdown("Duplicate model name '%s'", value);
				}
				...
				if(newchar->icon > -1){
					free(buf);
					_shutdown("Error: model '%s' has multiple icons defined", filename);
				}
				...
				if(stricmp(value, "none")==0){
					...
					free(buf);
					_shutdown("Model '%s' has invalid type: '%s'", filename, value);
				}
			}
				...
				if(stricmp(value, "biker")==0){
					...
					free(buf);
					_shutdown("Model '%s' has invalid subtype: '%s'", filename, value);
				}
			}
				...
				if(!load_colourmap(newchar, value, value2)){
					free(buf);
					_shutdown("Failed to create colourmap from images\n\t'%s'\nand\n\t'%s'.", value, value2);
				}
			}
				...
				if(newanim==NULL){
					free(buf);
					_shutdown("Not enough memory for animations!");
				}
				if(stricmp(value, "idle")==0){
					...
					free(buf);
					_shutdown("Invalid animation name '%s'", value);
				}
				...
				if(newanim == NULL){
					free(buf);
					_shutdown("Can't set loop: no animation specified!");
				}
				value = findarg(buf+pos, 1);
				...
				if(newanim==NULL){
					free(buf);
					_shutdown("Cannot set range: no animation!");
				}
				...
				if(newanim==NULL){
					free(buf);
					_shutdown("Cannot add frame: animation not specified!");
				}
				value = findarg(buf+pos, 1);
				...
				free(buf);
				_shutdown("Command '%s' not understood in file '%s'!", command, filename);
			}
		}
...
int load_models(){
	...
	if((handle=openpackfile(filename,packfile)) < 0) _shutdown("Error loading model list from %s", filename);
	size = seekpackfile(handle,0,SEEK_END);
	seekpackfile(handle,0,SEEK_SET);
	...
	if(buf==NULL){
		closepackfile(handle);
		_shutdown("Not enough memory to allocate %i-byte buffer for file %s", size, filename);
	}
	if(readpackfile(handle, buf, size) != size){
		free(buf);
		closepackfile(handle);
...
void add_level(char *filename, int diff){
	...
	if(num_levels[diff] >= MAX_LEVELS) _shutdown("Too many entries in level order (max. %i)!", MAX_LEVELS);

	levelorder[diff][num_levels[diff]] = (s_level_entry*)malloc(sizeof(s_level_entry));
	if(levelorder[diff][num_levels[diff]] == NULL) _shutdown("Out of memory loading level order!");
...
void load_levelorder(){
	...
	if((handle=openpackfile(filename,packfile)) < 0) _shutdown("Error loading level list from %s", filename);
	size = seekpackfile(handle,0,SEEK_END);
	seekpackfile(handle,0,SEEK_SET);
		...
		if(command[0]){
			if(stricmp(command, "set")==0){
				if(num_difficulties>=MAX_DIFFICULTIES){
					free(buf);
					_shutdown("Too many sets of levels (max %u)!", MAX_DIFFICULTIES);
				}
				++num_difficulties;
				++current_set;
				...
				if(current_set<0){
					free(buf);
					_shutdown("Error in level order: a set must be specified.");
				}
				...
				if(num_levels[current_set]<1){
					free(buf);
					_shutdown("Error in level order (next before file)!");
				}
				levelorder[current_set][num_levels[current_set]-1]->gonext = 1;
			}
			else{
				free(buf);
				_shutdown("Command '%s' not understood in level order!", command);
			}
		}
	...
	if(current_set<0) _shutdown("No levels were loaded!");
}
...
void load_level(char *filename){
	...
	if(level==NULL) _shutdown("FATAL: Out of memory!");
	memset(level, 0, sizeof(s_level));


	if((handle=openpackfile(filename,packfile)) < 0) _shutdown("Unable to load level file '%s'", filename);
	size = seekpackfile(handle,0,SEEK_END);
	seekpackfile(handle,0,SEEK_SET);
	...
	if(buf==NULL){
		closepackfile(handle);
		_shutdown("FATAL: out of memory!");
	}
	if(readpackfile(handle, buf, size) != size){
		free(buf);
		closepackfile(handle);
		_shutdown("FATAL: read error accessing file '%s'", filename);
	}
	buf[size] = 0;		// Terminate string (important!)
	closepackfile(handle);
	...
	while(pos<size){
		...
		if(command[0]){
			if(stricmp(command, "background")==0){
				...
				if(panels_loaded<1){
					free(buf);
					_shutdown("You must load the panels before entering the level layout!");
				}
				value = findarg(buf+pos, 1);
				i = 0;
				while(value[i] && level->numpanels < LEVEL_MAX_PANELS){
					...
					if(j>='A' && j<='Z') j-='A';
						...
						free(buf);
						_shutdown("Illegal character in panel order: '%c' (%02Xh)", j, j);
					}
					if(j >= panels_loaded){
						free(buf);
						_shutdown("Illegal panel index: %i (only %i panels loaded)", j, panels_loaded);
					}
				...
				if(level->numholes >= LEVEL_MAX_HOLES) _shutdown("Too many holes in level (max %i)!", LEVEL_MAX_HOLES);
				...
				if(level->numspawns >= LEVEL_MAX_SPAWNS) _shutdown("Level error: too many entries (max. %i)", LEVEL_MAX_SPAWNS);
				...
				free(buf);
				_shutdown("Command '%s' not understood!", command);
			}
		}
	...
	if(level->numpanels < 1) _shutdown("Level error: level has no panels");
}
...
void ent_set_anim(entity *ent, int aninum){
	...
	if(ent==NULL) _shutdown("FATAL: tried to set animation with invalid address (no such object)");
	if(aninum<0 || aninum>=MAX_ANIS) _shutdown("FATAL: tried to set animation with invalid index (%i)", aninum);
	ani = ent->model->animation[aninum];
	if(ani==NULL) _shutdown("FATAL: tried to set animation with invalid address (%s, %i)", ent->name, aninum);
...
void ent_set_model(entity * ent, char * modelname){
	...
	if(ent==NULL) _shutdown("FATAL: tried to change model of invalid object");

	m = find_model(modelname);
	if(m==NULL) _shutdown("Model not found: '%s'", modelname);
	ent->model = m;
...
entity * spawn(float x, float z, float a, char * name){
	...
	if(model==NULL){
		...
		return NULL;
		// _shutdown("FATAL: attempt to spawn object with invalid model (%s)!", name);
	}
...
void spawnplayer(int index){
	...
	if(player[index].ent==NULL) _shutdown("Fatal: unable to spawn player from '%s'", &p.name);
...
void startup(){
	...
	if((vscreen = allocscreen(320, 240)) == NULL){
		_shutdown("Not enough memory!\n");
	}
	clearscreen(vscreen);

	printf("Loading font...\n");

	if(!font_load(0, "data/sprites/font", packfile)) _shutdown("Unable to load font #1!\n");
	if(!font_load(1, "data/sprites/font2", packfile)) _shutdown("Unable to load font #2!\n");
	if(!font_load(2, "data/sprites/font3", packfile)) _shutdown("Unable to load font #3!\n");
	if(!font_load(3, "data/sprites/font4", packfile)) _shutdown("Unable to load font #4!\n");
	...
	if(savedata.usesound && sound_init(6)){
		...
		if(!sound_start_playback(savedata.soundstereo,savedata.soundbits,savedata.soundrate)){
	...
	if(!alloc_ents()){
		_shutdown("Not enough memory for game objects!\n");
	}
	...
	if(rescreen){
		if(!video_set_mode(rescreen->width, rescreen->height)){
			_shutdown("Unable to set video mode: %ix%i!\n", rescreen->width, rescreen->height);
		}
	}
	else if(!video_set_mode(320, 240)){
		_shutdown("Unable to set video mode: 320x240!\n");
	}
...
int playgif(char *filename, int x, int y){
	...
	if(background==NULL) _shutdown("Out of memory!");
		...
		for(i=0; i<2; i++){
			if(!ready[i]){
				if(player[i].lives <= 0 && (player[i].newkeys || immediate[i])){
					...
					if(example[i]==NULL) _shutdown("Failed to create player selection object!");
...
void playgame(int p1on, int p2on, unsigned which_set){
	...
	// _shutdown("Illegal set chosen: index %i (there are only %i sets)!", which_set, num_difficulties);
			...
			switch(selector){
				...
				case 2:
					...
					if(!sound_start_playback(savedata.soundstereo, savedata.soundbits, savedata.soundrate)){
						savedata.soundbits = DEFAULT_SOUNDBITS;
						savedata.soundrate = DEFAULT_SOUNDRATE;
						sound_start_playback(savedata.soundstereo, savedata.soundbits, savedata.soundrate);
					}
