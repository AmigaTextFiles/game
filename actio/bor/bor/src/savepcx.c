...
int savepcx(char *filename, s_screen *screen, char *pal){
	...
	head->xmin = 0;
	head->ymin = 0;
	head->xmax = SDL_SwapLE16( screen->width - 1 );
	head->ymax = SDL_SwapLE16( screen->height - 1 );
	head->hres = SDL_SwapLE16( screen->width );
	head->vres = SDL_SwapLE16( screen->height );
	head->hscreensize = SDL_SwapLE16( screen->width );
	head->vscreensize = SDL_SwapLE16( screen->height );
	head->colorplanes = 1;
	head->bpl = SDL_SwapLE16( screen->width+1 & 0xFFFE );
	head->paltype = 0;



	if((handle=open(filename, O_WRONLY|O_CREAT|O_TRUNC|O_BINARY, 0))==-1){
		success = 0;
		goto END;
	}
