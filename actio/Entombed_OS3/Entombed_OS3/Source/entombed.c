/*
  ...
  May 19, 2002 - January 17, 2005
*/

#define VERSION "2005.01.17b"
...
// Kjell
// Raised the number of starting breaks to 3... a bit low with just 2 
//#define STARTING_MAKEBREAK 2
#define STARTING_MAKEBREAK 3
...
const char * image_names[NUM_IMAGES] = {
  "data/images/title.jpg",
  "data/images/brick1.png",
  "data/images/brick2.png",
  "data/images/brick3.png",
  "data/images/brick4.png",
  "data/images/brick5.png",
  "data/images/man1.png",
  "data/images/man2.png",
  "data/images/woman1.png",
  "data/images/woman2.png",
  "data/images/zombie1.png",
  "data/images/zombie2.png",
  "data/images/zombie_mad1.png",
  "data/images/zombie_mad2.png",
  "data/images/makebreak1.png",
  "data/images/makebreak2.png",
  "data/images/makebreak3.png",
  "data/images/makebreak4.png",
  "data/images/makebreak5.png",
  "data/images/credits1.png",
  "data/images/credits2.png",
  "data/images/credits3.png",
  "data/images/credits4.png",
  "data/images/1.png",
  "data/images/2.png",
  "data/images/numbers.png",
  "data/images/paused.png"
};
...
const char * sound_names[NUM_SOUNDS] = {
  ...
  "data/sounds/makebreak_get.wav",
  "data/sounds/makebreak_use.wav",
  "data/sounds/zombie_appear.wav",
  "data/sounds/zombie_eat.wav",
  "data/sounds/zombie_mad.wav",
  "data/sounds/die.wav",
...
const char * music_names[NUM_MUSICS] = {
  "data/music/AMAZE.S3M",
  "data/music/Suggestive_vibes_01.mod"
};
...
int game(void)
{
  ...
  do
      ...
      while (SDL_PollEvent(&event) > 0)
	{
      ...
      if (map_position >= (MAP_HEIGHT - SCREEN_HEIGHT) * 32)
	{
	  level++;
	...
	pl_y[1] += 4;
      ...
      for (player = 0; player < num_players; player++)
        {
	  if (pl_y[player] > map_position + SCREEN_HEIGHT * 32 - 27)
	    {
	      pl_y[player] = map_position + SCREEN_HEIGHT * 32 - 27;
	    }
        }
	  ...
	  x = mirror_x(pl_x[player]);
          ...
          if (map[(pl_y[player] +  4) / 32][(x +  4) / 32] ||
	      map[(pl_y[player] + 27) / 32][(x +  4) / 32] ||
	      map[(pl_y[player] +  4) / 32][(x + 27) / 32] ||
	      map[(pl_y[player] + 27) / 32][(x + 27) / 32])
	    {
...
#define MAKEBREAK_DIST 35
      ...
      for (player = 0; player < num_players; player++)
        {
          if (pl_y[player] - map_position < 16)
	    {
	      do_die = player;
	    }
	}
	  ...
	  else if (zombie_x > ((MAP_WIDTH * 2) - 3) * 32)
	    {
	      zombie_x = ((MAP_WIDTH * 2) - 3) * 32;
	      zombie_dir = DIR_LEFT;
	    }

	  if (zombie_y > SCREEN_HEIGHT * 32 + map_position)
	    {
	      zombie_y = zombie_y - 2;
	      zombie_dir = DIR_UP;
	    }
	  ...
	  if (!zombie_mad)
	    {
	      ...
	      if (map[(zombie_y +  4) / 32][(x + 4) / 32] ||
		  map[(zombie_y + 27) / 32][(x + 4) / 32] ||
		  map[(zombie_y +  4) / 32][(x + 27) / 32] ||
		  map[(zombie_y + 27) / 32][(x + 27) / 32])
		{
	  ...
	  if (zombie_fadein == 0)
	    {
	      for (player = 0; player < num_players; player++)
		{
		  if (pl_x[player] >= zombie_x - 27 &&
		      pl_x[player] <= zombie_x + 27 &&
		      pl_y[player] >= zombie_y - 27 &&
		      pl_y[player] <= zombie_y + 27)
		    {
		      /* DIE */
      ...
      else
	{
	  /* Add a new zombie now and then! */
	      ...
	      zombie_y = ((map_position / 32) + SCREEN_HEIGHT) * 32;
	      do
		{
		  zombie_x = (rand() % (MAP_WIDTH * 2)) * 32;
		}
	      while (map[zombie_y / 32][mirror_x(zombie_x) / 32]);
	  ...
	  for (player = 0; player < num_players; player++)
	    {
	      if (pl_x[player] >= potion_x - 27 &&
	          pl_x[player] <= potion_x + 27 &&
	          pl_y[player] >= potion_y - 27 &&
	          pl_y[player] <= potion_y + 27)
	        {
	  ...
	  if (map[potion_y / 32][mirror_x(potion_x -
				          (16 * abs(potion_xm))) / 32])
	  {
	    potion_x = potion_x - potion_xm;
	    potion_xm = -potion_xm;
	  ...
	  potion_y = ((map_position / 32) + SCREEN_HEIGHT + 1) * 32;
	  do
	    {
	      potion_x = (rand() % (MAP_WIDTH * 2)) * 32;
	    }
	  while (map[potion_y / 32][mirror_x(potion_x) / 32]);
	  ...
	  if (zombie_fadein == 0)
	    ...
	    {
	      for (y = (map_position % 2); y < 32; y = y + 2)
		{
      ...
      for (y = 0; y < SCREEN_HEIGHT + 1; y++)
	{
	  for (x = 0; x < MAP_WIDTH; x++)
	    {
	      if (map[y + (map_position / 32)][x])
		{
		  dest.x = x * 32;
		  dest.y = y * 32 - (map_position % 32);
      ...
      dest.w = WIDTH;
      dest.h = HEIGHT - (SCREEN_HEIGHT * 32);
        ...
        for (x = 0; x < lives[player]; x++)
	{
      ...
      draw_num(160, SCREEN_HEIGHT * 32 + 4, makebreak[0]);
      ...
      if (num_players == 2)
        draw_num(WIDTH - 160 - 32, SCREEN_HEIGHT * 32 + 4, makebreak[1]);
      else
	draw_num(WIDTH - 160 - 32, SCREEN_HEIGHT * 32 + 4,
	         (map_position * 6) / (MAP_HEIGHT * 32) + ((level - 1) * 6));
	...
	{
	  pl_y[player] = (SCREEN_HEIGHT / 2) * 32 + map_position;
	}
	...
	for (y = (map_position / 32);
	     y <= (map_position / 32) + SCREEN_HEIGHT;
	     y++)
	{
...
// Kjell
  ...
  SDL_WM_SetCaption("Entombed! AmigaOS3 port by Arti", "Entombed! AmigaOS3 port by Arti");
  /* SDL_ShowCursor(0); */
...
void seticon(void)
{
  ...
  icon = IMG_Load("data/images/icon.png");
  if (icon == NULL)
    {
      printf(
              "\nError: I could not load the icon image: %s\n"
              "The Simple DirectMedia error that occured was:\n"
              "%s\n\n", "data/images/icon.png", SDL_GetError());
      exit(1);
    }
...
void reset_level(void)
{
  ...
  for (player = 0; player < 2; player++)
    {
      pl_y[player] = (SCREEN_HEIGHT / 2) * 32;
    }
...
int mirror_x(int real_x)
{
  ...
  if (real_x >= MAP_WIDTH * 32 - 27)
    {
      x = MAP_WIDTH * 32 - 32 - (real_x - MAP_WIDTH * 32);
    }
  else
    {
      x = real_x;
