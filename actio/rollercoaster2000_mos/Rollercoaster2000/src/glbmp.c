...
int GetRightShiftCount(ULONG dwVal);
int GetNumSet(ULONG dwVal);
...
int glBmpLoadBMP(glBmpImage* img, char *filename)
{
	...
	if (iHeader.biCompression != BI_RGB)
	{
		if (iHeader.biCompression == BI_BITFIELDS)
		{
			...
			{
				fread(&img->redMask, 1, sizeof(ULONG), bmpFile);
				fread(&img->greenMask, 1, sizeof(ULONG), bmpFile);
				fread(&img->blueMask, 1, sizeof(ULONG), bmpFile);
			}
		}
		else
		{
...
int glBmpMemLoadBMP(glBmpImage* img, char *filePtr)
{
	...
	if (iHeader.biCompression != BI_RGB)
	{
		if (iHeader.biCompression == BI_BITFIELDS)
		{
			if (img->colorDepth == 16)
			{
				memcpy(&img->redMask,filePtr,sizeof(ULONG));
				filePtr += sizeof(ULONG);
				memcpy(&img->greenMask,filePtr,sizeof(ULONG));
				filePtr += sizeof(ULONG);
				memcpy(&img->blueMask,filePtr,sizeof(ULONG));
				filePtr += sizeof(ULONG);
			}
		}
		else
