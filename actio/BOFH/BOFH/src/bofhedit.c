...
RANDOMLOCATION rloc[MAX_ACTOR*16];
...
void savemission(void)
{
	...
	char mapnamebuf[80];
        ...
        for (c=0;c<MAX_ACTOR*16;c++)
        {
                if(rloc[c].type == ACTOR_BIGMEDIKIT)
                {
                        writele32(handle, rloc[c].xb);
                        writele32(handle, rloc[c].yb);
                }
        ...
        for (c=0;c<MAX_ACTOR*16;c++)
        {
                if(rloc[c].type == ACTOR_PISTOLMAN)
                {
                        writele32(handle, rloc[c].xb);
                        writele32(handle, rloc[c].yb);
                }
