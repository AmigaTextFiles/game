...
void intro :: actualizar(void)
{
	...
	if (paso>5)
	{
		if (mensaje[pos] != '\0')
		{
			if (mensaje[pos] != '#')
			{
				imprimir_caracter(mensaje[pos], 10+posx_letra*26, 200+posy_letra*33, 0);
				posx_letra++;
			}
			else
			{
	...
	if (tecla[SDLK_SPACE] || tecla[SDLK_ESCAPE] || etapa > VS)
	{
		pmundo->audio.play(5);
		pmundo->cambiar_escena(MENU);
	}
...
void intro :: cambiar_etapa(int etapa)
{
	...
	{
		case PRESENTA:
		strcpy(mensaje,_("#######presenting ..."));
		break;

		case SENTADO:
		strcpy(mensaje,_("####the cowboy don ceferino#"\
					"lived very happy in#"\
					"his quite and modest#"\
		...
		case VS:
		strcpy(mensaje,_("###but this doesn't end#"\
					"here. don ceferino is#"\
