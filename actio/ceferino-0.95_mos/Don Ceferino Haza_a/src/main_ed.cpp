...
#ifdef __MORPHOS__
const char *version_tag = "$VER: ceferinoeditor 0.95 (2005-05-31)";
#endif
...
int iniciar(SDL_Surface **screen, FILE **arch)
{
	...
	*arch = fopen(SRC_DIR "/niveles/base.map", "rb+");

	if (!*arch)
	{
		printf("No se puede abrir el archivo'" SRC_DIR\
				"/niveles/base.map', no tiene permisos\n");
		return 1;
	}
...
#ifdef __MORPHOS__
	// make sure left mousebutton gets released
	while (mouse->botones == SDL_BUTTON(1)) {
		SDL_PumpEvents();
		mouse->botones = SDL_GetMouseState(NULL, NULL);
	}
#endif
}
