...
#ifdef __MORPHOS__
const char *version_tag = "$VER: ceferinosetup 0.95 (2005-05-31)";
#endif
...
int iniciar(SDL_Surface **screen, SDL_Surface **fondo, SDL_Surface **items)
{
	...
	*fondo = IMG_Load(SRC_DIR "/ima/fondo.jpg");

	if (*fondo == NULL)
	{
		printf("error: no se encuentra la imagen '" SRC_DIR\
				"/ima/fondo.jpg'\n");
		return 1;
	}
	
	*items = IMG_Load(SRC_DIR "/ima/op_items.png");
...
#else
#ifdef __MORPHOS__
	strcpy(cadena, "Progdir:.ceferino");
#else
