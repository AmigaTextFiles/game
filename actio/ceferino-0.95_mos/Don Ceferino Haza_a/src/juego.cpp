...
int juego :: avanzar_nivel(void)
{
	...
	strcpy(mensaje, _("level "));
	mensaje[6] = (nivel_actual / 10) + '0';
	mensaje[7] = (nivel_actual % 10) + '0';
	mensaje[8] = '\0';
	
	procesos->crear_mensaje(pmundo->libgrafico.ima_fuente_1, mensaje);
...
void juego :: procesar_tiempo(void)
{
	if (SDL_GetTicks() - tick_ant > 1500) //paso un segundo
	{
		...
		if (tiempo<0)
		{
			procesos->crear_mensaje(\
					pmundo->libgrafico.ima_fuente_1,\
					_("time out"));
...
void juego :: restar_vidas(void)
{
	...
	if (vidas<0)
	{
		procesos->crear_mensaje(pmundo->libgrafico.ima_fuente_1,\
				_("game over"));
		procesos->pausar_gaucho();
		procesos->pausar_tiros();
		procesos->pausar_enemigos();
	...
	{
		procesos->crear_mensaje(pmundo->libgrafico.ima_fuente_1, mensajes[rand()%3]);
		procesos->pausar_gaucho();
		procesos->pausar_tiros();
		procesos->pausar_enemigos();
...
void juego :: crear_mensaje_bonus_tiempo(int segundos)
{
	...
	mensaje[6] = (segundos % 10) + '0';
	mensaje[7] = ' ';
	mensaje[8] = '\0';
	
	procesos->crear_mensaje_rapido(pmundo->libgrafico.ima_fuente_1, mensaje);
}
...
void juego :: imprimir_marca(SDL_Surface *screen, SDL_Rect *rect_actual, int *lim_actual)
{
	...
	for (i=0; mensaje1[i] != '\0'; i++)
	{
		if (mensaje1[i] != ' ')
		{
			pmundo->libgrafico.ima_fuente_2->imprimir(mensaje1[i]-'a', screen, &(rect_actual[*lim_actual]), i*30 +10, 100, 1);
			(*lim_actual)++;
		}
	}
	...
	for (i=0; mensaje2[i] != '\0'; i++)
	{
		if (mensaje2[i] != ' ')
		{
			pmundo->libgrafico.ima_fuente_2->imprimir(mensaje2[i]-'a', screen, &(rect_actual[*lim_actual]), i*30 +60, 150, 1);
			(*lim_actual)++;
		}
	}
}
...
{
	int i;
	int x_inicial; // posicion de centrado
	SDL_Rect cursor = {0,0,15,1};
	
	x_inicial = 320 - (strlen(nombre))*15;
	
	// imprime el nombre escrito
	for (i=0; nombre[i] != '\0'; i++)
	{
		if (nombre[i] != ' ')
		{
			pmundo->libgrafico.ima_fuente_2->imprimir(nombre[i]-'a'+40, screen, &(rect_actual[*lim_actual]), i*30 +x_inicial, 200, 1);
			(*lim_actual)++;
		}
	}

	// imprime el cursor
	cursor.x = i*30 +x_inicial;
	cursor.y = 250;
	SDL_FillRect(screen, &cursor, SDL_MapRGB(screen->format, 255, 255, 255));
	rect_actual[*lim_actual] = cursor;
	(*lim_actual)++;
}
...
void juego :: imprime_pausa(SDL_Surface *screen, SDL_Rect *rect_actual, int *lim_actual)
{
	...
	{
		pmundo->libgrafico.ima_fuente_1->imprimir(mensaje[i]-'a'+40, screen, &(rect_actual[*lim_actual]), i*30 + 230, 230, 1);
		(*lim_actual)++;
	}
