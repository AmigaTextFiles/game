...
void hmasked_sprite(BITMAP *dst, BITMAP *src, BITMAP *mask, int x, int y)
{
	...
	case 15:
		...
		for (i = 0; i < h; i++) {
			...
			// CHQ_BRANCH: Accessing video bitmaps directly is illegal
			//sd = (unsigned short *) dst->line[dy + i] + dx;
			sd = (unsigned short *) bmp_write_line(dst, dy + i) + dx;
	...
	case 24 :
		...
		for (i = 0; i < h; i++) {
			...
			cd = (unsigned char *) bmp_write_line(dst, dy + i) + dx*3;
			cm = mask->line[sy+i] + sx;
			for (j = 0; j < w; j++) {
				if ((ucm = *(cm++)) == 256 || (uls = *((unsigned long *)cs)) == col){
					cd += 3;
					cs += 3;
					continue;
				}
				uld = *((unsigned long *)cd);
				...
				*((unsigned long *)cd) = uld & (0xFF000000) | (r << 16) | (g << 8) | b;
				cd += 3;
				cs += 3;
			}
		}
	...
	case 32:
		...
		for (i = 0; i < h; i++) {
			ls = (unsigned long *) src->line[sy + i] + sx;
			// CHQ_BRANCH: Accessing video bitmaps directly is illegal
			//ld = (unsigned long *) dst->line[dy + i] + dx;
			ld = (unsigned long *) bmp_write_line(dst, dy + i) + dx;
			cm = mask->line[sy+i] + sx;
			for (j = 0; j < w; j++) {
...
void hmasked_spritetcm(BITMAP *dst, BITMAP *src, BITMAP *mask, int x, int y)
{
	...
	case 24 :
		...
		for (i = 0; i < h; i++) {
			...
			for (j = 0; j < w; j++) {
				if ((ucm = getb24(*((unsigned short*)cm))) == 255 || (uls = *((unsigned long *)cs)) == col){
					cm += 3;
					cd += 3;
					cs += 3;
					continue;
