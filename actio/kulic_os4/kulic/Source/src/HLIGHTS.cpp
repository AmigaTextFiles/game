...
void hdraw_light_sprite(BITMAP *dst, BITMAP *src, int x, int y){
	...
	case 16 :
		...
		for (i = 0; i < h; i++) {
			sr = src->line[sy + i] + sx;
			l  = (unsigned long*)dst->line[dy + i] + dx/2;
			for (j = 0; j < w/2; j++) {
	...
	case 32:
		...
		for (i = 0; i < h; i++) {
			sr = src->line[sy + i] + sx;
			l   = (unsigned long *) dst->line[dy + i] + dx;
			for (j = 0; j < w; j++) {
				if (*sr != 0 && *l != 0 && *l != col) {
					l2 = *l;
					alp = ((*sr)+64);
...
void hdraw_light_spritetcm(BITMAP *dst, BITMAP *src, int x, int y){
	...
	case 32:
		col = makecol32(255, 255, 255);
		for (i = 0; i < h; i++) {
			sr32 = (unsigned long *) src->line[sy + i] + sx;
			l    = (unsigned long *) dst->line[dy + i] + dx;
			for (j = 0; j < w; j++) {
...
void hdraw_shadow_spritetcm(BITMAP *dst, BITMAP *src, int x, int y){
	...
	case 24 :
		...
		for (i = 0; i < h; i++) {
			...
			for (j = 0; j < w; j++) {
				l2   = (*((unsigned long*) c) & 0xFFFFFF);
				sr24 = (*((unsigned long*) c2) & 0xFFFFFF);
				if (sr24 != 0 && l2 != 0) {
					l =     (unsigned long*) c;
					alpr = (getr24(sr24)+64);
					alpg = (getg24(sr24)+64);
