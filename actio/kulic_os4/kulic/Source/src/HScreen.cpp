...
bool HScreen::Init(int w, int h, int bpp, int fliptype, int card, bool aabuffer)
{
	...
	// chnges resolution
// CHQ_BRANCH: This stuff doesn't work on OS4
/*#ifdef _DEBUG
	set_color_depth(32);
...
//	if(key[KEY_C] && key[KEY_ALT]) SaveScreen(NULL);
	...
	if (m_fps_draw) 
		textprintf(m_back, font, m_w-80, 25, makecol(200,200,200), "%.2f FPS",m_fps);
...
void HScreen::FadeDownTruecolor()
{
	...
	acquire_screen();
	// CHQ_BRANCH: Flipping video pages doesn't make screen == video_page!
	//blit(screen, tmp, 0, 0, 0, 0, 640, 480); // nemuzeme blitnout
	blit(GetVisible(), tmp, 0, 0, 0, 0, 640, 480); // nemuzeme blitnout
	release_screen();
