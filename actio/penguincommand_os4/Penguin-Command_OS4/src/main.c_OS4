...
#ifdef AMIGA
void ReadToolTypes(struct WBStartup *wbmsg)
{
  struct WBArg *wba;
  ...
  if (dob = GetDiskObject(wba->wa_Name)) {
    if (FindToolType(dob->do_ToolTypes,"WINDOWED"))
      fullscreen = 0;
    if (FindToolType(dob->do_ToolTypes,"NOSOUND"))
      sound = 0;
    if (FindToolType(dob->do_ToolTypes,"LEFTHANDEDMOUSE"))
      LeftHandedMouse = 1;
    FreeDiskObject(dob);
  }
}
...
//    if ( SDL_Init(SDL_INIT_AUDIO ) < 0 ) ComplainAndExit();
    ...
    if (!(IconBase = OpenLibrary("icon.library",36)))
        exit(10);
    atexit(AmigaCleanup);
    if (argc == 0)  /* start from workbench, parse tool types */
