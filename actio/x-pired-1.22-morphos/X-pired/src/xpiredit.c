...
#ifdef __MORPHOS__
const char *version_tag = "$VER: X-pir-Ed-It 1.2 (06.09.2006)";
#endif
...
const SDL_VideoInfo *info;
	...
	if(FULLSCR){
#ifdef __MORPHOS__
		videoflags = SDL_SRCALPHA | SDL_FULLSCREEN;
#else
		...
		if ((S[0]!='\0')&&(S[0]!='#')){
			if ((S[0]=='>')&&(S[strlen(S)-1]=='-')){
				...
				for (J=9;J<=strlen(S);J++)
					Lvl[L].Pw[J-9]=S[J];
			}else if (!strncmp(S,"deadline=",5)){
				for (J=9;J<=strlen(S);J++)
					T[J-9]=S[J];
				Lvl[L].DL=myatoi(T);
			}else if (!strncmp(S,"background=",5)){
				for (J=11;J<=strlen(S);J++)
					T[J-11]=S[J];
				strcpy(Lvl[L].BgFile,T);
			}else if ((I<20)&&(strlen(S)>=20*7)&&(S[0]==',')){
				for (J=0;J<20*7;J+=7){
...
int I,J,C=1,Akt=0;
	Fls[0][0]='\0';
#ifdef __MORPHOS__
	F=fopen("Progdir:bgimages.txt","r");
#else
...
char buf[255];
  ...
  sprintf(ConfFName,"%sxpired.cfg",GAME_HOME);

  if(!LoadConfig(ConfFName))
  {
#ifdef __MORPHOS__
    sprintf(buf,"%sxpired.cfg",SHARE_DIR);
#else
...
#ifdef __MORPHOS__
  sprintf(LFile,"%sxpired.lvl",SHARE_DIR);
#else
...
#ifdef __MORPHOS__
  sprintf(buf,"%simg/icon.bmp",SHARE_DIR);
#else
