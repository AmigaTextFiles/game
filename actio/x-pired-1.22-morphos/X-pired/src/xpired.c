...
#ifdef __MORPHOS__
const char *version_tag = "$VER: X-pired 1.22 (06.09.2006)";
#endif
...
#ifdef __MORPHOS__
	sprintf(LevelF,"%sxpired.lvl",SHARE_DIR);
#else
...
// load demo
	sprintf(buf,"%sxpired.dmo",GAME_HOME);

  if(!LoadDemo(buf)){
#ifdef __MORPHOS__
		sprintf(buf,"%sxpired.dmo",SHARE_DIR);
#else
...
// load config
  ...
  if (Sound)
#ifdef __MORPHOS__
    if ( Mix_OpenAudio(22050, AUDIO_S16MSB, 1, 1024) < 0 ){
#else
