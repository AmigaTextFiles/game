...
// File Chooser
bool CUIFileChooser::SendChildMessage(int message,int p1,int p2,SDL_Surface *si)
{
	switch (message)
	{
	case UIEV_DRAW:
	...
	case INTERNEV_REDRAW:
		{
			...
			{		
				...
				textzone->SendMessage(INTERNEV_SETTEXT,(int)temp,0,si);
			}
			else
			{
				textzone->SendMessage(INTERNEV_SETTEXT,(int)strings.GetElementN(GetVal()),0,si);
			}
			return true;
		}
	default :
