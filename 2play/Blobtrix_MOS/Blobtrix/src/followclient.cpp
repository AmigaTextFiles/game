...
bool followclient::SetNick(char *nick) {
	if (strlen(nick)<=20) {
		return Player.SetNick(nick);
	} else return 0;
}

bool followclient::connect(IPaddress *address, char *errmsg[]) {
	...
	char data[1024];
	sprintf (data, "FOLLOW %s", Config->nick);
	...
	while (!ready) {
		...
		while (SDL_GetTicks() >= nextupdate) {
			...
			if (alone) {
				...
				if (SDL_GetTicks()-lastping > 3000) {
					// tell what happened

					fprintf (stderr, "Server hasn't pinged us for 3 seconds - disconnecting\n");
			...
			if (SDLNet_UDP_Recv(sock, packet) ) {
				if (packet->address.host == servaddress.host && packet->address.port == servaddress.port) {
					...
					if (variable::str_splitcompare( (char*)packet->data, 0, "YOURNICK") ) {
					   char *playernick = variable::str_split( (char*)packet->data, ' ', 1);
					   if (playernick) {
					      Player.SetNick(playernick);
					      fprintf (stderr, "Set player nick to '%s'\n", playernick);
					      delete[] playernick;
					   }
					}
					if (variable::str_splitcompare( (char*)packet->data, 0, "YOUARENONE") ) {
					   fprintf (stderr, "Don't want to follow zero players. Disconnecting.\n");
					   ready=true;
					}
					#ifndef NOSOUND
					if (variable::str_splitcompare( (char*)packet->data, 0, "PLAYSOUND") ) {
