...
double compat::scraptail(double p) {
	unsigned char *q = (unsigned char*)&p;
	*q = (*q) & 0xF0;
	return *(double*)q;
}

void compat::debug(FILE *f, const char *frm, ...) {
#if DEBUG == 1
	va_list ap;
	va_start(ap, frm);
	vfprintf (f, frm, ap);
...
int compat::Str2Int(char *str, int from) { // unused I hope
	int luku=0;
	int len=strlen(str);
	bool found=false;
	int minus=0;
...
Uint32 compat::Str2IP(char *str, int from) { // unused I hope
	int i=from;
	int len = strlen(str);
	int ip[4]={0};
	int cip=0;
...
char * compat::IP2Str(Uint32 ip) { // remember to delete! unused I hope
...
void compat::ToFile_Int(FILE *stream, Uint32 data) {
	Uint32 p = SWAP32(data);
	fwrite (&p, sizeof(Uint32), 1, stream);
}
void compat::ToFile_SInt(FILE *stream, int data) {
	int p = SWAP32(data);
	fwrite (&p, sizeof(int), 1, stream);
}
...
void compat::ToFile_ShortInt(FILE *stream, Uint16 data) {
	Uint16 p = SWAP16(data);
	fwrite (&p, sizeof(Uint16), 1, stream);
}
...
int compat::FromFile_ShortInt_EOF(FILE *stream, Uint16 *data) {
	int i = fread (data, sizeof(Uint16), 1, stream);
	*data = SWAP16(*data);
	return i;
}

char *compat::str_split(char *str, char ch, int num) { // unused I hope
	int i=0;
	int len=strlen(str);
	int n=0;
...
bool compat::str_splitcompare(char *str, int num, char *str2) { // unused I hope
	bool ret;
...
int compat::str_splitgetint(char *str, char ch, int num) { // unused I hope
	int r=-1;
...
char *compat::str_fullsplit(char *str, char ch, int num) { // unused I hope
	int i=0;
	int len=strlen(str);
	int n=0;
...
string compat::getstring(FILE *fp) {
...
void compat::putstring(FILE *fp, string str) {
