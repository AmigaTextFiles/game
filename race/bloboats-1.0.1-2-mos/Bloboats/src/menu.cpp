	...
	if (replay) { // record playing screen
		...
		while (state!=2) {
			...
			if (!drawn) {
				...
				Font.WriteString( (char*)ghostname.c_str(), Window.width/2-15*ghostname.size(), Window.height/2-70, 30);
...
void menu::resolution() {
	...
	menutext item[items]={
		...
		menutext("1024 X 768 "),
		menutext("1280 X 960 "),
		menutext("1280 X 1024"),
		menutext(""),
		menutext("BACK")
	};


	int selection=Config.resolution;
	...
	while (!ready) {
		...
		while (SDL_GetTicks() > nextframe) {
			...
			if (Keyboard.Pressed(SDLK_RETURN)) {
				...
				switch(selection) {
					case 1: // 640 x 480
						if (!Window.OpenWindow(640, 480, 32, flags) ) {
					...
					break;
					case 2: // 800 x 600
						if (!Window.OpenWindow(800, 600, 32, flags) ) {
							fprintf (stderr, "Couldn't set screenmode: %s\n", SDL_GetError());
							exit(1);
						}
						Config.resolution=2;
					...
					case 3: // 1024 x 768
						...
						if (!Window.OpenWindow(1024, 768, 32, flags) ) {
							fprintf (stderr, "Couldn't set screenmode: %s\n", SDL_GetError());
							exit(1);
						}
						Config.resolution=3;
					...
					break;
					case 4: // 1280 x 960
						if (!Window.OpenWindow(1280, 960, 32, flags) ) {
							fprintf (stderr, "Couldn't set screenmode: %s\n", SDL_GetError());
							exit(1);
						}
						Config.resolution=4;
					...
					break;
					case 5: // 1280 x 1024
						if (!Window.OpenWindow(1280, 1024, 32, flags) ) {
							fprintf (stderr, "Couldn't set screenmode: %s\n", SDL_GetError());
							exit(1);
						}
						Config.resolution=5;
