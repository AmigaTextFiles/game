...
{
  return MainMaterial ? MainMaterial->GetColor() : 0;
}

truth object::AddRustLevelDescription(festring& String, truth Articled) const
{
  return MainMaterial && MainMaterial->AddRustLevelDescription(String, Articled);
}
...
{
  return MainMaterial ? MainMaterial->GetAlpha() : 0;
}
...
{
  return (MainMaterial && MainMaterial->IsSparkling()) ? SPARKLING_A : 0;
}
...
{
  return MainMaterial ? MainMaterial->GetRustData() : 0;
}
