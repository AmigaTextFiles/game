...
RoomGraphics(r_buildEntry, "Foyer", "", GUILD_MAP_GROUP, 130, 53, drawGuild).
...
Scenery(r_buildEntry,
    ...
    "tree;palm."
    "door;small,wooden").
define tp_bguild proc enterBuild()status:
    list action la;

    la := Me()@p_pEnterActions;
    /* Could be nil on a directed machine like Packrat */
    if la ~= nil and FindElement(la, checkBuildButton) = -1 then
	AddTail(la, checkBuildButton);
	checkBuildButton();
    fi;
    continue
corp;
AddWestChecker(r_sw2, enterBuild, false).
AddEnterChecker(r_sw2, enterBuild, false).
...
RoomGraphics(r_buildHall1, "East", "Hallway", GUILD_MAP_GROUP, 90, 53,
	     drawGuild).
...
RoomGraphics(r_buildHall2, "West", "Hallway", GUILD_MAP_GROUP, 61, 53,
	     drawGuild).
...
RoomGraphics(r_buildLibrary, "Library", "", GUILD_MAP_GROUP, 21, 53,
	     drawGuild).
...
"  @object { sitin | siton | liein | lieon } <object-symbol> <count> - E.g.\n"
"\tto make the couch have room for two people to sit on\n"
...
"\tconditional is true if the character/room/object has/contains an\n"
"\tobject so flagged.\n"
"  random <chance> - the conditional is true one time out of <chance>.".
...
RoomGraphics(r_buildOffice1, "British", "Office", GUILD_MAP_GROUP, 89, 35,
	     drawGuild).
...
RoomGraphics(r_buildOffice2, "Modern", "Office", GUILD_MAP_GROUP, 89, 73,
	     drawGuild).
...
RoomGraphics(r_buildOffice3, "African", "Office", GUILD_MAP_GROUP, 60, 35,
	     drawGuild).
...
RoomGraphics(r_buildOffice4, "Western", "Office", GUILD_MAP_GROUP, 60, 73,
	     drawGuild).
...
RoomGraphics(r_buildStairs1, "Top of", "Stairs", GUILD_MAP_GROUP, 130, 18,
	     drawGuild).
...
define tp_bguild proc drawStairsBottom()void:
    ...
    GSetPen(nil, C_TAN);
    GAMove(nil, 65, 41);
    GRectangle(nil, 29, 18, false);
    GSetPen(nil, C_BLACK);
    GAMove(nil, 66, 42);
    GRectangle(nil, 27, 16, true);
    GSetPen(nil, C_BROWN);
    GAMove(nil, 75, 59);
    HorizontalDoor();
    DrawUpArrow(C_GOLD);
...
define tp_bguild proc questPhotoDesc()string:
    ...
    "Bring me a photograph of " + FormatName(Me()@p_pName) + "'s whatzit."
corp;
...
define tp_bguild proc questPhotoGive()status:
	...
	"whatzit;photograph,of," + FormatName(TrueMe()@p_pName) + "'s") ~= -1
    then
