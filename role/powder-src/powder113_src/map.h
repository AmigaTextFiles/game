...
public:
    ...
    void		 throwItem(int x, int y, int dx, int dy,
				ITEM *item, ITEM *launcher, MOB *thrower,
...
protected:
    ...
    // the building code.
    bool		 isTileWall(int tile);
    bool		 isMapWall(int x, int y);
    bool		 isMapExit(int x, int y);
    bool		 isRoomExit(const ROOM &room, int x, int y);
    bool		 isMapPathway(int x, int y);
