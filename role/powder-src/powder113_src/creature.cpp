...
//	mob->myMagicDie = 100;
    ...
    // mob->myHitDie = 38;
#endif
    if (glbStressTest && definition == MOB_AVATAR)
    {
	if (rand_chance(80))
	{
	    mob->buildAscensionKit();
...
{
    bool		doublesize = getSize() >= SIZE_GARGANTUAN;
    ...
    // If mob is huge, we gotta check rest of the mob..
    if (doublesize &&
...
MOB::move(int nx, int ny, bool interlevel)
{
	...
	{
	    if (!glb_squaredefs[glbCurLevel->getTile(nx, ny)].isforest)
	    {
...
MOB::doHeartbeat()
{
	...
	if (hasIntrinsic(INTRINSIC_REGENERATION))
	{
	    ...
	    if (allowregen ||
		(intrinsicFromWhat(INTRINSIC_NOREGEN) & INTRINSIC_FROM_TEMPORARY))
	    {
...
MOB::receiveWeaponEnchant(int bonus, bool shudder)
{
    ...
    {
	if (weapon->getAttackName() == ATTACK_MISUSED)
	{
	    // Not a real weapon...
...
MOB::formatToString(const char *str, const MOB *self, const ITEM *selfitem, const MOB *mob, const ITEM *item, BUF b1, BUF b2, BUF b3)
{
	...
	if (*str == '%')
	{
		...
		if (targettype == 0)
		{
			...
			buf.strcat(selfitem->getName(true, false, false));
		    else
		...
		else if (targettype == 1)
		{
		    ...
		    buf.strcat(item->getName(true, false, false));
	    }
	    else if (*str == 'B')
	    {
		...
		else if (targettype == 1)
		{
		    ...
		    buf.strcat(item->getName(true, false, false, true));
	    }
	    else if (*str == 'u')
	    {
		str++;
		if (targettype == 0)
		{
			...
			buf.strcat(selfitem->getName(false, false, false));
		    else
			buf.strcat("something");
		}
		else if (targettype == 1)
		{
		    ...
		    buf.strcat(item->getName(false, false, false));
	    }
	    else if (*str == 'R')
	    {
		str++;
		if (targettype == 0)
		{
			...
			buf.strcat(gram_makepossessive(selfitem->getName(true, false, false)));
		    else
		    ...
		    buf.strcat(gram_makepossessive(item->getName(true, false, false)));
	    }
	    else if (*str == 'r')
	    {
		str++;
...
MOB::determineClassiness()
{
    ...
    if (wizard || ranger || fighter || cleric || necro || barb)
    {
	item = getEquippedItem(ITEMSLOT_BODY);
	if (item)
	{
    ...
    if (wizard || ranger || fighter || cleric || barb)
    {
	item = getEquippedItem(ITEMSLOT_FEET);
	if (item)
	{
...
void
MOB::setTimedIntrinsic(MOB *inflictor, INTRINSIC_NAMES intrinsic, int turns)
{
    INTRINSIC_COUNTER	*counter;
...
MOB::zapCallbackStatic(int x, int y, bool final, void *data)
{
	...
	switch (tile)
	{
		...
		glbCurLevel->reportMessage(
			...
			x, y);
		glbCurLevel->setTile(x, y, SQUARE_DOOR);
		return false;
	    }

	    case SQUARE_VIEWPORT:
	    {
		glbCurLevel->reportMessage("The glass wall shatters.", x, y);
