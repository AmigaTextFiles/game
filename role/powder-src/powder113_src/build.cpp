...
bool
MAP::isRoomExit(const ROOM &room, int x, int y)
{
...
MAP::buildDistance(int px, int py)
{
    ...
    memset(ourDistanceMap, 0xff, MAP_WIDTH * MAP_HEIGHT * 16);
...
MAP::populateRoom(const ROOM &room)
{
	...
	roomToMapCoords(x, y, room, pt->x, pt->y);
	mySignList->append((SIGNPOST_NAMES) pt->data, x, y);
    }
}
