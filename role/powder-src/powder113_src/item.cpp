...
ITEM::unpetrify(MAP *map, MOB *unpetrifier)
{
	...
	formatAndReport("%U <collapse> into a meatball.");
	myDefinition = ITEM_MEATBALL;
	return true;
    }
...
ITEM::create(ITEM_NAMES definition, bool allowartifact, bool nocurse)
{
	...
	item->isJacket() || item->getAttackName() != ATTACK_MISUSED)
    {
...
BUF
ITEM::getName(bool article, bool shortname, bool neverpronoun, bool idclass) const
{
    BUF			 rawname;
	...
	return gram_createcount(rawname, getStackCount(), article);
    }
    else
    {
	...
	if (article && isArtifact() && getStackCount() == 1)
	{
	    buf.strcpy("the ");
	...
	{
	    buf = gram_createcount(nonarticle, getStackCount(), article);
	}
...
ITEM::actionZap(MOB *zapper, int dx, int dy, int dz)
{
	...
	case MAGICTYPE_WAND:
	{
		...
		case WAND_INVISIBLE:
		{
		    ...
		    tile = glbCurLevel->getTile(zx+dx, zy+dy);
	...
	{
	    // If we have not yet ided it, spam the revalation
	    if (MOB::getAvatar() && !isKnownClass())
	    {
		MOB::getAvatar()->formatAndReport("%U <learn> that %IU <I:be> %Ii.", this);
	    }
