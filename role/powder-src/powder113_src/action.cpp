...
MOB::actionEat(int ix, int iy)
{
	...
	else if (canDigest(item))
	{
		     ...
		     intrinsic = (INTRINSIC_NAMES) (intrinsic+1))
		{
			...
			{
			    setTimedIntrinsic(this, intrinsic,
					    foodval + rand_choice(foodval));
			}
		    }
		}
...
MOB::actionDip(int potionx, int potiony, int ix, int iy)
{
    ...
    if (!item)
    {
	formatAndReport("%U <dip> nothing into %IU.  Suprisingly, nothing happens!", item);
	return false;
    }
...
MOB::actionCast(SPELL_NAMES spell, int dx, int dy, int dz)
{
	...
	case SPELL_BLIZZARD:
	{
	    ...
	    // Create the blizzard.
	    glbCurLevel->reportMessage("A large blizzard fills the air with sleet and slow!", tx, ty);
