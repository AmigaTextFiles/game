...
world_new(void)
{
  ...
  wp->player_id = -1;
  wp->ancient_dragon_id = -1;
  wp->mature_dragon_id = -1;
  wp->baby_dragon_id = -1;
  wp->player_sheath = NULL;
  wp->player_weapon_slot = -1;
  wp->last_kill_type = -1;
  wp->player_z_max = 1;
  ...
  wp->log_head = 0;
  wp->pcache = NULL;
  wp->step_map_result = NULL;
  wp->step_map_queue = NULL;
  wp->creature_effect_func = NULL;
  wp->weapon_effect_func = NULL;
  ...
  wp->itm_size = 4;
  wp->itm = (item **) malloc(sizeof(item *) * wp->itm_size);
  if (wp->itm == NULL)
  {
    fprintf(stderr, "world_new: malloc(wp->itm) failed\n");
    world_delete(wp);
    wp = NULL;
    return NULL;
  }
  ...
  wp->player_sheath = (int *) malloc(sizeof(int) * 4);
  if (wp->player_sheath == NULL)
  {
    fprintf(stderr, "world_new: malloc(wp->player_sheath) failed\n");
    world_delete(wp);
    wp = NULL;
    return NULL;
  }
  ...
  wp->mode = (int *) malloc(sizeof(int) * MODE_SIZE);
  if (wp->mode == NULL)
  {
  ...
  wp->pcache = player_cache_new(PLAYER_SIGHT_RANGE);
  if (wp->pcache == NULL)
  {
  ...
  wp->step_map_result = array2_new(wp->size_x, wp->size_y);
  if (wp->step_map_result == NULL)
  {
    fprintf(stderr, "world_new: array2_new(step_map_result) failed\n");
    world_delete(wp);
    wp = NULL;
    return NULL;
  }

  wp->step_map_queue = array3_new(2, wp->size_x * wp->size_y, 2);
  if (wp->step_map_queue == NULL)
  {
    fprintf(stderr, "world_new: array3_new(step_map_queue) failed\n");
    world_delete(wp);
    wp = NULL;
    return NULL;
  }

  wp->creature_effect_func
    = (int (**)(struct _world *, combat_judge *, int))
    malloc(sizeof(int (*)(struct _world *, combat_judge *, int))
           * NUM_CREATURE);
  if (wp->creature_effect_func == NULL)
  {
    fprintf(stderr, "world_new: malloc(creature_effect_func) failed\n");
    world_delete(wp);
    wp = NULL;
    return NULL;
  }
  for (i = 0; i < NUM_CREATURE; i++)
    wp->creature_effect_func[i] = NULL;
  creature_effect_init(wp);

  wp->weapon_effect_func
    = (int (**)(struct _world *, combat_judge *, int))
    malloc(sizeof(int (*)(struct _world *, combat_judge *, int))
           * NUM_WEAPON);
  if (wp->weapon_effect_func == NULL)
  {
    fprintf(stderr, "world_new: malloc(weapon_effect_func) failed\n");
    world_delete(wp);
    wp = NULL;
    return NULL;
  }
  for (i = 0; i < NUM_WEAPON; i++)
    wp->weapon_effect_func[i] = NULL;
  weapon_effect_init(wp);
...
world_delete(world *wp)
{
  ...
  /* wp->step_map_result must be freed before the map size is reset */
    if (wp->step_map_result != NULL)
  {
    array2_delete(wp->step_map_result, wp->size_x, wp->size_y);
    wp->step_map_result = NULL;
  }

  /* wp->step_map_queue must be freed before the map size is reset */
  if (wp->step_map_queue != NULL)
  {
    array3_delete(wp->step_map_queue, 2, wp->size_x * wp->size_y, 2);
    wp->step_map_queue = NULL;
  }
  ...
  if (wp->creature_effect_func != NULL)
  {
    free(wp->creature_effect_func);
    wp->creature_effect_func = NULL;
  }

  if (wp->weapon_effect_func != NULL)
  {
    free(wp->weapon_effect_func);
    wp->weapon_effect_func = NULL;
  }
...
world_init_curses(world *wp)
{
  ...
  init_pair(CPAIR_YELLOW, COLOR_YELLOW, COLOR_BLACK);
  init_pair(CPAIR_BLUE, COLOR_BLUE, COLOR_BLACK);
  init_pair(CPAIR_MAGENTA, COLOR_MAGENTA, COLOR_BLACK);
  init_pair(CPAIR_CYAN, COLOR_CYAN, COLOR_BLACK);
  init_pair(CPAIR_WHITE, COLOR_WHITE, COLOR_BLACK);
  init_pair(CPAIR_BLACK, COLOR_BLACK, COLOR_BLACK);
...
world_expand_itm(world *wp)
{
  ...
  if (wp == NULL)
  {
    fprintf(stderr, "world_expand_itm: wp is NULL\n");
    return 1;
  }
  if (wp->itm_size <= 0)
  {
    fprintf(stderr, "world_expand_itm: wp->itm_size is non-positive (%d)\n",
            wp->itm_size);
    return 1;
  }

  itm_size_temp = wp->itm_size * 2;
  if (itm_size_temp <= 0)
  {
    fprintf(stderr, "world_expand_itm: itm_size overflowed\n");
    return 1;
  }
  itm_temp = (item **) realloc(wp->itm,
                               sizeof(item *) * itm_size_temp);
  if (itm_temp == NULL)
  {
    fprintf(stderr, "world_expand_itm: realloc failed\n");
    return 1;
  }

  wp->itm = itm_temp;
  for (i = wp->itm_size; i < itm_size_temp; i++)
    wp->itm[i] = NULL;
  wp->itm_size = itm_size_temp;
