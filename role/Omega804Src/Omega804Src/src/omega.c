...
//char *argv[];
{
  ...
  if (argc==2) {
#ifdef AMIGA
  	if (wbstartup != 0) sprintf(savestr, "%s%s", "PROGDIR:", argv[1]);
    else strcpy(savestr,argv[1]);
#endif
    ok = restore_game(savestr);
    if (! ok) {
      endgraf();
      printf("Trouble with the save file. Try again, please!\n");
      exit(0);
    }
  ...
  int continuing, count;
	...
	if (wbstartup != 1)
		...
		if (savefilename != NULL)
			{
			strcpy(buf, savefilename);
			own_argv[1] = (char *)buf;	own_argc = 2;
			}
		}
//	else printf("wblaunch\n");
#endif
  ...
  if (filecheck() == 0) { amiga_close(); exit(0); }
...
#ifdef AMIGA
	if (wbstartup != 1) continuing = game_restore(own_argc,own_argv);
	else continuing = game_restore(argc,argv);
#else
