...
#define SAVE_FILE_NAME "kusemono-save.txt"
#define OLD_SAVE_FILE_NAME "kusemono-save-old.txt"
#define TERMINATE_MARKER 0xdead
/* history of SAVE_FILE_VERSION:
 * 1:
 * 0: Sat, 12 Mar 2011 release (the 7DRL version)
 */
...
load_creature(world *wp, int_loader *ilp)
{
  ...
  for (i = 0; i < num_creature; i++)
  {
    ...
    who->successive_run = int_loader_get(ilp);
    who->attitude = int_loader_get(ilp);
    who->timer_attitude = int_loader_get(ilp);
    who->enchant_type = int_loader_get(ilp);
    who->enchant_duration = int_loader_get(ilp);
    for (j = 0; j < ACT_SIZE; j++)
...
save_game(world *wp)
{
  ...
  if (fprintf(fp, "# kusemono save file\n") < 0)
    error_found = 1;
  ...
  if (fprintf(fp, "# global_decision_flag\n") < 0)
    error_found = 1;
  if (fprintf(fp, "%d\n", wp->global_decision_flag) < 0)
    error_found = 1;

  if (fprintf(fp, "# player_shield\n") < 0)
    error_found = 1;
  if (fprintf(fp, "%d\n", wp->player_shield) < 0)
    error_found = 1;

  if (fprintf(fp, "# player_is_damaged\n") < 0)
    error_found = 1;
...
load_game(world *wp)
{
  ...
  wp->queen_id = int_loader_get(ilp);
  ...
  wp->global_decision_flag = int_loader_get(ilp);
  wp->player_shield = int_loader_get(ilp);
  wp->player_is_damaged = int_loader_get(ilp);
