...
static int monster_set_path(world *wp, creature *who,
                            int dest_z, int dest_x, int dest_y);
static int lost_monster_forget_path(world *wp);
                 ...
                 int dest_z, int dest_x, int dest_y)
{
  ...
  if (who->path_x[who->path_num - 1] != dest_x)
    return 0;
  if (who->path_x[who->path_num - 1] != dest_y)
    return 0;
                 ...
                 int dest_z, int dest_x, int dest_y)
{
  ...
  step_map = array2_new(wp->size_x, wp->size_y);
  if (step_map == NULL)
  {
    fprintf(stderr, "monster_set_path_1: array2_new failed\n");
    return 2;
  }
...
static int
lost_monster_forget_path(world *wp)
{
  int i;
  ...
  for (i = 0; i < wp->cr_size; i++)
  {
    ...
    if (i == wp->player_id)
      continue;
    if (wp->cr[i]->attitude != ATTITUDE_MONSTER_LOST)
      continue;
...
monster_wander(world *wp, creature *who)
{
  ...
  if (step_map == NULL)
  {
    fprintf(stderr, "monster_wander: array2_new failed\n");
    return 1;
  }
...
action_decide_monster_1(world *wp, creature *who)
{
  ...
  /* if we see the player, be aggressive */
  be_aggressive = 0;
  ...
  /* we may miss the player if we are not prepared */
  switch (who->attitude)
  {
  case ATTITUDE_MONSTER_UNINTERESTED:
  case ATTITUDE_MONSTER_CONCERNED:
  ...
  if (be_aggressive)
  {
    ...
    {
    case ATTITUDE_MONSTER_UNINTERESTED:
      /* immediately become active if the player is very close */
      if (clearness < 3)
      {
        monster_set_attitude(wp, who, ATTITUDE_MONSTER_CONCERNED);
        /* who->timer_attitude will be decremented this turn */
        who->timer_attitude = 3 + 1;
        /* send one of us to find what is happening */
        if (((wp->global_decision_flag & GLOBAL_DSCN_NO_MORE_SEARCH) == 0)
            && ((who->act[0] == 0)
                || (!action_check(wp, who->id, who->act))))
        {
          if (monster_set_path(wp, who,
                               player->z, player->x, player->y) == 0)
          {
            wp->global_decision_flag |= GLOBAL_DSCN_NO_MORE_SEARCH;
            /* forget as soon as we reach the destination */
            who->timer_attitude = 1;
            /* skip this turn */
            monster_do_nothing(wp, who);
            return 0;
          }
        ...
        {
          /* stop wandering, stay here and wait for the report */
          who->path_num = 0;
          who->path_now = 0;
        }
        break;
    ...
    /* if we have found the player, update the last known position */
  ...
  case ATTITUDE_MONSTER_UNINTERESTED:
    ...
    if (who->timer_attitude <= 0)
    {
      ...
      /* if we are far from the home position, return
  ...
  /* if we are lost, try a random walk */
  if (who->attitude == ATTITUDE_MONSTER_LOST)
  {
    who->act[0] = ACTION_WALK;
    who->act[1] = (rand() % 3) - 1;
    who->act[2] = (rand() % 3) - 1;
    ...
    /* can not walk in this direction, try something else */
    for (i = 0; i < 4; i++)
      who->act[i] = 0;
  }
