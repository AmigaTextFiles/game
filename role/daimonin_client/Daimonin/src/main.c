...
*********************************@þ    {"hp.png", PIC_TYPE_TRANS}, {"sp.png", PIC_TYPE_TRANS}, {"grace.png", PIC_TYPE_TRANS}, {"food.png", PIC_TYPE_TRANS},\    {"cmark_middle.png", PIC_TYPE_TRANS}, {"textwin_scroll.png", PIC_TYPE_DEF*********************************/
...
// Ice END
    ...
    {
// Ice BEGIN
        rt = SendVersion(csocket);
        if (rt == -1)
        {
            sprintf(buf, "SendVersion failed! %d",rt);
            draw_info(buf, COLOR_RED);
            GameStatus = GAME_STATUS_START;
        }
        else
...
//        char tmp[1024];
        ...
        sprintf(tmp, "Couldn't initialize SDL: %s\n", SDL_GetError());
        fprintf(stderr, tmp);
        exit(1);
    }
    atexit(SDL_Quit);
    ...
    */
// Ice BEGIN
    while (!done && GameStatus != GAME_STATUS_QUIT)
    {
// Ice END
        done = Event_PollInputDevice();
        if (GameStatus == GAME_STATUS_QUIT)
        {
           break;
        }
#ifdef INSTALL_SOUND
...
//                sprintf(tmp,"%s read: %d write: %d exc: %d",strerror(errno),FD_ISSET(csocket.fd, &tmp_read),FD_ISSET(csocket.fd, &tmp_write),FD_ISSET(csocket.fd,&tmp_exceptions));
//                fprintf(stderr, tmp);
                maxfd = csocket.fd + 1;
                pollret =  select(maxfd, &tmp_read, &tmp_write, &tmp_exceptions, &timeout);
                if (pollret == -1)
                {
                    sprintf(tmp,"%s read: %d write: %d exc: %d",strerror(errno),FD_ISSET(csocket.fd, &tmp_read),FD_ISSET(csocket.fd, &tmp_write),FD_ISSET(csocket.fd,&tmp_exceptions));
                    fprintf(stderr, tmp);
                    getsockopt(maxfd, SOL_SOCKET, SO_ERROR, (void *) &valopt, &lon);
                    sprintf(tmp, "Got errno %d on selectcall %s .\n", valopt,strerror(valopt));
                    fprintf(stderr, tmp);
                    GameStatus = GAME_STATUS_START;
                }
// Ice END
...
// Ice BEGIN
        if ((GameStatus < GAME_STATUS_REQUEST_FILES) && (GameStatus > GAME_STATUS_META))
