...
// Ice BEGIN
        sprintf(buf,"mb:%d mc:%d ab:%d ID:%d gfx:%d text:%s", mb, mb_clicked, active_button, id,gfxNr,text);
// Ice END
    ...
    for (i = 0; i < ATT_SUM; i++)
    {
        ...
        if (delta)
        {
            ...
            if (delta > 0)
            {
                ...
                {
                    sprintf(tmp, " stat: %d point: %d i:%d",new_character.stats[i],new_character.stat_points,i);
                    fprintf(stderr, tmp);
