...
port)
{
    ...
    unsigned int  oldbufsize, newbufsize = 65535, temp;
  ...
  //  socklen_t buflen = sizeof(int);
    ...
    *socket_temp = socket(PF_INET, SOCK_STREAM, protox->p_proto);
    ...
    if (fcntl(*socket_temp, F_SETFL, O_NONBLOCK) == -1)
    {
...
int write_socket(int fd, unsigned char *buf, int len)
{
    ...
    while (len > 0)
    {
        ...
        {
            /* We got an error */
            LOG(LOG_ERROR, "New socket (fd=%d) write failed -> %s .\n", fd,strerror(errno));
            draw_info("SOCKET ERROR: Server write failed.", COLOR_RED);
            return -1;
        }
        if (amt == 0)
