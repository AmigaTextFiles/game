...
define t_base p_pExitActions CreateActionListProp()$	/* do on player idle */
...
define t_base p_oEmptyChecker CreateActionProp()$
define t_base p_oLookInString CreateStringProp()$
define t_base p_oLookInAction CreateActionProp()$
define t_base p_oNotUnlockString CreateStringProp()$
define t_base p_oNotLockString CreateStringProp()$
define t_base p_oUnlockMeWithChecker CreateActionProp()$
define t_base p_oUnlockWithMeChecker CreateActionProp()$
define t_base p_oLockMeWithChecker CreateActionProp()$
define t_base p_oLockWithMeChecker CreateActionProp()$
define t_base p_oNotLockable CreateBoolProp()$
define t_base p_oLocked CreateBoolProp()$
...
/* kind codes for EditString */

define t_base EDIT_COOKED	0$
define t_base EDIT_RAW		1$
define t_base EDIT_CODE 	2$

/* comparison codes for FindIntOnList */

define t_base COMP_EQUAL		0$
define t_base COMP_NOT_EQUAL		1$
define t_base COMP_LESS_THAN		2$
define t_base COMP_LESS_OR_EQUAL	3$
define t_base COMP_GREATER_THAN 	4$
define t_base COMP_GREATER_OR_EQUAL	5$
...
/* Properties, etc. that control the automated backups and other globally
   settable things. */

define tp_misc GlobalThing CreateThing(nil)$
define tp_misc p_FlushNeeded CreateBoolProp()$
define tp_misc p_BackupInterval CreateIntProp()$

SetThingStatus(GlobalThing, ts_public)$ /* so parseInput can write it */
GlobalThing@p_FlushNeeded := false$
GlobalThing@p_BackupInterval := 0$

define tp_misc p_FreePoses CreateBoolProp()$
GlobalThing@p_FreePoses := false$

define tp_misc p_FreeBuilding CreateBoolProp()$
...
/*
 * CharacterNameS - a standardized form of a specific character name.
 */

define t_base proc utility public CharacterNameS(thing who)string:
    string name;

    name := FormatName(who@p_pName);
    if who@p_pStandard then
	name
    else
	"the " + name
    fi
...
/*
 * CharacterNameG - a standardized form of a generic character name.
 */

define t_base proc utility public CharacterNameG(thing who)string:
    string name;
...
/*
 * Note that none of these routines use 'SetEffectiveToNone'. This is a
 * part of the big security hole involving SysAdmin. SysAdmin should never
 * be used to 'play' on a MUD where untrusted people have been given
 * builder, apprentice or wizard status. Those characters can write code
 * and attach it to rooms, objects, etc. That code, if triggered by
 * SysAdmin, will run as SysAdmin. For this reason, SysAdmin should *never*
 * go into the PlayPen!.
 * Adding SetEffectiveToNone is a problem, because it prevents these
 * routines from doing entirely legitimate activities on behalf of the
 * active character. Character idle code for build description routines
 * is an example. Even SysAdmin can't shut down correctly in that case!
 */
 ...
 *	This routine specifically handles the case of the action
 *	deleting itself from the list.
 */
...
define t_base proc utility public DirImage(int dir)property string:
    case dir
    incase D_NORTH:
	p_rNorthImage
    incase D_NORTHEAST:
...
define t_base proc utility AddRoomLightChecker(thing room; action a;
	bool front)void:
    addChecker(room, p_rLightChecks, a, front);
corp;

define t_base proc utility DelRoomLightChecker(thing room; action a)void:
    delChecker(room, p_rLightChecks, a);
corp;
...
define t_base proc utility DoRoomLightChecks(thing room)status:
    DoChecks(room@p_rLightChecks)
corp;
...
define t_base proc utility public SetupMachine(thing machine)void:
    ...
    if machine@p_pEnterActions = nil then
	machine@p_pEnterActions := CreateActionList();
    fi;
    if machine@p_pExitActions = nil then
	machine@p_pExitActions := CreateActionList();
    fi;
corp;
