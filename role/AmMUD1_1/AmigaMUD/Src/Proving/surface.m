...
define tp_proving o_longSword WeaponSell(r_armoury, "sword;long.longsword",
...
define tp_proving o_twoHandedSword WeaponSell(r_armoury,
    "sword;two-handed.sword;two,handed,2-handed,2handed,twohanded",
...
WeaponSell(r_armoury, "axe,ax;war.waraxe,warax",
...
WeaponSell(r_armoury, "poleaxe,poleax.axe,ax;pole",
...
WeaponSell(r_armoury, "mail;chain.chainmail,chain-mail",
...
/* Do not call it 'plate' - that conflicts with the floor plate. */
WeaponSell(r_armoury, "mail;plate.platemail,plate-mail",
...
define tp_proving o_woodenShield WeaponSell(r_armoury,
    ...
    "The wooden shield is made from many strips of wood glued together to "
...
define tp_surface r_healer CreateThing(r_indoors)$
...
Scenery(r_healer,
    "tent;fairly,large."
    "area;curtained,off."
    "proprietor,man,healer;friendly-looking,friendly,looking."
    "robe;loose,brown."
    "sandal."
    "healing."
    "shelves,shelf,wall."
...
HealSell(r_healer, "heal;great.great.heal;major", 500, 500)$
r_healer@p_rBuyAction := HealingBuy$

/* and some more locations to play around in */

define tp_surface r_road1 CreateThing(r_provingRoad)$
...
define tp_surface PR_CROSSROADS_ID NextEffectId()$
define tp_surface proc crossRoadsDraw()void:
...
define tp_surface r_road2 CreateThing(r_provingRoad)$
...
Scenery(r_road2, "trail.path")$
define tp_surface PR_ROAD2_ID NextEffectId()$
define tp_surface proc road2Draw()void:
	...
	Else(nil);
	    ...
	    GAMove(nil, 0.0625, 0.0);
	    GRectangle(nil, 0.37, 0.33, true);
	    GSetPen(nil, C_BRICK_RED);
	    GAMove(nil, 0.234, 0.33);
	    HorizontalDoor();
	Fi(nil);
	EndEffect();
    fi;
...
define tp_surface r_provingStore CreateThing(r_indoors)$
...
WeaponSell(r_provingStore, "wrappings;fur",
    ...
    "weapons, teeth and claws. They also provide an ideal nesting place for "
...
define tp_surface o_flint AddForSale(r_provingStore,
    "lighter;flint.flint",
...
define tp_surface proc oilLampTick(thing lamp)void:
    int newState;
...
corp;
define tp_surface proc oilLampOn()status:
    thing lamp;
...
define tp_surface proc oilLampOff()status:
    status st;
...
define tp_surface proc oilLampPutIn(thing lamp, container)status:
...
define tp_surface proc oilLampLookIn(thing lamp)string:
    int timeLeft;
...
define tp_surface proc oilLampEmptyChecker(thing lamp)status:
    lamp@p_oState := 0;
    continue
corp;
...
define tp_surface proc refillBuy()status:
    thing me, lamp;
	...
	if lamp@p_oState = o_oilLamp@p_oState then
	    /* It's already full! */
	    Print("The shop owner smiles slyly and says "
		"\"There you are - all filled up!\"\n");
	else
...
define tp_surface o_oilCan AddForSale(r_provingStore,
    "oil;can,of.oilcan.oil-can.can;oil",
    "The can of oil is a small tin container with a pouring spout.",
    100, nil)$
o_oilCan@p_oState := 5$
define tp_surface proc oilCanDrop(thing can)status:
...
define tp_surface proc oilLampFillWith(thing lamp, source)status:
    int oilLeft;
...
define tp_surface proc oilCanLookIn(thing oilCan)string:
    int oilLeft;
...
define tp_surface proc oilCanEmptyChecker(thing oilCan)status:
    oilCan@p_oState := 0;
    continue
corp;
...
define tp_proving o_torch AddForSale(r_provingStore, "torch;reed", "",
    5, nil)$
define tp_surface proc torchDesc()string:
    int timeLeft;
...
o_torch@p_oState := 10$
define tp_surface proc torchTick(thing torch)void:
    int newState;
...
corp;
define tp_surface proc torchOn()status:
    thing torch;
...
define tp_surface proc torchOff()status:
    status st;
...
define tp_surface proc torchDrop(thing torch)status:
...
define tp_surface proc torchPutIn(thing torch, container)status:
...
define tp_surface r_alley1 CreateThing(r_provingRoad)$
...
Scenery(r_alley2, "path.armoury.pasture")$
define tp_surface PR_ALLEY2_ID NextEffectId()$
define tp_surface proc alley2Draw()void:
...
define tp_surface r_gateSouth CreateThing(r_provingField)$
...
Scenery(r_gateSouth, "gate.fence.path")$
r_gateSouth@p_Image := "Proving/gateSouth"$

define tp_surface r_pasture CreateThing(r_field)$
AutoGraphics(r_pasture, AutoOpenSpace)$
SetThingStatus(r_pasture, ts_readonly)$
monsterSetP(r_pasture)$

define tp_surface r_gateNorth CreateThing(r_pasture)$
...
define tp_surface r_pasture1 CreateThing(r_pasture)$
...
define tp_surface r_pasture2 CreateThing(r_pasture)$
...
define tp_surface r_pasture3 CreateThing(r_pasture)$
...
define tp_surface r_pasture4 CreateThing(r_pasture)$
...
define tp_surface o_cowpie CreateThing(nil)$
...
define tp_surface proc getCowPie(thing th)status:
...
define tp_surface r_pasture5 CreateThing(r_pasture)$
...
define tp_surface r_pasture6 CreateThing(r_pasture)$
...
define tp_surface r_pasture7 CreateThing(r_pasture)$
...
define tp_surface r_pasture8 CreateThing(r_pasture)$
...
define tp_surface r_pasture9 CreateThing(r_pasture)$
...
define tp_surface r_pasture10 CreateThing(r_pasture)$
...
define tp_surface r_pasture11 CreateThing(r_pasture)$
...
define tp_surface r_pasture12 CreateThing(r_pasture)$
...
define tp_surface r_pasture15 CreateThing(r_pasture)$
...
define tp_surface r_pasture16 CreateThing(r_pasture)$
...
define tp_surface r_pasture17 CreateThing(r_pasture)$
...
define tp_surface p_pAppleEatCount CreateIntProp()$
define tp_surface p_pAppleEatTime CreateIntProp()$

define tp_surface r_pasture18 CreateThing(r_pasture)$
...
define tp_surface o_appleTree CreateThing(nil)$
...
define tp_surface o_appleOnTree CreateThing(nil)$
...
define tp_surface proc appleDrop(thing apple)status:
    thing carryer, here;
...
define tp_surface proc appleEat()status:
    thing th, me;
    int now, current, max;
...
define tp_surface proc appleGive(thing target)status:
    thing me, apple;
...
define tp_surface o_appleInHand CreateThing(nil)$
...
define tp_surface proc appleGet(thing th)status:
    thing me;
...
define tp_surface proc appleTreeGet(thing th)status:
...
define tp_surface r_alley3 CreateThing(r_provingRoad)$
...
Scenery(r_alley4, "tent;healer's,healer.spot;vacant.path.trail")$
define tp_surface PR_ALLEY4_ID NextEffectId()$
define tp_surface proc alley4Draw()void:
...
define tp_surface r_behindHealer CreateThing(r_provingRoad)$
...
define tp_surface r_forest2 CreateThing(r_provingForest)$
...
define tp_surface PR_FOREST2_ID NextEffectId()$
define tp_surface proc forest2Draw()void:
...
define tp_surface r_forest3 CreateThing(r_provingForest)$
...
o_stream@p_oListenString := "You can hear the stream splashing along."$
define tp_surface o_drink CreateThing(nil)$
SetupObject(o_drink, r_forest3, "drink.swallow", "")$
o_drink@p_oInvisible := true$
define tp_surface proc takeDrink(thing drink)status:
    streamDrink()
corp;
o_drink@p_oGetChecker := takeDrink$
define tp_surface proc streamDraw()void:
    AutoPaths();
    GSetPen(nil, C_BLUE);
    GAMove(nil, 0.125, 0.0);
    GRectangle(nil, 0.0625, 1.0, true);
corp;
AutoGraphics(r_forest3, streamDraw)$

define tp_surface STREAM_SOUND_ID NextSoundEffectId()$
define tp_proving p_StreamSoundVolume CreateIntProp()$
define tp_surface proc doStartStreamSound()void:
    int volume;

    volume := Here()@p_StreamSoundVolume;
    if volume ~= 0 then
	if SOn(nil) then
	    SVolume(nil, volume);
	    SPlaySound(nil, "stream", 0, STREAM_SOUND_ID);
	    SVolume(nil, 10000);
	    Me()@p_StreamSoundVolume := volume;
	fi;
    fi;
corp;
/* These two are also needed for the upper sewer level. */
define tp_proving proc startStreamSound()status:
    thing me;

    me := Me();
    if Character(me@p_pName) ~= nil then
	if Here()@p_StreamSoundVolume ~= me@p_StreamSoundVolume then
	    if SOn(nil) and me@p_StreamSoundVolume ~= 0 then
		AbortEffect(nil, EFFECT_SOUND, STREAM_SOUND_ID);
	    fi;
	    if me@p_StreamSoundVolume = 0 and
		FindElement(me@p_pEnterActions, doStartStreamSound) = -1
	    then
		AddTail(me@p_pEnterActions, doStartStreamSound);
	    fi;
	    doStartStreamSound();
	fi;
    fi;
    continue
corp;
define tp_proving proc stopStreamSound()status:
    thing me;
	...
	me -- p_StreamSoundVolume;
	DelElement(me@p_pEnterActions, doStartStreamSound);
    fi;
    continue
corp;
AddAnyEnterChecker(r_forest1, stopStreamSound, false)$
r_forest3@p_StreamSoundVolume := 5000$
AddAnyEnterChecker(r_forest3, startStreamSound, false)$

define tp_surface r_forest4 CreateThing(r_provingForest)$
...
r_forest4@p_StreamSoundVolume := 5000$
AddAnyEnterChecker(r_forest4, startStreamSound, false)$
...
Scenery(r_forestByStream,
    ...
    "sewer;large")$
r_forestByStream@p_StreamSoundVolume := 10000$
AddAnyEnterChecker(r_forestByStream, startStreamSound, false)$
define tp_proving o_barGrating CreateThing(nil)$
SetupObject(o_barGrating, r_forestByStream,
    "grating,grate;iron,bar.bar;iron.iron",
...
o_barGrating@p_oNotLockable := true$
...
o_gratingLatch@p_oNotLockable := true$
...
define tp_surface r_forest5 CreateThing(r_provingForest)$
...
r_forest5@p_StreamSoundVolume := 5000$
AddAnyEnterChecker(r_forest5, startStreamSound, false)$

define tp_surface r_forest6 CreateThing(r_provingForest)$
...
define tp_surface PR_FOREST6_ID NextEffectId()$
define tp_surface proc forest6Draw()void:
	...
	Else(nil);
	    ...
	    GSetPen(nil, C_MEDIUM_BROWN);
	    GPolygonStart(nil);
	    GAMove(nil, 0.499, 0.15);
	    GADraw(nil, 0.35, 0.42);
	    GADraw(nil, 0.35, 0.58);
...
r_forest6@p_StreamSoundVolume := 5000$
AddAnyEnterChecker(r_forest6, startStreamSound, false)$
AddSouthEastChecker(r_forest6, stopStreamSound, false)$

define tp_surface r_forest7 CreateThing(r_provingForest)$
...
define tp_surface PR_FOREST7_ID NextEffectId()$
define tp_surface proc forest7Draw()void:
...
AddNorthEastChecker(r_forest6, stopStreamSound, false)$

define tp_surface r_fTrail2 CreateThing(r_provingField)$
...
Scenery(r_fTrail2, "forest.tree.bush.bushes.path.trail.road;cobbled")$
AutoGraphics(r_fTrail2, AutoPaths)$

define tp_surface r_fTrailHidden CreateThing(r_provingForest)$
SetupRoomP(r_fTrailHidden, "in the forest",
    "There are trees all around you.")$
HConnect(r_fTrail2, r_fTrailHidden, D_SOUTH)$
HConnect(r_fTrail1, r_fTrailHidden, D_NORTH)$
/* Want to replace the scenery to remove the trail/path. */
r_fTrailHidden@p_rScenery := "ground.sky.forest.tree"$
Scenery(r_fTrailHidden, "forest.tree")$
AutoGraphics(r_fTrailHidden, AutoPaths)$
