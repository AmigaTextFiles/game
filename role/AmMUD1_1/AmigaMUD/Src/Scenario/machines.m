...
"pocket until he needs it.";
    ...
    CreateMachine("Caretaker,car", Caretaker, r_crossSW, caretakerStep);
...
define tp_machines proc packratStep()void:
	...
	After(60.0, packratStep);
    else
	if me@p_mWantDelay then
	    me@p_mWantDelay := false;
	    After(IntToFixed(10 + Random(20)), packratStep);
	    me@p_mArrived := true;
...
define tp_machines proc packratNoNo()status:
    ...
    DoSay("'ere now - don't get fresh wi' me!");
    fail
corp;
...
"precariously on her head.";
    CreateMachine("Packrat,pac,pack", Packrat, r_es2, packratStep);
