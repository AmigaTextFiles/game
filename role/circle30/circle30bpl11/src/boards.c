...
int CMD_RREAD, CMD_LOOK, CMD_EXAMINE, CMD_WWRITE, CMD_REMOVE;
...
void init_boards(void)
{
  ...
  CMD_RREAD = find_command("read");
  CMD_WWRITE = find_command("write");
...
SPECIAL(gen_board)
{
  ...
  if (cmd != CMD_WWRITE && cmd != CMD_LOOK && cmd != CMD_EXAMINE &&
      cmd != CMD_RREAD && cmd != CMD_REMOVE)
    return 0;
