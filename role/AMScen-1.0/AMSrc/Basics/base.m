...
/* properties, etc. that control the automated backups */

define tp_misc BackupThing CreateThing(nil).
define tp_misc p_FlushNeeded CreateBoolProp().
define tp_misc p_BackupInterval CreateIntProp().

SetThingStatus(BackupThing, ts_public). /* so parseInput can write it */
BackupThing@p_FlushNeeded := false.
BackupThing@p_BackupInterval := 0.
...
define tp_base tb_firstActive CreateThingProp().
...
define tp_base proc callUserCode(thing object; action a)void:
...
define tp_base proc timerStep()void:
	...
	callUserCode(tl@tl_object, tl@tl_action);
	ClearThing(tl);
    od;
...
define tp_base proc timerStart()status:
...
define t_base proc utility public InsertCommand(string command)void:
    ...
    Print(">>> " + command + "\n");
    BackupThing@p_FlushNeeded := true;
