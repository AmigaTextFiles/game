...
define t_graphics proc public EnterRoomDraw()void:
	...
	/* Here we draw the autographics, then draw any image specified.
	   This way, the user without the image will get the autographics,
	   and the user with the image will get the image, without us
	   having to generate a custom effect for this room/image. */
	a := here@p_rAutoDrawAction;
	if a ~= nil then
	    call(a, void)();
	fi;
	if here@p_Image ~= "" then
	    GShowImage(nil, here@p_Image, 0, 0, 160, 100, 0, 0);
	fi;
