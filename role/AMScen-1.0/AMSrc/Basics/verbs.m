	...
	if st = succeed then
		...
		if st ~= fail then
			...
			if not me@p_pHidden and CanSee(here, me) then
			    OPrint(Capitalize(me@p_pName) + " " + s1 + "s " +
				s2 + whatName + ".\n");
			fi;
		    fi;
		    me@p_pPosition := pos;
...
define tp_verbs proc v_lookAround()bool:
	...
	if ShowRoomToMe(true) then
		...
		OPrint(Capitalize(me@p_pName) + " looks around.\n");
	    fi;
	    true
	else
	    false
...
define tp_verbs proc v_inventory()bool:
    ...
    if not me@p_pHidden and CanSee(Here(), me) then
	OPrint(Capitalize(me@p_pName) + " takes inventory.\n");
    fi;
    true
corp;
	...
	if st = continue then
		...
		if CarryItem(what) then
			...
			if who@p_pHidden then
			    ...
			    ABPrint(where, who, who, Capitalize(who@p_pName) +
...
define tp_verbs proc v_hide()bool:
	...
	if me@p_pHidden then
	    ...
	    me@p_pHidden := false;
	    OPrint(Capitalize(me@p_pName) + " fades into view.\n");
	    ForEachAgent(Here(), ShowIconOnce);
	else
	    Print("You are no longer visible to others.\n");
	    me@p_pHidden := true;
	    OPrint(Capitalize(me@p_pName) + " fades out of view.\n");
...
define tp_verbs proc DoBackup(thing obj)void:
    ...
    if BackupThing@p_BackupInterval ~= 0 then
	DoAfter(BackupThing@p_BackupInterval * 60, BackupThing, DoBackup);
    fi;
    if BackupThing@p_FlushNeeded then
	...
	Log("Starting automatic backup, interval = " +
	    IntToString(BackupThing@p_BackupInterval) + " minutes\n");
...
define tp_verbs proc v_backup(string arg)bool:
	...
	if arg = "" then
	    ...
	    n := StringToPosInt(arg);
	    if n = 0 then
		BackupThing@p_BackupInterval := 0;
		ignore CancelDoAfter(BackupThing, DoBackup);
		...
		BackupThing@p_BackupInterval := n;
		Print("Backup interval set to " + IntToString(n) +
		    " minutes.\n");
		ignore CancelDoAfter(BackupThing, DoBackup);
		DoAfter(n * 60, BackupThing, DoBackup);
