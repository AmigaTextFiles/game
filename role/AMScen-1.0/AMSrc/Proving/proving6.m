...
/*
 * proving6.m - the 3D maze area
 */

private tp_proving6 CreateTable().
use tp_proving6

define tp_proving6 p_pHasShovel CreateBoolProp().
define tp_proving6 p_pMazeX CreateIntProp().
define tp_proving6 p_pMazeY CreateIntProp().
define tp_proving6 p_pMazeZ CreateIntProp().
define tp_proving6 MAZE_MAX_X 15.
define tp_proving6 MAZE_MAX_Y 9.
define tp_proving6 MAZE_MAX_Z 9.
define tp_proving6 MAZE_ENTER_X 8.
define tp_proving6 MAZE_ENTER_Y 9.
define tp_proving6 MAZE_ENTER_Z 5.
define tp_proving6 MAZE_GOAL_X	11.
define tp_proving6 MAZE_GOAL_Y	3.
define tp_proving6 MAZE_GOAL_Z	5.
define tp_proving6 p_rLayerList CreateThingListProp().
define tp_proving6 p_rLayer CreateIntListProp().
define tp_proving6 MAZE_OPEN	0b00.
define tp_proving6 MAZE_NORMAL	0b01.
define tp_proving6 MAZE_BLOCK	0b10.
define tp_proving6 p_oCurrentUser CreateThingProp().

define tp_proving6 r_shovelChamber CreateThing(r_provingCave).
...
define tp_proving6 o_trollShovel CreateThing(nil).
...
define tp_proving6 r_trollMaze CreateThing(nil).
...
define tp_proving6 proc mazeNotify()void:
...
/* forward declaration */
define tp_proving6 proc mazeIdle()void: corp;

define tp_proving6 proc mazeActive(thing shovel)void:
    thing who;
...
define tp_proving6 proc touchShovel()status:
    thing me;
    action a;

    me := Me();
    Print("As you touch the shovel, you feel a sharp shock, as of electricity."
	" The shock momentarily clouds your mind, but even so you look with "
	"bewilderment as the shovel seems to melt into your hand. When you "
	"recover your senses, the shovel is gone, and you feel a strong "
	"desire to dig. There is a nagging feeling of something tugging at "
	"your mind, from somewhere to the northeast.\n");
    OPrint("As " + me@p_pName + " touches the shovel, it seems to cloud or "
...
define tp_proving6 proc getShovel(thing shovel)status:
    touchShovel()
corp;
o_trollShovel@p_oGetChecker := getShovel.
o_trollShovel@p_Image := "Proving/trollShovel".

define tp_proving6 proc leaveShovel()status:
    thing me;
...
define tp_proving6 proc shovelNoGo()status:
    if Me()@p_pHasShovel then
...
define tp_proving6 proc shovelUp()status:
    if Me()@p_pHasShovel then
...
define tp_proving6 proc shovelDown()status:
    if Me()@p_pHasShovel then
...
define tp_proving6 proc doShovelDig()void:
    if Me()@p_pHasShovel then
	Print("The stone of the floor is quite hard - you cannot go down that "
	    "way.\n");
    else
...
define tp_proving6 o_bridgeSign CreateThing(nil).
FakeObject(o_bridgeSign, r_trollMaze, "sign", "").
define tp_proving6 proc bridgeSignDesc()string:
    thing me;
...
define tp_proving6 proc bridgeSignRead()string:
    thing me;
...
define tp_proving6 proc trollMazeListen(string s)status:
    thing me;
	...
	do
		...
		GSetPen(nil, C_BROWN);
		GAMove(nil, MAZE_GOAL_X * 10 + 1, MAZE_GOAL_Y * 10 + 19);
		GRectangle(nil, 7, 11, true);
	    fi;
	od;
    fi;
    continue
...
define tp_proving6 proc questHeartDesc()string:
    "Find the heart of the trolls."
corp;
define tp_proving6 proc questHeartCheck()void:
corp;
define tp_proving6 proc questHeartHint()string:
    "It lies deep in the rock."
corp;
QuestDirect("Heart", questHeartDesc, questHeartCheck, questHeartHint).

define tp_proving6 proc maze(int z, y, x)int:
...
define tp_proving6 p_rSpaceCount CreateIntProp().
define tp_proving6 p_rSpaceVector CreateIntListProp().
r_trollMaze@p_rSpaceCount := 0.
r_trollMaze@p_rSpaceVector := CreateIntArray(6).
define tp_proving6 proc markSpace(int dir)void:
    int count;
...
corp;
define tp_proving6 proc dumpSpaces()bool:
    list int spaces;
    int count, i;
...
define tp_proving6 proc drawFloor(int x, y, z)void:
    int kind;
    ...
    GAMove(nil, x * 10, y * 10);
    GRectangle(nil, 9, 9, true);
corp;

define tp_proving6 proc drawBlock(int x, y, kind)void:
...
define tp_proving6 proc showMaze()void:
    thing me;
    int x, y, z, dx, dy, dz, x0, y0, kind;
    bool needNewLine;
	...
	if GOn(nil) then
		...
		GAMove(nil, x0 + 11, y0 + 31);
		GPixel(nil);
		GSetPen(nil, C_BROWN);
		GAMove(nil, x0 + 1, y0 + 19);
		if o_drawBridge@p_oState = 0 then
		    GRectangle(nil, 7, 2, true);
		else
		    GRectangle(nil, 7, 11, true);
		fi;
	    fi;
	    PlaceCursor(x * 10 + 1, y * 10 + 1);
	fi;
    else
	Print("It is dark.\n");
    fi;
...
define tp_proving6 proc mazeLookChecker()status:
    showMaze();
    succeed
corp;
AddRoomLookChecker(r_trollMaze, mazeLookChecker, false).

define tp_proving6 proc enterMaze()status:
    thing me;
	...
	Print("Amazingly enough, you are able to walk right into the wall. "
	    "The stone seems to be soft, and it parts before you and closes "
	    "behind you.\n");
	SetLocation(r_trollMaze);
	if LightAt(r_shovelChamber) then
	    if not me@p_pHidden then
		OPrint(me@p_pName + " disappears.\n");
...
define tp_proving6 proc mazeMove(int xDelta, yDelta, zDelta)status:
    thing me;
...
define tp_proving6 proc mazeMoveNorth()status:
    thing me;
...
corp;
define tp_proving6 proc mazeMoveEast()status:
    mazeMove(1, 0, 0)
corp;
define tp_proving6 proc mazeMoveSouth()status:
    thing me;
...
corp;
define tp_proving6 proc mazeMoveWest()status:
    mazeMove(-1, 0, 0)
corp;
define tp_proving6 proc mazeMoveUp()status:
    mazeMove(0, 0, 1)
corp;
define tp_proving6 proc mazeMoveDown()status:
    mazeMove(0, 0, -1)
corp;
...
define tp_proving6 proc shovelDig()void:
...
define tp_proving6 proc makeRow(string r)int:
    int row, i;
    string ch;
...
define tp_proving6 proc makeLayer(string y0,y1,y2,y3,y4,y5,y6,y7,y8,y9)void:
    thing th;
    list int li;
...
ignore DeleteSymbol(tp_proving6, "makeLayer").
ignore DeleteSymbol(tp_proving6, "makeRow").
