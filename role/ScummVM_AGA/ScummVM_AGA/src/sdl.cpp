...
#include "prefs.h"      // Screenmode Prefs protos
#include "grabscreen.h" // Grabscreen proto
#include "timer.h"
#include <SDL.h>
#include <SDL_timer.h>
#include <SDL_audio.h>
...
void FreeAGAResources(void) {
    printf("Freeing resources...\n");
    SDL_PauseAudio(1);
    Delay(10); // To avoid crashes when some data cannot be found
    SDL_Quit();
    if (T) DeleteTimer(T);
    if (BLANKPointer) FreeVec(BLANKPointer);
    if (screen) free(screen);
    if (win) CloseWindow(win);
    if (scr) CloseScreen(scr);
    }

void AllocAGAResources(void) {
    struct ScreenModeRequester *ScreenRequest;
    uint32 ModeID;
    int a;

    ModeID=readmodeid();   // Tries to read ModeID from file 'ScummAGA.Prefs'
    ...
    if (!AslRequestTags(ScreenRequest,    ASLSM_TitleText, (ULONG) "Pick 8Bit 320x200 Screenmode ",
        ...
        exit(-1);
            ...
            if (storemodeid(ModeID)) printf("New ModeID saved to 'ScummAGA.Prefs'\n");
                else printf("Warning: Cannot write ModeID to file 'ScummAGA.Prefs'\n");
       }
    }
  ...
  if (win)
  {
    ...
    if (BLANKPointer=(UWORD *)AllocVec(100,MEMF_CHIP|MEMF_CLEAR))
    {
       ...
       /*SetRGB32(&scr->ViewPort,1,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF);
       SetAPen(win->RPort,1);
       WritePixel(win->RPort,319,199);
       */
       }
    }
       ...
       while (msg = (struct IntuiMessage *)GetMsg(win->UserPort))
       {
           ...
           switch (msg->Class)
           {
             case IDCMP_VANILLAKEY:
              ...
              {
                case '+':
                    s->_defaultTalkDelay+=30;
                break;
                case '-':
                    s->_defaultTalkDelay-=30;
                    if (s->_defaultTalkDelay<=0) s->_defaultTalkDelay=0;
                break;
              }
              if (msg->Code == 'F')
                 {
             ...
             case IDCMP_RAWKEY:
                  ...
                  if ((key >= 0x50) && (key <= 0x58))
                  {
                    switch (key)
                    {
                        ...
                        case 0x54:
                            s->_keyPressed=0;
                            printf("Warning: F5 may produce an error with some games (ex. Monkey) \n");
                            key+=202;
		...
		if (!(s->_fastMode&2))
	    AGA_Delay(dontPause ? 25 : 300);
        //SDL_Delay2(dontPause ? 49: 500);
        //Delay(1);
     } while (!dontPause);
}
		...
		while (SDL_PollEvent(&event)) {
			...
			case SDL_KEYDOWN:
				...
				if (event.key.keysym.sym >= '0' && event.key.keysym.sym<='9')
                    {
					de 0 à 9: + shift: on sauve...
                    s->_saveLoadSlot = event.key.keysym.sym - '0';
					if (event.key.keysym.mod&KMOD_SHIFT) {
						sprintf(s->_saveLoadName, "Quicksave %d", s->_saveLoadSlot);
						s->_saveLoadFlag = 1;
                        de 0 à 9 + ctrl: on charge/sauve
...
void initGraphics(Scumm *s, bool fullScreen)
{
    ...
    printf("ScummAGA v0.38 (based on ScummVM 0.3 alpha)\n");
    ...
    // Alloc Intuition Screen/Window and Chunky Buffer
    AllocAGAResources();

	/* Clean up on exit */
 	atexit(FreeAGAResources);
    //if (scumm.sound) atexit(SDL_Quit);
