...
#else
    char path[strlen("Progdir")+ strlen(fname) + 2];
    sprintf(path, "Progdir:%s", fname);
#endif
...
#else
    char    path[strlen("Progdir")+ strlen(f) + 2];
#endif

#ifndef __MORPHOS__
    sprintf(path, "%s/%s", SRC_DIR, f);
#else
    sprintf(path, "Progdir:%s", f);
#endif
...
#else
        if ((icon = SDL_LoadBMP("Progdir:gfx/win_icon.bmp")) != NULL) {
#endif
