...
/*------------------------------------------------------------------------*/
char *ID = "$VER: aGibbet v1.52me (07.08.04)\r\n";
...
startnew:
    ...
    (*addr1++)=textsp[i++];
    *addr1=0;
    PrintIText(mywin->RPort, &my_intui_text, 135, topborder+12);
...
//    printf("%s\n",selword);
   ...
   PrintIText(mywin->RPort, &my_intui_text, 125, topborder+44);
   ...
   if (sigr & SIGBREAKF_CTRL_C) terminated = TRUE;
    ...
    RefreshDTObjects (dtobj, mywin, NULL, NULL);
        ...
        *addr1=0;
        ...
        case REFRESHWINDOW:
          if (ngflag==FALSE) PrintIText(mywin->RPort, &my_intui_text, 125, topborder+44);
          else
          {addr2=textwin;
          if (wrongcnt>6) addr2=textgib;
          addr1=my_text;
          while(*addr2!=0) (*addr1++)=(*addr2++);
          *addr1=0;
          PrintIText(mywin->RPort, &my_intui_text, 135, topborder+12);
          i=0;
          addr1=my_text;
