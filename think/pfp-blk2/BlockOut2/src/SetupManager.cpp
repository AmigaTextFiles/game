...
const char *RESOLUTION_NAME[]  = { "640x480" , "800x600" , "1024x768" , "1280x1024" };
const POINT RESOLUTION_VALUE[] = { {640,480} , {800,600} , {1024,768} , {1280,1024} };
const char *STYLE_NAME[] = { "Nostalgia" , "Marble", "Arcade" };
const char *SOUND_NAME[] = { "Blockout II" , "Blockout" };
const char *FRLIMITER_NAME[] = { "Off" , "50 fps", "60 fps", "75 fps", "100 fps" };
const float FRLIMITER_VALUE[] = { 0.0f, 50.0f, 60.0f, 75.0f, 100.0f };
...
void SetupManager::SetFrLimiter(int fLimit) {
  frLimit = Saturate(fLimit,FR_NOLIMIT,FR_LIMIT100);
}
...
void SetupManager::SetWindowSize(int size) {
  windowSize = Saturate(size,RES_640x480,RES_1280x1024);
}
...
void SetupManager::LoadHighScore() {
  ...
  FILE *file = fopen(LHD("hscore.dat"),"rb");

  if( file != NULL ) {

    int nbScore;
    DWORD nbRead;
    nbRead = fread(&nbScore,1,sizeof(int),file);

    if( (nbRead == sizeof(int)) && (nbScore <= 10*585) && (nbScore>=1) ) {

      scoreList = (SCOREREC *)malloc( sizeof(SCOREREC) );
      fread(scoreList, sizeof(SCOREREC), 1, file);
      ptr = scoreList;

      for( int i = 1; i < nbScore ; i++ ) {

        SCOREREC *nPtr = (SCOREREC *)malloc( sizeof(SCOREREC) );
        fread(nPtr, sizeof(SCOREREC), 1, file);
        ptr->next = nPtr;
        ptr = ptr->next;
...
void SetupManager::SaveHighScore() {
  ...
  FILE *file = fopen(LHD("hscore.dat"),"wb");
  ...
  if( file != NULL ) {
    ...
    // Write
    DWORD nbWritten;
    nbWritten = fwrite(&nbScore,1,sizeof(int),file);
    if( nbWritten == sizeof(int) ) {

      ptr = scoreList;
      while( ptr != NULL ) {
        fwrite(ptr,sizeof(SCOREREC),1,file);
        ptr = ptr->next;
      }
...
void SetupManager::WriteSetup() {
  ...
  FILE *file = fopen(LHD("setup.dat"),"wb");

  if( file != NULL ) {

    // Write
    DWORD nbWritten;
    int version = SETUP_VERSION;
    nbWritten = fwrite(&version,1,sizeof(int),file);
    if( nbWritten == sizeof(int) ) {

      fwrite(&pitWidth,sizeof(int),1,file);
      fwrite(&pitHeight,sizeof(int),1,file);
      fwrite(&pitDepth,sizeof(int),1,file);
      fwrite(&blockSet,sizeof(int),1,file);
      fwrite(&animationSpeed,sizeof(int),1,file);
      fwrite(&startLevel,sizeof(int),1,file);
      fwrite(&playSound,sizeof(BOOL),1,file);
      fwrite(&fullScreen,sizeof(BOOL),1,file);
      fwrite(&windowSize,sizeof(int),1,file);
      ...
      fwrite(&transparentFace,sizeof(BOOL),1,file);
      fwrite(httpHome,256,1,file);
      fwrite(httpProxy,256,1,file);
      fwrite(&httpProxyPort,sizeof(int),1,file);
      fwrite(&useHttpProxy,sizeof(BOOL),1,file);
      fwrite(&httpTimeout,sizeof(int),1,file);
      fwrite(&style,sizeof(int),1,file);
      fwrite(&soundType,sizeof(int),1,file);
      fwrite(&frLimit,sizeof(int),1,file);
    }
    fclose(file);
...
void SetupManager::LoadSetup() {
  ...
  FILE *file = fopen(LHD("setup.dat"),"rb");

  if( file != NULL ) {

    int version = 0;
    DWORD nbRead;
    nbRead = fread(&version,1,sizeof(int),file);

    if( nbRead == sizeof(int) ) {

      if( version<=SETUP_VERSION ) {

        fread(&pitWidth,sizeof(int),1,file);
        fread(&pitHeight,sizeof(int),1,file);
        fread(&pitDepth,sizeof(int),1,file);
        fread(&blockSet,sizeof(int),1,file);
        fread(&animationSpeed,sizeof(int),1,file);
        ...
        fread(&startLevel,sizeof(int),1,file);
        fread(&playSound,sizeof(BOOL),1,file);
        fread(&fullScreen,sizeof(BOOL),1,file);
        fread(&windowSize,sizeof(int),1,file);
        fread(&keyRx1,sizeof(BYTE),1,file);
        fread(&keyRy1,sizeof(BYTE),1,file);
        fread(&keyRz1,sizeof(BYTE),1,file);
        fread(&keyRx2,sizeof(BYTE),1,file);
        fread(&keyRy2,sizeof(BYTE),1,file);
        fread(&keyRz2,sizeof(BYTE),1,file);
        fread(&transparentFace,sizeof(BOOL),1,file);
        ...
        if( version>=3 ) {
          ...
          fread(httpHome,256,1,file);
          fread(httpProxy,256,1,file);
          fread(&httpProxyPort,sizeof(int),1,file);
          fread(&useHttpProxy,sizeof(BOOL),1,file);
          fread(&httpTimeout,sizeof(int),1,file);
        }
        if( version>=4 ) {
          // Load style and sound type
          fread(&style,sizeof(int),1,file);
          fread(&soundType,sizeof(int),1,file);
         }
        if( version>=5 ) {
          // Frame limiter
          fread(&frLimit,sizeof(int),1,file);				
				}
