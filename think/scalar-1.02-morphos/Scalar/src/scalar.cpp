...
#define SCALAR_VERSION "1.02"

#ifdef __MORPHOS__
const char *version_tag = "$VER: Scalar 1.02 (11.07.2006)";
#endif
...
ScalarEngine::~ScalarEngine()
{
	...
	if (m_LogoImage)
		SDL_FreeSurface(m_LogoImage);
	if (m_Background)
		SDL_FreeSurface(m_Background);
	if (m_SmallImage)
		SDL_FreeSurface(m_SmallImage);
	if (m_Toolbar)
		SDL_FreeSurface(m_Toolbar);
...
bool ScalarEngine::Init(bool Fullscreen, bool UseDGA)
{
	...
	if (!NjamLoad(&m_Toolbar, "data/toolbar.bmp"))
		return false;

	LogFile("Loading Fonts:\n");
	m_FontBlue = new NjamFont("data/font-blue.bmp", 14, 30);
	m_FontYellow = new NjamFont("data/font-uni.bmp", 14, 28);
...
void ScalarEngine::CreatePieces(Piece *parent)
{
	...
	if (!parent)	// create first piece
	{
		...
		for (Piece *temp = m_FirstPiece; temp; temp = temp->next)
		{
			...
			NjamSetRect(temp->image_rect, r->x - m_XOffset, r->y - m_YOffset, r->w, r->h);
		}
		return;
	}
	...
	if (NjamRandom(3) == 1 && w*h < 50000 && parent != m_FirstPiece)
		return;
...
void ScalarEngine::Draw(bool WithBackground)
{
	...
	m_FontYellow->WriteTextXY(m_Screen, 575, 11, buff);

	if (WithBackground && current_level > -1)
	{
		sprintf(buff, "POINTS: %d", current_points);
		m_FontYellow->WriteTextXY(m_Screen, 200, 12, buff);
	}
	...
	if (WithBackground)
	{
		...
		while (temp)
		{
			...
			if (m_GridOn || m_SelectedPiece == temp)
			{
				...
				NjamSetRect(src, image->x+1, image->y+1, image->w-2, image->h-2);
				NjamSetRect(dest, grid->x+1, grid->y+1, grid->w-2, grid->h-2);
...
void ScalarEngine::Message(char *text)
{
	...
	NjamSetRect(dest, (800-l*14-50)/2, 275, l*14+50, 55);
	SDL_FillRect(m_Screen, &dest, FillColor);

	FillColor = SDL_MapRGB(m_Screen->format, 180, 0, 0);
	NjamSetRect(dest, (800-l*14-30)/2, 285, l*14+30, 35);
	SDL_FillRect(m_Screen, &dest, FillColor);

	m_FontYellow->WriteTextXY(m_Screen, (800-l*14)/2, 290, text);
	SDL_Flip(m_Screen);
...
bool ScalarEngine::SetupGame()
{
	...
	if (w > 750 || h > 510)
	{
		double vk = w / (double)750;
		double hk = h / (double)510;
	...
	m_XOffset = 10 + (780 - m_ImageW)/2;
	m_YOffset = 50 + (550 - m_ImageH)/2;
...
void ScalarEngine::DrawHelp()
{
	...
	m_FontYellow->WriteTextXY(m_Screen, 200, 12, "VERSION " SCALAR_VERSION);
	m_FontBlue->WriteTextColRow(m_Screen, 14, 3, "by Milan Babuskov");

	// welcome message
	m_FontBlue->WriteTextColRow(m_Screen, 2, 5, "The goal of the game is to assemble the picture from");
	m_FontBlue->WriteTextColRow(m_Screen, 2, 6, "randomly shuffled pieces. Move the pieces by swapping");
	m_FontBlue->WriteTextColRow(m_Screen, 2, 7, "their positions. Number of swappings is limited for");
	m_FontBlue->WriteTextColRow(m_Screen, 2, 8, "each level. Swap the pieces by clicking on them.");
	m_FontBlue->WriteTextColRow(m_Screen, 2, 9, "Cancel the swap by clicking on the same piece twice.");

	m_FontBlue->WriteTextColRow(m_Screen, 2,11, "In each game you should assemble three images. For");
	m_FontBlue->WriteTextColRow(m_Screen, 2,12, "each image you get as many points as swappings you");
	m_FontBlue->WriteTextColRow(m_Screen, 2,13, "have got left, plus the following bonuses:");
	m_FontBlue->WriteTextColRow(m_Screen, 2,15, "30 points for level done in first try");
	m_FontBlue->WriteTextColRow(m_Screen, 2,16, "10 points for level done in second try");

	m_FontBlue->WriteTextColRow(m_Screen, 2,18, "      Click anywhere on the screen to continue");
...
void ScalarEngine::DrawHiscore()
{
	...
	NjamSetRect(r, 20, 60);
	SDL_BlitSurface(m_LogoImage, 0, m_Screen, &r);
	m_FontBlue->WriteTextColRow(m_Screen, 19, 3, "* TOP 10 PLAYERS *");

	// welcome message
	m_FontBlue->WriteTextColRow(m_Screen, 11, 5, "Name             Swaps      Points");
	char buff[80];
	for (int i=0; i<10; ++i)
	{
		sprintf(buff, "%-15s  %5d       %5d", m_hiscore.names[i], m_hiscore.swaps[i], m_hiscore.points[i]);
		m_FontBlue->WriteTextColRow(m_Screen, 11, 6+i, buff);
	}

	sprintf(buff, "Your score: %d", current_points);
	m_FontBlue->WriteTextColRow(m_Screen, 21,17, buff);
	m_FontBlue->WriteTextColRow(m_Screen, 2,18, "      Click anywhere on the screen to continue");
	SDL_Flip(m_Screen);
}
//-----------------------------------------------------------------------------
...
{
	char InfoText[6][30] = {
		"VIEW HISCORES", "EXIT GAME", "PLAY NEXT LEVEL",
		"RESTART GAME", "SHOW HELP", "TOGGLE GRID ON/OFF"
	};
	...
	while (true)	// loop main menu
	{
		...
		else
		{
			...
			if (option != LastInfo)
			{
				...
				NjamSetRect(r, 190, 10, 385, 30);
				SDL_FillRect(m_Screen, &r, 0);
				if (LastInfo > -1)
					m_FontYellow->WriteTextXY(m_Screen, 200, 12, InfoText[LastInfo]);
