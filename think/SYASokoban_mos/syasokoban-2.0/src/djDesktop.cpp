...
// djDesktop.cpp
//
// Copyright (C) 2001-2006 David Joffe
// http://scorpioncity.com/
//---------------------------------------------------------------------------
...
#ifndef __MORPHOS__
#define DEFAULT_SETTING_FULLSCREEN false
#else
#define DEFAULT_SETTING_FULLSCREEN true
#endif
...
{
	g_Settings.SetDefaultSettingBool(SETTING_FULLSCREENMODE, DEFAULT_SETTING_FULLSCREEN);
...
int CdjDesktop::InitSurface()
{
	...
	const int nDEPTH = 32;
#ifndef __MORPHOS__
        const int nDEF_FLAGS = SDL_HWSURFACE|SDL_DOUBLEBUF;
#else
        const int nDEF_FLAGS = SDL_SWSURFACE;
#endif

	if (m_bFullScreen)
	{
		m_pSurface = SDL_SetVideoMode(nWIDTH, nHEIGHT, nDEPTH, nDEF_FLAGS|SDL_FULLSCREEN);
	...
	{
		m_pSurface = SDL_SetVideoMode(nWIDTH, nHEIGHT, nDEPTH, nDEF_FLAGS);
		if (m_pSurface==NULL)
