...
#ifdef __MORPHOS__
const char *version_tag = "$VER: BlockRage 0.2.1 (21.09.2005)";
#endif
...
static int turn_interval_usec,board_x_center;
...
static int nx_x0[3],nx_y0[3]; /* "NEXT" column coordinates for P1,P2, single P.*/
...
static int player_init(player_t *p, int pn) {
  ...
  p->board=malloc((board_y_size+5)*board_x_size);
  if(!(p->board)) return 0;
  
  p->bflags=malloc((board_y_size+5)*board_x_size);
...
static void player_rise(player_t *p) {
  int x,y;
  int i,t,nsuit,*suits=malloc(sizeof(int)*(piecetypes+1));
  float fnsuit;
  ...
  /* obsolete: totaly random */
  
  /*for(x=0;x<board_x_size;x++)
    p->board[x]=1+(rand()%piecetypes);*/
...
#endif
  printf("saving to '%s'\n",fname);
  pcx_save(scr_x_size,scr_y_size,vscr,mypal,fname);
}
  ...
  if(!gameover && !paused) {
    for(i=0;i<players;i++) {
      ...
      if(scancode==key_drop[i])
        player[i].piece_spd=press?piece_spd_drop:piece_spd; 
    }
  }
...
static int piece_readinfo(char *name, int *frames, flp_t *delay) {
  ...
  fscanf(f,"%d",frames);
  delay->data=calloc(*frames,sizeof(float));
...
static int screen_init(void) {
  ...
  if(!(vscr=malloc(scr_x_size*scr_y_size*sizeof(pix_t)))) return 0;
  if(!(bground=calloc(board_x_size*box_x_size*board_y_size*box_y_size,sizeof(pix_t)))) return 0;
  ...
  if(!(piece=calloc(no_of_sets*P_PER_S,sizeof(piece_t)))) return 0;
...
static int process_args(int argc,char *argv[]) {
  ...
  while (i<argc) {
    ...
    if(strcmp(argv[i],"+d")==0) {
      ...
      if(res==1 && piecetypes>=4 && piecetypes<=6) {
        ;
      }
      else {
        fprintf(stderr,"Wrong difficulty #(must be 4-6) param %d '%s'\n",i,argv[i]);
	return 0;
      }
    } else
...
static int dataconfig_read(void) {
  ...
  strcpy(fname,datadir);
  cat_with_slash(fname,dataset);
  cat_with_slash(fname,"dataset.cfg");
  ...
  if(!(particle_frame=calloc(no_of_particle_frames,sizeof(frame_t)))) return 0;
...
#ifdef __MORPHOS__
	if(!rc_load("Progdir:blockrage.rc")) {
		return EXIT_FAILURE;
	}
#else
  if(!rc_load("./blockrage.rc")) {
    strcpy(fname,getenv("HOME"));
    cat_with_slash(fname,".blockrage/blockrage.rc");

    if(!rc_load(fname)) {      
      if(!rc_load(SYSCONFDIR "/blockrage.rc")) {
