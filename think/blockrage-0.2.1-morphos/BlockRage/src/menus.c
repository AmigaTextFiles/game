...
int topten_update(int newscore) {
  ...
  topscore[i].score=newscore;		// set new score
  strcpy(topscore[i].name,getenv("USERNAME"));	// & name
  ...
  do {
	...
	case SDLK_RETURN: if(topscore[i].name[0]!=0) finished=1; break;
	case SDLK_BACKSPACE: l=strlen(topscore[i].name);
		  if(l>0)
		    topscore[i].name[l-1]=0;
		  break;
	default: l=strlen(topscore[i].name);
	         if(l<32) {
		   topscore[i].name[l+1]=0;
		   topscore[i].name[l]=k;
		 }
    ...
    /* may be used to knock the user if asleep
       "hey! you're asleep ? you won, so enter your name, dummy!"
...
void menu_draw(int menupos) {
  ...
  particle_frame[no_of_particle_frames-1].ys,
  40,40);
    ...
    do {
      ...
      switch(k) {
        ...
        case SDLK_LEFT: // left arrow
          switch(menupos) {
  	    case 0: if(piecetypes>4) piecetypes--; break;
        ...
        case SDLK_RIGHT: // right arrow
          switch(menupos) {
	    case 0: if(piecetypes<6) piecetypes++; break;
