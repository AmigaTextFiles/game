...
int font_load(char *name, font_t *fnt) {
  ...
  strcpy(fname,name);
  strcat(fname,".pcx");
  if(!pcx_load(&fnt->bmp,fname)) return 0;
/*  printf("font 0: %dx%d (%dx%d)\n",fontcxs[0],fontcys[0],fontxs[0],fontys[0]);
  printf("font 1: %dx%d (%dx%d)\n",fontcxs[1],fontcys[1],fontxs[1],fontys[1]);*/
  fclose(f);
  return 1;
}
...
int font_load(char *name, font_t *fnt) {
  ...
  fscanf(f,"%d%d%d",&(fnt->h),&(fnt->sh),&(fnt->y0));
  
  for(i=0;i<128;i++) {
    fscanf(f,"%d %d %d %d",&(fnt->ofs[i]),&(fnt->width[i]),
    &(fnt->lspac[i]),&(fnt->rspac[i]));
  }
  
  for(i=1;i<128;i++) {
    fnt->ofs[i]+=fnt->ofs[i-1] + fnt->width[i-1];
  }
  
  strcpy(fname,name);
  strcat(fname,".pcx");
  if(!pcx_load(&tmp,fname)) return 0;
  fnt->bmp=tmp.data;
  fnt->xs=tmp.xs;
  fnt->ys=tmp.ys;
*  printf("font 0: %dx%d (%dx%d)\n",fontcxs[0],fontcys[0],fontxs[0],fontys[0]);
...
/*  printf("pcx_load: %dx%d, bpl:%d",
  ...
  up=calloc(xsize*(ysize+10),sizeof(pix_t));
...
void pcx_save(int xs, int ys, unsigned char *data, unsigned char *pal,
  ...
  phead->identifier=0x0a;
  phead->version=5;
  phead->encoding=1;
  phead->bitsperpixel=8;
  phead->xstart=0;
  phead->ystart=0;
  phead->xend=xs-1;
  phead->yend=ys-1;
  ...
  * phead->palette *
  phead->reserved1=0;
  phead->numbitplanes=1;
  phead->bytesperline=xs;
  phead->palettetype=1;
  phead->horzscreensize=scr_x_size;
  phead->vertscreensize=scr_y_size;
  for(i=0;i<54;i++)
    phead->reserved2[i]=0;


  fwrite(&ph, sizeof(PCX_HEADER),1, f);
  ...
  for(y=0;y<ys;y++) {
	  ...
	  if(*upp<0xc0) fputc(*upp,f); * save as one byte *
	    else {
	      fputc(0xc1,f); * one-pixel rle chunk (damn!) *
	      fputc(*upp,f);
	    }
