...
//Renvoie vrai si la balle est dQtruite, 0 sinon.
bool CBalle::deplacer(const int& iElapsedTime, CTerrain &terrain, SDL_Rect* RaqRect, CSon &mySon, CBonus &myBonus)
{
    ...
    //On passe par des int pour les cas nQgatifs.
    double x = xBalle;
    double y = yBalle;
    ...
    //Attention : la fonction peut modifier x et y, si nQcessaire.
    ...
    switch (retColl)
    {
        ...
        break;
        default:
        fprintf( stderr, "Erreur : Valeur non valable renvoyQe par collisionBriques\n");
    }
    ...
    if ((y>RaqRect->y && y<RaqRect->y+RaqRect->h) || (y+myRectBalle.h>RaqRect->y && y+myRectBalle.h<RaqRect->y+RaqRect->h))
        ...
        if ((x>RaqRect->x && x<RaqRect->x+RaqRect->w) || (x+myRectBalle.w>RaqRect->x && x+myRectBalle.w<RaqRect->x+RaqRect->w))
        {
            ...
            //Formule qui dQtermine l'angle de renvoie.
            ...
            //Permet d'Qviter les balles trop horizontales.
