...
void CJeu::initJeu(bool musiqueOn, bool sonsOn, char lang[MAX_LANG][256])
{
    ...
    playing=true; // variable permettant de dQterminer si le joueur n'a pas perdu.
    ...
    //Chargement de la police d'Qcriture des chiffres :
...
//Fonction qui permet de gQrer la durQe entre deux descentes puor une briques
...
//Permet d'enregistrer la derniøre image du jeu, pour lorsque l'on a perdu.
...
//Gøre le jeu lorsque des lignes sont dQtruites.
//A gQrer mieux ca va pas du tout !!
...
bool CJeu::allMoveFigure(int moveStrafe,bool bas,bool rotation, bool down)
{
    ...
    if (moveStrafe!=0)
    {
        if ((tempsActuel-m_iLastStrafe)>=3*m_iDureeStrafe)
        {
            m_iLastStrafe=tempsActuel+3*m_iDureeStrafe;
    ...
    if (down==true)
    {
        ...
        //On force le test de descente d'une ligne, qui fait poser la piøce.
    ...
    else
    {
        if (bas==true && ((tempsActuel-m_iLastChute)>=m_iDureeChute))
        {
            ...
            if (testDescente==true) { m_iLastDescente=tempsActuel-m_iDureeAttente;}
        }
    }
    ...
    if (tempsActuel-m_iLastDescente>=m_iDureeAttente)
    {
        ...
        if (myTerrain.moveFigure(0,1)==false)
        {
            ...
            if (myTerrain.dropFigure()==true)
            {
                ...
                myTerrain.findEmptyLine(); //Ne fais pas de points, car les lignes la ont QtQ cassQes en vol par le tetris.
                ...
                //VQrifions maintenant si le terrain est vide.
...
{
    //On dQplace la raquette.
    if (sens!=0) myRaquette.deplacer(iElapsedTime,sens,myTerrain.GetRect());
    if (modifRaquette!=-1 && modifRaquette+10000<SDL_GetTicks())
    {
        modifRaquette=-1;
...
//La gestion de la balle doit prendre en compte les diffQrents QlQments du terrain... Pas facile facile...
//Il faut de plus dQtruire les briques touchQes.
void CJeu::deplacerBalle(const int& iElapsedTime)
{
    int nbCaseDetruites;
    SDL_Rect* raqRect = myRaquette.GetRect();
    int raqMidX=raqRect->x + raqRect->w/2;
    //Ajouter ici Qventuellement une boucle pour gQrer plusieurs balles.

    //Il faut gQrer les collisions avec les briques, les murs, et la perte des balles.
    //Tous ceci se fait dans la fonction de dQplacement de la balle.
    //On lui enverra en paramøtre le terrain, pour qu'elle puisse procQder a ses test.
    for (int i=0;i<MAX_BALLES;i++)
    {
        if (myBalle[i].getStatutBalle()==1)
        {
            //Indique que la balle a QtQ dQtruite.
            if (myBalle[i].deplacer(iElapsedTime,myTerrain,raqRect,mySon,myBonus)==true)
            {
                m_iNbBalles--;
                mySon.playEffet(S_PERDU_BALLE);
            }
            //Indique les destructions que la balle a causQe.
        ...
        for (int i=0;i<nbBonus;i++)
        {
            ...
            switch (bonusObtenu[i])
            {
                ...
                case B_BLOQUE_STRAFE:
                    //On bloque la figure latQralement pendant 10 secondes.
                    m_iLastStrafe+=10000;
                break;
                case B_SMALL_RAQ:
...
//Gøre les dQplacement de raquette.
