...
void CTerrain::initTerrain(char lang[MAX_LANG][256])
{
    ...
    int posY[6]={43,193,240,290,340,444};
...
void CTerrain::rndFillTerrain(int nbLignes)
{
    ...
    for (int j=jDep;j<NBCASEY;j++)
    {
        for (int i=0;i<NBCASEX;i++)
        {
            if (i!=j) //Pour Qviter que toutes les cases soient remplies
            {
...
/*
Cette fonction permet de gQrer la collision entre la balle et le mur de briques.
Elle recoit en paramøtre la position de de la balle avant le mouvement (les oldXb, oldYb),
ainsi que la position aprøs le mouvement (xb, yb).
Il s'agit bien entendu de la position du coin supQrieur gauche de la balle.
Elle recoit aussi la largeur de la balle (sb), considQrQe comme un carrQ.
...
Une collision horizontale signifie que la balle a frappQ le dessus d'une ou plusieurs briques.
Une collision verticale signifie que la balle a frappQ le cotQ d'une ou plusieurs briques.
Cette fonction est une des plus complexes, a terme, du programme.

Voici en gros le dQroulement qu'elle devrait avoir :
...
//Approximation : on suppose que le dx et le dy de la balle sont trøs petits.
int CTerrain::collisionBriques(int xb, int yb, int oldXb, int oldYb, int sb)
{
    ...
    {
        //CoordonnQes de la case dans laquelle est le point en haut a gauche du carrQ de la balle.
        ...
        if (collision==true)
        {
            ...
            //Valeur absolue du coefficient directeur du dQplacement de la balle.
            ...
            //Valeur absolue du coefficient directeur du dQplacement entre la balle et la brique.
...
// :D Permet de savoir si la balle a dQtruite une ligne.
...
//On dQplace la figure en x et y
...
//Renvoie le nombre de lignes totalement cassQes
