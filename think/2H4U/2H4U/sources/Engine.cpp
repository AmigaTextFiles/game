...
CEngine::CEngine()
{
	...
	m_lLastRender   = 0;
	m_lLastThink    = 0;
	//m_iFrameLenght  = 15; //DurQe en millisecondes d'une frame.
    m_iFrameLenght=0;
	m_iWidth		= 800;
	m_iHeight		= 600;
	m_czTitle		= NULL;
...
//M me chose que prQcQdemment, mais en plein Qcran.
void CEngine::SetFullScreen(const int& iWidth, const int& iHeight)
{
    m_iWidth  = iWidth;
	m_iHeight = iHeight;
	//A vQrifier : l'ancienne surface de m_pScreen est-elle supprimQe ?
...
void CEngine::Init()
{
    ...
    //Ligne rajoutQ, au cas ou le programme est quittQ violement (kill).
    atexit(SDL_Quit);

    putenv("SDL_VIDEO_WINDOW_POS=center"); //pour centrer la fen tre

    //Chargement des paramøtres, afin de choisir si fen tre ou plein Qcran
    ParamInit(m_iWidth,m_iHeight);

	// If we fail, return error.
	if ( m_pScreen == NULL )
	{
		fprintf( stderr, "Impossible de dQfinir la vidQo: %s\n", SDL_GetError() );
		exit( 1 );
	}
...
//Le jeu sera en fait appellQ par le menu.
...
void CEngine::Start()
{
	...
	while ( !m_bQuit )
	{
        ...
        //On se passe du Delay, car l'affichage est limitQ (60 images par secondes)
	}
...
void CEngine::HandleInput()
{
	...
	while ( SDL_PollEvent( &event ) )
	{
        ...
        //Rajouter la gestion de 4 ou 5 touches supplQmentaires.
...
//NQcessite SDL_Image. Recoit le chemin d'accøs de l'icone en paramøtre.
