...
static struct NewMenu connect4menu[] =
{
...
static char *Page_titles[] = { "Players", "Network", NULL };

static char *cycle_playertype_strings[] = { "Local", "Remote", "Computer", NULL };

static char *cycle_acceptnet_strings[] = { "from known players", "from everyone", NULL };

static char *default_player_name[] = { "Player 1", "Player 2" };
...
BOOL loadsettings(char *filename)
{
    ...
    char fileheader[]={19, 31, 30, 30, 21, 19, 36, 4}; // ASCII values of each char in "CONNECT4" minus 48

    // Attempt to load the user prefs (player and network details)
    if (fp_in=Open (filename, MODE_OLDFILE))
    {
        Seek (fp_in, 0, OFFSET_END);
        filelength=Seek (fp_in, 0, OFFSET_BEGINNING);
...
void savesettings (char *filename, APTR playerlist, APTR netport, APTR netname)
{
    BPTR fp_out;
    char fileheader[]={19, 31, 30, 30, 21, 19, 36, 4}; // ASCII values of each char in "CONNECT4" minus 48
    char section_separator[]={1};
    char *stringptr;
    struct PAB_Entry *tempentryptr;

    if (fp_out=Open(filename, MODE_NEWFILE))
    {
        Write(fp_out, fileheader, 8);  // Write weird encoding of "CONNECT4" as file header
        ...
        Write(fp_out, section_separator, 1);
...
void problem_window (APTR app, char *windowtitle, char *errmsg)
{
    enum {okpressed=500};

    APTR problemwin=0,
         okbutton;
    char formattederrmsg[100], i;
...
SAVEDS APTR PAB_constructor_function (REG(a0, struct Hook *hook),
                                      REG(a2, APTR pool),
                                      REG(a1, struct PAB_Entry *entry))
{
...
SAVEDS void PAB_destructor_function (REG(a0, struct Hook *hook),
                                     REG(a2, APTR pool),
                                     REG(a1, struct PAB_Entry *entry))
{
...
SAVEDS LONG PAB_display_function (REG(a0, struct Hook *hook),
                                  REG(a2, char **array),
                                  REG(a1, struct PAB_Entry *entry))
{
      if (entry)
...
SAVEDS LONG PAB_compare_function (REG(a0, struct Hook *hook),
                                  REG(a2, struct PAB_Entry *entry1),
                                  REG(a1, struct PAB_Entry *entry2))
{
...
SAVEDS void menu_save_function (REG(a0, struct Hook *hook),
                                REG(a2, Object *obj),
                                REG(a1, ULONG *data))
{
...
SAVEDS void menu_savedefault_function (REG(a0, struct Hook *hook),
                                       REG(a2, Object *obj),
                                       REG(a1, ULONG *data))
{
    APTR playerlist=(APTR)*data++;
...
SAVEDS void menu_load_function (REG(a0, struct Hook *hook),
                                REG(a2, Object *obj),
                                REG(a1, APTR *data))
{
    APTR app=(APTR)*data++;
...
SAVEDS void delete_player_function (REG(a0, struct Hook *hook),
                                    REG(a2, Object *obj),
                                    REG(a1, APTR *data))
{
    APTR playerlist=(APTR)*data++;
...
SAVEDS void menu_aboutmui_function (REG(a0, struct Hook *hook),
                                    REG(a2, Object *obj),
                                    REG(a1, APTR *data))
{
...
SAVEDS void popchoice_function (REG(a0, struct Hook *hook),
                                REG(a2, Object *obj),
                                REG(a1, APTR *data))
{
...
SAVEDS void existing_player_edit_function (REG(a0, struct Hook *hook),
                                           REG(a2, Object *obj),
                                           REG(a1, APTR *data))
{
...
SAVEDS void reset_values_function (REG(a0, struct Hook *hook),
                                   REG(a2, Object *obj),
                                   REG(a1, APTR *data))
{
    APTR playerlist=(APTR)*data++;
...
SAVEDS void commit_player_function (REG(a0, struct Hook *hook),
                                    REG(a2, Object *obj),
                                    REG(a1, APTR *data))
{
...
SAVEDS void winner_function (REG(a0, struct Hook *hook),
                             REG(a2, Object *obj),
                             REG(a1, APTR *data))
{
...
SAVEDS void problem_function (REG(a0, struct Hook *hook),
                              REG(a2, Object *obj),
                              REG(a1, APTR *data))
{
    APTR app=(APTR)*data++;
...
SAVEDS void react_move_function (REG(a0, struct Hook *hook),
                                 REG(a2, Object *obj),
                                 REG(a1, APTR *data))
{
    APTR board=(APTR)*data;
...
SAVEDS void start_game_function (REG(a0, struct Hook *hook),
                                 REG(a2, Object *obj),
                                 REG(a1, APTR *data))
{
...
void connect4 (struct MUI_CustomClass *gameboardmcc, UBYTE *temppool)
{
