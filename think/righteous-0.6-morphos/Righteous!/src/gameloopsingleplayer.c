...
int gameloopsingleplayer(SDL_Surface * screen, int timelimit)
{
	...
	while(!done) {
		...
		if(timelimit!=0&&(timer[3]/1000)>=timelimit) done=1;
		...
		if(keystate[SDLK_LCTRL]||keystate[SDLK_RCTRL]||keystate[SDLK_RALT]||keystate[SDLK_LALT])
	      ...
	      if(boxP[0]==(boxP[1])-1) { // fix "sliding while at different heights"


		if(keystate[SDLK_DOWN]&&getkey==1) { 
		...
		if(boxP[0]>=0) { // fix movement while "above" the game grid
		  ...
		  if(keystate[SDLK_LEFT]&&getkey==1&&gridgetx(boxP[0])>0&&(squares[boxP[0]-1]==0)&&(squares[boxP[2]-1]==0)) { 

			boxP[0]--;boxP[1]--;boxP[2]--;boxP[3]--; 
		  }
		
		
		  else if(keystate[SDLK_RIGHT]&&getkey==1&&gridgetx(boxP[1])<15&&(squares[boxP[1]+1]==0)&&(squares[boxP[3]+1]==0)) { 
		  ...
		  if(keystate[SDLK_LEFT]&&boxP[0]>-32&&getkey==1) {
		  ...
		  if(keystate[SDLK_RIGHT]&&boxP[3]<-1&&getkey==1) {
		  ...
		  if(keystate[SDLK_LEFT]&&boxP[2]>0&&getkey==1) {
		  ...
		  if(keystate[SDLK_RIGHT]&&boxP[3]<15&&getkey==1) {
		...
		if(keystate[SDLK_SPACE]&&boxP[2]>=0) { // quickly lower the box & fix for newly spawned boxes
		...
		if(timelimit==0) valuetoscreen(screen, num, (timer[3]/1000), 245);
		else valuetoscreen(screen, num, timelimit-(timer[3]/1000), 245);
	...
	if(score>=highscore&&timelimit==0)
