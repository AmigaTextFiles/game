...
#ifdef SLOW_MODE /* Slow-Mode */
#define BOWL_DOWN_PERC_CHANGE_SLOW  0.02
#define BOWL_DOWN_PERC_CHANGE_FAST  0.085
#else /* SLOW_MODE */
...
#ifdef SLOW_MODE
        bowl->block_vert_vel += ( BOWL_DOWN_VEL_END - bowl->block_vert_vel ) * (config.slow ? BOWL_DOWN_PERC_CHANGE_SLOW : BOWL_DOWN_PERC_CHANGE_FAST);
#else /* SLOW_MODE */
...
====================================================================
*/
int bowl_add_line( Bowl * bowl, int wanted_holes )
{
...
void bowl_finish_game( Bowl *bowl )
{
    ...
    write_text( bowl->font,  offscreen, bowl->sx + bowl->sw / 2, bowl->sy + bowl->sh / 2, "Game Over", OPAQUE );
    write_text( bowl->font, sdl.screen, bowl->sx + bowl->sw / 2, bowl->sy + bowl->sh / 2, "Game Over", OPAQUE );
#ifdef SOUND
        ...
        for ( i = 0; i < BOWL_COUNT; i++ )
            if ( bowls[i] && bowls[i] != bowl && !bowls[i]->game_over) {
                for ( j = 0; j < send_count; j++ )
                    if ( !bowl_add_line( bowls[i], config.holes ) ) {
...
int bowl_check_block_insertion( Bowl *bowl )
{
    ...
    if ( bowl->controls && keystate[bowl->controls->down] && config.clear_keystate )
...
void bowl_init_block_masks() 
{
	...
	block_masks[0].ry = 0;
	block_masks[0].id = 0;
	memset(block_masks[0].mask, 0, sizeof( int ) * 4 * 4 * 4 );
	...
	block_masks[1].ry = 0;
	block_masks[1].id = 1;
	memset(block_masks[1].mask, 0, sizeof( int ) * 4 * 4 * 4 );
	...
	block_masks[2].ry = 1;
	block_masks[2].id = 2;
	memset(block_masks[2].mask, 0, sizeof( int ) * 4 * 4 * 4 );
	...
	block_masks[3].ry = 1;
	block_masks[3].id = 3;
	memset(block_masks[3].mask, 0, sizeof( int ) * 4 * 4 * 4 );
	...
	block_masks[4].ry = 1;
	block_masks[4].id = 4;
	memset(block_masks[4].mask, 0, sizeof( int ) * 4 * 4 * 4 );
	...
	block_masks[5].ry = 1;
	block_masks[5].id = 5;
	memset(block_masks[5].mask, 0, sizeof( int ) * 4 * 4 * 4 );
	...
	block_masks[6].ry = 1;
	block_masks[6].id = 6;
	memset(block_masks[6].mask, 0, sizeof( int ) * 4 * 4 * 4 );
...
void bowl_show( Bowl *bowl )
{
    ...
    sprintf( aux, "%i Lvl: %i", bowl->lines, bowl->level );
...
void bowl_update( Bowl *bowl, int ms, int game_over )
{
    ...
    if ( !bowl->paused )
    {
        if ( delay_timed_out( &bowl->add_delay, ms ) )
        {
            if ( bowl->add_lines )
            {
                bowl_add_line( bowl, bowl->add_line_holes );
                bowl->draw_contents = 1;
            }
            if ( bowl->add_tiles )
                bowl_add_tile( bowl );
...
void bowl_draw_frames( Bowl *bowl )
{
    ...
    write_text( bowl->font, bkgnd, dx + 4, dy + 4, "Player:", OPAQUE );
    bowl->font->align = ALIGN_X_RIGHT | ALIGN_Y_TOP;
    write_text( bowl->font, bkgnd, dx + dw - 4, dy + 4, bowl->name, OPAQUE );
    bowl->font->align = ALIGN_X_LEFT | ALIGN_Y_CENTER;
    write_text( bowl->font, bkgnd, dx + 4, dy + dh / 2, "Score:", OPAQUE );
    bowl->font->align = ALIGN_X_LEFT | ALIGN_Y_BOTTOM;
    write_text( bowl->font, bkgnd, dx + 4, dy + dh - 4, "Lines:", OPAQUE );
    /* preview */
    if ( bowl->preview_center_sx != -1 )
        draw_3dframe( bkgnd, 
