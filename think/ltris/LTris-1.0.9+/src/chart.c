...
int chart_load_from_path( char *path )
{
    ...
    if ( file ) {
		...
		if ( strequal( ">>>", aux ) ) {
			...
			chart = chart_set_create( gametype_names[1] );
			chart_read_entries( file, file_name, chart );
			list_add( charts, chart );
		}
        fclose( file );
...
void chart_load()
{
    ...
    chart_level_offset = text_width( ccfont, "name.-----" ) + text_width( ccfont, "Level" ) / 2; /* level offset centered */
}
/*
====================================================================
Delete resources
...
void chart_show( Set_Chart *chart, int x, int y, int w, int h )
{
    ...
    write_text( ccfont, sdl.screen, x + ( chart_w >> 1 ), y,  chart->name, OPAQUE );
	/* caption */
