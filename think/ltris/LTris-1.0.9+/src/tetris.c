...
/* a game type index is mapped to the specified highscore */
char *chart_map[8] = {
    "Demo",
    "Classic",
    "Figures",
...
void tetris_run()
{
                ...
                case SDL_KEYUP:    
                    switch ( event.key.keysym.sym ) {
                        case SDLK_ESCAPE: 
                            ...
                            escape = 1;
                            if ( confirm( large_font, "End Game? y/n", CONFIRM_YES_NO ) ) 
                                for ( i = 0; i < BOWL_COUNT; i++ )
    ...
    for ( i = 0; i < BOWL_COUNT; i++ )
        if ( bowls[i] ) 
            chart_add( chart_set_query( chart_map[config.gametype] ), bowls[i]->name, bowls[i]->level, counter_get( bowls[i]->score ) );
    SDL_ShowCursor( 1 );
}
...
void tetris_make_stat()
{
    ...
    memset( lines, 0, sizeof( int ) * 1024 );
