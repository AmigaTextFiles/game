...
{	
    ...
    if (font_dir!= "")
    	filename.PrefixDirectory (font_dir);
...
//    printf("buf: %d %s\n", file.GetPosition(), buf);
	...
	while (1)
	{
		...
		SDL_Surface *surfLetter = SDL_CreateRGBSurface(SDL_SWSURFACE | SDL_SRCALPHA, x2-x1, y2-y1, fmt->BitsPerPixel, fmt->Rmask, fmt->Gmask, fmt->Bmask, amask);
