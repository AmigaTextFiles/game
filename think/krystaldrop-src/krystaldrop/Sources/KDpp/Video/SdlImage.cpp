...
void KD_SDLImage::ConvertToColorKey(unsigned int key, int alphaTrigger)
{
	...
	SDL_Surface *surf = SDL_CreateRGBSurface(SDL_SWSURFACE, image->w, image->h, image->format->BitsPerPixel, image->format->Rmask, image->format->Gmask, image->format->Bmask, 0);
...
KD_Image *KD_SDLImage::copy(int x, int y, int width, int height)
{
	...
	SDL_Surface *imageCopy = SDL_CreateRGBSurface(SDL_SWSURFACE | SDL_SRCALPHA, width, height, image->format->BitsPerPixel, image->format->Rmask, image->format->Gmask, image->format->Bmask, amask);
