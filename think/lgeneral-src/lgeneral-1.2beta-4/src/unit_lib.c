...
====================================================================
*/
static void unit_lib_eval_unit( Unit_Lib_Entry *unit )
{
...
int unit_lib_load( char *fname, int main )
{
    ...
    if ( main == UNIT_LIB_MAIN && unit_lib_main_loaded ) {
        fprintf( stderr, "%s: can't load as main unit library (which is already loaded): loading as 'additional' instead\n",
                 fname );
        main = UNIT_LIB_ADD;
    }
    /* parse file */
    sprintf( path, "%s/units/%s", SRC_DIR, fname );
    sprintf( log_str, "  Parsing '%s'", fname );
    ...
    if ( main == UNIT_LIB_MAIN ) {
        write_line( sdl.screen, log_font, "  Loading Main Definitions", log_x, &log_y ); refresh_screen( 0, 0, 0, 0 );
        ...
        if ( trgt_type_count > TARGET_TYPE_LIMIT ) {
            fprintf( stderr, "%i target types is the limit!\n", TARGET_TYPE_LIMIT );
        ...
        while ( ( sub = list_next( entries ) ) ) {
            ...
            if ( !parser_get_string( sub, "name", &trgt_types[i].name ) ) goto parser_failure;
            i++;
        }
        /* movement types */
        ...
        while ( ( sub = list_next( entries ) ) ) {
            ...
            if ( !parser_get_string( sub, "name", &mov_types[mov_type_count].name ) ) goto parser_failure;
#ifdef WITH_SOUND                
        ...
        while ( ( sub = list_next( entries ) ) ) {
            ...
            if ( !parser_get_string( sub, "name", &unit_classes[unit_class_count++].name ) ) goto parser_failure;
    ...
    sprintf( path, "units/%s", str );
    write_line( sdl.screen, log_font, "  Loading Tactical Icons", log_x, &log_y ); refresh_screen( 0, 0, 0, 0 );
    ...
    while ( ( sub = list_next( entries ) ) ) {
        ...
        /* name */
        if ( !parser_get_string( sub, "name", &unit->name ) ) goto parser_failure;
        /* class id */
        unit->class = 0;
        ...
        parser_get_int( sub, "end_year", &unit->end_year );
        /* icon */
        /* icon id */
