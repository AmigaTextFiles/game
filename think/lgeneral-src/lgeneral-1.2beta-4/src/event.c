...
{
    int i;
    for (i = 0; i < sizeof buttonstate/sizeof buttonstate[0]; i++)
        if (buttonstate[i]) return 1;
    return 0;
}
...
/*
====================================================================
Ensure that no mouse button is pressed at exit.
The function will wait until this condition holds true.
Call this before any status screen to ensure that it is not closed
on the button release from a pressed button when the screen was
to be displayed
====================================================================
*/
void event_clear_all_input()
{
    int dummy;
    /* if button is pressed on entrance, wait for buttonup event before proceeding */
    if ( event_is_button_pressed() ) {
      do { SDL_PumpEvents(); SDL_Delay(20); } while ( !event_get_buttonup( &dummy, &dummy, &dummy ) );
    }
}
