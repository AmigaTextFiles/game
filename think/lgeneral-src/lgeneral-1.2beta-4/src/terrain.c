...
int terrain_load( char *fname )
{
    ...
    while ( ( sub = list_next( entries ) ) ) {
        ...
        if ( !parser_get_string( sub, "name", &weather_types[i].name ) ) goto parser_failure;
    ...
    sprintf( path, "terrain/%s", str );
    if ( ( terrain_icons->cross = anim_create( load_surf( path, SDL_SWSURFACE ), 1000, hex_w, hex_h, sdl.screen, 0, 0 ) ) == 0 )
        goto failure;
    anim_hide( terrain_icons->cross, 1 );
    if ( !parser_get_value( pd, "explosion", &str, 0 ) ) goto parser_failure;
    sprintf( path, "terrain/%s", str );
    if ( ( terrain_icons->expl1 = anim_create( load_surf( path, SDL_SWSURFACE ), 50, hex_w, hex_h, sdl.screen, 0, 0 ) ) == 0 )
        goto failure;
    anim_hide( terrain_icons->expl1, 1 );
    if ( ( terrain_icons->expl2 = anim_create( load_surf( path, SDL_SWSURFACE ), 50, hex_w, hex_h, sdl.screen, 0, 0 ) ) == 0 )
        goto failure;
    ...
    while ( ( sub = list_next( entries ) ) ) {
        ...
        /* name */
        if ( !parser_get_string( sub, "name", &terrain_types[i].name ) ) goto parser_failure;
