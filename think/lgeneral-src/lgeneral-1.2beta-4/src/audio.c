...
int audio_open()
{
    if ( Mix_OpenAudio( MIX_DEFAULT_FREQUENCY, MIX_DEFAULT_FORMAT /*AUDIO_S16SYS*/, MIX_DEFAULT_CHANNELS, 4096 ) < 0 ) {
        fprintf( stderr, "%s\n", SDL_GetError() );
        audio_ok = 0;
        return 0;
    }
...
Wav* wav_load( char *fname, int channel )
{
    ...
    sprintf( path, "%s/sounds/%s", SRC_DIR, fname );
...
void wav_play( Wav *wav )
{
    ...
    if (wav->chunk) Mix_PlayChannel( wav->channel, wav->chunk, 0 );
}
