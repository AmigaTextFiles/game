...
static void shp_read_icon_header( FILE *file, Icon_Header *header )
{
    memset( header, 0, sizeof( Icon_Header ) );
    {
        ...
        fread( &dummy16, 2, 1, file );
        header->height = SDL_SwapLE16( dummy16 );

        header->height++; /* if y1 == y2 it is at least one line anyway */

        fread( &dummy16, 2, 1, file );
        header->width = SDL_SwapLE16( dummy16 );

        header->width++;

        fread( &dummy16, 2, 1, file );
    	header->cx = SDL_SwapLE16( dummy16 );

        fread( &dummy16, 2, 1, file );
        header->cy = SDL_SwapLE16( dummy16 );
	}

    {
        Sint32 dummy32;

        fread( &dummy32, 4, 1, file );
        header->x1 = SDL_SwapLE32( dummy32 );

        fread( &dummy32, 4, 1, file );
        header->y1 = SDL_SwapLE32( dummy32 );

        fread( &dummy32, 4, 1, file );
    	header->x2 = SDL_SwapLE32( dummy32 );

        fread( &dummy32, 4, 1, file );
    	header->y2 = SDL_SwapLE32( dummy32 );
    }
//printf("%d %d %d %d %ld %ld %ld %ld\n", header->width, header->height, header->cx, header->cy, header->x1, header->y1, header->x2, header->y2);
...
{
    // This code used memcpy, but it fails probably anywhere else than with 32-bit surfaces
    int pos = y * surf->pitch + x * surf->format->BytesPerPixel;

    switch( surf->format->BytesPerPixel )
	{
        case 4:
			*(Uint32*) ( (Uint8*) surf->pixels + pos) = pixel;
            break;

        case 2:
			*(Uint16*) ( (Uint8*) surf->pixels + pos) = pixel;
            break;

        case 1:
			*( (Uint8*) surf->pixels + pos) = pixel;
			break;

        default:
            printf("Error in shp.c/set_pixel() - %d!\n", surf->format->BytesPerPixel);
            break;
    }
...
Uint32 get_pixel( SDL_Surface *surf, int x, int y )
{
	...
	{
        case 4:
			pixel = *(Uint32*) ( (Uint8*) surf->pixels + pos);
            break;

        case 2:
			pixel = *(Uint16*) ( (Uint8*) surf->pixels + pos);
            break;

        case 1:
			pixel = *( (Uint8*) surf->pixels + pos);
			break;

        default:
            printf("Error in shp.c/get_pixel() - %d!\n", surf->format->BytesPerPixel );
            break;
    }
...
int shp_all_to_bmp( void )
{
    ...
    while ( ( dirent = readdir( dir ) ) != 0 ) {
        ...
        sprintf( path, "%s/.view/%s.bmp", dest_path, dirent->d_name );
