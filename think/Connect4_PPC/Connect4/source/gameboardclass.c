...
STATIC SAVEDS ULONG gameboardNew (struct IClass *cl, Object *obj, Msg *msg)
{
    ...
    gb_data->ehnode.ehn_Object  = obj;
    gb_data->ehnode.ehn_Class   = cl;
    gb_data->ehnode.ehn_Flags   = MUI_EHF_GUIMODE;
...
STATIC SAVEDS ULONG gameboardDispose (struct IClass *cl, Object *obj, Msg *msg)
{
...
STATIC SAVEDS ULONG gameboardSet (struct IClass *cl, Object *obj, struct opSet *msg)
{
    {
...
STATIC SAVEDS ULONG gameboardGet (struct IClass *cl, Object *obj, struct opGet *msg)
{
    struct gameboarddata *gb_data=INST_DATA(cl, obj);
...
STATIC SAVEDS ULONG gameboardDraw (struct IClass *cl, Object *obj, struct MUIP_Draw *msg)
{
...
STATIC SAVEDS ULONG gameboardHandleEvent (struct IClass *cl, Object *obj, struct MUIP_HandleEvent *msg)
{
...
STATIC SAVEDS ULONG gameboardSetup (struct IClass *cl, Object *obj, struct MUIP_Setup *msg)
{
...

    {
        ...
        STATIC CONST UBYTE mask_body[270] =
        {
            /* Plane 0 */
        ...
        STATIC CONST UBYTE blank_body[270] =
        {
            /*Plane 0*/
        ...
        gb_data->ehnode.ehn_Events = IDCMP_MOUSEBUTTONS;

        DoMethod(_win(obj), MUIM_Window_AddEventHandler, (ULONG)&gb_data->ehnode);
    }
...
STATIC SAVEDS ULONG gameboardCleanup (struct IClass *cl, Object *obj, Msg msg)
{
    struct gameboarddata *gb_data=INST_DATA(cl, obj);
    struct Screen *screen=_screen(obj);
...
*/
LONG testS (struct gameboarddata *gb_data, int column, int row)
{
    if (row>=3)
    {
        LONG total, i;
...
**          red_player or yellow_player, or -1 if no-one has won
*/
LONG testWE (struct gameboarddata *gb_data, int column, int row)
{
    LONG total, i;
...
**          red_player or yellow_player, or -1 if no-one has won
*/
LONG testNESW(struct gameboarddata *gb_data, int column, int row)
{
    LONG diagrow=0;
    LONG total=0;
    LONG diagcolumn;
...
**          red_player or yellow_player, or -1 if no-one has won
*/
LONG testNWSE(struct gameboarddata *gb_data, int column, int row)
{
    LONG diagrow=0;
    LONG total=0;
    LONG diagcolumn;
...
STATIC SAVEDS ULONG gameboardMakeMove (struct IClass *cl, Object *obj)
{
    struct gameboarddata *gb_data=INST_DATA(cl, obj);
...
{
#else
STATIC SAVEDS ULONG gameboardDispatcherPPC (void);
struct EmulLibEntry gameboardDispatcher = { TRAP_LIB, 0, (void (*)())&gameboardDispatcherPPC };
STATIC SAVEDS ULONG gameboardDispatcherPPC (void)
{
    ...
    switch (msg->MethodID)
    {
        case OM_NEW             : return(gameboardNew           (cl, obj, (APTR)msg));
        case OM_DISPOSE         : return(gameboardDispose       (cl, obj, (APTR)msg));
        ...
        case MUIM_HandleEvent   : return(gameboardHandleEvent   (cl, obj, (struct MUIP_HandleEvent *)msg));
        case MUIM_Setup         : return(gameboardSetup         (cl, obj, (struct MUIP_Setup *)msg));
        case MUIM_Cleanup       : return(gameboardCleanup       (cl, obj, msg));
        case MUIM_Gameboard_MakeMove  : return(gameboardMakeMove (cl,obj));
    }
