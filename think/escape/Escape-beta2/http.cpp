...
void httpreal::destroy () {
  /* ? */
  if (conn) {
    DMSG (itos((int)(void*)this) + " destroy\n");
    SDLNet_TCP_Close(conn);
    conn = 0;
  }
  delete this;
...
bool httpreal::connect(string chost, int port) {
  ...
  DMSG (itos((int)(void*)this) + " connect '" + chost + "':" + itos(port) + "\n");
  
  /* should work for "snoot.org" or "128.2.194.11" */
  if(SDLNet_ResolveHost(&remote, (char *)chost.c_str(), port)) {
    DMSG (itos((int)(void*)this) + " can't resolve: " + (string)(SDLNet_GetError()) + "\n");
    return false;
  }

  host = chost;

  DMSG (itos((int)(void*)this) + " ok\n");
...
httpresult httpreal::req_general(string req, string & res, bool tofile) {
  ...
  DMSG (itos((int)(void*)this) + " conn@" + itos((int)conn) + " req_general: \n[" + req + "]\n");

  /* we don't use keep-alive now. each request is a new
     connection. */
  bye();
  if (! (conn = SDLNet_TCP_Open(&remote))) {
    DMSG (itos((int)(void*)this) + " can't connect: " + (string)(SDLNet_GetError()) + "\n");
    return HT_ERROR;
  }

  DMSG (itos((int)(void*)this) + " connected.\n");
 
  if (!sendall(conn, req)) {
    /* error. try again? */
    DMSG (itos((int)(void*)this) + " can't send: " + (string)(SDLNet_GetError()) + "\n");
    SDLNet_TCP_Close(conn);
    conn = 0;
    return HT_ERROR;
  }

  DMSG (itos((int)(void*)this) + " sent request.\n");
  ...
  */
  for(;;) {
    if (SDLNet_TCP_Recv(conn, &c, 1) != 1) {
      DMSG (itos((int)(void*)this) + " can't recv: " + (string)(SDLNet_GetError()) + "\n");
	...
	if (status != "200") {
	  /* XXX or whatever ... */
	  DMSG (itos((int)(void*)this) + " got status code " + status + "\n");
