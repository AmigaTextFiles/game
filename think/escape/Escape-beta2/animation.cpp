...
/* broken */
#define deadrobot_forward_0 deadrobot
#define deadrobot_forward_1 deadrobot
#define deadrobot_backward_0 deadrobot
#define deadrobot_backward_1 deadrobot
#define deadrobot_left_0 deadrobot
#define deadrobot_left_1 deadrobot
#define deadrobot_right_0 deadrobot
#define deadrobot_right_1 deadrobot

static aframe frames_deadrobotforward[] =
  { {&animation::deadrobot_forward_1,  0, 4, 0},
    {&animation::deadrobot_forward_1,  0, 8, 20},
    {&animation::deadrobot_forward_1,  0, 12, 40},
    {&animation::deadrobot_forward_0,  0, 16, 20},
    {&animation::deadrobot_forward_0,  0, 20, 20},
    {&animation::deadrobot_forward_0,  0, 24, 20},
    {&animation::deadrobot_forward_1,  0, 28, 40},
    {&animation::deadrobot_forward_1,  0, 32, 20},
    {0, 0, 0, 0} };

static aframe frames_deadrobotbackward[] =
  { {&animation::deadrobot_backward_1,  0, -4, 0},
    {&animation::deadrobot_backward_1,  0, -8, 20},
    {&animation::deadrobot_backward_1,  0, -12, 40},
    {&animation::deadrobot_backward_0,  0, -16, 20},
    {&animation::deadrobot_backward_0,  0, -20, 20},
    {&animation::deadrobot_backward_0,  0, -24, 20},
    {&animation::deadrobot_backward_1,  0, -28, 40},
    {&animation::deadrobot_backward_1,  0, -32, 20},
    {0, 0, 0, 0} };

static aframe frames_deadrobotright[] =
  { {&animation::deadrobot_right_1,  4,  0,  0},
    {&animation::deadrobot_right_1,  8,  0,  20},
    {&animation::deadrobot_right_1,  12, 0,  40},
    {&animation::deadrobot_right_0,  16, 0,  20},
    {&animation::deadrobot_right_0,  20, 0,  20},
    {&animation::deadrobot_right_0,  24, 0,  20},
    {&animation::deadrobot_right_1,  28, 0,  40},
    {&animation::deadrobot_right_1,  32, 0,  20},
    {0, 0, 0, 0} };

static aframe frames_deadrobotleft[] =
  { {&animation::deadrobot_left_1,  -4,  0,  0},
    {&animation::deadrobot_left_1,  -8,  0,  20},
    {&animation::deadrobot_left_1,  -12, 0,  40},
    {&animation::deadrobot_left_0,  -16, 0,  20},
    {&animation::deadrobot_left_0,  -20, 0,  20},
    {&animation::deadrobot_left_0,  -24, 0,  20},
    {&animation::deadrobot_left_1,  -28, 0,  40},
    {&animation::deadrobot_left_1,  -32, 0,  20},
    {0, 0, 0, 0} };
/* end broken */

typedef ptrlist<animation> alist;

static inline SDL_Surface * FACING_FRAME(dir d, bot entt) {
  switch(entt) {
  default:
  case B_PLAYER:
    switch(d) {
...
void animation::start(drawing & dr, 
  ...
  case tag_lasered : {
	...
	alist::push(anims, l);
      ...
      /* optional */
      alist::push(anims, new aninplace(xx, yy, 1, frames_zap));
  ...
  case tag_liteup: {
    ...
    if (dr.onscreen(al->x, al->y, xx, yy)) {
      aframe * frames = 0;
      switch(al->what) {
      default: /* XXX error anim */
      case T_BLIGHT: frames = frames_bliteup; break;
      case T_RLIGHT: frames = frames_rliteup; break;
      case T_GLIGHT: frames = frames_gliteup; break;
	break;
      }

      animation * ar = new anwait(al->delay * WAIT_BETWEEN_WIRE);
...
#define JUICEFRAME(DD, dd) \
	...
	JUICEFRAME(WE, we);
      ...
      animation * ar = new anwait(aj->count * WAIT_BETWEEN_WIRE);
      ar->next = new aninplace(xx, yy, 1, frames);
  ...
  case tag_button: {
	...
	return ;
      ...
      /* PERF could use non-erasing (and non-alpha) blit
	 for these animations */
      anplacetile * ap = new anplacetile(ab->whatold, xx, yy);
      animation * ar = new aninplace(xx, yy, 1, frames_pressing);
  ...
  case tag_toggle: {
	...
	break;
	 ...
	 for these animations */
      ...
      if (at->delay) {
	animation * aw = new anwait(at->delay * WAIT_BETWEEN_WIRE);
	aw->next = ap;
	alist::push(anims, aw);
      } else {
	alist::push(anims, ap);
...
#define PICKFRAMES(C, c)                                               \
  ...
  case tag_press: {
    ...
    int sx, sy;
    if (dr.onscreen(as->x, as->y, sx, sy)) {
      anfinale * ag = new anfinale(FACING_FRAME(as->d, B_PLAYER),
  ...
  case tag_stand: {
    ...
    int sx, sy;
    if (dr.onscreen(as->x, as->y, sx, sy)) {
      anfinale * ag = new anfinale(FACING_FRAME(as->d, as->entt),
  ...
  case tag_walk: {
	...
	switch(aw->d) {
	default:
	case DIR_UP: wa = frames_walkbackward; break;
	case DIR_DOWN: wa = frames_walkforward; break;
	case DIR_RIGHT: wa = frames_walkright; break;
	case DIR_LEFT: wa = frames_walkleft; break;
	}
	break;
      case B_DALEK:
	...
	switch(aw->d) {
	default:
	case DIR_UP: wa = frames_dalekbackward; break;
	case DIR_DOWN: wa = frames_dalekforward; break;
	case DIR_RIGHT: wa = frames_dalekright; break;
	case DIR_LEFT: wa = frames_dalekleft; break;
	}
	break;
      case B_HUGBOT:
	...
	switch(aw->d) {
	default:
	case DIR_UP: wa = frames_hugbotbackward; break;
	case DIR_DOWN: wa = frames_hugbotforward; break;
	case DIR_RIGHT: wa = frames_hugbotright; break;
	case DIR_LEFT: wa = frames_hugbotleft; break;
	}
	break;
      case B_BROKEN:
	...
	switch(aw->d) {
	default:
	case DIR_UP: wa = frames_deadrobotbackward; break;
	case DIR_DOWN: wa = frames_deadrobotforward; break;
	case DIR_RIGHT: wa = frames_deadrobotright; break;
	case DIR_LEFT: wa = frames_deadrobotleft; break;
	}
	break;
      default: ;
	/* XXX Add other bot walking anims */
      }

      if (!wa) break;

      aninplace * af =
	new aninplace(sx, sy - overlapy, 1, wa);
      
      anfinale * ag = new anfinale(FACING_FRAME(aw->d, aw->entt),
				   sdx, sdy - overlapy);
...
void animation::clearent(drawing & dr, int entx, int enty, int overlap) {
  ...
  if (dr.onscreen(entx, enty, sx, sy))
    drawing::drawtile(sx, sy, dr.lev->tileat(entx, enty));

  if (enty > 0 && dr.onscreen(entx, enty - 1, sx, sy))
    drawing::drawtile(sx, sy, dr.lev->tileat(entx, enty - 1));
