...
void textbox::draw(int posx, int posy, int f) {
  ...
  if (f) color = SDL_MapRGBA(screen->format, 255, 255, 45,  255);
  ...
  */
  ...
  for(int i = 0; i < count; i ++) {
    ...
    fon->draw(posx, posy + y * fon->height, lastline);
...

  {
    ...
    /* Uint32 color = SDL_MapRGBA(screen->format, 120, 120, 180, 255); */
    SDL_Rect dst;

    /* left side */
    dst.x = posx + fon->sizex(lastline) + 1;
  ...
  while(rest && y < charsh) {
    ...
    while (w.length () > 0) {
	  ...
	  fon->draw(posx + (x * (fon->width - fon->overlap)),
