...
void drawing::drawbot(bot b, dir d,
  ...
  case B_BROKEN:
    ...
    s = animation::pic_deadrobot[zoomfactor]; break;
    break;
  case B_HUGBOT:
...
#   endif
     ...
     wrt to the guy as well */
  {
    for(int i = 0; i < lev->nbots; i ++) {
      int bsx, bsy;
      int bx, by;
      lev->where(lev->boti[i], bx, by);

      if (onscreen(bx, by, bsx, bsy))
	drawbot(lev->bott[i], lev->botd[i], bsx, bsy,
		zoomfactor, surf);
    }
  }
  ...
  {
    int gsx, gsy;
    if (onscreen(lev->guyx, lev->guyy, gsx, gsy)) {
      drawguy(GUY_OFFICE, lev->guyd, gsx, gsy, zoomfactor, surf, isdead);
    }
    /* XXX else cool to draw "magnifying glass" */   
  }
...
void drawing::drawsmall(int y, 
  ...
  /* if it fits, center it horizontally */
  dr.posx = (l->w >= TILESWIDE)?4 : 
    (4 + (((TILESWIDE - l->w) * (TILEW >> 2)) >> 1));
  dr.posy = y + ((l->h >= TILESHIGH)?8 :
		 (8 + (((TILESHIGH - l->h) * (TILEH >> 2)) >> 1)));
  dr.width = TILESWIDE * (TILEW >> 2);
  dr.height = TILESHIGH * (TILEH >> 2);
  dr.zoomfactor = 2;
  ...
  /* borders */
  int textx = 24 + ((TILEW >> 2) * TILESWIDE);
  ...
  if (date) {
    char buf[256];
    /* available on win32?? */
    strftime(buf, 250, "%d %b %Y", localtime((time_t*)&date));
