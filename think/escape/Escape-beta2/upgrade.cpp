...
upresult upgradereal::doupgrade(http * hh, string & msg,
  ...
  for(ulist * hd = upthese;
	...
	(string)"/" + util::replace(hd->head.filename, "/", "_");
...
#       else
	  ...
	  if (n < 0 ||
	      (string)(char*)(&buf) != dst) {
	    /* bad */
	    say(font::pad(fi, 16) + (string)RED " --> " POP WHITE
	    ...
	    say(font::pad(fi, 16) + (string)GREY " --> "
