...
struct ra_entry {
  ...
  ra_entry(string s, ratestatus vv, int d, int sr) 
    : filename(s), v(vv), date(d), speedrecord(sr) {}
  ra_entry() {}
...
struct di_real : public dirindex {
  ...
  virtual bool getentry(string filename, ratestatus & v, int &, int &);
...
bool di_real::getentry(string filename, ratestatus & v, int & d, int & sr) {
...
bool dirindex::isindex(string f) {
  return (hasmagic(f, INDEXMAGIC) ||
	  hasmagic(f, INDEX3MAGIC));
}
...
dirindex * dirindex::fromfile(string f) {
  ...
  string iii = readfilemagic(f, INDEXMAGIC);
...
void di_real::addentry(string f, ratestatus v, int date, int speedrecord) {
  tab->insert(new ra_entry(f, v, date, speedrecord));
}
