...
# define GUYCAP (CAP_ISPLAYER | CAP_CANTELEPORT | CAP_PUSHBOTS)
...
# define AFFECTENTEX(ei) if (ei == -1) ctx->affectplayer(this, etail); \
                         else ctx->affectbot(ei, this, etail);
# define AFFECTENT AFFECTENTEX(enti)
...
# define LITEWIRE(a, b, c, d) do { ; } while (0)
# define AFFECT(a, b) do { ; } while(0)
# define AFFECTI(a) do { ; } while(0)
# define AFFECTENT /* nuthin' */
# define AFFECTENTEX(a) do { ; } while(0)
    ...
    case T_PANEL: {
      ...
      if (pushing) {
        ...
        if (travel(newx, newy, d, farx, fary)) {
           ...
           /* deal with targeting new panels */
           if (ftarget == T_PANEL) {
           ...
           if (target == T_PANEL) {
              /* avoid double flip,
                 since pushing ent is
                 stepping onto this 
                 panel now */           
              /* do nothing */
           } else {
              CHECKSTEPOFF(newx, newy);
           }
        ...
        /* might push into other bot, elec? */
      } else {
    ...
    case T_BUTTON: {
      ...
      for(dir dd = FIRST_DIR; dd <= LAST_DIR; dd ++) {
        ...
        while(pd != DIR_NONE &&
          ...
          /* should be wire by invariant */
          if (back) { 
             LITEWIRE(pulsex, pulsey, targ, dist);
             dist --;
          } else {
            dist ++;
          }
