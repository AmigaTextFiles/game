...
struct ulreal : public upload {
  ...
  virtual upresult up(player * p, string file);
...
upresult ulreal::up (player * p, string f) {
  redraw();

  string levcont = readfilemagic(f, LEVELMAGIC);
  ...
  say(YELLOW + itos(slong->length) + GREY " -> " POP +
      itos(opt->length) + POP);
