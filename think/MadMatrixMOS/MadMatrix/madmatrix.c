...
/*  struct TagItem *tags,*tag;*/
  ...
  if (data->matrice)
  {
    ...
    if (data->port)
    {
      data->timer_io.tr_node.io_Message.mn_ReplyPort  = data->port;
      data->timer_io.tr_node.io_Message.mn_Length     = sizeof(data->timer_io);

      if (OpenDevice("timer.device", UNIT_MICROHZ, (struct IORequest *)&data->timer_io, 0) == 0)
      {
        data->timer_ok = 1;
...
{
#else
ULONG Madmatrix_DispatcherPPC(void)
{
...
void Madmatrix_Rotate(struct IClass *cl,Object *obj, int sens, double pas)
{
  ...
  for ( data->pas=0; data->pas <= 1; data->pas+=pas )
  {
    ...
    data->timer_io.tr_time.tv_micro   = 5000;
    DoIO((struct IORequest *)&data->timer_io);
    #endif
    MUI_Redraw(obj,MADF_DRAWUPDATE);
  }
  ...
  if ( data->taille != data->ntaille )
  {
    if ( data->groupe )
    {
      if (DoMethod(data->groupe,MUIM_Group_InitChange))
      {
        ...
        DoMethod(data->groupe,OM_REMMEMBER,(ULONG)obj);
        data->taille=data->ntaille;
        DoMethod(data->groupe,OM_ADDMEMBER,(ULONG)obj);
...
#ifdef __MORPHOS__
struct EmulLibEntry Madmatrix_Dispatcher = { TRAP_LIB, 0, (void (*)())&Madmatrix_DispatcherPPC };
#endif
