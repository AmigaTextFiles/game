...
#macro Quader(x0,y0,z0,x1,y1,z1,D)
  isosurface {
    ...
    Textur(pigment {
      granite
      colour_map {
        [0.35 rgb <0.5,0.5,0.55>]
        [0.4 rgb <0.5,0.4,0.3>]
        [0.45 rgb <0.5,0.53,0.5>]
        [0.7 rgb <0.5,0.53,0.5>]
        [0.7 rgb <0.54,0.5,0.5>]
      }
      scale <7,7,1>/3
...
#declare Wand = union {
  ...
  #while (I<Reihen)
    Quader(
      mod(I,2)/3,I*Hoehe/Reihen,0,
      (2+mod(I,2))/3,(I+1)*Hoehe/Reihen,1/3,
      10*I)
    #local I=I+1;
  #end
}
...
#declare Block = union {
  BlockAusWand(Wand)
  Quader(1/3,0,1/3,2/3,Hoehe,2/3,-10)
}
