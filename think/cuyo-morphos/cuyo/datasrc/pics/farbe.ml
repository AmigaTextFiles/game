...
let reduziere_farben1 verteilung zielanzahl =
  ...
  halbiere zielanzahl (anzahl verteilung) teil
...
let rec verteile palette (teil : int array) a =
    (* Erzeugt einen (int * bool) ref Baum, der genau die durch teil
...
let rec fueg_ein palette baum farbe i = match baum  with
    (* Nimmt eine neue Farbe in einen unit baum auf. *)
| Blatt (i',()) -> if i=i'
  then Blatt (i',())
  else
...
let rec zaehle farbe baum = match baum  with
    (* Geht von einem (int * bool) ref baum aus und teilt ihm mit, daß eine
       neue Farbe aufgenommen wurde. Entsprechend wird die Zusatzinformation
       angepasst. *)
...
let reduziere_farben2 palette zielanzahl =
  if zielanzahl >= Array.length palette
    then palette
      ...
      zaehle farbe1 gross;
      zaehle farbe2 gross;
      sammle (fueg_ein palette (Blatt (i1,())) farbe2 i2) 2)
