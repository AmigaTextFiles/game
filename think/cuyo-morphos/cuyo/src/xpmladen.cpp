...
bool ladeDateiLow(Str na, bool gz) {
  ...
  while ( gz  ?  !gzeof(gzf)  :  !feof(f) ) {
    ...
    if (gz) {
      ...
      if (neu < 0) {
        gzclose(gzf);
	/* Hier könnte man schönere Fehlermeldungen mit gzerror()
	   bekommen... */
...
void leerUndKommentarWeg() {
  while (1) {
    ...
    while (gDatBei[0] != '*' || gDatBei[1] != '/') {
      if (!gDatBei[0])
        throw Fehler(_("Endless comment."));
      gDatBei++;
    }
    gDatBei += 2;
  }
...
union suchbaum {
  ...
  suchbaum(int tiefe) {
    if (tiefe)
      for (int i=0; i<256; i++) weiter[i]=0;
  }

  inline Uint32* rein(int tiefe) {
    suchbaum* such = this;
    for (;tiefe--;) {
  ...
  inline Uint32 raus(int tiefe) {
    suchbaum* such = this;
    for (;tiefe--;) {
      if (!such->weiter[(unsigned int)*gDatBei])
        throw Fehler(_("Undefined pixel name"));
      such = such->weiter[(unsigned int)*gDatBei++];
    }
...
SDL_Surface * ladXPM(Str na) {
  ...
  try {
    ...
    tiefe = getInt()-1;
    if (tiefe<0)
      throw Fehler(_("The pixels consist of 0 chars. That's not yet supported in .xpm.gz files."));
    leerWeg();
    erwarte('"');
    ...
    for (int i = 0; i < farb; i++) {
      ...
      Uint32* index = farben.rein(tiefe);
      leerWeg();
      char typ = *gDatBei++;
      CASSERT(typ == 'c');
      leerWeg();
      ...
      if (kommtString("None")) {
        // durchsichtig
	*index = SDL_MapRGBA(pf, 0, 0, 0, 0);
      } else if (kommtString("black")) {
        *index = SDL_MapRGBA(pf, 0, 0, 0, 255);
      } else if (kommtString("white")) {
        *index = SDL_MapRGBA(pf, 255, 255, 255, 255);
      } else if (kommtString("Background")) {
        // Farbe vom Level-Hintergrund (für Explosion)
        *index = ld->mHintergrundFarbe.getPixel(pf);
	...
	int f_g = getHex();
	int f_b = getHex();
	*index = SDL_MapRGBA(pf, f_r, f_g, f_b, 255);
      }
      liesBis('"');
    }
    ...
    for (int y = 0; y < groesse_y; y++) {
      ...
      for (int x = 0; x < groesse_x; x++)
        SDLTools::getPixel32(s, x, y) = farben.raus(tiefe);
      erwarte('"');
    }
    ...
    if (gDebug)
      fprintf(stderr, _("Fast xpm loading did not work for \"%s\":\n%s\n"),
