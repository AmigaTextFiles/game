...
bool Fall::erzeug() {
  mPos.x = grx / 2 - 1;
  mPos.yy = mSpf->getHetzrandYPix() - gric + ld->mFall_langsam_pix;
...
void Fall::spielSchrittIntern() {
  ...
  if (mSchnell || mPos.r == richtung_einzel)
    fp2.yy += ld->mFall_schnell_pix;
  else
    fp2.yy += ld->mFall_langsam_pix;
  /* ld->mFall_langsam_pix könnte ja negativ sein. Also clippen. */
  if (fp2.yy < mSpf->getHetzrandYPix() - gric + ld->mFall_langsam_pix)
    fp2.yy = mSpf->getHetzrandYPix() - gric + ld->mFall_langsam_pix;
...
int Fall::testBelegt(FallPos p) const {
  ...
  /* Sonderfall: Zwei senkrechte Blops => Blop 0 braucht nicht getestet
     zu werden, da er in der Blop-1-Spalte liegt. Außerdem: Wenn im Senkrecht-
