...
void BildStapel::speichereBild(Sorte * so, int dat, int pos, int viertel,
  ...
  if (mAnz >= mMaxAnz)
    throw iFehler(_("Internal error: Too many picts drawn for one single blob"));
...
void BildStapel::print() const {
  fprintf(stderr, "d1=%d,d2=%d,p=%d", mDebugOut1, mDebugOut2, mAmPlatzen);
  for (int i = 0; i < mAnz; i++)
    fprintf(stderr, " %s:%d:%d:%d:%d", mStapel[i].mSorte->getName().data(),
                  mStapel[i].mDat, mStapel[i].mPos, mStapel[i].mViertel,
		  mStapel[i].ebene);
  fprintf(stderr, "\n");
}
