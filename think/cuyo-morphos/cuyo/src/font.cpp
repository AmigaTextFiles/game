...
void Font::drawText(const Str & text, int x, int y, TextAlign align) const {
  switch (align & AlignVMask) {
    case AlignBottom: y -= getTextHeight(text.data()); break;
    case AlignVCenter: y -= getTextHeight(text.data()) / 2; break;
  }

  const char * bei = text.data();
  while (1) {
...
void Font::wordBreak(Str & text, int width) const {
  int l = text.length();
  int w = 0;
  ...
  for (int i = 0; i < l; i++) {
	...
	if (last_break_pos == 0) last_break_pos = i;
	text[last_break_pos] = '\n';
	i = last_break_pos;
	w = 0;
	last_break_pos = 0;
...
int Font::getLineWidth(const char * text) const {
  int w = 0;
  while (*text != 0 && *text != '\n') {
...
void Font::init() {
  gGame = new Font("font-big.xpm", 24, 24, c2pBig, true, 1);
...
void Font::setGameColor(const Color & c) {
  gGame->mChars.bildNachbearbeiten(c);
}
