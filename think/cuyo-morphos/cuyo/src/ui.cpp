...
void init() {
  ...
  PrefsDaten::init();
  SDLTools::initSDL(L_fenster_breite, L_fenster_hoehe);
  Area::init();
  Font::init();
  Sound::init();
  BlattMenu::initMenus();
...
void doEvent(SDL_Event & evt) {
  ...
  /* Ggf. die Koordinaten der Events umrechnen (im Kleinmodus)
      Waere eigentlich schoener, wenn das irgendwie
      in SDLTools statt faende, weil da auch die
      restlichen Koordinatenumrechungen sind. */
  if (gKlein) {
    if (evt.type == SDL_MOUSEMOTION) {
      /* Einfach xrel und yrel mit 4/3 multiplizieren fuehrt zu
         Rundungsfehlern, die oft zu Problemen fuehren. */
      Uint16 xalt, yalt;
      xalt = evt.motion.x - evt.motion.xrel;
      yalt = evt.motion.y - evt.motion.yrel;
      evt.motion.x = evt.motion.x * 4 / 3;
      evt.motion.y = evt.motion.y * 4 / 3;
      xalt = xalt * 4 / 3;
      yalt = yalt * 4 / 3;
      evt.motion.xrel = evt.motion.x - xalt;
      evt.motion.yrel = evt.motion.y - yalt;
    } else if (evt.type == SDL_MOUSEBUTTONDOWN ||
               evt.type == SDL_MOUSEBUTTONUP) {
      evt.button.x = evt.button.x * 4 / 3;
      evt.button.y = evt.button.y * 4 / 3;
    }
  }
