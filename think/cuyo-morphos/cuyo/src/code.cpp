...
int Code::getStapelHoehe(int & nsh) const {
  ...
  //case bitset_acode:
    ...
    case undefiniert_code:
      throw iFehler(_("%s: Internal error in Code::getStapelHoehe(): Aufruf fuer Code %d"),
  ...
  try {
	...
	*/
    ...
    //case bitset_acode:
      ...
      case undefiniert_code:
	throw iFehler(_("Internal error in Code::eval(): Code-Art undefined_code"));
	break;
    }
    ...
    /* Fehlermeldung verbessern */
    if (!fe.mMitZeile) {
      /* Da ist noch keine Zeilenangabe dabei. Also eine hinzufügen. */
