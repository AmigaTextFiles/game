...
DrawDing::DrawDing(const Str & text, int hotkey,
  ...
  switch (align) {
    ...
    case AlignHCenter:
      ...
      break;
    default: throw iFehler(_("Invalid alignment"));
  }
  #if HOT_ALIGN
      if (hotkey >= 0) {
...
void DrawDing::anzeigen(int subBereich, int x, int y) const {
  ...
  case dda_Text: {
	...
	int y_ = y + (mY0+mY1)/2;
	Bilddatei & bild = BlattMenu::gMenuPics[menupic_highlight];
  ...
  case dda_Icon:
    BlattMenu::gMenuPics[mBild].malBildchen(x+mXPos,y+mY0,mBildchen);
    break;
  case dda_Bild: {
      Bilddatei & bi = BlattMenu::gMenuPics[mBild];
...
void MenuEintrag::updateDrawDinge() {
  ...
  mX0 = L_fenster_breite;
  mX1 = -L_fenster_breite;
...
void MenuEintrag::anzeigen(int x, int y, bool graue) {
  ...
  Area::fillRect(0, y, L_fenster_breite, mHoehe, Color(30, 30, 70));
  Area::updateRect(0, y, L_fenster_breite, mHoehe);

  /* Position von Highlight-Hintergrund und Grauen bestimmen */

  int hx0 = L_fenster_breite, hx1 = 0; /* Hintergrund */
  int hy0 = L_fenster_hoehe, hy1 = 0;

  int gx0 = L_fenster_breite, gx1 = 0; /* Graue */
  ...
  if (hx1 > 0) {
    ...
    // Waagerechte Kanten
    int dx;
    for (dx=0; dx+gric<=w2; dx+=gric) {
      bild.malBildAusschnitt(hx0+w1+dx,hy0,
			     SDLTools::rect(gric,gric-h1,gric,h1));
      bild.malBildAusschnitt(hx0+w1+dx,hy1-h3,
			     SDLTools::rect(gric,2*gric,gric,h3));
    }
    bild.malBildAusschnitt(hx0+w1+dx,hy0,
			   SDLTools::rect(gric,gric-h1,w2-dx,h1));
    bild.malBildAusschnitt(hx0+w1+dx,hy1-h3,
			   SDLTools::rect(gric,2*gric,w2-dx,h3));
    // Senkrechte Kanten
    int dy;
    for (dy=0; dy+gric<=h2; dy+=gric) {
      bild.malBildAusschnitt(hx0,hy0+h1+dy,
			     SDLTools::rect(gric-w1,gric,w1,gric));
      bild.malBildAusschnitt(hx1-w3,hy0+h1+dy,
			     SDLTools::rect(2*gric,gric,w3,gric));
    }
    bild.malBildAusschnitt(hx0,hy0+h1+dy,
			   SDLTools::rect(gric-w1,gric,w1,h2-dy));
    bild.malBildAusschnitt(hx1-w3,hy0+h1+dy,
			   SDLTools::rect(2*gric,gric,w3,h2-dy));
    // Fläche
    if (w2>0 && h2>0)
  ...
  /* Graue malen */
  if (graue) {
    BlattMenu::gMenuPics[menupic_pfeile].malBildchen(x+gx0-L_grausep-gric,
						     y+mHoehe/2-gric/2,
						     0);
    BlattMenu::gMenuPics[menupic_pfeile].malBildchen(x+gx1+L_grausep,
						     y+mHoehe/2-gric/2,
						     1);
  }
}
...
MenuEintragBild::MenuEintragBild(BlattMenu * papi, int nr):
   MenuEintrag(papi, "", MenuEintrag::Art_deko, BlattMenu::gMenuPics[nr].getHoehe()),
   mBildNr(nr) {
}
...
MenuEintragAuswahlmenu::MenuEintragAuswahlmenu(BlattMenu * papi,
    ...
    MenuEintragSubmenu(papi, name,
		       ...
		       accel,
		       L_menueintrag_defaulthoehe+L_menueintrag_schrifthoehe),
...
void MenuEintragAuswahlmenu::updateDDIntern() {
  int y = mHoehe/2-L_menueintrag_schrifthoehe/2;
  int x0,x1;
  neuDraw() = DrawDing(mName,mAccIndex, subbereich_default, 0, y, AlignHCenter,
		       NULL, &x0, &x1);

  int x_mitte = (x0 + x1) / 2;
  y += L_menueintrag_schrifthoehe;
...
//     
//     /*
//     neuDraw() = DrawDing(menupic_pfeile,2,x0-L_grausep-gric/2,y);
//     neuDraw() = DrawDing(menupic_pfeile,2+1,x1+L_grausep+gric/2,y);
//     */
//   }
}
