...
TankClass::TankClass(Stream& stream) : TrackedUnit(stream)
{
    TankClass::init();
...
void TankClass::save(Stream& stream) const
{
...
void TankClass::blitToScreen()
{
    ...
    if(drawnTurretAngle < 0 || drawnTurretAngle > 7) {
        fprintf(stdout,"Invalid drawnTurretAngle(%d)\n", drawnTurretAngle);
    }
...
int TankClass::getCurrentAttackAngle() {
