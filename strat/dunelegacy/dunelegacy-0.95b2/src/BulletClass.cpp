...
BulletClass::BulletClass(Stream& stream)
{
	...
	if(x < MAX_PLAYERS) {
		owner = currentGame->player[x];
	} else {
		owner = currentGame->player[0];
	}
...
int BulletClass::init()
{
	...
	int houseNum = (owner == NULL) ? HOUSE_HARKONNEN : owner->getHouse();
	...
	switch(bulletID) {
        case(Bullet_Inf_Gun): {
            ...
            speed = 9.0;
            numFrames = 1;
            graphic = pDataManager->getObjPic(ObjPic_Bullet_Small, houseNum);
        } break;
        ...
        case(Bullet_DRocket): {
            ...
            speed = 4.0;
            numFrames = 16;
            graphic = pDataManager->getObjPic(ObjPic_Bullet_MediumRocket, houseNum);
        } break;
        ...
        case(Bullet_LargeRocket): {
            ...
            speed = 4.0;
            numFrames = 16;
            graphic = pDataManager->getObjPic(ObjPic_Bullet_LargeRocket, houseNum);
        } break;
        ...
        case(Bullet_Shell): {
            ...
            speed = 9.0;
            numFrames = 1;
            graphic = pDataManager->getObjPic(ObjPic_Bullet_Medium, houseNum);
        } break;
        ...
        case(Bullet_SmallRocket): {
            ...
            speed = 4.0;
            numFrames = 16;
            graphic = pDataManager->getObjPic(ObjPic_Bullet_SmallRocket, houseNum);
        } break;
        ...
        case(Bullet_Sonic): {
            ...
            speed = 4.0;
            numFrames = 1;
            SDL_Surface* tmpSurface = pDataManager->getObjPic(ObjPic_Bullet_Sonic, houseNum);
            graphic =  copySurface(tmpSurface);	//make a copy of the image
        } break;

        case(Bullet_Sandworm): {
            fprintf(stdout,"BulletClass: Unknown Bullet_Sandworm not allowed.\n");
            inaccuracy = 0;
            graphic = NULL;
        } break;

        default: {
            fprintf(stdout,"BulletClass: Unknown Bullet type %d.\n",bulletID);
            inaccuracy = 0;
            graphic = NULL;
        } break;
	}
...
void BulletClass::blitToScreen()
{
    ...
    if(bulletID == Bullet_Sonic) {
        SDL_Surface *mask = pDataManager->getObjPic(ObjPic_Bullet_Sonic,(owner == NULL) ? HOUSE_HARKONNEN : owner->getHouse());
		...
		if(distance_from(source.x, source.y, realX, realY) >= distance_from(source, destination))	{
			...
			if((location.x != oldLocation.x) || (location.y != oldLocation.y)) {
				...
				currentGameMap->damage(shooterID, owner, realPos, bulletID,
                                        (int) (((double)damage)/((double)currentGame->objectData.data[Unit_SonicTank].weaponrange)),
                                        damageRadius, false);
			}
...
void BulletClass::destroy()
{
    ...
    switch(bulletID) {
        ...
        case Bullet_DRocket: {
            ...
            currentGame->getExplosionList().push_back(new Explosion(Explosion_Gas,position,houseNum));
        } break;
            ...
            for(int i = 0; i < 5; i++) {
                for(int j = 0; j < 5; j++) {
                    if (((i != 0) && (i != 4)) || ((j != 0) && (j != 4))) {
                        ...
                        currentGame->getExplosionList().push_back(new Explosion(explosionID,position,houseNum));
                    }
                }
            }
        } break;
        ...
        {
            currentGameMap->damage(shooterID, owner, position, bulletID, damage, damageRadius, airAttack);
            currentGame->getExplosionList().push_back(new Explosion(Explosion_Small,position,houseNum));
        } break;

        case Bullet_Shell: {
            currentGameMap->damage(shooterID, owner, position, bulletID, damage, damageRadius, airAttack);
            currentGame->getExplosionList().push_back(new Explosion(Explosion_Shell,position,houseNum));
        } break;
