...
void ListBox::HandleMouseMovement(Sint32 x, Sint32 y) {
	scrollbar.HandleMouseMovement(x - GetSize().x + scrollbar.GetSize().x,y);
}
...
void ListBox::UpdateList() {
	...
	// create surfaces
	pForeground = GUIStyle::GetInstance().CreateEmptySurface(GetSize().x - 4, GetSize().y - 2,true);
	...
	for(int i = FirstVisibleElement; i < FirstVisibleElement + numVisibleElements; i++) {
		...
		SDL_Surface* pSurface = GUIStyle::GetInstance().CreateListBoxEntry(GetSize().x - 4,GetEntry(i),(i==SelectedElement) ? true : false);
		dest.w = pSurface->w;
		dest.h = pSurface->h;
	...
	scrollbar.SetRange(0,std::max(0,GetNumEntries() - numVisibleElements + 1));
}
