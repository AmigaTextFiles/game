...
LoadSaveWindow::LoadSaveWindow(bool bSave, std::string caption, std::string directory, std::string extension)
 : Window(0,0,0,0) {
	saveWindow = bSave;
	...
	// set up window
	SDL_Surface *surf;
	surf = pDataManager->getUIGraphic(UI_LoadSaveWindow);
	...
	TitleText.SetTextColor(110);
	TitleText.SetAlignment(Alignment_Center);
	TitleText.SetText(caption);
	Main_VBox.AddWidget(&TitleText);

	Main_VBox.AddWidget(VSpacer::Create(6));

	Main_VBox.AddWidget(&FileList_HBox);
	FileList_HBox.AddWidget(VSpacer::Create(bSave ? 100 : 130));
	...
	Button_OK.SetText(bSave ? "SAVE" : "LOAD");
...
void LoadSaveWindow::UpdateEntries() {
	std::list<std::string> Files = GetFileNames(directory,extension,true);
