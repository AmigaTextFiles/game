			...
			for(int x=0;x<scenes[i]->loops;x++)
			{
				...
				for(int j=0;j<scenes[i]->nFrames;j++)
				{
					if(!skipScene && !skipIndividScene)
					{
						...
						if(SDL_BlitSurface(scenes[i]->frames[j], NULL, screen, &cutScenePos) != 0) {
							fprintf(stdout,"DuneCutScene::playCutScene(): Error while Bliting!\n");
							fflush(stdout);
						}
...
void DuneCutScene::loadPalette(char *filename)
{
	...
	if(f == NULL) {
		fprintf(stdout,"DuneCutScene::loadPalette(): Unable to open file %s!\n",filename);
		fflush(stdout);
		exit(EXIT_FAILURE);
	}
...
{
	currentGame->AddToNewsTicker("loading scenes...");
	SDL_Flip(screen);
	...
	scenes[0]->voSounds[0]->sound = Mix_LoadWAV("data/sound/introvoc/resampled/dune.wav");
	...
	scenes[0]->voSounds[1]->sound = Mix_LoadWAV("data/sound/introvoc/resampled/blding.wav");
	...
	scenes[0]->voSounds[2]->sound = Mix_LoadWAV("data/sound/introvoc/resampled/dynasty.wav");
		...
		while((line = fgets(line, 1024, f)) != NULL)
		{
			...
			if(line[0] != '#')
			{
				...
				else if(strstr(line, "BASEPATH"))
				{
					...
					scenes[sNum]->basePath = (char *)malloc(strlen(path)+1);
				...
				else if(strstr(line, "TEXT") && strstr(line, "TEXTS") == NULL && strstr(line, "VOSOUND") == NULL)
				{
					...
					else
					{
						...
						sscanf(line, "%d TEXT %d %s\n", &sNum, &vosNum, path);
						scenes[sNum]->subtitles[vosNum]->text = (char *)malloc(strlen(path)+1);
				...
				else if(strstr(line, "VOSOUND") && strstr(line, "VOSOUNDS") == NULL)
				{
					...
					else if(strstr(line, "PATH"))
					{
						...
						if((scenes[sNum]->voSounds[vosNum]->sound = Mix_LoadWAV(path)) == NULL) {
							fprintf(stdout,"DuneCutScene::parseScenesFile(): Cannot load %s!\n",path);
							exit(EXIT_FAILURE);
						}
					}
					else if(strstr(line, "TEXT"))
					{
						...
						scenes[sNum]->voSounds[vosNum]->text = (char *)malloc(strlen(path)+1);
...
void DuneCutScene::loadSceneImages(Dune_Scene* scene)
{
	...
	if(!scene->hasSets)
	{
		...
		for(int i=1;i<=scene->nFrames;i++)
		{
			...
			if((scene->frames[i -1] = SDL_LoadBMP(path)) == NULL) {
				fprintf(stdout,"DuneCutScene::loadSceneImages(): Cannot load picture %s!\n",getFramePath(scene->basePath, i));
				exit(EXIT_FAILURE);
			}
...
void DuneCutScene::showText(int x, int y, char *text, int colour)
{
	...
	{
		if(hasPalette) {
			textSurface = pFontManager->createSurfaceWithText(text, colour, FONT_STD24);
		} else {
			fprintf(stdout,"ERROR: DuneCutScene::showText(): This picture has no palette.\n");
			exit(EXIT_FAILURE);
			//TTF_RenderText_Solid(font[FONT24], text, palette->colors[colour]);
		}
		textPos.x = x;
		textPos.y = y;
