...
void Explosion::init()
{
    switch(explosionID) {
        case Explosion_Small: {
            graphic = pDataManager->getObjPic(ObjPic_ExplosionSmall);
            numFrames = 5;
        } break;

        case Explosion_Medium1: {
            graphic = pDataManager->getObjPic(ObjPic_ExplosionMedium1);
            numFrames = 5;
        } break;

        case Explosion_Medium2: {
            graphic = pDataManager->getObjPic(ObjPic_ExplosionMedium2);
            numFrames = 5;
        } break;

        case Explosion_Large1: {
            graphic = pDataManager->getObjPic(ObjPic_ExplosionLarge1);
            numFrames = 5;
        } break;

        case Explosion_Large2: {
            graphic = pDataManager->getObjPic(ObjPic_ExplosionLarge2);
            numFrames = 5;
        } break;

        case Explosion_Gas: {
            graphic = pDataManager->getObjPic(ObjPic_Hit_Gas, house);
            numFrames = 5;
        } break;

        case Explosion_Shell: {
            graphic = pDataManager->getObjPic(ObjPic_Hit_Shell);
            numFrames = 3;
        } break;

        case Explosion_SmallUnit: {
            graphic = pDataManager->getObjPic(ObjPic_ExplosionSmallUnit);
            numFrames = 2;
        } break;

        case Explosion_Flames: {
            graphic = pDataManager->getObjPic(ObjPic_ExplosionFlames);
            numFrames = 21;
        } break;

        default: {
            fprintf(stdout,"Explosion: Unknown explosion type %d.\n",explosionID);
            exit(EXIT_FAILURE);
        } break;
    }
}
