...
HouseChoiceInfoMenu::HouseChoiceInfoMenu(int newHouse) : MentatMenu(newHouse)
{
	DisableQuiting(true);
	...
	switch(house) {
		case HOUSE_ATREIDES:
		case HOUSE_FREMEN:
			anim = pDataManager->getAnimation(Anim_AtreidesPlanet);
			break;
		case HOUSE_ORDOS:
		case HOUSE_MERCENARY:
			anim = pDataManager->getAnimation(Anim_OrdosPlanet);
			break;
		case HOUSE_HARKONNEN:
		case HOUSE_SARDAUKAR:
		default:
			anim = pDataManager->getAnimation(Anim_HarkonnenPlanet);
			break;
	}
	...
	std::string desc = pDataManager->GetBriefingText(0,MISSION_DESCRIPTION,house);
	...
	surf = pDataManager->getUIGraphic(UI_MentatYes);
	surfPressed = pDataManager->getUIGraphic(UI_MentatYes_Pressed);
	...
	surf = pDataManager->getUIGraphic(UI_MentatNo);
	surfPressed = pDataManager->getUIGraphic(UI_MentatNo_Pressed);
