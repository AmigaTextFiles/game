...
#include <FileClasses/adl/emuopl.h>
...
CadlPlayer::CadlPlayer(SDL_RWops* rwop, int freq)
  : numsubsongs(0), _trackEntries(), _soundDataPtr(0), m_freq(freq)
{
  opl = new CEmuopl(m_freq, true, false);
...
Mix_Chunk* CadlPlayer::getSubsong(int Num) {
	...
	while(update()) {
		for(towrite = (unsigned long) (22050 / getrefresh()); towrite; towrite -= write) {
			write = 1; //(towrite > BUFSIZE ? BUFSIZE : towrite);
		...
		if(bufsize > 1024*1024*8) {
			fprintf(stdout,"CadlPlayer::getSubsong(): Decoding aborted after 16MB have been decoded.\n");
			break;		
		}
	}
...
Mix_Chunk* CadlPlayer::getUpsampledSubsong(int Num, int TargetFrequency, Uint16 TargetFormat, int channels) {
	...
	float ConversionRatio = ((float) TargetFrequency) / 22050.0;
...
bool CadlPlayer::load(SDL_RWops* rwop)
{
  ...
  if((file_size = SDL_RWseek(rwop,0,SEEK_END)) <= 0) {
  	fprintf(stdout,"CadlPlayer::load(): Cannot seek in SDL_RWop!\n");
	return false;
  }
  ...
  if(SDL_RWread(rwop,file_data,1,file_size) != file_size) {
  	fprintf(stdout,"CadlPlayer::load(): Cannot read from SDL_RWop!\n");
	return false;
  }
