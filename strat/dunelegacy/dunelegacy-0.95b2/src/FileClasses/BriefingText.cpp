...
BriefingText::BriefingText(SDL_RWops* RWop) {
	int BriefingTextFilesize;
	unsigned char* Filedata;
	Uint16* Index;

	if(RWop == NULL) {
		fprintf(stdout, "BriefingText: RWop == NULL!\n");
		exit(EXIT_FAILURE);
	}

	BriefingTextFilesize = SDL_RWseek(RWop,0,SEEK_END);
	if(BriefingTextFilesize <= 0) {
		fprintf(stdout,"BriefingText: Cannot determine size of this *.wsa-File!\n");
		exit(EXIT_FAILURE);
	}

	if(BriefingTextFilesize < 2) {
		fprintf(stdout, "BriefingText: No valid Briefing-File: File too small!\n");
		exit(EXIT_FAILURE);
	}

	if(SDL_RWseek(RWop,0,SEEK_SET) != 0) {
		fprintf(stdout,"BriefingText: Seeking in this Briefing-File failed!\n");
		exit(EXIT_FAILURE);
	}

	if( (Filedata = (unsigned char*) malloc(BriefingTextFilesize)) == NULL) {
		fprintf(stdout,"BriefingText: Allocating memory failed!\n");
		exit(EXIT_FAILURE);
	}

	if(SDL_RWread(RWop, Filedata, BriefingTextFilesize, 1) != 1) {
		fprintf(stdout,"BriefingText: Reading this Briefing-File failed!\n");
		exit(EXIT_FAILURE);
	}

	numBriefingStrings = ((int)SDL_SwapLE16(((Uint16*) Filedata)[0]))/2 - 1;
	Index = (Uint16*) Filedata;
	for(int i=0; i <= numBriefingStrings; i++) {
		Index[i] = SDL_SwapLE16(Index[i]);
	}

	BriefingString = new std::string[numBriefingStrings];

	for(int i=0; i < numBriefingStrings;i++) {
		std::string tmp = (const char*) (Filedata+Index[i]);
		BriefingString[i] = decodeString(tmp);
	}
	...
	for(unsigned int i = 0; i < text.length(); i++) {
		...
		switch(databyte) {
			...
			case 0x1B: {
				...
				i++;
				if(i == text.length()) {
					fprintf(stdout,"BriefingText::decodeString: Special character escape sequence at end of string!\n");
					exit(EXIT_FAILURE);
				}
