...
static Uint8 *LoadVOC_RW(SDL_RWops* rwop, Uint32 &size, Uint32 &rate) {
	...
	if(SDL_RWread(rwop,(char*) description,20,1) != 1) {
		fprintf(stdout,"loadVOCFromStream: Invalid header!\n");
		return NULL;
	}
	...
	if(offset != sizeof(description) + sizeof(offset) + sizeof(version) + sizeof(id)) {
		fprintf(stdout,"loadVOCFromStream: Invalid datablock offset in header!\n");
		return NULL;
	}
	...
	if(!(version == 0x010A || version == 0x0114 || version == 0x0100)) {
		fprintf(stdout,"loadVOCFromStream: Invalid version (0x%X) in header!\n",version);
		return NULL;
	}

	if(id != (~version + 0x1234)) {
		fprintf(stdout,"loadVOCFromStream: Invalid id in header!\n");
		return NULL;
	}
	...
	while (SDL_RWread(rwop,&code,sizeof(Uint8),1) == 1) {
		...
		if(SDL_RWread(rwop,tmp,1,3) != 3) {
			fprintf(stdout,"loadVOCFromStream: Invalid block length!\n");
			return ret_sound;
		}
		len = tmp[0];
		len |= tmp[1] << 8;
		...
		switch (code) {
			case VOC_CODE_DATA: {
				...
				if(SDL_RWread(rwop,&time_constant,sizeof(Uint8),1) != 1) {
					fprintf(stdout,"loadVOCFromStream: Cannot read time constant!\n");
					return ret_sound;
				}

				Uint8 packing;
				if(SDL_RWread(rwop,&packing,sizeof(Uint8),1) != 1) {
					fprintf(stdout,"loadVOCFromStream: Cannot read packing!\n");
					return ret_sound;
				}
				len -= 2;
				Uint32 tmp_rate = getSampleRateFromVOCRate(time_constant);
				if((rate != 0) && (rate != tmp_rate)) {
					fprintf(stdout,"This voc-file contains data blocks with different sampling rates: old rate: %d, new rate: %d\n",rate,tmp_rate);
				}
				rate = tmp_rate;

				//fprintf(stdout,"VOC Data Block: Rate: %d, Packing: %d, Length: %d\n", rate, packing, len);
				...
				if (packing == 0) {
					...
					if(SDL_RWread(rwop,ret_sound + size,1,len) != len) {
						fprintf(stdout,"loadVOCFromStream: Cannot read data!\n");
						return ret_sound;
					}

					size += len;
				} else {
					fprintf(stdout,"VOC file packing %d unsupported!\n", packing);
				}
			} break;
			...
			case VOC_CODE_SILENCE: {
				...
				if(SDL_RWread(rwop,&SilenceLength,sizeof(Uint16),1) != 1) {
					fprintf(stdout,"loadVOCFromStream: Cannot read silence length!\n");
					return ret_sound;
				}
				...
				if(rate != 0) {
					...
					fprintf(stdout,"The silence in this voc-file is right at the beginning. Therefore it is not possible to adjust the silence sample rate to the sample rate of the other sound data in this file!\n");
			...
			default:
				fprintf(stdout,"Unhandled code in VOC file : %d\n", code);
				return ret_sound;
		}
	}
	return ret_sound;
