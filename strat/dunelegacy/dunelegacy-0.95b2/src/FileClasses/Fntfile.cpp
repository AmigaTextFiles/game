...
Fntfile::Fntfile(SDL_RWops* RWop)
{
	...
	Uint32 FntFilesize;
	if(RWop == NULL) {
		fprintf(stdout, "Fntfile: RWop == NULL!\n");
		exit(EXIT_FAILURE);
	}

	FntFilesize = SDL_RWseek(RWop,0,SEEK_END);
	if(FntFilesize <= 0) {
		fprintf(stdout,"Fntfile: Cannot determine size of this *.fnt-File!\n");
		exit(EXIT_FAILURE);
	}

	if(SDL_RWseek(RWop,0,SEEK_SET) != 0) {
		fprintf(stdout,"Fntfile: Seeking in this *.fnt-File failed!\n");
		exit(EXIT_FAILURE);
	}

	if( (Filedata = (unsigned char*) malloc(FntFilesize)) == NULL) {
		fprintf(stdout,"Fntfile: Allocating memory failed!\n");
		exit(EXIT_FAILURE);
	}

	if(SDL_RWread(RWop, Filedata, FntFilesize, 1) != 1) {
		fprintf(stdout,"Fntfile: Reading this *.fnt-File failed!\n");
		exit(EXIT_FAILURE);
	}
...
void Fntfile::DrawText(SDL_Surface* pSurface, const char* text, char BaseColor) {
	if(text == NULL) {
		return;
	}
...
*/
int	Fntfile::getTextWidth(const char* text) {
	if(text == NULL) {
		return 0;
	}
...
void Fntfile::readfile(unsigned char* Filedata, Uint16 Filesize) {
	...
	if(Filesize < 20) {
		fprintf(stdout,"Fntfile::readfile(): A valid fnt-file must be at least 20 bytes long!\n");
		exit(EXIT_FAILURE);
	}
	...
	if((size != Filesize) || (magic1 != 0x0500)  || (magic2 != 0x000E) || (magic3 != 0x1012)) {
		fprintf(stdout,"Fntfile::readfile(): Invalid filesize or magic number!\n");
		exit(EXIT_FAILURE);
	}
	...
	if(		(SDL_SwapLE16(((Uint16*) Filedata)[4]) >= Filesize)
		...
		fprintf(stdout,"Fntfile::readfile(): Invalid data positions!\n");
		exit(EXIT_FAILURE);
	}
	...
	if(	( (unsigned char*) (chars+NumCharacters) > (Filedata+Filesize))
		...
		fprintf(stdout,"Fntfile::readfile(): Invalid fnt-File. File too small and cannot contain all characters!\n");
		exit(EXIT_FAILURE);
	}

	if((Character = (FntfileCharacter*) calloc(sizeof(FntfileCharacter) * NumCharacters,1)) == NULL) {
		fprintf(stdout,"Fntfile::readfile(): Cannot allocate memory!\n");
		exit(EXIT_FAILURE);
	}
	...
	for(Uint32 i = 0; i < NumCharacters; i++) {
		...
		if(chardata + charHeight*((Character[i].width+1)/2) > (Filedata+Filesize)) {
			fprintf(stdout,"Fntfile::readfile(): Invalid character graphic. The graphic goes beyond end of file!\n");
			exit(EXIT_FAILURE);
		}

		if(yPos+charHeight > CharacterHeight) {
			fprintf(stdout,"Fntfile::readfile(): Invalid character graphic. The character height is bigger than the font height!\n");
			exit(EXIT_FAILURE);
		}
