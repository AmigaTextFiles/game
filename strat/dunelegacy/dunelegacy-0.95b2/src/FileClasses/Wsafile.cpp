...
*/
Wsafile::Wsafile(SDL_RWops* RWop) : Decode()
{
	readdata(1,RWop);
}
...
*/
Wsafile::Wsafile(SDL_RWops* RWop0, SDL_RWops* RWop1) : Decode()
{
...
*/
Wsafile::Wsafile(SDL_RWops* RWop0, SDL_RWops* RWop1, SDL_RWops* RWop2) : Decode()
{
...
*/
Wsafile::Wsafile(SDL_RWops* RWop0, SDL_RWops* RWop1, SDL_RWops* RWop2, SDL_RWops* RWop3) : Decode()
{
...
SDL_Surface * Wsafile::getAnimationAsPictureRow() {
	...
	for(int i = 0; i < NumFrames; i++) {
		Image = decodedFrames + (i * SizeX * SizeY);
...
/**
	This helper method decodes on frame.
...
void Wsafile::decodeFrames(unsigned char* Filedata, Uint32* Index, int NumberOfFrames, unsigned char* pDecodedFrames, int x, int y)
{
	...
	for(int i=0;i<NumberOfFrames;i++) {
		if( (dec80 = (unsigned char*) malloc(x*y*2)) == NULL) {
			fprintf(stdout, "Error: Unable to allocate memory for decoded WSA-Frames!\n");
			exit(EXIT_FAILURE);
		}
...
unsigned char* Wsafile::readfile(SDL_RWops* RWop, int* filesize) {
	...
	if(filesize == NULL) {
		fprintf(stdout, "Wsafile: filesize == NULL!\n");
		exit(EXIT_FAILURE);
	}

	if(RWop == NULL) {
		fprintf(stdout, "Wsafile: RWop == NULL!\n");
		exit(EXIT_FAILURE);
	}

	WsaFilesize = SDL_RWseek(RWop,0,SEEK_END);
	if(WsaFilesize <= 0) {
		fprintf(stdout,"Wsafile: Cannot determine size of this *.wsa-File!\n");
		exit(EXIT_FAILURE);
	}

	if(WsaFilesize < 10) {
		fprintf(stdout, "Wsafile: No valid WSA-File: File too small!\n");
		exit(EXIT_FAILURE);
	}

	if(SDL_RWseek(RWop,0,SEEK_SET) != 0) {
		fprintf(stdout,"Wsafile: Seeking in this *.wsa-File failed!\n");
		exit(EXIT_FAILURE);
	}

	if( (Filedata = (unsigned char*) malloc(WsaFilesize)) == NULL) {
		fprintf(stdout,"Wsafile: Allocating memory failed!\n");
		exit(EXIT_FAILURE);
	}

	if(SDL_RWread(RWop, Filedata, WsaFilesize, 1) != 1) {
		fprintf(stdout,"Wsafile: Reading this *.wsa-File failed!\n");
		exit(EXIT_FAILURE);
	}
...
void Wsafile::readdata(int NumFiles, va_list args) {
	...
	if((Filedata = (unsigned char**) malloc(sizeof(unsigned char*) * NumFiles)) == NULL) {
		fprintf(stdout, "Wsafile::readdata(): Unable to allocate memory!\n");
		exit(EXIT_FAILURE);
	}
	...
	for(int i = 0; i < NumFiles; i++) {
		...
		if(i == 0) {
			...
			if( (SizeX != (SDL_SwapLE16(*((Uint16*) (Filedata[i] + 2)) )))
				...
				fprintf(stdout, "Wsafile: The wsa-files have different picture dimensions. Cannot concatinate them!\n");
				exit(EXIT_FAILURE);
			}

			if(FpsTimes1024ms != SDL_SwapLE32( *((Uint32*) (Filedata[i]+6)) )) {
				Uint32 NewFpsTimes1024ms = SDL_SwapLE32( *((Uint32*) (Filedata[i]+6)) );
				fprintf(stdout, "Wsafile: Warning! The wsa-files have different framerates. 1. File: fps=%d*1024ms=%f, %d. File: fps=%d*1024ms=%f\n",FpsTimes1024ms, FpsTimes1024ms/1024.0,i+1, NewFpsTimes1024ms,NewFpsTimes1024ms/1024.0);
			}
		}
		...
		if(Index[i][0] == 0) {
			// extended animation
			if(i == 0) {
				fprintf(stdout,"Extended WSA-File!\n");
			}
			Index[i]++;
			NumberOfFrames[i]--;
			extended[i] = true;
		...
		if(i == 0) {
			if(Index[0][NumberOfFrames[0]+1] == 0) {
				...
				// => looped animation
				//	fprintf(stdout,"Looped WSA-File!\n");
				looped = true;
			}
		}
	...
	if( (decodedFrames = (unsigned char*) calloc(1,SizeX*SizeY*NumFrames)) == NULL) {
		fprintf(stdout, "Wsafile: Unable to allocate memory for decoded WSA-Frames!\n");
		exit(EXIT_FAILURE);
	}
