...
MapEditor::MapEditor() {
	...
	// Load GameBar
	pGameBarSurface = pDataManager->getUIGraphic(UI_GameBar);
	...
	// Load TopBar
	pTopBarSurface =  pDataManager->getUIGraphic(UI_TopBar);
	TopBarPos.w = pTopBarSurface->w;
	TopBarPos.h = pTopBarSurface->h;
	TopBarPos.x = TopBarPos.y = 0;

	// Load Terrain Surface
	TerrainSprite = pDataManager->getObjPic(ObjPic_Terrain);
...
void MapEditor::InitMap(int x, int y) {
	...
	if(Tiles == NULL) {
		fprintf(stdout,"MapEditor::InitMap(): Cannot allocate memory!\n");
		exit(EXIT_FAILURE);
	}
