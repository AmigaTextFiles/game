...
public:
	...
	*/
	inline void SetCount(int newCount) { count = newCount; }
	...
	*/
	virtual inline void Draw(SDL_Surface* screen, Point Position) {
		...
		surface = pDataManager->getUIGraphic(UI_MissionSelect);
		dest.x = Position.x;
		dest.y = Position.y;
		dest.w = surface->w;
		dest.h = surface->h;
		...
		surface = pDataManager->getUIGraphic(UI_CreditsDigits);
	...
	*/
	virtual Point GetMinimumSize() const {
		SDL_Surface* surface = pDataManager->getUIGraphic(UI_MissionSelect);
		if(surface != NULL) {
