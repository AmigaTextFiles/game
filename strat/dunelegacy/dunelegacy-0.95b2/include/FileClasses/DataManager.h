...
public:
	...
	Mix_Chunk*		GetVoice(unsigned int id, int house);
	Mix_Chunk*		GetSound(unsigned int id);
...
private:
	...
	Mix_Chunk* 		GetVoice_English(unsigned int id, int house);

	void			LoadVoice_German();
	Mix_Chunk* 		GetVoice_German(unsigned int id, int house);

	SDL_Surface*	ExtractSmallDetailPic(char* filename);
	...
	BriefingText*	BriefingStrings[3];
