...
{
    int32_t val;
    if(!PHYSFS_readSLE32(file, (PHYSFS_sint32 *)&val))
        throw Exception("read error: %s", PHYSFS_getLastError());
    return val;
}
...
{
    uint32_t val;
    if(!PHYSFS_readULE32(file, (PHYSFS_uint32 *)&val))
        throw Exception("read error: %s", PHYSFS_getLastError());
    return val;
}
...
{
    int32_t val;
    if(!PHYSFS_readSBE32(file, (PHYSFS_sint32 *)&val))
        throw Exception("read error: %s", PHYSFS_getLastError());
    return val;
}
...
{
    uint32_t val;
    if(!PHYSFS_readUBE32(file, (PHYSFS_uint32 *)&val))
        throw Exception("read error: %s", PHYSFS_getLastError());
    return val;
}
