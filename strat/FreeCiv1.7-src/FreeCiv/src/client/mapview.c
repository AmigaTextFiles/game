			     ...
			     struct unit *punit1, int hp1)
{
  /***********************************************************************
   *                                                                     *
   * - problem fix: amiga compile sometimes freezes while select()'ing;  *
   *   now usleep() is used instead of select()                          *
   * - new feature: decreasing healthpoints is now really smooth;        *
   *   every fight takes approx. 1 seconds, the needed count of steps    *
  ...
  /* NEW: calculate count of steps */
  unsigned long steps = ((punit0->hp-hp0)<(punit1->hp-hp1)?(punit0->hp-hp0):(punit1->hp-hp1));
  unsigned long usecs = (unsigned long) 1000000 / (steps==0 ? 1 : steps);

  /* debug */
  /* puts("decrease_unit_hp_smooth - START"); */
  ...
  do {
     ...
     **************************/
    usleep(100);	/* wait approx. 100 microseconds */
  ...
  /* debug */
  /* puts("decrease_unit_hp_smooth - ENDE"); */
}
