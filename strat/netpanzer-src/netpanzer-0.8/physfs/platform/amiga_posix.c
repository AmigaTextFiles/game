...
/*
    AmigaOS 4 variant hacked by Juha Niemimäki
*/
...
#ifndef __amigaos4__
#   error "Check your project files. You may not be building AmigaOS 4 version!"
#endif

#ifdef __NEWLIB__
// Since newlib's getenv() is not implemented, try to use DOS library directly
#	include <proto/dos.h>
#else
#	warning "Not tested in other than newlib environment!"
#endif
...
char *__PHYSFS_platformCopyEnvironmentVariable(const char *varname)
{
    ...
    LONG len = IDOS->GetVar( varname, buffer, 1024, GVF_GLOBAL_ONLY );
    ...
    switch ( len )
    {
        ...
        default:
            retval = (char*) allocator.Malloc(strlen(buffer) + 1);
            if ( retval != NULL )
            {
...
static char *getUserNameByUID(void)
{
    ...
    retval = (char *) allocator.Malloc(strlen(username) + 1);
    if (retval != NULL)
    {
...
{
    char *retval = NULL;
    const char * dir = "/progdir"; // this relies on the -lunix linking which may be baaad

    retval = (char *) allocator.Malloc(strlen(dir) + 1);
    if (retval != NULL)
    {
        strcpy(retval, dir);
    }
...
{
    int fd = *((int *) opaque);
    #warning "fsync() removed here"

    //BAIL_IF_MACRO(fsync(fd) == -1, strerror(errno), 0);
    //BAIL_IF_MACRO(fflush( fdopen(fd, "r+w") ) != 0, strerror(errno), 0);
