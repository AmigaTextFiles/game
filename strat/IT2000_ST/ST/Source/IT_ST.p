...
var Technology,Project                  :StrArr42;
...
begin
   ...
   Save.SystemName:=StrArr('Sonne',    'Centauri','Wega',      'Rigel',    'Cassiopeia',
                           'Deneb',    'Castor',  'Regulus',   'Sirius',   'Spica',
                           'Aldebaran','Arktur',  'Beteigeuze','Kapella',  'Kepheus',
                           'Canopus',  'Mira',    'Antares',   'Achernar', 'Atair',
                           'Prokyon',  'Pollux',  'Algol',     'Fomalhaut','Altar');
   ...
   Save.TechCosts[1]:=LongArr42(0,
                           ...
                           85000,135000);
   Project:=StrArr42('BIOSPHERE II',   'FÖDERATION DER PLANETEN','KREBSHEILUNG',
                {4}  'PANDORA-Projekt','BIG-BANG-Projekt',       'GENESIS-Projekt',
                {7}  'Q-Projekt',

                {8}  'Wostok',      'Mir',      'Spaceshuttle','Starwing',
               {12}  'Galaxy',      'Catamaran','Quasar',      'Destroyer',
               {16}  'Enterprise A','Deltawing','Tristars',    'Enterprise B',
               {20}  'Warbird',     'Stargate', 'Enterprise C','Cruiser',
               {24}  'Starburner',

               {25}  'Raumdock','Siedler-Einheit','Landungstruppen','Kontinentale Union',

               {29}  'Globale Union',      'Recycling-Anlage',     'Fusions-Kraftwerk',
               {32}  'Hydrolyse-Kraftwerk','Teilchenbeschleuniger','SDI',
               {35}  'Internet',           'virtuelle Universität','intelligente Fabrik',
               {38}  'Informationhighway', 'von-Neumann-Sonde',    'Spacephalanx',
               {41}  'Microiden',          'Wetter-Station');
   ...
   ShipData[16]:=r_ShipData(10, 79,12,10);   {Enterprise A,25}
   ShipData[17]:=r_ShipData(15, 93,10,11);   {Delta-Wing,  30}
   ShipData[18]:=r_ShipData( 9,107,16,12);   {Tristars,    33}
   ShipData[19]:=r_ShipData(19,121,14,14);   {Enterprise B,32}
   ShipData[20]:=r_ShipData(22,129,21,15);   {Warbird,     35}
   ShipData[21]:=r_ShipData( 4, 60,20,12);   {Stargate,    31}
   ShipData[22]:=r_ShipData(25,136,19,16);   {Enterprise C,38}
   ShipData[23]:=r_ShipData(28,149,27,18);   {Cruiser,     37}
   ShipData[24]:=r_ShipData(33,160,24,20);   {Starburner,  36}
   ShipData[27]:=r_ShipData(28,190,27,30);   {Borgschiff     }
   { Maximal:               --,---,27,-                      }
   Romanum:=StrArrMAXPLANETS('I','II','III','IV','V','VI','VII','VIII','IX','X','XI');
   PNames[0]:=StrArr11('Talos','Scalos','Iconia','Odon','Genesis','Rosod',
                       'Munaxx','Tzenketi','Yamos','Jake','Andmun');
   PNames[1]:=StrArr11('Merkur','Venus', 'Erde',  'Mars','Jupiter',
                       'Saturn','Uranus','Neptun','Pluto','','');
   PNames[2]:=StrArr11('Darukis','Akaneb', 'Cronos','Sedokub','Praxis',
                       'Dasekab','Khitomer','Vedekab', 'Damakeb','Khetamar',
                       'Zhalakab');
   PNames[3]:=StrArr11('Akusum', 'Celadum','Romulus','Dianum','Ecanum',
                       'Jamaris','Lorius', 'Noratum','Tamarus','Pekateg',
                       'Raletis');
   PNames[4]:=StrArr11('Inarett',  'Ferraccud','Kardassia','Latecca',  'Ooro',
                       'Reedameec','Soorov',   'Zalee',    'Zaalasena','Veda',
                       'Sodoora');
   PNames[5]:=StrArr11('Deragi',  'Sodaki',  'Ferengi Nar','Zolati', 'Plauti',
                       'Olsatebi','Munakati','Rastoki',    'Mosavli','Berali',
                       'Dresadai');
   PNames[6]:=StrArr11('Bos','Dak','Bajor','Zeda','Co',
                       'Zud','Fec','Paq', 'Loa', 'Zate',
                       'Wid');
   PNames[7]:=StrArr11('Wolaron','Unareus', 'Vulkan','Ataneia','Peratham',
...
function GETCIVADJ(CivVar :byte):string;

begin
   case CivVar of
      1: GETCIVADJ:='terranische';
      2: GETCIVADJ:='klingonische';
      3: GETCIVADJ:='romulanische';
      4: GETCIVADJ:='kardassianische';
      5: GETCIVADJ:='Ferengi-';
      6: GETCIVADJ:='bajoranische';
      7: GETCIVADJ:='vulkanische';
      8: case Save.WorldFlag of
            WFLAG_DCON:   GETCIVADJ:='tkonianische';
            WFLAG_JAHADR: GETCIVADJ:='Jem´Hadar-';
            otherwise GETCIVADJ:=GETCIVADJ(GETCIVVAR(Save.WorldFlag));
         end;
      otherwise GETCIVADJ:='ERROR: '+intstr(Civvar);
   end;
end;



function GETCIVNAME(CivVar :byte):string;

begin
   case CivVar of
      1: GETCIVNAME:='Terraner';
      2: GETCIVNAME:='Klingonen';
      3: GETCIVNAME:='Romulaner';
      4: GETCIVNAME:='Kardassianer';
      5: GETCIVNAME:='Ferengis';
      6: GETCIVNAME:='Bajoraner';
      7: GETCIVNAME:='Vulkanier';
      8: case Save.WorldFlag of
            WFLAG_CEBORC: GETCIVNAME:='Borg';
            WFLAG_DCON:   GETCIVNAME:='Tkonianer';
            WFLAG_JAHADR: GETCIVNAME:='Jem´Hadar';
            otherwise GETCIVNAME:=GETCIVNAME(GETCIVVAR(Save.WorldFlag))
         end;
      9: GETCIVNAME:='Der Maquis';
      otherwise GETCIVNAME:='ERROR: '+intstr(CivVar);
   end;
end;
...
begin
   ...
   if Mode in [MODE_REDRAW,MODE_STARGATE] then begin
      ...
      WRITE(576,418,0,16,MyScreen[1]^,4,'Regierung');
      WRITE(576,446,0,16,MyScreen[1]^,4,'Hauptmenü');
      WRITE(576,474,8,16,MyScreen[1]^,4,'Rundenende');
   ...
   if Informed then for j:=1 to MAXHOLES do with MyWormHole[j] do for i:=1 to 2 do
    if (CivKnowledge[ActPlayer]=FLAG_KNOWN)
    and (System[1]<=Save.SYSTEMS) and (System[2]<=Save.SYSTEMS) then begin
...
begin
   ...
   if Mode=MODE_REDRAW then begin
      if Display in [0,100] then begin
         for i:=1 to 3 do DrawImage(^MyScreen[1]^.RastPort,^GadImg1,518,500-i*28);
         WRITE(576,418,0,16,MyScreen[1]^,4,'Suchen');
         WRITE(576,446,0,16,MyScreen[1]^,4,'Sternenkarte');
         WRITE(576,474,8,16,MyScreen[1]^,4,'Rundenende');
      end;
      Display:=ActSys;
...
type StrArr=array [1..20] of str;
type FontArr=array [1..20] of byte;
...
{*****************************************************************************}
   ...
   ClipBlit(^MyScreen[AScr]^.RastPort,0,200,^MyScreen[3-AScr]^.RastPort,0,200,640,150,192);
   VObj[1]:=VectorObj(0,0,ROTATE_PY+ROTATE_PX,
...
{*****************************************************************************}
   ...
   if not DISPLAYIMAGE(s,0,235,640,37,5,MyScreen[AScr]^,0) then begin end;
   WRITE(320,285,31,16,MyScreen[AScr]^,5,'A TRIBUTE TO STAR TREK');
   ClipBlit(^MyScreen[AScr]^.RastPort,0,235,^MyScreen[3-AScr]^.RastPort,0,235,640,100,192);
   VObj[1]:=VectorObj(0,0,ROTATE_PY+ROTATE_PX,
   ...
   SArr:=StrArr('Software & Design',                 'OXYGENIC',
                ...
                'Music by',                          'LUDWIG v. BEETHOVEN',
                'RICHARD WAGNER',                    'N.N. IKONNIKOW',
                'JERRY GOLDSMITH',                   'ALEXANDER COURAGE',
                'Credits go to',                     'C. MORELL',
                'JAKOB GAARDSTED',                   'GEORGE MOORE',
                'RIKARD CEDERLUND',                  'ANDY JONES',
                'ADAM BENJAMIN',                     '',
                'DolbySurround®-Sounds created with','WAVETRACER DS');
   FArr:=FontArr(4,5, 4,5, 4,5, 5,5, 5,5, 4,5,  5,5, 5,5, 5,5, 4,5);
   ...
   repeat
      ...
      BltBitMapRastPort(^MybitMap,0,145,^MyScreen[AScr]^.RastPort,0,220,640,55,192);
      ...
      if ISize>1 then begin
         WRITE(320,220,125,16,MyScreen[AScr]^,FArr[l],SArr[l]);
         WRITE(320,245,125,16,MyScreen[AScr]^,FArr[l+1],SArr[l+1]);
      end;
      ISize:=ISize+1;
      if ISize>44 then begin
         l:=l+2; ISize:=0;
      end;
...
begin
   ...
   WRITE(554,156,45,0,MyScreen[1]^,4,'Jahr');
   WRITE(535,207,45,0,MyScreen[1]^,4,'Guthaben');
   WRITE(533,258,45,0,MyScreen[1]^,4,'Population');
   s:=PathStr[6]+'ST/DeskImages.img';
   if not RAWLOADIMAGE(s,0,32,640,32,4,ImgBitMap4) then exit;
...
procedure BORGMESSAGE(CivVar :byte);

begin
   MAKEBORDER(MyScreen[1]^,20,70,490,225,12,6,0);
   WRITE(256, 90,1,16,MyScreen[1]^,4,'Unsere Analyse hat ergeben, das die Zivilisation');
                                  s:='der '+GETCIVNAME(CivVar)+' nicht in der Lage ist, sich';
   WRITE(256,112,1,16,MyScreen[1]^,4,s);
   WRITE(256,134,1,16,MyScreen[1]^,4,'erfolgreich gegen uns zu verteidigen. ');
   WRITE(256,156,1,16,MyScreen[1]^,4,'Ihre Zivilisation wird assimiliert.');
   WRITE(256,190,1,16,MyScreen[1]^,4,'WIDERSTAND IST ZWECKLOS!');
   WAITLOOP(false);
   RECT(MyScreen[1]^,0,20,70,490,225);
   REFRESHDISPLAY;
end;
...
begin
   ...
   WRITE(256,151,12,1+16,MyScreen[1]^,4,'beginnen Krieg gegen');
   ...
   if (CivVar1=8) and (Save.WorldFlag=WFLAG_CEBORC) and (random(10)=0)
   and (Save.CivPlayer[CivVar2]<>0) then BORGMESSAGE(CivVar2);
...
begin
   ...
   for i:=1 to Save.SYSTEMS do if i<>ActSys then begin
      ...
      if (l<SysEntfernung) and (random(2)=0) then begin
         ...
         or (MyShipPtr^.Fracht=0))) or (CivVar=9) then begin
            SysEntfernung:=l; SysID:=i;
            if (SystemHeader[ActSys].FirstShip.SType=TARGET_STARGATE)
             and (SystemHeader[SysID].FirstShip.SType=TARGET_STARGATE)
             then SysEntfernung:=255;
...
begin
   ...
   if Mode=SCREEN_PLANET then begin
      ...
      WRITE(62,126,2,0,MyScreen[2]^,4,'Biosphäre');
      WRITE(62,175,2,0,MyScreen[2]^,4,'Infrastruktur');
      WRITE(62,224,2,0,MyScreen[2]^,4,'Industrialisierung');
      WRITE(165,250,2,0,MyScreen[2]^,4,'Millionen');
      WRITE(62,273,2,0,MyScreen[2]^,4,'Population');
      WRITE(185,312,2,0,MyScreen[2]^,4,'Kreativität');
      WRITE(185,359,2,0,MyScreen[2]^,4,'Produktivität');
      WRITE(120,461,2,16,MyScreen[2]^,4,'Ändern');
      WRITE(262,461,2,16,MyScreen[2]^,4,'Kaufen');
...
begin
   ...
   WRITE(320,60,127,80,MyScreen[2]^,5,'IMPERIUM TERRANUM HIGHSCORES');
   s:=PathStr[5]+'ST/HiScore.dat';
   FHandle:=OPENSMOOTH(s,MODE_OLDFILE);
   if FHandle=0 then begin
      CREATEHIGHSCORE;
   ...
   for i:=1 to 8 do with HiScore do begin
      s:=intstr(Points[i]);
      WRITE(120,100+i*30,GETCOLOR(CivVar[i]),32+64,MyScreen[2]^,5,s);
      WRITE(140,100+i*30,GETCOLOR(CivVar[i]),64,MyScreen[2]^,5,Player[i]);
   end;
...
begin
   ...
   WRITE(320,100,1,16,MyScreen[2]^,4,'Neuer Highscore!');
   if not MultiPlayer then WRITE(320,150,2,16,MyScreen[2]^,4,'Bitte Namen eingeben:') else begin
      s:='Spieler '+intstr(Save.CivPlayer[ActPlayer])+', bitte Namen eingeben:';
   ...
   with MyPlanetHeader^ do begin
      ...
      if ProjectID<>0 then begin
         if ProjectID>0 then begin
            ...
            -3: begin
                   s:='Biosphäre reinigen';
                   l:=Biosphäre div 2;
                end;
            -2: begin
                   s:='Infrastruktur reparieren';
                   l:=Infrastruktur div 2;
                end;
            otherwise begin
                   s:='Industrieanlagen reparieren';
                   l:=Industrie div 2;
            end;
         end;
...
begin
   ...
   with MyPlanetHeader^ do begin
      ...
      s:='System: '+Save.SystemName[ActSys]; WRITE(5,5,1,1,MyScreen[2]^,4,s);
      s:='Planet: '+PName;                   WRITE(5,25,1,1,MyScreen[2]^,4,s);
      s:='Klasse: ';
      case Class of
      ...
      s:='Größe: '+realstr(Size/10,2)+'-facher Erdradius';
      WRITE(5,65,1,1,MyScreen[2]^,4,s);

      if Save.ActTech[ActPlayer]>0 then begin
         s:='Technologie: '+Technology[Save.ActTech[ActPlayer]];
         WRITE(275,5,1,1,MyScreen[2]^,4,s);
      end else WRITE(275,5,1,1,MyScreen[2]^,4,'keine Entwicklung');
      ...
      if FirstShip.NextShip<>NIL then begin
         ...
         repeat
            ...
            MyShipPtr:=MyShipPtr^.NextShip;
         until MyShipPtr=NIL;
         s:='Schiffe im Orbit: '+intstr(i);
         WRITE(275,45,1,1,MyScreen[2]^,4,s);
      end;
      if Ethno<>PFlags and ActPlayerFlag then begin
         s:='Ethnische Minderheit der '+GETCIVNAME(GETCIVVAR(Ethno));
         ...
         if (LData^ and 64=0) then begin
            if (IBase^.MouseX in [56..187]) and (IBase^.MouseY in [455..480]) then begin
               ...
               if Biosphäre<200 then begin
                  NewProject[j]:='Biosphäre reinigen';
               ...
               if Infrastruktur<200 then begin
                  NewProject[j]:='Infrastruktur reparieren';
               ...
               if Industrie<200 then begin
                  NewProject[j]:='Industrieanlagen reparieren';
               ...
               WRITE(365,474,4,1,MyScreen[2]^,4,'Was soll gebaut werden?');
            ...
            and (ProjectID>0) then begin
               ...
               WRITE(498,361,2,16,MyScreen[2]^,4,'Kosten für Projekt:');
               l:=XProjectCosts-XProjectPayed;
               if l<0 then l:=0;
               s:=intstr(l);
               WRITE(498,382,4,16,MyScreen[2]^,2,s);
               WRITE(498,403,2,16,MyScreen[2]^,4,'Guthaben:');
...
begin
   for i:=1 to Save.SYSTEMS do
    ...
    and (LastSystem<>i) then begin
      LastSystem:=i;
      RECT(MyScreen[1]^,0,522,9,629,116);
      WRITE(525,12,12,0,MyScreen[1]^,4,Save.SystemName[i]);
      if (Save.CivPlayer[ActPlayer]<>0)
      and not (SystemFlags[ActPlayer,i] and FLAG_KNOWN=0) then begin
         s:=intstr(SystemHeader[i].Planets)+' Planeten';
         WRITE(525,29,12,0,MyScreen[1]^,4,s);
         s:=GETCIVNAME(GETCIVVAR(SystemFlags[1,i] and FLAG_CIV_MASK));
         WRITE(525,46,SystemFlags[1,i] and FLAG_CIV_MASK,0,MyScreen[1]^,4,s);
      end else WRITE(525,29,12,0,MyScreen[1]^,4,'unbekannt');
      if ActSys>0 then begin
         ...
         WRITE(550,87,12,0,MyScreen[1]^,4,'Jahre');
...
begin
   ...
   s:=PathStr[6]+'ST/'+Project[MyShipPtr^.SType]+'Tech.img';
   if not DISPLAYIMAGE(s,6,7,256,498,5,MyScreen[2]^,0) then begin end;
   with MyShipPtr^ do begin

      WRITE(290,15,1,0,MyScreen[2]^,4,Project[MyShipPtr^.SType]);
      case Weapon of
         WEAPON_GUN:       s:='Feuerwaffen';
         WEAPON_LASER:     s:='Laserkanonen';
         WEAPON_PHASER:    s:='Phaserbänke';
         WEAPON_DISRUPTOR: s:='Disruptorbänke';
         WEAPON_PTORPEDO:  s:='Protonentorpedo-Slots';
         otherwise s:=intstr(Weapon);
      end;
      s:='- Bewaffnung:  '+s;
      WRITE(290,42,1,0,MyScreen[2]^,3,s);
      s:='- Angriffsstärke:  '+intstr(round(ShipData[SType].WeaponPower*(Weapon/10+1)));
      WRITE(290,62,1,0,MyScreen[2]^,3,s);
      s:='- Schilde:  '+intstr(ShipData[SType].MaxShield);
      WRITE(290,82,1,0,MyScreen[2]^,3,s);
      s:='- Kampferfahrung:  '+intstr(ShieldBonus);
      WRITE(290,102,1,0,MyScreen[2]^,3,s);
      s:='- Zustand des Schiffes:  '+realstr(Shield/ShipData[SType].MaxShield*100,2)+' %';
      WRITE(290,122,1,0,MyScreen[2]^,3,s);
      s:='- Bewegungsrate:  '+intstr(ShipData[SType].MaxMove);
      WRITE(290,142,1,0,MyScreen[2]^,3,s);
      s:='- Siedler an Bord:  '+intstr((Ladung and MASK_SIEDLER) div 16);
      WRITE(290,162,1,0,MyScreen[2]^,3,s);
      s:='- Landungstrupps an Bord:  '+intstr(Ladung and MASK_LTRUPPS);
      WRITE(290,182,1,0,MyScreen[2]^,3,s);
      s:='- Frachträume zu '+intstr(round(Fracht / ShipData[Stype].MaxLoad*100))+'% belegt';
      WRITE(290,202,1,0,MyScreen[2]^,3,s);
      s:='- Baujahr: ';
      if Age<200 then s:=s+intstr(Year-Age) else s:=s+' Schiff ist völlig veraltet!!';
...
begin
   if LogoSMemA[JingleID]=0 then begin
      FName:=PathStr[10]+'ST/'+intstr(JingleID)+'.RAW';
      FHandle:=OPENSMOOTH(FName,MODE_OLDFILE);
      if FHandle=0 then exit;
      l:=DosSeek(FHandle,0,OFFSET_END);
...
begin
   ...
   if LogoMemA[0]<>0 then begin
      if LogoMemA[ActPlayer]=0 then begin
         ...
         FName:=PathStr[11]+'ST/'+intstr(ActPlayer)+'.img';
         FHandle:=OPENSMOOTH(FName,MODE_OLDFILE);
         if FHandle<>0 then begin
            ISize:=DosSeek(FHandle,0,OFFSET_END);
            ISize:=DosSeek(FHandle,0,OFFSET_BEGINNING);
...
begin
   ...
   if Multiplayer or (Year=1900) then begin
      if not Save.PlayMySelf then RECT(MyScreen[1]^,0,0,0,511,511);
      s:='Spieler '+intstr(Save.CivPlayer[ActPlayer]);
      PLAYERJINGLE(ActPlayer);
      MAKEBORDER(MyScreen[1]^,35,80,475,290,12,6,0);
      if Year>1900 then
       WRITE(256,100,12,16,MyScreen[1]^,4,'Nächster am Zug:')
      else begin
         case ActPlayerFlag of
            FLAG_TERRA:  WRITE(256,100,ActPlayerFlag,16,MyScreen[1]^,4,'Föderation der Planeten:');
            FLAG_KLEGAN: WRITE(256,100,ActPlayerFlag,16,MyScreen[1]^,4,'Klingonisches Reich:');
            FLAG_REMALO: WRITE(256,100,ActPlayerFlag,16,MyScreen[1]^,4,'Romulanisches Imperium:');
            FLAG_CARDAC: WRITE(256,100,ActPlayerFlag,16,MyScreen[1]^,4,'Kardassianisches Zentralkommando:');
            FLAG_FERAGI: WRITE(256,100,ActPlayerFlag,16,MyScreen[1]^,4,'Ferengi-Allianz:');
            FLAG_BAROJA: WRITE(256,100,ActPlayerFlag,16,MyScreen[1]^,4,'Bajoraner:');
            FLAG_VOLKAN: WRITE(256,100,ActPlayerFlag,16,MyScreen[1]^,4,'Vulkanier:');
         end;
      end;
            ...
            with MyPlanetHeader^ do if GETCIVVAR(PFlags)=HomeWorld then begin
               if (SplitWorld=0) and (Ethno<>PFlags and FLAG_CIV_MASK) then begin
                  ...
                  if Save.CivPlayer[ActPlayer]<>0 then begin
                     ...
                     WRITE(256,136,GETCIVFLAG(HomeWorld),1+16,MyScreen[1]^,4,'Der Fall der Heimatwelt der');
                     s:=GETCIVNAME(HomeWorld)+' verursacht eine Spaltung';
                     WRITE(256,156,GETCIVFLAG(HomeWorld),1+16,MyScreen[1]^,4,s);
                     WRITE(256,176,GETCIVFLAG(HomeWorld),1+16,MyScreen[1]^,4,'des Imperiums in einen loyalen Teil');
                     WRITE(256,196,SplitWorld,1+16,MyScreen[1]^,4,'und einen aufständischen,');
                     s:=GETCIVADJ(GETCIVVAR(Splitworld))+'n Teil!';
...
begin
   ...
   if EndText=0 then begin
      s:='Die '+GETCIVNAME(CivVar)+' beenden den Bürgerkrieg und';
      if Save.CivPlayer[CivVar]<>0 then REQUEST(s,'vereinigen ihr Imperium wieder!',CivFlag,CivFlag);
   end else if EndText=1 then begin
      {REBELLEN-SIEG}
      if Save.CivPlayer[CivVar]<>0 then case CivFlag of
         FLAG_TERRA:  REQUEST('Die Sternenflotte kapituliert vor den Rebellen',
                              'und übergibt die Macht über die Föderation.',CivFlag,CivFlag);
         FLAG_KLEGAN: REQUEST('Das Militär kapituliert vor dem Hause der Duras und',
                              'übergibt die Macht über das klingonische Reich.',CivFlag,CivFlag);
         FLAG_REMALO: REQUEST('Das Militär kapituliert vor den Talshiar und',
                              'übergibt die Macht über das Imperium.',CivFlag,CivFlag);
         FLAG_CARDAC: REQUEST('Das Militär kapituliert vor dem obsidianischen',
                              'Orden und übergibt die Macht im Zentralkommando.',CivFlag,CivFlag);
         FLAG_FERAGI: REQUEST('Das Militär kapituliert vor den Rebellen',
                              'und übergibt die Macht über die Ferengiallianz.',CivFlag,CivFlag);
         otherwise REQUEST('Die Militärführung kapituliert vor den Rebellen',
                           'und übergibt das Kommando über das Imperium.',CivFlag,CivFlag);
      end;
   ...
   end else if EndText=2 then if Save.CivPlayer[CivVar]<>0 then begin
      {MILITÄR-SIEG}
      s:='Die Rebellen kapitulieren und stimmen der';
      if Save.CivPlayer[CivVar]<>0 then case CivFlag of
         FLAG_TERRA:  REQUEST(s,'Reintegration in die Föderation zu.',CivFlag,CivFlag);
         FLAG_KLEGAN: REQUEST('Die Duras kapitulieren und stimmen der','Reintegration in das Reich zu.',CivFlag,CivFlag);
         FLAG_REMALO: REQUEST('Die Talshiar kapitulieren und stimmen','der Reintegration in das Imperium zu.',CivFlag,CivFlag);
         FLAG_CARDAC: REQUEST('Der obsidianische Orden kapituliert und','stimmt der reintegration zu.',CivFlag,CivFlag);
         FLAG_FERAGI: REQUEST(s,'Reintegration in die Ferengiallianz zu.',CivFlag,CivFlag);
         otherwise REQUEST('Die Rebellen kapitulieren und stimmen der','Reintegration in das Imperium zu.',CivFlag,CivFlag);
      end;
   end;
...
begin
   ...
   WRITE(340,50,1,0,MyScreen[2]^,3,'Wissenschaftler entdecken neue Technologie:');
   WRITE(340,70,1,0,MyScreen[2]^,4,Technology[TechID]);
   if TechUse1[TechID]>0 then begin
      WRITE(340,110,1,0,MyScreen[2]^,4,'Benötigt:');
      WRITE(370,130,1,0,MyScreen[2]^,4,Technology[TechUse1[TechID]]);
      if TechUse1[TechID]<>TechUse2[TechID] then WRITE(370,150,1,0,MyScreen[2]^,4,Technology[TechUse2[TechID]]);
   end;
   if TechID<>27 then begin
      WRITE(340,190,1,0,MyScreen[2]^,4,'Ermöglicht:');
      l:=0;
      ...
      if l>0 then WRITE(370,210,1,0,MyScreen[2]^,4,Technology[l]);
      if (l<>Offset) and (Offset in [1..42]) then WRITE(370,230,1,0,MyScreen[2]^,4,Technology[Offset]);
   end;
   l:=320;
   for CivVar:=1 to 42 do if ProjectNeedsTech[CivVar]=TechID then begin
      if CivVar in [1..7] then s:='Großprojekt:'
      else if CivVar in [8..24] then s:='Raumschiff:'
      else s:='Bauprojekt:';
      s:=s+'  '+Project[CivVar];
...
begin
   ...
   and ((Save.CivPlayer[CivVar1]<>0) or (Save.CivPlayer[CivVar2]<>0)) then begin
      MAKEBORDER(MyScreen[1]^,85,118,425,200,12,6,0);
      s:=GETCIVNAME(CivVar1)+' übernehmen Technologie:';
...
begin
   ...
   WRITE(256,152,12,1+16,MyScreen[1]^,4,'und');
   s:=GETCIVNAME(CivVar2);
   WRITE(256,172,CivFlag2,1+16,MyScreen[1]^,4,s);
   WRITE(256,192,12,1+16,MyScreen[1]^,4,'schließen Frieden');
   if Save.PlayMySelf then delay(PAUSE);
   WAITLOOP(Save.PlayMySelf);
...
begin
   ...
   if MyPlanets>0 then begin
      ...
      s:='System: '+Save.SystemName[SysID];
      WRITE(256,260,SystemFlags[1,SysID] and FLAG_CIV_MASK,0+16,MyScreen[1]^,4,s);
      s:='Bevölkerung: '+intstr(SysPop)+' Millionen';
      WRITE(256,290,ThePlayerFlag,0+16,MyScreen[1]^,4,s);
      s:='Planeten: '+intstr(SystemHeader[SysID].Planets)+', davon '+intstr(MyPlanets)
        +' '+GETCIVADJ(GETCIVVAR(ThePlayerFlag));
      WRITE(256,310,ThePlayerFlag,0+16,MyScreen[1]^,4,s);
      if Buildings<3 then s:='praktisch nicht entwickelt'
      else if Buildings<5 then s:='geringe Entwicklungsstufe'
      else if Buildings<10 then s:='mäßig hoch entwickelt'
      else if Buildings<18 then s:='hoch entwickelt'
      else if Buildings>=34 then s:='höchste Entwicklungsstufe';
...
begin
   ...
   if Mode=1 then begin
      ...
      WRITE(118,202,0,16,MyScreen[1]^,4,'Annehmen');
      WRITE(388,202,0,16,MyScreen[1]^,4,'Ablehnen');
   end;
end;
...
begin
   ...
   case CivFlag of
      FLAG_TERRA:  CivStr:='Der Vertreter des Föderationsrates'
      FLAG_KLEGAN: CivStr:='Der Khalessor des Klingonischen Reiches';
      FLAG_REMALO: CivStr:='Der Deqaresto des Romulanischen Imperiums';
      FLAG_CARDAC: CivStr:='Der Legat des kardassianischen Zentralkommandos';
      FLAG_FERAGI: CivStr:='Der Gesandte der Ferengiallianz';
      FLAG_BAROJA: CivStr:='Der Vertreter des Bajoranischen Hohen Rates';
      FLAG_VOLKAN: CivStr:='Der Botschafter der vulkanischen Regierung';
      FLAG_OTHER:  case Save.WorldFlag of
                      WFLAG_DCON:   CivStr:='Der Vertreter des Tkon-Imperiums';
                      WFLAG_JAHADR: CivStr:='Der Vertreter des Dominion';
                      otherwise CivStr:='Die Militärführung der Rebellen';
                   end;
      otherwise exit;
   end;
   XSystem:=0;
   ...
   if (CivVar=8) and (Save.WorldFlag=WFLAG_JAHADR) then begin
      if Save.WarState[ActPlayer,8]<>LEVEL_WAR then begin
         ...
         if Mode=MODE_FORCE then begin
            s:='verurteilt das Verhalten der '+GETCIVNAME(ActPlayer)+' und';
            WRITE(256,110,FLAG_OTHER,16,MyScreen[1]^,4,s);
            WRITE(256,130,FLAG_OTHER,16,MyScreen[1]^,4,'fordert als Bestrafung und für die Wieder-');
            WRITE(256,150,FLAG_OTHER,16,MyScreen[1]^,4,'herstellung des Status Quo die Zahlung einer');
         end else begin
            WRITE(256,110,FLAG_OTHER,16,MyScreen[1]^,4,'bedauert den Überfall auf Stellungen');
            WRITE(256,130,FLAG_OTHER,16,MyScreen[1]^,4,'der Jem´Hadar und fordert als Entschädigung und für');
            WRITE(256,150,FLAG_OTHER,16,MyScreen[1]^,4,'die Wiederherstellung des Friedens die Zahlung einer');
         end;
         if Save.JSteuer[ActPlayer]=0 then s:='jährlichen Abgabe von '+intstr(i)+'% an das Dominion.'
         else s:='erhöhten jährlichen Sicherheitsabgabe.';
         ...
         KLICKGAD(330,200);
         INIT(0);
         WRITE(256,110,CivFlag,0+16,MyScreen[1]^,4,'erklärt den uneingeschränkten Krieg bis zur');
         WRITE(256,130,CivFlag,0+16,MyScreen[1]^,4,'Kapitulation oder der vollständigen');
         s:='Ausrottung der '+GETCIVADJ(ActPlayer)+' Rasse.';
         ...
         if (Year mod 6=0) then begin
            INIT(1);
            WRITE(256,110,CivFlag,0+16,MyScreen[1]^,4,'weist auf die militärische Überlegenheit seiner');
            WRITE(256,130,CivFlag,0+16,MyScreen[1]^,4,'Einheiten hin und verlangt als Bedingung für einen');
            WRITE(256,150,CivFlag,0+16,MyScreen[1]^,4,'Frieden die Zahlung einer Sicherheitsabgabe');
            i:=2*(1+(Save.WarPower[8] div Save.WarPower[ActPlayer]));
            if i>53 then i:=53;
            s:='in Höhe von '+intstr(i)+'% jährlich';
            ...
            INIT(0);
            WRITE(256,115,CivFlag,0+16,MyScreen[1]^,4,'ist nicht dazu bereit, Verhandlungen aufzunehmen.');
            WAITLOOP(false);
            RECT(MyScreen[1]^,0,30,80,480,230);
            exit;
         end;
   ...
   if Mode=MODE_BELEIDIGUNG then begin
      INIT(0);
      WRITE(256,110,CivFlag,0+16,MyScreen[1]^,4,'faßt die Ablehnung seines Angebotes als Beleidigung');
      WRITE(256,130,CivFlag,0+16,MyScreen[1]^,4,'und als Schwere Verletzung der Ehre seines Volkes');
      WRITE(256,150,CivFlag,0+16,MyScreen[1]^,4,'auf und erklärt den Krieg');
      WAITLOOP(false);
      RECT(MyScreen[1]^,0,30,80,480,230);
      Save.WarState[CivVar,ActPlayer]:=LEVEL_WAR;
      Save.WarState[ActPlayer,CivVar]:=LEVEL_WAR;
      ...
      if Save.WarState[CivVar,ActPlayer]<>LEVEL_WAR then begin
         ...
         s2:='verurteilt die '+GETCIVADJ(ActPlayer)+' Aggression auf das';
         TArr4:=TextArr4(s2,'schärfste und fordert für die Wiederherstellung',
                         '','');
         if Mode=MODE_MONEY then begin
            s:='gabe der Gewinne in Höhe von '+intstr(XCosts)+', die den';
            s2:='Ferengis durch das unfaire '+GETCIVADJ(ActPlayer);
            TArr4:=TextArr4('verlangt für den Erhalt des Friedens die Heraus-',
                            s,s2,
                            'Geschäftsgebaren entgangen sind!');
         end else if (Save.WarPower[ActPlayer]<Save.WarPower[CivVar]*3) and (XSystem>0) then begin
            TArr4[3]:='des Friedens die bedingungslose Übergabe des';
            s:='Systems '+Save.SystemName[XSystem]+'.';
            TArr4[4]:=^s;
            SYSINFO(XSystem,ActPlayerFlag);
         end else if (Save.WarPower[ActPlayer]<Save.WarPower[CivVar]*4) and (XTech>0) then begin
            s:=' '+Technology[XTech]+'.';
            if XTech in [12,13,15,18,20,23,24,28,30,33,36] then s:='den'+s
            else if XTech in [3,17,21,25,31] then s:='das'+s else s:='die'+s;
            TArr4[3]:='des Friedens die Übergabe von Informationen über';
            TArr4[4]:=^s;
         end else begin
            s:=intstr(XCosts);
            TArr4[3]:='des Friedens Reparationszahlungen in Höhe von';
            TArr4[4]:=^s;
         end;
         ...
         KLICKGAD(330,200);
         INIT(0);
         WRITE(256,110,CivFlag,0+16,MyScreen[1]^,4,'teilt den Abbruch aller Beziehungen');
         WRITE(256,130,CivFlag,0+16,MyScreen[1]^,4,'und die Ausrufung des Krieges gegen');
         s:='die '+GETCIVNAME(ActPlayer)+' mit.';
         WRITE(256,150,CivFlag,0+16,MyScreen[1]^,4,s);
         WAITLOOP(false);
         RECT(MyScreen[1]^,0,30,80,480,230);
         Save.WarState[CivVar,ActPlayer]:=LEVEL_WAR;
      ...
      and ((Year mod 6=0) or (Mode=MODE_FORCE)) then begin
         INIT(1);
         WRITE(256,110,CivFlag,0+16,MyScreen[1]^,4,'bietet den Abschluß eines');
         WRITE(256,130,CivFlag,0+16,MyScreen[1]^,4,'Friedensvertrages an.');
         repeat
            delay(RDELAY);
         until (LData^ and 64=0) and (IBase^.MouseY in [200..220]) and (IBase^.MouseX in [60..176,330..446]);
         if IBase^.MouseX in [60..176] then begin
         ...
         MAKEBORDER(MyScreen[1]^,85,120,425,200,12,6,0);
         s:='Die Forderungen der '+GETCIVNAME(ActPlayer);
         WRITE(256,140,CivFlag,16,MyScreen[1]^,4,s);
         WRITE(256,165,CivFlag,16,MyScreen[1]^,4,'wurden zurückgewiesen.');
         WAITLOOP(false);
         RECT(MyScreen[1]^,0,85,120,425,200);
         REFRESHDISPLAY;
      end;
      ...
      if XSystem<>0 then begin
         ...
         if Mode=MODE_TERRITORIUM then begin
            WRITE(256,110,CivFlag,0+16,MyScreen[1]^,4,'verurteilt die Verletzung seines Hoheitsgebietes');
            WRITE(256,130,CivFlag,0+16,MyScreen[1]^,4,'und verlangt als Entschädigung für diese Aggression');
            WRITE(256,150,CivFlag,0+16,MyScreen[1]^,4,'die bedingungslose Übergabe des');
         end else begin
            WRITE(256,110,CivFlag,0+16,MyScreen[1]^,4,'weist auf die militärische Überlegenheit seiner');
            WRITE(256,130,CivFlag,0+16,MyScreen[1]^,4,'Zivilisation hin und verlangt als Vorbedingung');
            WRITE(256,150,CivFlag,0+16,MyScreen[1]^,4,'für einen Friedensvertrag die Übergabe des');
         end;
         s:=GETCIVADJ(ActPlayer)+'n Systems '+Save.SystemName[XSystem]+'.';
         WRITE(256,170,CivFlag,0+16,MyScreen[1]^,4,s);
         SYSINFO(XSystem,ActPlayerFlag);
         repeat
            delay(RDELAY);
         ...
         INIT(0);
         if Mode=MODE_TERRITORIUM then begin
            WRITE(256,110,CivFlag,0+16,MyScreen[1]^,4,'kann das illegale Eindringen auf sein Hoheits-');
            WRITE(256,130,CivFlag,0+16,MyScreen[1]^,4,'gebiet nicht ungestraft hinnehmen. Er erklärt hiermit');
            s:='den Krieg gegen die '+GETCIVNAME(ActPlayer)+'!';
            WRITE(256,150,CivFlag,0+16,MyScreen[1]^,4,s);
         end else begin
            WRITE(256,110,CivFlag,0+16,MyScreen[1]^,4,'erteilt dem Wunsch nach Friedensverhandlungen');
            WRITE(256,130,CivFlag,0+16,MyScreen[1]^,4,'eine Absage und bricht sämtliche Beziehungen');
            s:='zur '+GETCIVADJ(ActPlayer)+'n Zivilisation ab.';
            WRITE(256,150,CivFlag,0+16,MyScreen[1]^,4,s);
         end;
         WAITLOOP(false);
         RECT(MyScreen[1]^,0,30,80,480,230);
...
begin
   ...
   with PlanetHeader^ do if (PFlags and FLAG_CIV_MASK<>ActPlayerFlag) then begin
      ...
      s:=realstr(Size/10,2)+' facher Erdradius';
      ...
      if Class in [CLASS_SATURN,CLASS_GAS] then s:='- Temperaturen im Bereich '
      else s:='- Anteil Wasser der Oberfläche: ';
      case Class of
         CLASS_DESERT:    begin
                             s:=s+intstr(Water div Size)+'%';
                             SA6:=SArr6('- wüstenartiger Planet','- Lebensformen möglich',
                                        '- Rohstoffe vorhanden','- ca. 60% Besiedelung möglich',
                                        '- sehr geringe Wasservorkommen',s);
                             l:=5;
                          end;
         CLASS_HALFEARTH: begin
                             s:=s+intstr(Water div Size)+'%';
                             SA6:=Sarr6('- erdähnlicher Planet','- gute Bedingungen für Lebensformen',
                                        '- Rohstoffe vorhanden','- ca. 80% Besiedelung möglich',
                                        '- Wasservorkommen, Athmosphäre, Wetter',s);
                             l:=4;
                          end;
         CLASS_EARTH:     begin
                             s:=s+intstr(Water div Size)+'%';
                             SA6:=SArr6('- erdartiger Planet','- sehr gute Bedingungen für Lebensformen',
                                        '- Rohstoffe vorhanden','- ca. 95% Besiedelung möglich',
                                        '- große Wasservorkommen, Athmosphäre, Wetter',s);
                             l:=6;
                          end;
         CLASS_SATURN:    begin
                             s:=s+intstr(l)+' .. '+intstr(l+(12-i)*10)+ '°C';
                             SA6:=SArr6('- Planet der Saturnklasse','- Lebensformen nicht möglich',
                                        '- keine Rohstoffe','- Besiedelung nicht möglich',
                                        '- Athmosphäre, Gasplanet',s);
                             l:=3;
                          end;
         CLASS_GAS:       begin
                             s:=s+intstr(l)+' .. '+intstr(l+(12-i)*10)+ '°C';
                             SA6:=SArr6('- sonnenähnlicher Gasplanet','- Lebensformen nicht möglich',
                                        '- keine Rohstoffe','- Besiedelung nicht möglich',
                                        '- Athmosphäre mit starken Turbulenzen',s);
                             l:=1;
                          end;
         CLASS_ICE:       begin
                             s:=s+intstr(Water div Size)+'%';
                             SA6:=SArr6('- Eisplanet','- Lebensformen möglich',
                                        '- Rohstoffe','- ca. 60% Besiedelung möglich',
                                        '- große Wasservorkommen, Athmosphäre, Wetter',s);
                             l:=8;
                          end;
         CLASS_PHANTOM:   begin
                             SA6:=SArr6('- Phantomplanet','- keine Informationen',
                                        '','','','');
                             l:=7;
                          end;
         CLASS_STONES:    begin
                             s:=s+intstr(Water div Size)+'%';
                             SA6:=SArr6('- frühes Entwicklungsstadium','- Lebensformen nicht möglich',
                                        '- wenig Rohstoffe','- ca. 75% Besiedelung möglich',
                                        '- Athmosphäre, Wetter, starke Erdbeben',s);
                             l:=0;
                          end;
         CLASS_WATER:     begin
                             s:=s+intstr(Water div Size)+'%';
                             SA6:=SArr6('- Wasserplanet','- Lebensformen möglich',
                                        '- Rohstoffe','- ca. 60% Besiedelung möglich',
                                        '- über 90% Wasser, Athmosphäre, Wetter',s);
                             l:=2;
                          end;
         otherwise;
      end;
...
begin
   ...
   for i:=1 to (random(6)+3) do begin
      Save.Bevölkerung[8]:=Save.Bevölkerung[8]+1;
      l:=AllocMem(sizeof(r_ShipHeader),MEMF_CLEAR);
      MyShipPtr:=ptr(l);
      MyShipPtr^:=r_ShipHeader(0,27,FLAG_OTHER,0,100,1,0,random(20)-40,random(20)-40,
                               ShipData[27].MaxShield,WEAPON_PTORPEDO,1,
                               ShipData[27].MaxMove,0,0,0,NIL,NIL,NIL);
...
begin
   ...
   if Save.WorldFlag=0 then begin
      if Save.CivPlayer[CivVar]<>0 then begin
         ModC:=GETTHESOUND(2);
         ModL:=ModMemL[2];
         s:='Das Militär der '+GETCIVNAME(CivVar)+' putscht und spaltet';
         case CivFlag of
            FLAG_TERRA:  REQUEST('Teile der Sternenflotte putschen und spalten',
                                 'die Föderation in zwei Teile!',CivFlag,CivFlag);
            FLAG_KLEGAN: REQUEST('Das Haus der Duras putscht und',
                                 'spaltet das Reich in zwei Teile!',CivFlag,CivFlag);
            FLAG_REMALO: REQUEST('Die Talshiar putschen und spalten das',
                                 'Romulanische Imperium in zwei Teile!',CivFlag,CivFlag);
            FLAG_CARDAC: REQUEST('Der obsidianische Orden putscht und',
                                 'spaltet das Militär in zwei Teile!',CivFlag,CivFlag);
            FLAG_FERAGI: REQUEST('Das Militär putscht und spaltet die',
                                 'Ferengiallianz in zwei Teile!',CivFlag,CivFlag);
            otherwise REQUEST(s,'das Imperium in zwei Teile!',CivFlag,CivFlag);
         end;
      end;
      for i:=1 to pred(MAXCIVS) do begin
...
procedure JAHADRINVASION(ActPlayer :byte);

var ShipID,SysID        :byte;
var l                   :long;
var MyShipPtr           :ShipHeader;
...
begin
   ...
   if Year>0 then Save.Staatstopf[8]:=Save.Staatstopf[8]+abs(Year)*120
   else Save.Staatstopf[8]:=200000;
   l:=0;
   ShipID:=0;
   for i:=24 downto 8 do if (Save.TechCosts[ActPlayer,ProjectNeedsTech[i]]<=0) then
    if (l<3) then begin
      l:=l+1;
      ShipID:=i;
   end;
   if ShipID<10 then ShipID:=10;
   l:=1000;
   for i:=1 to Save.SYSTEMS do if SystemX[i]+SystemY[i]<l then begin
      SysID:=i;
      l:=SystemX[i]+SystemY[i];
   end;

   for i:=1 to pred(MAXCIVS) do
    if Save.WarState[8,i] in [LEVEL_PEACE,LEVEL_UNKNOWN,LEVEL_DIED]
    then Save.WarState[8,i]:=LEVEL_COLDWAR;
   for i:=1 to (random(12)+3) do begin
      l:=AllocMem(sizeof(r_ShipHeader),MEMF_CLEAR);
      MyShipPtr:=ptr(l);
      MyShipPtr^:=r_ShipHeader(0,ShipID,FLAG_OTHER,0,15,$F1,0,random(20)-40,random(20)-40,
                               ShipData[ShipID].MaxShield,WEAPON_PTORPEDO,1,
                               ShipData[ShipID].MaxMove,0,0,0,NIL,NIL,NIL);
      LINKSHIP(MyShipPtr,^SystemHeader[SysID].FirstShip,0);
   end;
end;
   ...
   repeat
      case random(6) of
         ...
         1: if Save.WorldFlag in [0,WFLAG_JAHADR] then for i:=1 to MAXCIVS-2 do
            if (i<>ActPlayer) and not b and (Save.WarPower[i]>=Save.WarPower[ActPlayer])
            then begin
               CREATEJAHADR(i);
               b:=true;
            end;
         ...
         5: if Save.WorldFlag in [0,WFLAG_JAHADR] then begin
               JAHADRINVASION(ActPlayer);
               b:=true;
            end;
      end;
   until b;
   REQUEST('Der Schwierigkeitsgrad des Spieles wurde erhöht.','Viel Vergnügen!',12,12);
end;
...
begin
   ...
   if Save.WorldFlag=WFLAG_FIELD then begin
      ...
      Save.WorldFlag:=0;
      PLAYSOUND(3,420);
      MAKEBORDER(MyScreen[1]^,50,100,460,175,12,6,0);
      WRITE(256,110,ActPlayerFlag,16,MyScreen[1]^,4,'Störung im Raum-Zeit-Kontinuum!');
      WRITE(256,130,ActPlayerFlag,16,MyScreen[1]^,4,'Die Anomalie wurde beseitigt, alle');
      WRITE(256,150,ActPlayerFlag,16,MyScreen[1]^,4,'Planeten des Quadranten sind wieder frei.');
      WAITLOOP(false);
      RECT(MyScreen[1]^,0,50,100,460,175);
      REFRESHDISPLAY;
      exit;
   end;
   ...
   case random(5) of
      ...
      1: begin        { NEUVERTEILUNG DER KRÄFTE }
            ...
            MAKEBORDER(MyScreen[1]^,50,100,460,175,12,6,0);
            WRITE(256,110,ActPlayerFlag,16,MyScreen[1]^,4,'Riß im Raum-Zeit-Kontinuum!');
            WRITE(256,130,ActPlayerFlag,16,MyScreen[1]^,4,'Der Spieler wurde dadurch in ein');
            WRITE(256,150,ActPlayerFlag,16,MyScreen[1]^,4,'Parallel-Universum versetzt!');
            for i:=1 to Save.SYSTEMS do begin
               repeat
      ...
      2: begin   { CEBORCS }
            ...
            MAKEBORDER(MyScreen[1]^,50,100,460,175,12,6,0);
            WRITE(256,110,ActPlayerFlag,16,MyScreen[1]^,4,'Riß im Raum-Zeit-Kontinuum!');
            WRITE(256,130,ActPlayerFlag,16,MyScreen[1]^,4,'Das Universum hatte zeitweilig Kontakt');
            WRITE(256,150,ActPlayerFlag,16,MyScreen[1]^,4,'zu einem Parallel-Universum!');
            CEBORCATTACK(0);
            WAITLOOP(false);
            RECT(MyScreen[1]^,0,50,100,460,175);
         end;
      3: begin   { DCON-IMPERIUM }
            ...
            MAKEBORDER(MyScreen[1]^,50,100,460,175,12,6,0);
            WRITE(256,110,ActPlayerFlag,16,MyScreen[1]^,4,'Riß im Raum-Zeit-Kontinuum!');
            WRITE(256,130,ActPlayerFlag,16,MyScreen[1]^,4,'Das Sonnensystem wurde in eine');
            WRITE(256,150,ActPlayerFlag,16,MyScreen[1]^,4,'andere Zeit versetzt!');
            ...
            for i:=1 to MAXSYSTEMS do if i<>ActSys then begin
               Save.SystemName[i]:='Tk'+NArr[random(6)]+NArr[random(20)+6]
               ...
               for j:=1 to SystemHeader[i].Planets do begin
                  MyPlanetHeader:=ptr(SystemHeader[i].PlanetMemA+pred(j)*sizeof(r_PlanetHeader));
                  with MyPlanetHeader^ do begin
                     PName:='Tk'+NArr[random(6)]+NArr[random(20)+6];
                     if PFlags<>0 then begin
                        PFlags:=FLAG_OTHER;
                        Ethno:=FLAG_OTHER;
                        Biosphäre:=200;
      ...
      4: begin   { ZEIT-ANOMALIE }
            ...
            MAKEBORDER(MyScreen[1]^,50,100,460,175,12,6,0);
            WRITE(256,110,ActPlayerFlag,16,MyScreen[1]^,4,'Störung im Raum-Zeit-Kontinuum!');
            WRITE(256,130,ActPlayerFlag,16,MyScreen[1]^,4,'Allen Planeten des Quadranten melden');
            WRITE(256,150,ActPlayerFlag,16,MyScreen[1]^,4,'eine Zeitverschiebung von 79,1 Sekunden!');
...
begin
   ...
   WRITE(36,56,12,0,MyScreen[1]^,1,'Schiff   Schiffstyp');
   WRITE(232,56,12,0,MyScreen[1]^,1,'Siedler  Landgtr.    Kampf.      Status');
   i:=1; ShipFactor:=0;
   DRAWSHIPS;
   if (i>12) and (MyShipPtr<>NIL) then begin
      MoreThanShown:=true;
      DrawImage(^MyScreen[1]^.RastPort,^GadImg1,300,457);
      WRITE(335,460,0,0,MyScreen[1]^,4,'Mehr');
...
begin
   ...
   with MyPlanetHeader^ do begin
      ...
      s:='Bevölkerung: '+intstr(Population)+' -> ';
      ...
      s:='Infrastruktur: '+intstr(Infrastruktur div 2)+'% -> ';
...
begin
   ...
   s:=PathStr[7]+'ST/'+s+'.RAW';    {SFX/}
end;


begin
   INITIMAGES:=false;
   s:=PathStr[6]+'ST/'+Project[ShipPtr1^.SType]+'.img';       { SHIPS/ }
   if ShipPtr1^.SType=27 then s:=PathStr[6]+'ST/Borg.img';
   if not RAWLOADIMAGE(s,0,32,512,32,4,ImgBitMap4) then exit;
   s:=PathStr[6]+'ST/'+Project[ShipPtr2^.SType]+'.img';       { SHIPS/ }
   if ShipPtr2^.SType=27 then s:=PathStr[6]+'ST/Borg.img';
   if not RAWLOADIMAGE(s,0,64,512,32,4,ImgBitMap4) then exit;

   BSFSoundMemA[2]:=IMemA[0];
   INITSOUNDNAMES(ShipPtr1^.Weapon);
   if ShipPtr1^.SType=27 then s:=PathStr[7]+'ST/Borg.RAW';
   if not LOADSAMPLE(s,2) then exit;
   BSFSoundMemA[3]:=BSFSoundMemA[2]+BSFSoundSize[2]*2;
   INITSOUNDNAMES(ShipPtr2^.Weapon);
   if ShipPtr2^.SType=27 then s:=PathStr[7]+'ST/Borg.RAW';
...
begin
   MAKEBORDER(MyScreen[1]^,85,120,425,200,12,6,0);
   s:='Die '+GETCIVADJ(ActPlayer)+'n Forderungen';
   if (ActPlayer=GETCIVVAR(FLAG_FERAGI))
    or ((ActPlayer=8) and (Save.WorldFlag=WFLAG_JAHADR)) then s:='Die '+GETCIVADJ(ActPlayer)+'Forderungen';
   WRITE(256,140,ActPlayerFlag,16,MyScreen[1]^,4,s);
   WRITE(256,165,ActPlayerFlag,16,MyScreen[1]^,4,'wurden zurückgewiesen.');
   WAITLOOP(false);
   RECT(MyScreen[1]^,0,85,120,425,200);
   REFRESHDISPLAY;
end;
...
begin
   ...
   Save.WarState[CivVar,ActPlayer]:=LEVEL_PEACE;
   MAKEBORDER(MyScreen[1]^,85,120,425,200,12,6,0);
   s:='Die '+GETCIVADJ(ActPlayer)+'n Bedingungen';
   if (ActPlayer=GETCIVVAR(FLAG_FERAGI))
    or ((ActPlayer=8) and (Save.WorldFlag=WFLAG_JAHADR)) then s:='Die '+GETCIVADJ(ActPlayer)+'Forderungen';
   WRITE(256,140,ActPlayerFlag,16,MyScreen[1]^,4,s);
   WRITE(256,165,ActPlayerFlag,16,MyScreen[1]^,4,'wurden akzeptiert.');
   WAITLOOP(false);
   RECT(MyScreen[1]^,0,85,120,425,200);
   REFRESHDISPLAY;
end;
...
begin
   ...
   s2:='Spieler '+intstr(Save.CivPlayer[CivVar])+', sind sie bereit,';
   ...
   WRITE(115,192,8,16,MyScreen[1]^,4,'Annehmen');
   WRITE(397,192,8,16,MyScreen[1]^,4,'Ablehnen');
   repeat
      delay(RDELAY);
...
begin
   PLAYERJINGLE(CivVar);
   MAKEBORDER(MyScreen[1]^,35,80,475,290,12,6,0);
   WRITE(256,95,12,16,MyScreen[1]^,4,'Friedensverhandlungen');
...
begin
   ...
   if Save.WarState[ActPlayer,CivVar]=LEVEL_WAR then begin
      OptArr[Opts]:='Frieden ohne Bedingungen';
      OptID[Opts]:=OPT_NONE;
      Opts:=2;
      if XSystem>0 then begin
         OptArr[Opts]:='Frieden gegen Sonnensystem';
         OptID[Opts]:=OPT_SYSTEM;
         Opts:=Opts+1;
      end;
      if XTech>0 then begin
         OptArr[Opts]:='Frieden gegen Technologie';
         OptID[Opts]:=OPT_TECH;
         Opts:=Opts+1;
      end;
      OptArr[Opts]:='Frieden gegen Reparationen';
      OptID[Opts]:=OPT_MONEY;
      Opts:=Opts+1;
   end else begin
      if XSystem>0 then begin
         OptArr[Opts]:='Sonnensystem für Frieden';
         OptID[Opts]:=OPT_SYSTEM;
         Opts:=Opts+1;
      end;
      if XTech>0 then begin
         OptArr[Opts]:='Technologie für Frieden';
         OptID[Opts]:=OPT_TECH;
         Opts:=Opts+1;
      end;
      OptArr[Opts]:='Tributzahlungen für Frieden';
      OptID[Opts]:=OPT_MONEY;
      Opts:=Opts+1;
      OptArr[Opts]:='Gemeinsamen Feldzug anbieten';
      OptID[Opts]:=OPT_WAR;
      Opts:=Opts+1;
      if Save.CivPlayer[CivVar]=0 then begin
         OptArr[Opts]:='Entwicklungshilfe leisten';
         OptID[Opts]:=OPT_HELP;
         Opts:=Opts+1;
         if Save.WarPower[CivVar]<Save.WarPower[ActPlayer] then begin
            OptArr[Opts]:='Schiff übergeben';
            OptID[Opts]:=OPT_SHIP;
            Opts:=Opts+1;
         end;
      end;
   ...
   if (Save.CivPlayer[CivVar]<>0) and not Save.PlayMySelf then begin
      ...
      case OptID[l] of
         OPT_NONE:   begin
                        if SMALLREQUEST('einen bedingungslosen Frieden zu akzeptieren?')
                        then POSITIVEANSWER else NEGATIVEANSWER;
                     end;
         OPT_SYSTEM: begin
                        s2:='das System '+Save.SystemName[XSystem]+' zu übergeben?';
         ...
         OPT_TECH:   begin
                        s2:='Informationen über '+Technology[XTech]+' zu übergeben?';
                        if not SMALLREQUEST(s2) then NEGATIVEANSWER else begin
                           POSITIVEANSWER;
         ...
         OPT_MONEY:  begin
                        s2:=intstr(XCosts)+' an Reparationen zu zahlen?';
                        if not SMALLREQUEST(s2) then NEGATIVEANSWER else begin
                           POSITIVEANSWER;
                           Save.Staatstopf[CivVar]:=Save.StaatsTopf[CivVar]-XCosts;
                           Save.Staatstopf[ActPlayer]:=Save.StaatsTopf[ActPlayer]+XCosts;
         ...
         OPT_WAR:    begin
                        ...
                        if Opts>0 then begin
                           ...
                           if l=0 then exit;
                           CALLOTHERPLAYER;
                           s2:='Krieg gegen die '+GETCIVNAME(OptID[l])+' zu führen?';
                           if not SMALLREQUEST(s2) then NEGATIVEANSWER else begin
                              POSITIVEANSWER;
                           ...
                           MAKEBORDER(MyScreen[1]^,85,120,425,200,12,6,0);
                           WRITE(256,140,12,16,MyScreen[1]^,4,'Keine weiteren');
                           WRITE(256,160,12,16,MyScreen[1]^,4,'Zivilisationen bekannt.');
                           WAITLOOP(false);
                           RECT(MyScreen[1]^,0,85,120,425,200);
                           REFRESHDISPLAY;
                        end;
   ...
   case OptID[l] of
      ...
      OPT_MONEY:  if (Save.WarPower[CivVar]*2>Save.WarPower[ActPlayer]) or (XCosts<10000) then NEGATIVEANSWER
                     ...
                     Save.Staatstopf[ActPlayer]:=Save.StaatsTopf[ActPlayer]+XCosts;
                     MAKEBORDER(MyScreen[1]^,85,120,425,200,12,6,0);
                     WRITE(256,127,ActPlayerFlag,16,MyScreen[1]^,4,'Sie erhalten Reparationen');
                     WRITE(256,147,ActPlayerFlag,16,MyScreen[1]^,4,'in Höhe von');
                     s[1]:=intstr(XCosts);
      ...
      OPT_HELP:   begin
                     ...
                     if i in [0,1] then begin
                        ...
                        if XTech=0 then i:=2 else begin
                           i:=0;
                           s2:=Technology[XTech]+'.';
                           REQUEST('Sie übergeben Informationen über',s2,ActPlayerFlag,ActPlayerFlag);
                     ...
                     if i=2 then begin
                        ...
                        if XCosts=0 then i:=3 else begin
                           i:=0;
                           s2:='von '+intstr(XCosts)+'.';
                           REQUEST('Sie zahlen Beihilfen in Höhe',s2,ActPlayerFlag,ActPlayerFlag);
                           Save.Staatstopf[CivVar]:=Save.Staatstopf[CivVar]+XCosts;
                           Save.Staatstopf[ActPlayer]:=Save.Staatstopf[ActPlayer]-XCosts;
                           Save.WarState[CivVar,ActPlayer]:=LEVEL_ALLIANZ;
                           Save.WarState[ActPlayer,CivVar]:=LEVEL_ALLIANZ;
                     ...
                     if i=3 then REQUEST('Leider haben sie nichts, was','diese Zivilisation gebrauchen könnte!',ActPlayerFlag,ActPlayerFlag);
                     RECT(MyScreen[1]^,0,85,120,425,200);
                     REFRESHDISPLAY;
                  end;
      OPT_SHIP:   begin
...
begin
   ...
   WRITE(133,203,ActPlayerFlag,0,MyScreen[1]^,4,'Siedler');
   WRITE(232,203,ActPlayerFlag,0,MyScreen[1]^,4,'Landungstr.');
   WRITE(363,203,ActPlayerFlag,0,MyScreen[1]^,4,'Wasser.');
end;
...
begin
   ...
   with MyPlanetHeader^ do begin
      ...
      for i:=1 to GadCnt do begin
         ...
         case GadSet[i] of
            GADGET_LADEN:    WRITE(256,103+i*22,0,16,MyScreen[1]^,4,'Be-/Entladen');
            GADGET_ORBIT:    WRITE(256,103+i*22,0,16,MyScreen[1]^,4,'Orbit');
            GADGET_LANDUNG:  WRITE(256,103+i*22,0,16,MyScreen[1]^,4,'Einnehmen');
            GADGET_DIPLOMAT: WRITE(256,103+i*22,0,16,MyScreen[1]^,4,'Verhandeln');
            otherwise WRITE(256,102+i*22,0,16,MyScreen[1]^,4,'Angriff');
         end;
      end;
      b:=false;
                                      ...
                                      if (MyPlanetHeader^.PFlags and FLAG_CIV_MASK<>ActPlayerFlag) then begin
                                         ...
                                         if LTOut>0 then begin
                                            ...
                                            if MyPlanetHeader^.XProjectPayed>1000 then begin
                                               ...
                                               REQUEST('Erbeutetes Vermögen:',s,ActPlayerFlag,12);
...
procedure DOWORMHANDLING(AMyShipPtr :ShipHeader);
...
begin
   ...
   WRITE(256,Offset+13,ActPlayerFlag,16,MyScreen[1]^,4,'Raumschiff gerät in Wurmloch!' );
   WRITE(256,Offset+35,ActPlayerFlag,16,MyScreen[1]^,4,'Das Ende des Loches befindet sich');
   s:='im System '+Save.SystemName[SysID];
   ...
   if random(100)<10 then REQUEST('Glück gehabt, es war nicht','der Gamma-Quadrant!',12,12);
   REFRESHDISPLAY;
                                  ...
                                  and (MyShipPtr^.Ladung and MASK_SIEDLER=0) then begin
                                     ...
                                     if (Save.WorldFlag=WFLAG_CEBORC) and (CivVar=8) then begin
                                        ...
                                        with MyPlanetHeader^ do begin
                                           ...
                                           s:='Maquis plündern Planeten '+PName;
                                           s2:='und rauben '+intstr(XProjectPayed div 5)+'!';
...
begin
   ...
   WRITE(256,124,0,16,MyScreen[1]^,4,'Sterne');
   WRITE(256,146,0,16,MyScreen[1]^,4,'Sprengen');
   if UseShipPtr^.Target=TARGET_POSITION then WRITE(256,168,0,16,MyScreen[1]^,4,'Ablösen')
   else WRITE(256,168,0,16,MyScreen[1]^,4,'Position');
   if MyShipPtr^.SType<>21 then WRITE(256,190,0,16,MyScreen[1]^,4,'Bewässern')
     else WRITE(256,190,0,16,MyScreen[1]^,4,'Stargate');
   WRITE(256,212,0,16,MyScreen[1]^,4,'Warten');
   repeat
      delay(RDELAY);
      if (LData^ and 64=0) then begin
            ...
            if IBase^.MouseY in [122..142] then begin
               ...
               if MyShipPtr^.SType=21 then with SystemHeader[ActSys] do begin
                  if round(SQRT((MyShipPtr^.PosX*MyShipPtr^.PosX)+(MyShipPtr^.PosY*MyShipPtr^.PosY)))>=10 then begin
                     ...
                     WRITE(256,125,ActPlayerFlag,16,MyScreen[1]^,4,'Bahnradius für Stargate' );
                     WRITE(256,147,ActPlayerFlag,16,MyScreen[1]^,4,'ist zu groß!');
                     WRITE(256,169,ActPlayerFlag,16,MyScreen[1]^,4,'Radius muß kleiner 10 sein!');
                     WAITLOOP(false);
                     ...
                     REFRESHDISPLAY;
                     delay(15);
                  end else REQUEST('Stargate ist bereits','installiert!',ActPlayerFlag,ActPlayerFlag)
                  ...
                  then begin
                     if not FleetUsed then begin
                        ...
                        MyShipPtr^.Moving:=0;
                     end else REQUEST('Flotten können nicht Bewässern!','Es sind einzelne Schiffe aus der Flotte zu lösen!',ActPlayerFlag,ActPlayerFlag);
                  end else REQUEST('Das Schiff hat noch Ladung an Bord!','Schiff zuerst entladen!',ActPlayerFlag,ActPlayerFlag);
...
begin
   ...
   if GadID in [0,6] then begin
      ...
      RECT(MyScreen[2]^,0,410,341,510,360);
      s:=intstr(Level);
   ...
   if GadID in [0,1] then begin
      if Save.PlayMySelf then begin
         DRAWGAD(80,1);
         WRITE(240,115,40,1,MyScreen[2]^,4,'Computer');
      end else begin
         DRAWGAD(80,2);
         WRITE(240,115,40,1,MyScreen[2]^,4,'Trekkie');
      end;
   end;
   if GadID in [0,2] then begin
      if Save.SmallFight then begin
         DRAWGAD(160,1);
         WRITE(240,195,40,1,MyScreen[2]^,4,'aus');
      end else begin
         DRAWGAD(160,2);
         WRITE(240,195,40,1,MyScreen[2]^,4,'ein');
      end;
   end;
   if GadID in [0,3] then begin
      if Save.SmallLand then begin
         DRAWGAD(240,1);
         WRITE(240,275,40,1,MyScreen[2]^,4,'Computer');
      end else begin
         DRAWGAD(240,2);
         WRITE(240,275,40,1,MyScreen[2]^,4,'Spieler');
      end;
   end;
   if GadID in [0,4] then begin
      if Save.NoWorm then begin
         DRAWGAD(320,1);
         WRITE(240,355,40,1,MyScreen[2]^,4,'Computer');
      end else begin
         DRAWGAD(320,2);
         WRITE(240,355,40,1,MyScreen[2]^,4,'Spieler');
      end;
   end;
   if GadID in [0,5] then begin
      if Save.FastMove then begin
         DRAWGAD(400,1);
         WRITE(240,435,40,1,MyScreen[2]^,4,'schnell');
      end else begin
         DRAWGAD(400,2);
         WRITE(240,435,40,1,MyScreen[2]^,4,'normal  ');
      end;
   end;
end;
...
begin
   ...
   WRITE(320,45,40,16,MyScreen[2]^,5,'SPIELOPTIONEN');
   ...
   WRITE(240,90,40,0,MyScreen[2]^,4,'Spieler');
   ...
   WRITE(240,170,40,0,MyScreen[2]^,4,'Kampfszenen');
   ...
   WRITE(240,250,40,0,MyScreen[2]^,4,'Planeten-Attacke');
   ...
   WRITE(240,330,40,0,MyScreen[2]^,4,'Wurmloch-Flug');
   ...
   WRITE(240,410,40,0,MyScreen[2]^,4,'Schiffsbewegungen');
   ...
   MAKEBORDER(MyScreen[2]^,407,339,513,362,14,40,1);

   WRITE(555,100,40,16,MyScreen[2]^,4,'Spieler');
   if Mode=1 then begin
   ...
   if (Player>1) and (Mode=1) then begin
      ...
      for i:=1 to Player do begin
         SWITCHDISPLAY;
         INITMENU;
         s:='Spieler '+intstr(i)+' bitte eine Zivilisation auswählen!';
         ...
         for j:=1 to btx do begin
            ...
            if Save.CivPlayer[j]=0 then s:=GETCIVNAME(j) else s:='Spieler '+intstr(Save.CivPlayer[j]);
      ...
      SWITCHDISPLAY;
      INITMENU;
      WRITE(320,100,40,16,MyScreen[2]^,4,'Mit wieviel Heimatplaneten soll begonnen werden?');
      for i:=1 to 5 do begin
...
begin
   ...
   PLAYSOUND(1,300);
   GETSAVENAME:=PathStr[9]+'IMPTST.'+intstr(j);
end;
...
begin
   REQUEST('Nicht genug Speicher vorhanden!','Spielstand kann nicht geladen werden!',8,8);
end;
...
begin
   ...
   WRITE(255,124,0,16,MyScreen[1]^,4,'Laden');
   WRITE(255,146,0,16,MyScreen[1]^,4,'Speichern');
   WRITE(255,168,0,16,MyScreen[1]^,4,'Löschen');
   WRITE(255,190,0,16,MyScreen[1]^,4,'Optionen');
   WRITE(255,212,0,16,MyScreen[1]^,4,'Highscore');
   WRITE(255,234,8,16,MyScreen[1]^,4,'ENDE');
   b:=false;
   repeat
      delay(RDELAY);
      if (LData^ and 64=0) or (Auto>0) then begin
         if (IBase^.MouseX in [198..314]) or (Auto>0) then begin
            if (IBase^.MouseY in [122..142]) or (Auto=2) then begin
               KLICKGAD(198,122);
               b:=true;
               s:=GETSAVENAME('Spielstand laden');
               if s<>'' then begin
                  FHandle:=OPENSMOOTH(s,MODE_OLDFILE);
                  if FHandle<>0 then begin
                     FREESYSTEMMEMORY;
               ...
               KLICKGAD(198,144);
               b:=true;
               s:=GETSAVENAME('Spielstand speichern');
               if s<>'' then begin
               ...
               KLICKGAD(198,166);
               s:=GETSAVENAME('Spielstand LÖSCHEN');
...
begin
   case CivFlag of
      FLAG_TERRA:  GETMYADJ:='Terranisches';
      FLAG_KLEGAN: GETMYADJ:='Klingonisches';
      FLAG_REMALO: GETMYADJ:='Romulanisches';
      FLAG_CARDAC: GETMYADJ:='Kardassianisches';
      FLAG_FERAGI: GETMYADJ:='Ferengi-';
      FLAG_BAROJA: GETMYADJ:='Bajoranisches';
      FLAG_VOLKAN: GETMYADJ:='Vulkanisches';
      FLAG_OTHER:  case Save.WorldFlag of
                     WFLAG_CEBORC: GETMYADJ:='Borg-';
                     WFLAG_DCON:   begin
                                      GETMYADJ:='Tkonianische Schiffe erreichen';
                                      DconDone:=true;
                                   end;
                     WFLAG_JAHADR: GETMYADJ:='Jem´Hadar-';
                     otherwise GETMYADJ:=GETMYADJ(Save.WorldFlag);
                  end;
      FLAG_MAQUES: GETMYADJ:='Maquis-';
                        ...
                        and (Save.CivPlayer[ActPlayer]<>0) and not DconDone then begin
                           ...
                           if not DconDone then
                            if MyShipPtr^.SType=SHIPTYPE_FLEET then begin
                              s[length(s)]:=chr(0);
                              s:=s+' Flotte erreicht'
                           end else s:=s+' Schiff erreicht';
                           WRITE(280,136,MyShipPtr^.Owner,1+16,MyScreen[1]^,4,s);
                           s:='System '+Save.SystemName[i];
...
begin
   ...
   if SysID>0 then begin
      ...
      ActPProject:=MyPlanetHeader^.ProjectPtr;
      s:='System: '+Save.SystemName[SysID]+'   Planet: '+MyPlanetHeader^.PName;
      repeat
...
begin
   ...
   repeat
      ...
      while not (ActShipPtr^.SType in [8..24,27,SHIPTYPE_FLEET,TARGET_STARGATE])
...
begin
   ...
   with MyPlanetHeader^ do begin
      ...
      s1:='Schwere ethnische Konflikte ausgebrochen';
      if PFlags and FLAG_CIV_MASK=ActPlayerFlag then s2:='Unabhängigkeit wurde erklärt';
      for i:=1 to pred(MAXCIVS) do
      ...
      if PFlags and FLAG_CIV_MASK=ActPlayerFlag then begin
         case ActPlayerFlag of
            FLAG_TERRA:  s2:='Übertritt zur Föderation wurde erklärt';
            FLAG_KLEGAN: s2:='Übertritt ins Reich wurde erklärt';
            FLAG_FERAGI: s2:='Übertritt zur Allianz wurde erklärt';
            otherwise s2:='übertritt ins Imperium wurde erklärt'
         end;
      end;
      PlanetLose:=true;
   end;
...
begin
   ...
   with MyPlanetHeader^ do begin
      ...
      if (Year mod 10=0) and (odd(Year div 10)=odd(PlanetNum)) then begin
         s1:='Energienotstand durch Energiemangel';
         if ActPProjects^[41]=0 then begin
            if ActPProjects^[30]=0 then begin
               ...
               and (ProjectID<>30) then begin                      { RECYCLINGANLAGE }
                  TheProject:=30;
                  s1:='Massensterben durch verseuchte Landstriche';
               end else exit;
               ...
               and (ProjectID<>42) then begin
                  TheProject:=42;
                  s1:='Schwere Unwetter verwüsten Städte';
               end else exit;
            end else exit;
         end else exit;
         INFORMUSER;
         ...
         case i of
            0: if ActPProjects^[28]=0 then begin    {kontinentale Union}
                  TheProject:=28;  s1:='Bürgerkrieg ausgebrochen';
               end else exit;
            1: if ActPProjects^[29]=0 then begin    {globale Union}
                  TheProject:=29;  s1:='Kriege zwischen den Nationen';
               end else exit;
            2: if Save.ProjectCosts[ActPlayer,4]<>0 then s1:='Massenepidemie durch unbekannten Virus' else exit;
            3: s1:='Klimakatastrophe durch Kometeneinschläge';
      ...
      if (Save.CivPlayer[GETCIVVAR(PFlags)]<>0) or (TheProject=-1) then begin
         ...
         s:='System: '+Save.SystemName[ActSys]; WRITE(256,117,PFlags and FLAG_CIV_MASK,1+16,MyScreen[1]^,4,s);
         s:='Planet: '+PName;                   WRITE(256,137,PFlags and FLAG_CIV_MASK,1+16,MyScreen[1]^,4,s);
         if (TheProject>0) and (Save.TechCosts[ActPlayer,ProjectNeedsTech[TheProject]]<=0) then begin
            s:='Bürger fordern '+Project[TheProject];
         ...
         if (Save.CivPlayer[GETCIVVAR(PFlags)]<>0) and (ActPlayer=GETCIVVAR(PFlags)) then begin
            ...
            WRITE(198,220,0,16,MyScreen[1]^,4,'Planet');
            WRITE(338,220,0,16,MyScreen[1]^,4,'weiter');
            b:=false;
...
begin
   ...
   with MyPlanetHeader^ do begin
      ...
      s:='System: '+Save.SystemName[ActSys]; WRITE(256,127,PFlags and FLAG_CIV_MASK,1+16,MyScreen[1]^,4,s);
      s:='Planet: '+PName;              WRITE(256,147,PFlags and FLAG_CIV_MASK,1+16,MyScreen[1]^,4,s);
      WRITE(256,170,12,16,MyScreen[1]^,4,'Dramatischer Bevölkerungsrückgang!');
      WRITE(256,190,12,16,MyScreen[1]^,4,'Biosphäre reinigen!');
      if Save.PlayMySelf then delay(PAUSE);
      WAITLOOP(Save.PlayMySelf);
...
begin
   ...
   if (Save.Bevölkerung[Smallest]>1000) and ((Save.CivPlayer[Smallest]=0) or Save.PlayMySelf)
    then begin
      ...
      Save.Staatstopf[Biggest]:=Save.Staatstopf[Biggest]-(Save.WarPower[Biggest]*90);
      Save.Staatstopf[Smallest]:=Save.Staatstopf[Smallest]+(Save.WarPower[Biggest]*90);
   end;
...
begin { ROTATEPLANETS }
   ...
   for i:=1 to Save.SYSTEMS do if SystemHeader[i].Planets>0 then begin
      ...
      if (ActPlayer=1) and (SystemHeader[i].FirstShip.SType=TARGET_STARGATE) then with SystemHeader[i] do begin
         ...
         if (FirstShip.PosX in [-3..2]) and (FirstShip.PosY in [-3..2]) then begin
            ...
            WRITE(256,151,12,16,MyScreen[1]^,4,'Umlaufbahn kollabiert,');
            WRITE(256,171,12,16,MyScreen[1]^,4,'Stargate zerstört');
            if Save.PlayMySelf then delay(PAUSE);
            WAITLOOP(Save.PlayMySelf);
            RECT(MyScreen[1]^,0,85,120,425,200);
            FirstShip.SType:=0;
         ...
         with PlanetHeader^ do begin
            ...
            CLASS_HALFEARTH,CLASS_DESERT]) then begin
               ...
               and (Save.CivPlayer[ActPlayer]<>0) and (Year>1900) then begin
                  INFORMUSER;
                  MAKEBORDER(MyScreen[1]^,85,120,425,200,12,6,0);
                  s:='System: '+Save.SystemName[i]; WRITE(256,127,ActPlayerFlag,1+16,MyScreen[1]^,4,s);
                  s:='Planet: '+PName;         WRITE(256,147,ActPlayerFlag,1+16,MyScreen[1]^,4,s);
                  s:='wird zu Planet der Klasse ';
                  case Class of
                     CLASS_DESERT    : s:=s+'D';
                     CLASS_HALFEARTH : s:=s+'H';
                     CLASS_EARTH     : s:=s+'M';
            ...
            if PFlags and FLAG_CIV_MASK=ActPlayerFlag then begin
               ...
               if ProjectID<>0 then begin
                  ...
                  if (l=1) and (Save.CivPlayer[ActPlayer]<>0) then begin
                     ...
                     s:='System: '+Save.SystemName[i]; WRITE(256,127,ActPlayerFlag,1+16,MyScreen[1]^,4,s);
                     s:='Planet: '+PName;              WRITE(256,147,ActPlayerFlag,1+16,MyScreen[1]^,4,s);
                     if ProjectID>0 then s:='baut '+Project[ProjectID]
                     else case ProjectID of
                        -3: s:='Biosphäre gereinigt';
                        -2: s:='Infrastruktur repariert';
                        otherwise s:='Industrieanlagen repariert';
                     end;
                     WRITE(256,173,12,1+16,MyScreen[1]^,4,s);
                     delay(5);
                     if (ProjectID in [8..24]) and not Save.PlayMySelf then begin
                        ...
                        for k:=1 to 2 do DrawImage(^MyScreen[1]^.RastPort,^GadImg1,k*140,218);
                        WRITE(198,220,0,16,MyScreen[1]^,4,'Space');
                        WRITE(338,220,0,16,MyScreen[1]^,4,'Orbit');
                        b:=false;
                        repeat
                           delay(RDELAY);
                           if (LData^ and 64=0) then begin
                  ...
                  if ((l=1) or (ProjectID in [-3..0])) and ((Save.CivPlayer[ActPlayer]=0) or Save.PlayMySelf) then begin
                     ...
                     if (ActPlayer=8) and (Save.WarPower[8]>5000) then ProjectID:=0;
                        {*** DCON`s begrenzen ***}
   ...
   if ActPlayer<MAXCIVS then if (Year>1901) or (Year<0) then begin
         ...
         if NewTech[1]<>0 then begin
            if (NewTech[1]=42) and (Save.ProjectCosts[ActPlayer,6]>0) then begin
               if Year mod 13=0 then begin
                  ...
                  WRITE(256,110,ActPlayerFlag,0+16,MyScreen[1]^,4,'Wissenschaftler fordern');
                  WRITE(256,132,ActPlayerFlag,0+16,MyScreen[1]^,4,'Durchführung des');
                  WRITE(256,154,12,0+16,MyScreen[1]^,4,'GENESIS-Projects');
                  WAITLOOP(false);
               ...
               WRITE(100,110,ActPlayerFlag,0,MyScreen[1]^,4,'Was soll entwickelt werden?');
               for j:=1 to 6 do if NewTech[j]>0 then WRITE(70,j*20+120,12,0,MyScreen[1]^,4,Technology[NewTech[j]]);
               i:=0;
               repeat
                  delay(RDELAY);
                  ...
                  if (i<>j) and (j in [1..6]) then begin
                     i:=j;
                     for j:=1 to 6 do if NewTech[j]>0 then WRITE(70,j*20+120,12,0,MyScreen[1]^,4,Technology[NewTech[j]]);
                     if NewTech[i]>0 then WRITE(70,i*20+120,ActPlayerFlag,0,MyScreen[1]^,4,Technology[NewTech[i]]);
                  end;
                  if (LData^ and 64=0) then begin
                     PLAYSOUND(1,300);
      ...
      for i:=1 to pred(MAXCIVS) do begin
         ...
         then begin
            ...
            WRITE(256,156,12,1+16,MyScreen[1]^,4,'bieten Allianz gegen');
            s:=GETCIVNAME(j);
            WRITE(256,176,GETCIVFLAG(j),1+16,MyScreen[1]^,4,s);
            WRITE(256,196,12,1+16,MyScreen[1]^,4,'an');
            DrawImage(^MyScreen[1]^.RastPort,^GadImg1,105,225);
            DrawImage(^MyScreen[1]^.RastPort,^GadImg1,290,225);
            WRITE(162,227,8,16,MyScreen[1]^,4,'Annehmen');
            WRITE(348,227,8,16,MyScreen[1]^,4,'Ablehnen');
            repeat
               delay(RDELAY);
         ...
         or ((Save.Bevölkerung[8]=0) and (i=8) and not (Save.WorldFlag in [0,WFLAG_FIELD])) then begin
            ...
            if Save.CivPlayer[ActPlayer]<>0 then begin
               ...
               s:='Zivilisation der '+GETCIVNAME(i);
               WRITE(256,140,GETCIVFLAG(i),1+16,MyScreen[1]^,4,s);
               WRITE(256,165,12,1+16,MyScreen[1]^,4,'ist verschwunden.');
               if Save.PlayMySelf then delay(PAUSE);
               WAITLOOP(Save.PlayMySelf);
   ...
   and (Year mod 5=0) then for i:=1 to (MAXCIVS-2) do if (l=0) and (i<>ActPlayer)
    and (Save.JSteuer[i]>0) and (Save.WarState[ActPlayer,i]=LEVEL_WAR) then begin
      if (Save.CivPlayer[ActPlayer]<>0) and not Save.PlayMySelf then begin
         ...
         WRITE(256,136,FLAG_OTHER,1+16,MyScreen[1]^,4,'Das Dominion fordert die Einstellung');
         WRITE(256,156,FLAG_OTHER,1+16,MyScreen[1]^,4,'aller Feindseligkeiten gegen die');
         s:=GETCIVNAME(i)+' bei Zahlung von';
         WRITE(256,176,FLAG_OTHER,1+16,MyScreen[1]^,4,s);
         l:=abs(Year)*11;   s:='Reparationen in Höhe von '+intstr(l);
   ...
   and not Save.PlayMySelf then begin
      INFORMUSER;
      REQUEST('Es droht die Zahlungsunfähigkeit!','Bitte Steuern erhöhen!',ActPlayerFlag,ActPlayerFlag);
   end;
   ...
   (Save.CivPlayer[ActPlayer]<>0) and not Save.PlayMySelf then begin
      INFORMUSER;
      REQUEST('Es droht der endgültige Bankrott!','Bitte unbedingt Steuern erhöhen!',ActPlayerFlag,ActPlayerFlag);
   end;
   if Save.Staatstopf[ActPlayer]<-10000 then begin
      ...
      if (btx>0) and (Save.CivPlayer[ActPlayer]<>0) then begin
         ...
         s:=GETCIVNAME(ActPlayer)+' haben Schulden!';
         WRITE(256,127,12,16,MyScreen[1]^,4,s);
         s:='kann '+Project[btx]+' nicht erhalten!';
   ...
   if (Year>1973) and (Save.CivPlayer[ActPlayer]<>0) then begin
      if (Save.ImperatorState[ActPlayer]<700) and (Warnung[ActPlayer]=0) then begin
         ...
         WRITE(256, 95,ActPlayerFlag,16,MyScreen[1]^,4,'Der äußerst aggressive Führungsstil des');
         WRITE(256,115,ActPlayerFlag,16,MyScreen[1]^,4,'Imperators löst großen Unmut bei der');
         WRITE(256,135,ActPlayerFlag,16,MyScreen[1]^,4,'Bevölkerung aus.');
         WRITE(256,155,ActPlayerFlag,16,MyScreen[1]^,4,'Es werden aus allen Provinzen und Kolonien');
         WRITE(256,175,ActPlayerFlag,16,MyScreen[1]^,4,'schwere Unruhen gemeldet!');
         Warnung[ActPlayer]:=1;
         ...
         WRITE(256, 95,ActPlayerFlag,16,MyScreen[1]^,4,'Der anhaltend brutale und faschistische');
         WRITE(256,115,ActPlayerFlag,16,MyScreen[1]^,4,'Führungsstil des Imperators verursacht');
         WRITE(256,135,ActPlayerFlag,16,MyScreen[1]^,4,'weitere Unruhen und offenen Aufruhr auf allen');
         WRITE(256,155,ActPlayerFlag,16,MyScreen[1]^,4,'Planeten des Imperiums!');
         Warnung[ActPlayer]:=2;
         WAITLOOP(false);
   ...
   and (Save.CivPlayer[ActPlayer]<>0) and (Year>2065) and (Save.MaxWarPower[ActPlayer]=0) then begin
      ...
      if j>1 then begin
         ...
         s:='Die '+GETCIVNAME(ActPlayer)+' trennen sich von Spieler '+intstr(Save.CivPlayer[ActPlayer])+',';
         WRITE(256,140,ActPlayerFlag,16,MyScreen[1]^,4,s);
         WRITE(256,160,ActPlayerFlag,16,MyScreen[1]^,4,'da dieser offenbar nicht in der Lage ist,');
         WRITE(256,180,ActPlayerFlag,16,MyScreen[1]^,4,'eine Zivilisation erfolgreich zu führen!');
         WAITLOOP(false);
   ...
   and (ActPlayer<=(MAXCIVS-2)) and (Save.Bevölkerung[ActPlayer]<=0) then begin
      ...
      if SystemFlags[1,j] and FLAG_CIV_MASK=0 then begin
         ...
         if (Year<2120) and (random(45)=0) then CREATEJAHADR(ActPlayer)
         else if (Year>2120) and (random(255)=0) then JAHADRINVASION(ActPlayer);
      end;
   end;
   if (random(219)=0) and (random(219)=0) then CEBORCATTACK(0);
   if (random(219)=0) and (random(219)=0) then JAHADRINVASION(ActPlayer);
         ...
         then begin
            ...
            with MyShipPtr^ do begin
               if SType=SHIPTYPE_FLEET then begin
                  ...
                  s:=Project[SType];
                  WRITE(40,y,ActPlayerFlag,0,MyScreen[1]^,3,s);
                  s:='Siedler: '+intstr((Ladung and MASK_SIEDLER) div 16);
                  WRITE(135,y,12,0,MyScreen[1]^,3,s);
                  s:='Landungstrupps: '+intstr(Ladung and MASK_LTRUPPS);
                  WRITE(230,y,12,0,MyScreen[1]^,3,s);
                  s:='Beladung: '+intstr(round(Fracht / ShipData[Stype].MaxLoad*100))+' %';
    ...
    if (SystemFlags[ActPlayer,i] and FLAG_KNOWN=FLAG_KNOWN) then with SystemHeader[i] do for j:=1 to Planets do begin
      ...
      ((CivFlag=0) and not (PFlags and FLAG_CIV_MASK=ActPlayerFlag) and (PFlags>0)) then begin
         ...
         if CivFlag=ActPlayerFlag then begin
            case ProjectID of
               -3: s:='Biosphäre reinigen';
               -2: s:='Infrastruktur reparieren';
               -1: s:='Industrie aufbauen';
                0: s:='---';
            ...
            s:='Größe: '+realstr(Size/10,2);
            WRITE(170,y,12,0,MyScreen[1]^,3,s);
            if Ethno=ActPlayerFlag then begin
               s:=GETCIVADJ(ActPlayer)+' Minderheit';
...
begin
   ...
   WRITE(255,124,12,16,MyScreen[1]^,4,'Planeten');
   WRITE(255,146,0,16,MyScreen[1]^,4,'eigene');
   WRITE(255,168,0,16,MyScreen[1]^,4,'fremde');
   WRITE(255,190,12,16,MyScreen[1]^,4,'Schiffe');
   WRITE(255,212,0,16,MyScreen[1]^,4,'Position');
   WRITE(255,234,0,16,MyScreen[1]^,4,'Bewässern');
   b:=false;
   repeat
      delay(RDELAY);
      if (LData^ and 64=0) then begin
            ...
            repeat
               ...
               if (PSys[ThisP]<>0) and (ThisP<>LastP) and (IBase^.MouseX in [0..511]) then begin
                  if (LastP<36) and (PSys[LastP]<>0) then begin
                     ...
                     then s:='Flotte' else s:=Project[ShipPos[LastP]^.SType];
                  ...
                  if ThisP<36 then begin
                     ...
                     then s:='Flotte' else s:=Project[ShipPos[ThisP]^.SType];
         ...
         if (LData^ and 64=0) and not Save.PlayMySelf then begin
            ...
            if (IBase^.MouseX in [0..511]) and (IBase^.MouseY in [0..511]) then begin
               ...
               if FINDOBJECT(ActSys,IBase^.MouseX,IBase^.MouseY,NIL) then case ObjType of
                  TYPE_PLANET:   begin
                                    MyPlanetHeader:=ObjPtr;
                                    s:='Standardorbit '+MyPlanetHeader^.PName;
...
begin
   ...
   if (Year>1900) and (Save.CivPlayer[ActPlayer]<>0) then begin
      if (Save.WarState[ActPlayer,ActPlayer]<>LEVEL_DIED) then for i:=1 to pred(MAXCIVS) do begin
         if Save.LastWarState[ActPlayer,i]<>Save.WarState[ActPlayer,i] then begin
            j:=j+1;
            if Save.WarState[ActPlayer,i]=LEVEL_DIED then s:='Zivilisation der '+GETCIVNAME(i)+' ist verschwunden'
            else if (Save.WarState[ActPlayer,i]=LEVEL_PEACE)
             and (Save.LastWarState[ActPlayer,i]=LEVEL_WAR) then s:=GETCIVNAME(i)+' haben Friedensvertrag unterschrieben'
            else if Save.WarState[ActPlayer,i]=LEVEL_WAR then s:=GETCIVNAME(i)+' haben Krieg gegen '+GETCIVNAME(ActPlayer)+' begonnen'
            else if Save.WarState[ActPlayer,i]=LEVEL_ALLIANZ then s:=GETCIVNAME(i)+' haben Bündnis mit '+GETCIVNAME(ActPlayer)+'n geschlossen'
            else j:=j-1;
         end;
         if j>1 then begin
      ...
      FLAG_FERAGI,FLAG_BAROJA,FLAG_VOLKAN]) then begin
         if not Informed then CREATEINFOBOX;
         j:=j+1;
         s:=GETCIVNAME(FLAG_OTHER)+' spalten sich in zwei Teile und';
         if not Informed then CREATEINFOBOX;
         WRITE(255,100+j*18,12,16,MyScreen[1]^,4,s);
         j:=j+1;
         WRITE(255,100+j*18,12,16,MyScreen[1]^,4,'geraten in einen Bürgerkrieg!');
      end;
      ...
      and (GETCIVVAR(Save.WorldFlag) in [1..7]) then begin
         j:=j+1;
         s:=GETCIVNAME(FLAG_OTHER)+' beenden den Bürgerkrieg';
         if not Informed then CREATEINFOBOX;
         WRITE(255,100+j*18,12,16,MyScreen[1]^,4,s);
      end;
      ...
      if GetPlanetSys[ActPlayer]<>0 then begin
         j:=j+1;
         s:='Planet '+GetPlanet[ActPlayer]^.PName+' ';
         case ActPlayer of
            FLAG_TERRA:  s:=s+'ist in Föderation übergetreten';
            FLAG_KLEGAN: s:=s+'ist ins Reich übergetreten';
            FLAG_FERAGI: s:=s+'ist in die Allianz übergetreten';
            otherwise s:=s+'ist ins Imperium übergetreten';
         end;
         if not Informed then CREATEINFOBOX;
         WRITE(255,100+j*18,12,16,MyScreen[1]^,4,s);
      end;
      if Verschrottung[ActPlayer]>0 then begin
         j:=j+1;
         if Verschrottung[ActPlayer]>1 then s:=intstr(Verschrottung[ActPlayer])+' Schiffe wurden verschrottet'
         else s:='1 Schiff wurde verschrottet';
         if not Informed then CREATEINFOBOX;
         WRITE(255,100+j*18,12,16,MyScreen[1]^,4,s);
      ...
      if vNSonde[ActPlayer] then begin
         j:=j+1;
         s:=GETCIVNAME(ActPlayer)+' entdecken unbekannte Sonde';
         if not Informed then CREATEINFOBOX;
         WRITE(255,100+j*18,12,16,MyScreen[1]^,4,s);
   ...
   if (Save.CivPlayer[ActPlayer]<>0) and Informed then begin
      RECT(MyScreen[1]^,0,520,291,632,308);
      WRITE(521,292,12,1,MyScreen[1]^,4,'Fertig');
   end;
end;
...
begin
   SWITCHDISPLAY;
   s:='Lqqxt3Uz7D+=>.GE1IGL56789:;^ixY@XRSD^V\HYZ';
   INITMENU;
   ...
   WRITE(320,207,21,16,MyScreen[2]^,4,'Neues Spiel beginnen');
   WRITE(320,247,21,16,MyScreen[2]^,4,'Gesicherten Spielstand laden');
   ScreenToFront(MyScreen[2]);
   l:=0;
   repeat
      delay(RDELAY);
...
begin
   ...
   WRITE(255,80,ActPlayerFlag,16,MyScreen[1]^,4,'Militärische Ausbildung');
   WRITE(110,125,12,0,MyScreen[1]^,4,'IntensivTraining');
   WRITE(110,155,12,0,MyScreen[1]^,4,'ExtremTraining');
   WRITE(110,185,12,0,MyScreen[1]^,4,'Gehirnwäsche');
   WRITE(110,215,12,0,MyScreen[1]^,4,'Psychodrogen');
   if Save.TechCosts[ActPlayer,9]<=0 then s:='gentechnische Manipulation' else s:='---  (benötigt '+Technology[9]+')';
   WRITE(110,245,12,0,MyScreen[1]^,4,s);
   if Save.TechCosts[ActPlayer,23]<=0 then s:='bioelektronische Gehirnimplantate' else s:='--- (benötigt '+Technology[23]+')';
   WRITE(110,275,12,0,MyScreen[1]^,4,s);
   WRITE(70,310,ActPlayerFlag,0,MyScreen[1]^,4,'Imperatorstatus:');
   WRITE(70,335,ActPlayerFlag,0,MyScreen[1]^,4,'Kampferfahrung:');
   WRITE(70,360,ActPlayerFlag,0,MyScreen[1]^,4,'Kostenzuwachs:');
   DRAWDATA(0);
   repeat
      delay(RDELAY);
      if (LData^ and 64=0) then begin
...
begin
   ...
   WRITE(256,110,ActPlayerFlag,16,MyScreen[1]^,4,'Verteilung Finanzen');
   WRITE(80,147,12,0,MyScreen[1]^,4,'ins Guthaben');
   WRITE(80,192,ActPlayerFlag,0,MyScreen[1]^,4,'Einnahmen für Projekte');
   WRITE(80,237,12,0,MyScreen[1]^,4,'für Geheimdienst');
   ...
   then begin
      ...
      WRITE(80,316,FLAG_OTHER,0,MyScreen[1]^,4,'Sicherheitsabgabe');
      MAKEBORDER(MyScreen[1]^,291,322,370,345,12,6,1);
      WRITE(80,333,FLAG_OTHER,0,MyScreen[1]^,4,'an Dominion');
   end;
   WRITEDATA(0);
   repeat
      delay(RDELAY);
...
begin
   ...
   WRITE(175,60,12,32,MyScreen[1]^,3,'Status');
   WRITE(275,60,12,32,MyScreen[1]^,3,'Bevölkerung');
   WRITE(345,60,12,32,MyScreen[1]^,3,'Militär');
   WRITE(430,60,12,32,MyScreen[1]^,3,'Guthaben');
   WRITE(500,60,12,32,MyScreen[1]^,3,'Wertung');
   ...
   not (Save.WorldFlag in [0,WFLAG_FIELD])) then begin
      ...
      (Save.SSMoney[ActPlayer,i]>Save.WarPower[i]*39) or (Cheat) then begin
         ...
         or Cheat) and ((i<>j) or (GETCIVVAR(Save.WorldFlag)=i)) then begin
            ...
            then WRITE(j*73-35,267+i*18,GETCIVFLAG(i),21,MyScreen[1]^,3,'Krieg')
            else WRITE(j*73-35,267+i*18,GETCIVFLAG(i),16,MyScreen[1]^,3,s);
         end;
      end else WRITE(210,i*20+70,GETCIVFLAG(i),0,MyScreen[1]^,3,'keine Informationen');
...
begin
   ...
   if j<500 then begin
      ...
      WRITE(255,YPos[4]-15,12,16,MyScreen[1]^,1,'Projekte noch im Bau:');
      for i:=1 to Save.SYSTEMS do with SystemHeader[i] do if PlanetMemA>0 then begin
         for j:=1 to SystemHeader[i].Planets do begin
            MyPlanetHeader:=ptr(PlanetMemA+pred(j)*sizeof(r_PlanetHeader));
            if MyPlanetHeader^.PFlags and FLAG_CIV_MASK<>0 then with MyPlanetHeader^ do begin
...
begin
   ...
   Str9:=XStr9('Forschung:','Produktivität:','Biosphären:','Infrastruktur:',
               'Industrie:','Einwohner/Planet (Mio):','Planetengröße:',
               'ethnische Probleme:','Entwicklungsstufe:');
   ...
   WRITE(30,240,ActPlayerFlag,0,MyScreen[1]^,4,'Bevölkerung:');
   WRITE(60,260,ActPlayerFlag,0,MyScreen[1]^,4,'Zufriedenheit:');
   WRITE(60,280,ActPlayerFlag,0,MyScreen[1]^,4,'Stimmung:');
   ...
   if Planeten>0 then begin
      ...
      case round(l/31) of
         0: s:='äußerst gefährlich'
         1: s:='sehr gefährlich'
         2: s:='gefährlich'
         3: s:='sehr schlecht'
         4: s:='schlecht'
         5: s:='mäßig'
         6: s:='durchschnittlich'
         7: s:='gut'
         8: s:='sehr gut'
         otherwise s:='hervorragend';
      end;
      WRITE(280,280,ActPlayerFlag,0,MyScreen[1]^,4,s);

      l:=l div 3;
      if l<50 then s:='Urwald'
      else if l in [50..55] then s:='steinzeitlich'
      else if l in [56..60] then s:='Jäger und Sammler'
      else if l in [61..65] then s:='feudalistisch'
      else if l in [66..70] then s:='frühkapitalistisch'
      else if l in [71..75] then s:='frühindustriell'
      else if l in [76..80] then s:='durchschnittlich'
      else if l in [81..85] then s:='gut'
      else if l in [86..90] then s:='sehr gut'
      else if l>90 then s:='erstklassig';
      WRITE(260,210,12,0,MyScreen[1]^,4,s);
   end;
   s:='Militärausgaben: '+intstr(Militärausgaben[ActPlayer]);
   WRITE(30,310,12,0,MyScreen[1]^,4,s);
   s:='Entwicklungszeit: '+intstr(Save.TechCosts[ActPlayer,Save.ActTech[ActPlayer]] div succ(AllCreative[ActPlayer]))+' Jahre';
   WRITE(30,330,12,0,MyScreen[1]^,4,s);
   s:='Imperatorstatus: '+intstr(Save.ImperatorState[ActPlayer])+' Punkte';
...
begin
   ...
   WRITE(255,10,ActPlayerFlag,16,MyScreen[1]^,4,'Technologien');
   for i:=1 to 2 do for j:=1 to 21 do if Save.TechCosts[ActPlayer,pred(i)*21+j]<=0 then
    WRITE(pred(i)*245+10,j*14+15,12,0,MyScreen[1]^,3,Technology[pred(i)*21+j])
   else
    WRITE(pred(i)*245+10,j*14+15,29,0,MyScreen[1]^,3,Technology[pred(i)*21+j]);

   MAKEBORDER(MyScreen[1]^,0,331,510,400,12,6,0);
   if Save.ActTech[ActPlayer]>0 then begin
      s:=Technology[Save.ActTech[ActPlayer]]+', '+intstr(Save.TechCosts[ActPlayer,Save.ActTech[ActPlayer]] div succ(AllCreative[ActPlayer]))+' Jahre';
   ...
   end else WRITE(255,342,ActPlayerFlag,16,MyScreen[1]^,4,'keine Entwicklung');

   {ShipData:    MaxLoad,MaxShield,MaxMove,WeaponPower}
   MAKEBORDER(MyScreen[1]^,0,401,510,510,12,6,0);
   WRITE(10,410,12,0,MyScreen[1]^,3,'Schiffstyp');
   WRITE(110,410,12,0,MyScreen[1]^,3,'Laderaum');
   WRITE(210,410,12,0,MyScreen[1]^,3,'Bewegung');
   WRITE(310,410,12,0,MyScreen[1]^,3,'Schilde');
   WRITE(410,410,12,0,MyScreen[1]^,3,'Bewaffnung');
   l:=0;
...
begin
   ...
   WRITE(255,124,0,16,MyScreen[1]^,4,'Militär');
   WRITE(255,146,0,16,MyScreen[1]^,4,'Finanzen');
   WRITE(255,168,0,16,MyScreen[1]^,4,'Dossier');
   WRITE(255,190,0,16,MyScreen[1]^,4,'Projekte');
   WRITE(255,212,0,16,MyScreen[1]^,4,'Statistik');
   WRITE(255,234,0,16,MyScreen[1]^,4,'Forschung');
   WRITE(255,256,8,16,MyScreen[1]^,4,'Zu leicht!!');
   b:=false;
   repeat
      delay(RDELAY);
      if (LData^ and 64=0) then begin
