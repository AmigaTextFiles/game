 ...
 * $Id: servermain.d,v 1.10 90/07/09 00:32:50 DaveWT Exp $
 * $Log:	servermain.d,v $
Revision 1.10  90/07/09  00:32:50  DaveWT
Added the TEST option to the workbench interface, and added the new
NONEWS (-n) option. Users who specified a very short update time
(1 to 5 secs) reported that too many files were created during an attack.
It is also my opinion that if you are playing a game fast enough for
this to occur, you probobly don't need the news files anyway, as you
will be playing the game in one sitting.

Revision 1.9  90/06/23  00:26:43  DaveWT
Forgot to add the version number to the start up and shut down log
messages.

Revision 1.8  90/06/23  00:15:11  DaveWT
Added more "***" headers to errors that get logged to make them stand
out.

Revision 1.7  90/06/02  02:23:15  DaveWT
Now uses w_doFlush to decide if it should flush buffers when a client
terminates.

Revision 1.6  90/05/08  14:36:16  DaveWT
Added CG's -t command line flag to use "Empire test port" instead of
"Empire port" to allow testing of Empire while the regular Empire
game is running.
...
proc startChat(register *Client_t clNew)void:
    ...
    cl := Clients;
    while cl ~= nil do
        if cl ~= clNew and cl*.cl_chatting then
            ml := AllocMem(sizeof(MessageList_t), 0);
            if ml ~= nil then
...
proc readTelegram(register *Client_t cl; register *Request_t rq)void:
    ...
    if cl*.cl_fd = 0 and cou*.c_telegramsTail ~= 0 then
        ...
        incase TELE_DELETE:
            ...
            len := len + 1;
            fileName[len] := '\e';
            FileDestroy(&fileName[0]);
            ignore FileCreate(&fileName[0]);
        incase TELE_KEEP:
            cou*.c_telegramsNew := cou*.c_telegramsTail;
        default:
    ...
    while IdleClients ~= nil or BusyClients ~= nil or not quitRequested do
        ...
        do
            ...
            incase rt_createOffer:
                ...
                writeOffer(rq*.rq_whichUnit, &rq*.rq_u.ru_offer);
            default:
                logN("bad request type ", rq*.rq_type - rt_nop, " received");
            esac;
...
proc startServing()void:

    openFiles(&Path[0], &HelpDir, &DocDir);
    EmpirePort := FindPort(if DoDebug then "Empire test port" else
                               EMPIRE_PORT_NAME fi);
    if EmpirePort = nil then
        EmpirePort := CreatePort(if DoDebug then "Empire test port" else
                                    EMPIRE_PORT_NAME fi, 0);
        if EmpirePort = nil then
            abort(if DoDebug then "can't create test port" else
                       "can't create Empire port" fi);
        fi;
        if not readWorld(&World, &Country) then
            DeletePort(EmpirePort);
            abort("can't read world/countries");
        fi;
        IdleClients := nil;
        BusyClients := nil;
        log(if DoDebug then "Test server 2.2w started up." else
           "Empire server 2.2w started up." fi);
        handleRequests();
        DeletePort(EmpirePort);
        writeWorld(&World, &Country);
        log(if DoDebug then "Test server 2.2w shut down." else
           "Empire server 2.2w shut down." fi);
    else
        abort(if DoDebug then "Test port already exists" else
                   "Empire port already exists" fi);
    fi;
    closeFiles();
corp;
...
proc useage()void:
    ...
    mess("Useage is: EmpServ [-t] [-n]\n");
    mess("    -t = use test port\n");
    mess("    -n = don't generate news files\n");
corp;
                        ...
                        if wa*.wa_Lock ~= 0 then
                            ...
                            then
                                ...
                                if dob ~= nil then
                                    ...
                                    value :=
                                        FindToolType(dob*.do_ToolTypes,"TEST");
                                    if value ~= nil then
                                        DoDebug := true;
                                    fi;
                                    value :=
                                        FindToolType(dob*.do_ToolTypes,"NONEWS");
                                    if value ~= nil then
                                        DoNews := false;
                                    fi;
                                    FreeDiskObject(dob);
