 ...
 * $Id: FileIO.d,v 1.2 90/05/11 11:38:11 DaveWT Exp $
 * $Log:	FileIO.d,v $
Revision 1.2  90/05/11  11:38:11  DaveWT
Changed the default log to be "empire.log" instead of "ram:empire.log"
as per CG.
...
proc sectorFlush()void:
    ...
    if SectorFree ~= 0 then
        ...
        do
            ...
            if Write(SectorFd, &lowest*.sc_sector, sizeof(Sector_t)) ~=
                    sizeof(Sector_t)
            then
...
proc shipFlush()void:
    ...
    if ShipFree ~= 0 then
        ...
        do
            ...
            if Write(ShipFd, &lowest*.shc_ship, sizeof(Ship_t)) ~=
                    sizeof(Ship_t)
            then
...
proc getFileName(*char defName)void:
    ...
    if FileFd = 0 then
        ...
        if FileName[i] ~= '\n' then
            Close(FileFd);
            abort("Bad file name in name file");
        fi;
        FileName[i] := '\e';
    fi;
corp;
...
proc writeWorld(*World_t pWorld; *[COUNTRY_MAX] Country_t pCountry)void:
    ...
    if Seek(CountryFd, 0, OFFSET_BEGINNING) = -1 then
        abort("Can't rewrite countries");
    fi;
    if Write(CountryFd, pCountry, COUNTRY_MAX * sizeof(Country_t)) ~=
            COUNTRY_MAX * sizeof(Country_t)
    then
        abort("Can't write countries");
    fi;
    if Seek(WorldFd, 0, OFFSET_BEGINNING) = -1 then
        abort("Can't rewrite world file");
    fi;
    if Write(WorldFd, pWorld, sizeof(World_t)) ~= sizeof(World_t) then
...
proc readSector(uint rowCol; *Sector_t s)void:
    ...
    if sectorLookup(rowCol) then
        ...
        if Read(SectorFd, &SectorHead*.sc_sector, sizeof(Sector_t)) ~=
                sizeof(Sector_t)
        then
...
proc readShip(uint shipNumber; *Ship_t ship)void:
    ...
    if shipLookup(shipNumber) then
        ...
        if Read(ShipFd, &ShipHead*.shc_ship, sizeof(Ship_t)) ~= sizeof(Ship_t)
        then
...
proc readFleet(uint fleetNumber; *Fleet_t fleet)void:
    ...
    if Read(FleetFd, fleet, sizeof(Fleet_t)) ~= sizeof(Fleet_t) then
...
proc writeFleet(uint fleetNumber; *Fleet_t fleet)void:
    ...
    if Write(FleetFd, fleet, sizeof(Fleet_t)) ~= sizeof(Fleet_t) then
...
proc readLoan(uint loanNumber; *Loan_t loan)void:
    ...
    if Read(LoanFd, loan, sizeof(Loan_t)) ~= sizeof(Loan_t) then
...
proc writeLoan(uint loanNumber; *Loan_t loan)void:
    ...
    if Write(LoanFd, loan, sizeof(Loan_t)) ~= sizeof(Loan_t) then
...
proc readOffer(uint offerNumber; *Offer_t offer)void:
    ...
    if Read(OfferFd, offer, sizeof(Offer_t)) ~= sizeof(Offer_t) then
...
proc writeOffer(uint offerNumber; *Offer_t offer)void:
    ...
    if Write(OfferFd, offer, sizeof(Offer_t)) ~= sizeof(Offer_t) then
