...
[range(ShipType_t)] uint
    ...
    SHIP_SIZE = (6, 1, 25, 15, 20, 20, 20, 25),
    SHIP_RANGE = (40, 30, 60, 40, 30, 30, 30, 40),
    SHIP_SPEED = (480, 960, 960, 685, 1200, 1200, 800, 960),
    SHIP_SH_RANGE = (1, 2, 8, 3, 0, 1, 1, 2),
    SHIP_DAMAGE = (3, 2, 1, 2, 2, 2, 2, 1);

[range(ItemType_t)][range(ShipType_t)] uint SHIP_CAPACITY = (
    ( 0,  0,   0,  0, 127,  0,   0,  0),
    (10, 25, 127, 80,   0, 25, 100, 60),
    (10, 25, 127, 40, 127, 10, 127, 40),
    ( 1,  2,   4,  2, 127,  1,  30,  2),
    ( 0,  0,   0,  0,   0,  0,  0, 127),
    ( 0,  0,   0,  0, 127,  0,  0,   0),
    ( 0,  0,   0,  0, 127,  0,  0,   0)
...
bool Aborting;                          /* true -> do no more work */
...
proc initializeCountry(uint country)void:
    ...
    for i from 0 upto REALM_MAX - 1 do
        c*.c_realms[i].r_top := 0;
        c*.c_realms[i].r_bottom := 0;
        c*.c_realms[i].r_left := 0;
        c*.c_realms[i].r_right := 1;
    od;
    c*.c_telegramsNew := 0;
    c*.c_telegramsTail := 0;
...
proc abort(*char message)void:
    ...
    mess(" *** - aborting.\n");
    flush();
    Aborting := true;           /* set to true AFTER the 'flush' call */
corp;

/*
 * abort3 - print a 3 string message and start aborting.
 */

proc abort3(*char m1, m2, m3)void:
...
/*
 * getFileName - get the next file path from 'empire.files'. If it isn't
 *      open, use the passed path.
 */

proc getFileName(*char defName)void:
    register uint pos;

    if Aborting or NameFd = 0 then
        ...
        od;
        if FileName[pos] ~= '\n' then
            abort("bad file name from empire.files");
        fi;
        FileName[pos] := '\e';
    fi;
corp;
...
proc zapFile()void:
    ...
    if not Aborting then
        ...
        if fd = 0 then
            abort3("can't create ", &FileName[0], "");
        else
            Close(fd);
        fi;
    fi;
...
/*
 * newFile - delete and recreate the next file name in 'empire.files'.
 */
...
proc getLine()LineStatus_t:
    ...
    while
        if InputBuffPos = InputBuffMax then
            InputBuffPos := 0;
            len := Read(StdIn, &InputBuff[0], BUFF_SIZE);
            InputBuffMax := len;
            if len <= 0 then
                return(l_eof);
            fi;
...
proc writeFiles()void:
    ...
    /* basically, we start a new 'if not Aborting' block after each action
       that could cause 'Aborting' to be set. Make sure we close everything
       we open, however. */
            ...
            for i from 0 upto Count - 1 do
                for j from 0 upto Count - 1 do
                    if not Aborting then
                        ...
                        if Write(fd, &s, sizeof(Sector_t)) ~= sizeof(Sector_t)
                        then
    ...
    clearFile(LOGOUT_MESSAGE_FILE);
    clearFile(BULLETIN_FILE);
...
proc repNum(*char prompt; uint def, min, max)uint:
    ...
    if IsInteractive(StdIn) then
        ...
        incase rn_empty:
        incase rn_error:
            abort3("can't read value for ", prompt, "");
        incase rn_ok:
            if n < min then
                abort3("value for ", prompt, " too small");
            elif n > max then
                abort3("value for ", prompt, " too large");
            fi;
        esac;
    fi;
    n
...
proc ask(*char question)bool:
    ...
    if IsInteractive(StdIn) then
        ...
        incase l_eof:
        incase l_empty:
            abort3("couldn't read answer for >", question, "<");
        incase l_ok:
            ch := InputLine[InputLinePos];
            if ch ~= 'y' and ch ~= 'Y' and ch ~= 'n' and ch ~= 'N' then
                abort3("invalid answer for >", question, "<");
                ch := 'y';
            fi;
        esac;
    fi;
...
proc build()void:
    ...
    w*.w_weather.we_hiMax := 10;
    w*.w_weather.we_loMin := -10;
    ...
    if not Aborting then
        for i from Count - 1 downto 0 do
            ...
            if Cell[i] = nil then
                abort("can't allocate memory for 'Cell'");
            fi;
            Bits[i] := AllocMem(Count * sizeof(byte), 0);
            if Bits[i] = nil then
                abort("can't allocate memory for 'Bits'");
            fi;
            if not Aborting then
...
proc openWindow(*Process_t thisProcess)bool:
    ...
    *Window_t saveWindow;
    ...
    CharsConcat(&windowDesc[0], "/Amiga Empire (EmpCre) by Chris Gray");
