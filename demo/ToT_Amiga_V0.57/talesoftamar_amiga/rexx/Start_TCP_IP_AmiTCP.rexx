...
ADDRESS COMMAND
	...
	IF ~SHOW('P','AMITCP') THEN
	  DO
	    'run >NIL: AmiTCP:AmiTCP >NIL:'		/* Existiert TCPIP-Stack überhaupt dort ?*/
	    DO UNTIL SHOW('P','AMITCP')			/* Auf TCPIP-Stack warten warten	*/
	      SIGNAL OFF ERROR				/* Fehler-Meldung solange auschalten	*/
	      'WaitForPort AMITCP'			/* Isser nu da ?			*/
	      SIGNAL ON ERROR				/* Fehler-Meldungen wieder ein		*/
	    END						
	  END
	ENDIF
        tcpipport = AMITCP				/* ist auf jeden Fall der aktive Port	*/
