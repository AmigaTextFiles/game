...
void collision(struct anim_struct *,struct anim_struct *,
...

{
   ...
   if (bm3)
      copy_bitmap(vp.bitmap1,bm3);   /* set up spare bitmap for background restore */
   gs_draw_anims(dlist);
   check_bounds();
...

{
   ...
   for (cnt=0; cnt < RECTANGLES; cnt++)
      ...
      width=gs_random(swidth/2);
      height=gs_random(sheight/2);
      x=gs_random(swidth-width);
      y=gs_random(sheight-height);
   ...
   for (cnt=0; cnt < WORDS; cnt++)
      ...
      */
      Move(&rastport,gs_random(swidth-80),gs_random(sheight-16)+15); /* position drawing pen */
