...

{
  ...
  printf("\2331;37;40mCore Design Protected Format file ripper\2330;31;40m - by JOTD 1997\n");
  ...
  switch(argc)
   ...
   case 1:
   case 2:
	printf("Usage : ripcorefiles <unit number> <destination-dir> [<diskid>]\n");
	if (!argc) Delay(200);
	exit(0);
	 			...
	 			if (!dcrslt)
				...
				else
					{
						if (retries>NB_RETRIES)
						{
							switch(dcrslt)
							{	
							case 1: printf("** Decode error Track %d\n",i);/*CloseAll("Decode disk read error!");*/
									readok=1;
									trkptr+=TRACK_LEN;
									break;
