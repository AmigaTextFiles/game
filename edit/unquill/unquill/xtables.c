/* UnQuill: Disassemble games written with the "Quill" adventure game system
    Copyright (C) 1996-2000, 2013, 2016  John Elliott
...
uchr xnextbyte_uc(ushrt *n)
{
	return (255-zmem((*n)++));
}

uchr xnextbyte_dict(ushrt *n)
{
	return zmem( (*n)++) & 0x7F;
}

uchr (*xnextbyte)(ushrt *) = xnextbyte_uc;
...
void xexpch(uchr cch, ushrt *n)
{
	...
	switch (cch)
	{
		...
		case 0x17:
		fprintf (outfile,"<tab cols=\"%d\" />", xnextbyte(n) & 0x1F);
		...
		case 16:  
		fprintf (outfile,"<ink col=\"%d\" />", xnextbyte(n));
		break;

		case 17:  
		fprintf (outfile,"<paper col=\"%d\" />", xnextbyte(n));
		break;

	        case 18:
		fprintf (outfile,"<flash val=\"%d\" />", xnextbyte(n));
		break;

		case 19:
		fprintf (outfile,"<bright val=\"%d\" />", xnextbyte(n));
		break;

		case 20:
		fprintf (outfile,"<inverse val=\"%d\" />", xnextbyte(n));
		break;

		case 21:
		fprintf (outfile,"<over val=\"%d\" />", xnextbyte(n));
		break;

		case 6:
		n0 = *n;
		/* Two htabs in succession are a line break */
		if (xpos < 15 && xnextbyte(&n0) == 6)
		{
			fprintf (outfile,"<br />\n");
			xpos = 0;
			++(*n);
		...
		/* Block graphics */
		else if (cch > 127 && cch < 144) 
		{	
			fprintf(outfile, "&#x%04x;", zx_entity[cch-128]);
			++xpos;
		}
		/* UDGs go to the private use area */
		else if (cch > 143 && cch < 165) 
		{
			fprintf(outfile, "&#x%04x;", cch - 144 + 0xE000);
			++xpos;
		}
		else if (cch > 164) xexpdict (cch);
		else 
		{
			fprintf(outfile, "&#x%04x;", cch);
			++xpos;
		}
		if (xpos >= 32)
		{
