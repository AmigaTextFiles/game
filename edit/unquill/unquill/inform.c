/* UnQuill: Disassemble games written with the "Quill" adventure game system
    Copyright (C) 1996-2020  John Elliott
...
static char wordchar(char c)
{
	if (arch == ARCH_C64) return (0xFF - c) & 0x7F;
	return 0xFF - c;
}
...
static char *lookup_word(ushrt wid)
{
	...
	{
		for (m = 0; m < 4; m++) w[m] = wordchar(zmem(m+n));
	}
	return w;
}
