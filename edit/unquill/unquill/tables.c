/* UnQuill: Disassemble games written with the "Quill" adventure game system
    Copyright (C) 1996-2000, 2016 John Elliott
...
void expdict(uchr cch)
{
	ushrt d=dict;
	xnextbyte = xnextbyte_dict;
 	...
 	if (dbver > 0) /* Early games aren't compressed & have no dictionary */
	{
      ...
      /* d=address of expansion text */
		while (1)
		{
			expch (zmem(d++) & 0x7F, &d);
			--d;
			if (zmem(d) & 0x80) break;
			++d;
		}
	}
	xnextbyte = xnextbyte_uc;
}
...
void expch(uchr cch, ushrt *n)
{
	...
	switch (oopt)
	{
		...
		else if ((cch > 0x0F) && (cch<0x16))
		{
			switch (cch)
			{
				case 0x10: (console->ink)(xnextbyte(n)); break;
				case 0x11: (console->paper)(xnextbyte(n)); break;
				case 0x12: (console->flash)(xnextbyte(n)); break;
				case 0x13: (console->bright)(xnextbyte(n)); break;
				case 0x14: (console->inverse)(xnextbyte(n)); break;
				case 0x15: (console->over)(xnextbyte(n)); break;
			}
		}
		break;
		...
		switch (cch)
		{
			...
			param = xnextbyte(n);
			xpos += param;
			fprintf (outfile,"<TAB%d>", param & 0x1F);
			xnextbyte(n);	/* Skip high byte of tab control word */
			break;
			...
			case 16:  
			fprintf (outfile,"<INK=%d>",xnextbyte(n));
			break;

			case 17:  
			fprintf (outfile,"<PAPER=%d>",xnextbyte(n));
			break;

		        case 18:
			if (xnextbyte(n) == 0x00)  fprintf (outfile,"</BLINK>");
			else fprintf (outfile,"<BLINK>");
			break;

			case 19:
			if (xnextbyte(n) == 0x00) fprintf (outfile,"</BRIGHT>");
			else fprintf (outfile,"<BRIGHT>");
			break;

			case 20:
			if (xnextbyte(n) == 0x00) fprintf (outfile,"</INVERSE>");
			else fprintf (outfile,"<INVERSE>");
			break;

			case 21:
			if (xnextbyte(n) == 0x00) fprintf (outfile,"</OVER>");
...
void listgfx(void)
{
	...
	/* Check that the location graphics pointers are all valid
	 * before attempting to follow them */
	for (n = 0; n < gfx_count; n++)
	{
		if (!address_valid(gfx_flags + n)  ||
		    !address_valid(gfx_ptrs + 2*n) ||
		    !address_valid(zword(gfx_ptrs + 2*n)))
		{
			return;
		}
	}
