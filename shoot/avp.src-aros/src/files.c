...
static char *FixFilename(const char *filename, const char *prefix, int force)
{
	...
	//printf("fixfilename(%s, %s, %d)\n", filename, prefix, force);
	...
	strcpy(f, prefix);
#ifdef __AROS__
	if ((prefix[plen - 2] != '/') && (prefix[plen - 2] != ':')) // don't add another dir separator
#endif
...
void *OpenGameDirectory(const char *dirname, const char *pattern, int type)
{
	...
	//printf("OpenGameDirectory(%s, %s, %d)\n",dirname, pattern, type);
...
void InitGameDirectories(char *argv0)
{
	...
	*/
	...
	if (!check_game_directory(gamedir)) {
#ifdef __AROS__
		AROS_ErrorMessage("Unable to find AvP gamedata. Make sure the game is correctly installed.");
#else
