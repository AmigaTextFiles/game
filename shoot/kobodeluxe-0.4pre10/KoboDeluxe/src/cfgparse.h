...
#ifdef __MORPHOS__
	// laccept for local accept -> accept seems to be an existing macro in MOS
	void laccept(int &var)			{ _accept(&var); }
	void laccept(float &var)			{ _accept(&var); }
	void laccept(cfg_string_t &var)		{ _accept(&var); }
#else
