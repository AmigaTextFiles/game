...
//FIXME: Temporary hack. If we're using IDAL, we should
//FIXME: not mess with the display surface directly.
	screen_surface = SDL_GetVideoSurface();
	if(!screen_surface)
	{
		fprintf(stderr, "No video surface!\n");
		return -3;
	}
	...
	switch(buffermode)
	{
	  ...
	  case GFX_BUFFER_SEMITRIPLE:
		...
		{
		  case GFX_DRIVER_SDL2D:
			if(!(screen_surface->flags & SDL_SWSURFACE))
			{
				buffermode = GFX_BUFFER_SINGLE;
				printf("WARNING: Failed to get software screen surface."
	...
	fullwin->init(this);
	fullwin->place(0, 0, (_width + xsc - 1) / xsc,
			(_height + ysc - 1) / ysc);
	  ...
	  case GFX_BUFFER_SEMITRIPLE:
		for(i = 0; i < dirtyrects[backpage]; ++i)
			SDL_BlitSurface(softbuf, &dirtytable[backpage][i],
					screen_surface, &dirtytable[backpage][i]);
