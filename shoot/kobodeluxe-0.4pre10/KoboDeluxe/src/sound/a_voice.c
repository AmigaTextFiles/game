...
static inline unsigned int __endframes(audio_voice_t *v, unsigned int frames)
{
	...
	{
		voice_kill(v);
		printf("INTERNAL ERROR: Voice locked up! (Too high pitch "
...
void voice_process_mix(audio_voice_t *v, int *busses[], unsigned frames)
{
	...
	while(frames)
	{
		...
		while( !(frag_frames = aev_next(&v->port, s)) )
		{
			...
			{
			  case VE_START:
				EVDBG(printf("\tVE_START %d", ev->arg1);)
			  ...
			  case VE_SET:
				...
				{
					printf("BUG! VC index out of range!");
					break;
				}
#endif
				...
				break;
			  case VE_IRAMP:
				EVDBG(printf("\tVE_IRAMP");)
#ifdef	CHECKPOINTS
				if(ev->index >= VIC_COUNT)
				{
					printf("BUG! VIC index out of range!");
					break;
				}
#endif
				if(ev->arg2)
		...
		while(frag_s < frag_frames)
		{
			...
			if(do_frames)
			{
				...
				{
					printf("BUG! position = %u while section_end = %u.",
							v->position, v->section_end);
					printf(" (step = %u)\n", v->step >> FREQ_BITS);
					v->position = 0;
				}
