...
int midi_open(midisock_t *output)
{
	...
	{
		fprintf(stderr, "ERROR: Failed to open MIDI input device!\n");
		return -1;
	}
# ifdef MIDI_THRU
	if(snd_rawmidi_open(&m_out_fd, 0, 0, SND_RAWMIDI_OPEN_OUTPUT))
	{
		fprintf(stderr, "ERROR: Failed to open MIDI output device!\n");
		return -2;
	}
# endif
	return 0;
...
#endif /*HAVE_OSS*/
#else /*USE_MIDI*/
	fprintf(stderr, "ERROR: MIDI support not compiled in!\n");
	return -3;
#endif /*USE_MIDI*/
}
