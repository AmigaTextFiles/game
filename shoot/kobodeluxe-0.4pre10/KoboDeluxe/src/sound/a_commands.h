...
typedef struct
{
	enum
	{
		...
		eCMD_STOP_ALL,
		eCMD_PLAY,
		eCMD_CCONTROL,	/* Channel Control */
		eCMD_GCONTROL,	/* Group Control */
		eCMD_MCONTROL,	/* Mixer Control */
		eCMD_WAIT	/* Advance Audio Time */
	} action;
	signed char	cid;
	unsigned char	index;
	int		tag;
...
/*
 * Advance audio time for subsequent calls by 'ms' milliseconds.
 */
...
/*
 * Like sound_play(), but with velocity control.
 * 'energy' scales the pitch and velocity. 65536
 * is "normal", whereas 0 means silent and 7
 * semitones lower than the default pitch.
 */
void sound_play_e(int wid, int x, int y, int energy);

/*
 * Play a sound at world coordinates (x, y), using
 * default pitch (60<<16) and velocity (65536).
 */
static inline void sound_play(int wid, int x, int y)
{
	sound_play_e(wid, x, y, 65536);
}
