/*
------------------------------------------------------------
	filemap.C - Simple Portable File Path Mapper
------------------------------------------------------------
 * © David Olofson, 2001
...
#warning This platform lacks stat(). File/directory probing may be unreliable!
	...
	DBG2(printf("probe(\"%s\") (with fopen())\n", syspath);)
	if(f)
	{
		::fclose(f);
		return FM_FILE;
...
int filemapper_t::test_file_create(const char *syspath)
{
	...
	if(f)
	{
		...
		if(exists)
		{
			DBG2(printf("  File is writable!\n");)
			return FM_FILE;
		}
		else
		{
			DBG2(printf("  File created!\n");)
...
int filemapper_t::test_file_dir_any(const char *syspath, int kind)
{
	...
	switch (res)
	{
	  ...
	  case FM_DIR:
		...
		{
			DBG2(printf("  Found! (Dir)\n");)
			return 1;
		}
		break;
	  case FM_ANY:
		fprintf(stderr, "INTERNAL ERROR: filemapper_t::probe()"
				" returned FM_ANY...\n");
		break;
	}
	if(res != FM_ERROR)
		...
		{
		  case FM_FILE:
			DBG2(printf("  Not a file.\n");)
			break;
		  case FM_DIR:
			DBG2(printf("  Not a directory.\n");)
			break;
		  default:
			break;
		}
		...
		int level, int build)
{
	...
	if(!obj)
	{
		...
		buffer[FM_BUFFER_SIZE-1] = 0;
		DBG2({for(int s=0;s<level;++s)printf("  ");})
		DBG2(printf("('%s') (no key!)\n", buffer);)
		DBG2(printf("Trying '%s'...", ref));
		sys_slashes(buffer);
	...
	while( (k = getkey(k, kclass)) )
	{
		...
		if(dt)
		{
			DBG2(printf("Reparsing '%s'", k->path));
	...
	DBG2(	if(!hits)
		{
			for(int s=0;s<level;++s)printf("  ");
			printf("No paths registered for '%s'!\n", kclass);
		}
	);
	return 0;
}
...
const char *filemapper_t::get(const char *ref, int kind)
{
	DBG2(
		if(kind == FM_FILE_CREATE)
			printf("Trying to create '%s'...", ref);
		else if(kind == FM_DIR)
			printf("Looking for '%s'...", ref);
		else
			printf("Looking for '%s'...", ref);
	);
...
void filemapper_t::get_all(const char *ref, int kind)
{
	...
	if(kind == FM_FILE_CREATE)
	{
		fprintf(stderr, "filemapper_t: Tried to create files"
				" in get_all()!\n");
		return;
	}
	DBG2(
	else if(kind == FM_DIR)
		printf("get_all('%s', DIR)...", ref);
	else
		printf("get_all('%s')...", ref);
	)
...
const char *filemapper_t::get_next()
{
	while(1)
	{
		if(!current_dir)
		{
			...
			switch(probe(current_obj->path))
			{
			  case FM_FILE:
			  {
				...
				DBG2(printf("get_next() found file '%s'.\n", res);)
				return res;
			  }

			  case FM_DIR:
				current_dir = ::opendir(current_obj->path);
				DBG2(printf("get_next() found dir '%s'.\n",
						current_obj->path);)
				break;

			  default:
				DBG2(printf("get_next(): Object '%s' skipped!\n",
						current_obj->path);)
				break;		//Error. Skip this object.
			}
		}
		if(!current_dir)
			continue;
...
#endif
			{
				...
				switch(probe(path))
				{
				  ...
				  case FM_DIR:
					...
					break;
				  default:
					DBG2(printf("get_next() found crap in dir.\n");)
					break;
				}
			}
		}
		else
		{
			...
			current_dir = NULL;
			current_obj = current_obj->next;
			DBG2(printf("get_next(): end-of-dir\n");)
		}
	}
}
...
{
	DBG2(fprintf(stderr, "filemapper_t::fopen()"));
...
{
	DBG2(fprintf(stderr, "filemapper_t::opendir()"));
...
{
	fprintf(stderr, "filemapper_t::mkdir() not yet implemented!\n");
	return -1;
#if 0
