...
int bio_read(bio_file_t *bio, unsigned char *buf, int count)
{
...
int eel_error(const char *format, ...)
{
	...
	{
		count = fprintf(stderr, "(EEL) in file \"%s\":\n",
				eel_scripttab[eel_current.script].name);
		last_script = eel_current.script;
	}
	if(eel_current.arg)
		count += fprintf(stderr, "(EEL) line %d, arg %d: ",
				line, eel_current.arg);
	else
		count += fprintf(stderr, "(EEL) line %d: ", line);
