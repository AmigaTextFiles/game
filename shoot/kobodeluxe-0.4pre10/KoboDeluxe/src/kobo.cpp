...
ULONG __stack = 65536;
const char *version_tag = "$VER: KoboDeluxe 0.4pre10 (26.09.2005)";
#endif
...
// Frame rate limiter
static float max_fps_filter = 0.0;
static int max_fps_begin = 0;
...
void progress(int percent, const char *msg)
{
	int buffers;
	int x, y, w, h;
	...
	while(buffers--)
	{
		...
		if(msg)
		{
			...
			r.h = 12;
			wbase.foreground(wbase.map_rgb(0x000000));
			wbase.fillrect(r.x, r.y, r.w, r.h);
			wbase.font(B_NORMAL_FONT);
			wbase.center(wbase.height() - 40, msg);
			wbase.invalidate(&r);
		}
...
#define	PROGRESS(x, y)	progress(x, y); if(quit_requested()) return -999;
...
int load_graphics(prefs_t *p)
{
	...
	PROGRESS(25, "Loading fonts & framework graphics");
	...
	gengine.scalemode(GFX_SCALE_NEAREST);
	fn = fmap.get("GFX>>noise.pcx");
	if(!fn || gengine.loadtiles(B_NOISE, NOISE_SIZEX, NOISE_SIZEY, fn) < 0)
		return -1;
	gengine.scalemode((gfx_scalemodes_t) p->scalemode);
	PROGRESS(90, NULL);
...
//	int i;
	...
	if(!sounds_loaded)
	{
		PROGRESS(5, "Loading sound effects");
	...
	PROGRESS(80, "Preparing audio engine");
...
skobo_gfxengine_t::skobo_gfxengine_t()
{
	time = 0;
}
...
void skobo_gfxengine_t::post_render()
{
	...
	if(prefs.max_fps)
	{
		int rtime = nt - max_fps_begin;
		max_fps_begin = nt;
		max_fps_filter += (float)(rtime - max_fps_filter) * 0.3;
		int delay = (int)(1000.0 / prefs.max_fps -
				max_fps_filter + 0.5);
		if((delay > 0) && (delay < 1100))
			SDL_Delay(delay);
		max_fps_begin = (int)SDL_GetTicks();
	}
}
