...
#if !defined(WIN32) && !defined(__amigaos4__) && !defined(__BEOS__) && !(defined(mac) /*&& !defined(TARGET_API_MAC_CARBON)*/)
...
#if !defined(WIN32) && !defined(__BEOS__) && !defined(__amigaos4__) && !(defined(mac) /*&& !defined(TARGET_API_MAC_CARBON)*/)
...
#if defined(__BEOS__) || defined(__MWERKS__) || defined(__amigaos4__)
...
SDLNetxint_CollectBroadcastAddresses(UDPsocket inSocket) {
    ...
    int theResult = ioctl(theSocketFD, SIOCGIFCONF, (char*) &theConfigRequest);
    ...
    while(theConfigRequest.ifc_len - theOffset >= int(sizeof(struct ifreq))) {
        ...
        if(theReq->ifr_addr.sa_family == AF_INET) {
            ...
            // broadcast address.
            theResult = ioctl(theSocketFD, SIOCGIFBRDADDR, (char*) theReq);
