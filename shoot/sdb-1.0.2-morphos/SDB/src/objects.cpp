...
void Shockwave::update()
{
  if (active)
  {
    innerRad += innerRadSpeed * l_timer.dT();
    outerRad += outerRadSpeed * l_timer.dT();
    
    shockTime -= l_timer.dT();
  ...
  {        
    ...
    for (int i = 0; i < numWalls + 4*currLevel.grid[cell].door.size(); i++)
    {
      ...
      (currWall->CollFlags() & COLLIDE_BULLET && IS_BULLET(type)))
      {
        ...
        Vector2D offset = vel;           
        offset.normalize();
        offset *= objType[type].boundRadius+(vel.mag()*l_timer.dT());
              ...
              objType[obj->Type()].boundRadius))
          {
            ...
            {                
              ...
              if (fabs(v1i) > fabs(v2i))
                pos += Vector2D(cos(a1), sin(a1)) * ((vel*l_timer.dT()).mag());
              else
                obj->addPos(Vector2D(cos(a2), sin(a2)) * ((obj->Vel()*l_timer.dT()).mag()));
  ...
  if (currLevel.inGrid(pos))
  {
    ...
    {       
      ...
      if (vel.mag() < WALL_COLLISION_THRESHOLD)
        pos += closestHitWall->normal * ((vel*l_timer.dT()).mag()+0.1);
      else
      {
        pos = impactPoint;
  ...
  {    
    ...
    if (objType[type].maxHealth != INF)
    {
      if (type == ENT_PLAYER && config.cheat_invul && currSurfaceHealth < 0) {}
        ...
        changeHealth(currSurfaceHealth * l_timer.dT());
    }
...
void Particle::update()
{
  ...
  if (active)
  {
    ...
    speed += partType[pType].accel * l_timer.dT();
    ...
    if (partType[pType].startFade == 0 || health <= partType[pType].startFade)
      alpha -= fadeSpeed * l_timer.dT();
     
    if (alpha <= 0)
      health = 0;
      
    rotation += rotVel * l_timer.dT();
    
    if (partType[pType].scaleStop == 0 ||
      (partType[pType].scaleStop > 0 && scale <= partType[pType].scaleStop))
        scale += partType[pType].scaleSpeed * l_timer.dT();
...
void Laser::update()
{
  if (active)
  {
    ...
    alpha -= lasType[type].fadeSpeed * l_timer.dT();
...
void initObjectTypes()
{
  for (int i = 0; i < NUM_OBJ_TYPES; i++)
    switch(i)
    {
      // float im, float iw, float ih, float ir, float imv, float ijv, float imtv, float ia, float ita, float hlth, bool grav, bool shad)
      case ENT_PLAYER:
        ...
        break;
      case ENT_BOX:
        objType[i].set(140, 3.0, 3.0, 2.0,  0, 0, 0, 0, 0, 25, DEATH_WOOD, true, true, true);
        objType[i].setModels(MDL_BOX, NO_MODEL);
        break;
      case ENT_BARREL:
        objType[i].set(180, 2.5, 2.5, 1.25,  0, 0, 0, 0, 0, 35, DEATH_EXPLOSION, true, true, true);
      ...
      case ENT_UNARMED_GUARD:
        objType[i].set(100, 3.0, 3.0, 1.5,  15, 0, torad(150), 25, torad(5000), 50, DEATH_SMALL_EXPLOSION, true, true, true);
        objType[i].setModels(MDL_UNARMED_GUARD, NO_MODEL);
        break;
      case ENT_MIB:
        objType[i].set(100, 3.0, 3.0, 1.5,  14, 0, torad(150), 25, torad(5000), 50, DEATH_NONE, true, true, true);
      ...
      case ENT_TURRET1: case ENT_SLAVE_TURRET:
        ...
        break;
      case ENT_ARMED_DRONE:
        objType[i].set(200, 4.0, 4.0, 1.25, 13, 0, torad(200), 25, torad(4000), 75, DEATH_ROBOT, true, true, true);
        objType[i].setModels(MDL_ARMED_DRONE, NO_MODEL);
        break;
      case ENT_HUNTER:
        objType[i].set(450, 4.0, 4.0, 1.25, 14, 0, torad(200), 25, torad(4000), 100, DEATH_ROBOT, true, true, true);
        objType[i].setModels(MDL_HUNTER, NO_MODEL);
        break;
      case ENT_GLADIATOR:
        objType[i].set(700, 4.0, 4.0, 1.5, 8, 0, torad(200), 25, torad(4000), 250, DEATH_GLADIATOR, true, true, true);
