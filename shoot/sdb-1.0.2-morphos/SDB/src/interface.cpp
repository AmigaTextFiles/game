...
void TerminalWindow::addString(int style, char *fmt, ...)
{
  va_list ap;
  va_start(ap, fmt);
  char buf[513];
...
void TerminalWindow::changeString(int line, int style, char *fmt, ...)
{
  va_list ap;
  va_start(ap, fmt);
  char buf[513];
...
int setMenu(TerminalWindow *m, int type, Uint8 *keystate)
{
  ...
  switch(type)
  {
    ...
    case TITLE_HELP:
      ...
      m->addString(0, "  press enter to return to menu");
      m->activate();
      break;
    case TITLE_CREDITS:
      ...
      m->addString(0, "");
      m->addString(0, "Version %s; %s", GAME_VERSION, GAME_DATE);
...
int title(bool first, bool playInSequence)
{
  ...
  while (!retVal)
  {
    ...
    scroll1 += SCROLL1_RATE * l_timer.dT();
    scroll2 += SCROLL2_RATE * l_timer.dT();
    ...
    if (screen != TITLE_CREDITS && screen != TITLE_RECORDS && screen != TITLE_HELP)
    {
      ...
      font.printf(-80.0, -40, 2.5, 2.5,    "A project of the Game Creation Society - sdb.gamecreation.org");
      font.printf(-80.0, LAST_Y, 2.5, 2.5, "Ver. %s", GAME_VERSION);
    }
    
    if (screen == TITLE_NOMENU)
      font.printf(CENTERED, LAST_Y, 2.5, 2.5, "Press Enter");
    ...
    if (keystate[SDLK_RETURN])
    {
      if (!enterPressed)
      {
        if (!textDone)
        {
          ...
          window.addString(0, "");
          window.addString(1, "press enter");
          window.flush();
          textDone = true;
...
void levelWinScreen(bool playInSequence)
{
  ...
  if (P.levelRecords[l].time < 0.0 || config.cheating() || !playInSequence || (P.augmented && !currLevel.playerAugmented))
  {
    ...
    stats.addString(0, "");
    stats.addString(0, "  press enter to continue");
  }
  else
  {
...
{
  glColor3f(1.0, 1.0, 1.0);
  font.printf(-98.0, SCREEN_TOP-3.0, 4.0, 3.2, "FPS       %03d", l_timer.FPS());
  font.printf(-98.0, LAST_Y, 4.0, 3.2, "\x7FT        %0.5f", l_timer.dT());
...
void errorMessage(int retVal, char *heading, char *msg, ...)
{
  va_list ap;
  va_start(ap, msg);
...
{ 
  ...
  int resolutions[] = {640,480, 800,600, 1024,768, 1152,864, 1280,1024,
  1400,1050, 1600,1200, 2048,1536,
  -1280,768, -1280,800, -1280,854, -1440,900, -1680,1050, -1920,1200};
  ...
  while (!done)
  {
    ...
    if (alertTimer > 0.0)
    {
      alertTimer -= l_timer.dT();
      if (alertTimer <= 0.0)
        alertTimer = 0.0;
    }
    if (keyAlertTimer > 0.0)
    {
      keyAlertTimer -= l_timer.dT();
      if (keyAlertTimer <= 0.0)
        keyAlertTimer = 0.0;
    }
...
void finalCredits()
{
  ...
  lines.push_back("http://www.contrib.andrew.cmu.edu/~msarnoff");
  lines.push_back("");
  lines.push_back(GAME_DATE);
  ...
  while (loop)
  {
    ...
    font.printf(CENTERED, LAST_Y, 2.0, 1.6, "press enter");
...
void takeScreenshot()
{
  ...
  if (screenPixels)
  {
    ...
    char filename[256];
#ifdef __MORPHOS__
    snprintf(filename, 255, "%sshot%04d.bmp", DATA_DIRECTORY, screenshotCount);
#else
    snprintf(filename, 255, "%s%sshot%04d.bmp", getenv("HOME"), DATA_DIRECTORY, screenshotCount);
#endif
