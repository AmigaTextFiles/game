...
*/
/*
 * $Source: /usr/CVS/descent/3d/interp.c,v $
 * $Revision: 1.2 $
 * $Author: hfrieden $
 * $Date: 1998/04/09 17:08:55 $
 * 
 * Polygon object interpreter
 * 
 * $Log: interp.c,v $
 * Revision 1.2  1998/04/09 17:08:55  hfrieden
 * Experimential routine for ViRGE briefings
 *
...
static char rcsid[] = "$Id: interp.c,v 1.2 1998/04/09 17:08:55 hfrieden Exp $";
...
#ifdef VIRGIN
bool g3_draw_polygon_model_offset(void *model_ptr,grs_bitmap **model_bitmaps,vms_angvec *anim_angles,fix model_light,fix *glow_values, fix offsetx, fix offsety)
{
	ubyte *p = model_ptr;
	int current_poly = 0;
			...
			case OP_FLATPOLY: {
				...
				if (g3_check_normal_facing(vp(p+4),vp(p+16)) > 0) {
					...
					for (i=0;i<nv;i++)
						point_list[i] = Interp_point_list + wp(p+30)[i];
					for (i=0; i<nv; i++) {
						g3_project_point(point_list[i]);
						point_list[i]->p3_sx += offsetx;
						point_list[i]->p3_sy += offsety;
					}
					g3_draw_poly(nv,point_list);
					for (i=0; i<nv; i++) {
						point_list[i]->p3_sx -= offsetx;
