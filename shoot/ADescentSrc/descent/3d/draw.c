...
*/
/*
 * $Source: /usr/CVS/descent/3d/draw.c,v $
 * $Revision: 1.4 $
 * $Author: nobody $
 * $Date: 1998/12/21 17:12:15 $
 * 
 * Drawing routines
 * 
 * $Log: draw.c,v $
 * Revision 1.4  1998/12/21 17:12:15  nobody
 * *** empty log message ***
 *
 * Revision 1.3  1998/03/30 18:30:45  hfrieden
 * Added ViRGE disc drawing stuff for automapper
 *
 * Revision 1.2  1998/03/22 15:53:49  hfrieden
 * ViRGE direct drawing stuff added
 *
 * 
 */

#pragma off (unreferenced)
static char rcsid[] = "$Id: draw.c,v 1.4 1998/12/21 17:12:15 nobody Exp $";
...
#ifdef POLY_STAT
extern int g3_stat_poly;
extern int g3_stat_tmap;
#endif
...
#ifdef VIRGIN
bool direct_must_clip_line(g3s_point *p0,g3s_point *p1,ubyte codes_or)
{
	bool ret;
...
bool g3_direct_draw_line(g3s_point *p0,g3s_point *p1)
{
	ubyte codes_or;
	...
	if (codes_or & CC_BEHIND)
		return direct_must_clip_line(p0,p1,codes_or);
	...
	gr_direct_line(p0->p3_sx, p0->p3_sy, p1->p3_sx, p1->p3_sy);
	return (bool)0;
}
#endif
...
#ifdef VIRGIN
int g3_direct_draw_sphere(g3s_point *pnt,fix rad)
{
	if (! (pnt->p3_codes & CC_BEHIND)) {
		...
		if (! (pnt->p3_codes & PF_OVERFLOW)) {
			...
			r2 = fixmul(rad,Matrix_scale.x);
			if (checkmuldiv(&t,r2,Canv_w2,pnt->p3_z))
				return gr_direct_disk(pnt->p3_sx,pnt->p3_sy,t);
		}
	}
