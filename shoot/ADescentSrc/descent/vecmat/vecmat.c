...
*/
/*
 * $Source: /usr/CVS/descent/vecmat/vecmat.c,v $
 * $Revision: 1.4 $
 * $Author: tfrieden $
 * $Date: 1998/04/01 21:20:51 $
 * 
 * C version of vecmat library
 * 
 * $Log: vecmat.c,v $
 * Revision 1.4  1998/04/01 21:20:51  tfrieden
 * Added two more function pointers
 *
 * Revision 1.2  1998/03/25 22:11:48  tfrieden
 * Now uses inlined functions
 *
 * Revision 1.1.1.1  1998/03/03 15:12:46  nobody
...
static char rcsid[] = "$Id: vecmat.c,v 1.4 1998/04/01 21:20:51 tfrieden Exp $";
...
// Function pointers for vecmat assembler version
// vm_vec_dotprod
fix (*vm_vec_dotprod_func)(vms_vector *v0 __asm("a0"),vms_vector *v1 __asm("a1"));
extern fix vm_vec_dotprod_fpu(vms_vector *v0 __asm("a0"),vms_vector *v1 __asm("a1"));
extern fix vm_vec_dotprod_int(vms_vector *v0 __asm("a0"),vms_vector *v1 __asm("a1"));
// vm_vec_dot3 (only internally used)
#define vm_vec_dot3(a,b,c,d) (*vm_vec_dot3_func)((a),(b),(c),(d))
fix (*vm_vec_dot3_func)(fix x __asm("d2"),fix y __asm("d3"),fix z __asm("d4"),vms_vector *v __asm("a0"));
extern fix vm_vec_dot3_fpu(fix x __asm("d2"),fix y __asm("d3"),fix z __asm("d4"),vms_vector *v __asm("a0"));
extern fix vm_vec_dot3_int(fix x __asm("d2"),fix y __asm("d3"),fix z __asm("d4"),vms_vector *v __asm("a0"));
// vm_vec_mag
fix (*vm_vec_mag_func)(vms_vector *v __asm("a0"));
extern fix vm_vec_mag_fpu(vms_vector *v __asm("a0"));
extern fix vm_vec_mag_int(vms_vector *v __asm("a0"));
fix vm_vec_mag_old(vms_vector *v __asm("a0"));
// vm_vec_crossprod
vms_vector *(*vm_vec_crossprod_func)(vms_vector *dest __asm("a0"),vms_vector *src0 __asm("a1"),vms_vector *src1 __asm("a2"));
extern vms_vector *vm_vec_crossprod_fpu(vms_vector *dest __asm("a0"),vms_vector *src0 __asm("a1"),vms_vector *src1 __asm("a2"));
extern vms_vector *vm_vec_crossprod_int(vms_vector *dest __asm("a0"),vms_vector *src0 __asm("a1"),vms_vector *src1 __asm("a2"));
vms_vector *vm_vec_crossprod_old(vms_vector *dest __asm("a0"),vms_vector *src0 __asm("a1"),vms_vector *src1 __asm("a2"));



void vecmat_init(void) {

	vm_vec_dotprod_func = vm_vec_dotprod_int;
	vm_vec_dot3_func = vm_vec_dot3_int;
	vm_vec_mag_func = vm_vec_mag_old;
	vm_vec_crossprod_func = vm_vec_crossprod_old;

	if (FindArg("-fpu")) {
		vm_vec_dotprod_func = vm_vec_dotprod_fpu;
		vm_vec_dot3_func = vm_vec_dot3_fpu;
		vm_vec_mag_func = vm_vec_mag_fpu;
		vm_vec_crossprod_func = vm_vec_crossprod_fpu;
	}
}
...
vms_vector *vm_vec_avg_old(vms_vector *dest,vms_vector *src0,vms_vector *src1)
{
...
fix vm_vec_dotprod_old(vms_vector *v0,vms_vector *v1)
{
	quad q;
...
fix vm_vec_dot3_old(fix x,fix y,fix z,vms_vector *v)
{
	quad q;
