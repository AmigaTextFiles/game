...
*/
/*
 * $Source: /usr/CVS/descent/main/modem.c,v $
 * $Revision: 1.6 $
 * $Author: nobody $
 * $Date: 1998/04/25 08:40:01 $
 * 
 * Modem support code
 *
 * $Log: modem.c,v $
 * Revision 1.6  1998/04/25 08:40:01  nobody
 * Transport layer now selectable
 *
 * Revision 1.5  1998/04/24 14:29:30  tfrieden
 * Multiplayer now really works
 *
 * Revision 1.4  1998/04/14 23:55:42  tfrieden
 * socket stuff added
 *
 * Revision 1.3  1998/04/13 01:18:29  tfrieden
 * *** empty log message ***
 *
 * Revision 1.2  1998/04/11 22:14:55  hfrieden
 * First try of an Amiga serial mode
 *
 * Revision 1.1.1.1  1998/03/03 15:12:24  nobody
 * reimport after crash from backup
 *
 * Revision 1.1.1.1  1998/02/13  20:20:49  hfrieden
 * Initial Import
 */
...
static char rcsid[] = "$Id: modem.c,v 1.6 1998/04/25 08:40:01 nobody Exp $";
...
extern short socket_other_valid;
extern char socket_hostname[];
...
short socket_serial = 0;                //  0 use serial, 1 use socket
...
//  short *ptr;
	...
	db_printf(( "com port %x.\n", portaddr));
	
	switch( detect_UART(portaddr, &loc, &code) )    {                                               
	case 0:  // No UART
		db_printf(( "No UART detected. (LOC:%d, CODE:0x%x)\n", loc, code));
		return -1;
	case 1: // 8250
		db_printf(( "UART type is 8250.\n" ));
		return(16450);
	case 2: // 16450 or 8250 with scratch reg.
		db_printf(( "UART is type 16450, or an 8250 with scratch register.\n" ));
		return(16450);
	case 3: // 16550
		db_printf(( "UART is type 16550, with FIFO bug.\n" ));
	...
	case 4: // 16550A,  which is the only UART the FIFO mode works with.
		db_printf(( "UART is type 16550A, no FIFO bug.\n" ));
		return(16550);
	}
	return (-1);
}
...
//  }
	...
	if (socket_serial == 0) {
		...
		com_port = (PORT *)malloc(sizeof(PORT));
		com_port->status = 1;
		if (socket_open(NULL) > 0)
			com_port->status = 0;
	}
...
#ifndef NDEBUG
	{
		...
		db_printf(( "Current port settings: base %x, irq %x.\n", curr_base, curr_irq ));
	}
#endif
	...
	#ifndef NDEBUG
	db_printf(( "Com port opened.  Status = %d\n", com_port->status));
	#endif
...
//  UseRtsCts(com_port, OFF);
	...
	if (rc != ASSUCCESS) {
		#ifndef NDEBUG
		db_printf(( "PortClose returned %d!\n", rc));
		#endif
	}
...
com_load_settings(void)
{
	...
	if (cfg_size <= 273 )   {               // Old 15 char LEN_PHONE_NUM's
		db_printf((  "Reading old pre 1.1 phone.cfg\n" ));
		for (i = 0; i < NUM_PHONE_NUM; i++)
		{
	...
	if (fread(&com_custom_port, sizeof(int), 1, settings) != 1) {
		db_printf(( "Reading old file format for serial.cfg.\n"));
		goto close;
	}
...
{
	#ifndef NDEBUG
	db_printf(( "Called serial_leave_game.\n"));
	#endif
//  com_abort();
...
void old_com_send_ptr(char *ptr, int len)
{
	register    int count;
	register char dat;
...
#ifndef NDEBUG
			if (i != 0) {
				db_printf(( "EOR followed by unexpected value %d.\n", i));
			}
#endif
			eor_recv = 0;
			return(-2);                         
...
com_check_message(char *checkbuf, int len)
{
	...
	{
		#ifndef NDEBUG
		printf(( "message type %d too short to be a real message!\n", checkbuf[0]));
		#endif
		goto error;
	}
	...
	{
		#ifndef NDEBUG
		printf( "message type %d out of range.\n", checkbuf[0]);
		#endif
		goto error;
	}
	...
	{
		#ifndef NDEBUG
		printf( "error in message type %d, length %d, checksum %d != %d\n", checkbuf[0], len, check, *(ushort *)(checkbuf+(len-2)));
		#endif
		goto error;
	}
	...
	{
		#ifndef NDEBUG
		db_printf(( "Warning, missed 1 or more messages.\n"));
		#endif
	}
	rx_seqnum = seqnum;
//  db_printf(( "message type %d len %d OK!\n", checkbuf[0], len));
	return 0; 

error:
	//db_printf(("Line status: %d.\n", GetLineStatus(com_port)));
...
com_process_menu(char *buf, int len)
{
	...
	db_printf(( "com_process_menu: type %d.\n", buf[0]));
...
nextmessage:
	...
	{
//      db_printf(( "Exiting com_process_input due to mode switch.\n"));
		return;
	}
#if 0
	...
	len = MAX_MULTI_MESSAGE_LEN+4;

	if (socket_serial == 0) {
		dat = com_receive_ptr(syncbuffer, &len);
	} else {
		dat = socket_receive_ptr(syncbuffer, &len);
	}
...
newmenu:
  ...
  /*      ADD_ITEM(TXT_DIAL_MODEM, MENU_MODEM_CALL, KEY_D);
		...
		if (serial_active & 1) {
			ADD_ITEM("Start serial link", MENU_SERIAL_LINK_START, KEY_S);
			ADD_ITEM("Com settings", MENU_SERIAL_SETUP, KEY_C);
		}
		if (serial_active & 2) {
			ADD_ITEM("Start TCP/IP link", 20, KEY_T);
			ADD_ITEM("TCP/IP settings", 21, KEY_T);
		}
	}
	else
	{
	...
	db_printf(( "main menu choice was %d.\n", choice));
	...
	if (choice > -1) 
	{
		...
		switch (menu_choice[choice])
		{
			...
			case MENU_SERIAL_GAME_START:
				...
				break;
			case 20:    // tcp start
				n[0].type = NM_TYPE_TEXT;
				n[0].text = "Connecting";
//                nm_messagebox("Connecting", 1
				socket_serial = 1;
				socket_error = socket_open(socket_hostname);
				if (socket_error <= 0) {
					switch (socket_error) {
						case 0:
							nm_messagebox("Error", 1, TXT_OK, "No host to connect to");
							goto newmenu;
							break;
						case -1:
							nm_messagebox("Error", 1, TXT_OK, "Can`t create socket");
							goto newmenu;
							break;
						case -2:
							nm_messagebox("Error", 1, TXT_OK, "Can`t bind port");
							goto newmenu;
							break;
						case -3:
							nm_messagebox("Error", 1, TXT_OK, "Can`t connect");
							goto newmenu;
							break;
						case -4:
							nm_messagebox("Error", 1, TXT_OK, "Unknown host");
							goto newmenu;
							break;
						default:
							nm_messagebox("Error", 1, TXT_OK, "Unknown error");
							goto newmenu;
							break;
					}
				}
...
newmenu:
	...
	if (mmn == menu_save) 
	{
		if ((new_irq == default_irq[com_port_num]) && (new_base == default_base[com_port_num])) {
			com_custom_port = -1;
			db_printf(( "Custom com settings not changed.\n"));
			return;
		}
...
com_start_game()
{
	...
	db_printf(( "Entered com_start_game\n"));

	com_reset_game();

	db_printf(( "Game mode is %x\n", Game_mode));
	...
	else // Slave
	{
		...
		if (com_process_mode == COM_PROCESS_NORMAL)
		{
			...
			Game_mode = (unsigned char)other_sync.game_mode & 0xff;
#ifndef SHAREWARE
...
void serial_link_start(void)
{
	...
	srandom(clock());
	my_sync.sync_time = random();
	db_printf(( "My rand set to %d.\n", my_sync.sync_time));
...
com_level_sync(void)
{
	...
	db_printf(( "entered com_level_sync()\n"));
	...
	if (Game_mode & GM_MULTI_COOP)
		...
		my_sync.difficulty = random()%MAX_NUM_NET_PLAYERS; // My starting position
...
com_process_sync(char *buf, int len)
{
	...
	switch(buf[0])
	{
		...
		case MULTI_BEGIN_SYNC:
		{
			db_printf(( "Incoming begin_sync message.\n"));
			if (got_sync)
				break;
			...
			{
				db_printf(( "Other sync invalid id, %d != %d.\n", other_sync.sync_id, my_sync.sync_id));
			}
			else
#endif
			{           
				db_printf(( "got other sync size %d.\n", sizeof(com_sync_pack)-3));
				got_sync = 1;
				com_send_end_sync();
			}
			break;
...
void com_sync_poll(int nitems, newmenu_item *menus, int *key, int citem)
{
	...
	if (t1==0) t1=timer_get_approx_seconds();
	...
	if (com_process_mode == COM_PROCESS_NORMAL)
	{
		*key = -2;
		com_send_sync();
		db_printf(( "Sync finished.\n"));
		return;
	}
	if (com_process_mode == COM_PROCESS_MENU)
	{
		*key = -3;
		db_printf(( "Sync denied by other side.\n"));
		return;
	}
	return;
}
