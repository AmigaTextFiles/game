...
*/
/*
 * $Source: /usr/CVS/descent/main/automap.c,v $
 * $Revision: 1.9 $
 * $Author: tfrieden $
 * $Date: 1998/04/24 14:26:38 $
 * 
 * Routines for displaying the auto-map.
 * 
 * $Log: automap.c,v $
 * Revision 1.9  1998/04/24 14:26:38  tfrieden
 * Bugfixes
 *
 * Revision 1.8  1998/04/09 17:15:23  hfrieden
 * Added new automapper stuff for ViRGE
 *
 * Revision 1.7  1998/04/02 13:43:27  tfrieden
 * Raised edge limit
 *
 * Revision 1.6  1998/03/30 18:40:42  hfrieden
 * New hardware-accelerated Automapper for ViRGE
 *
 * Revision 1.5  1998/03/29 21:22:23  tfrieden
 * Made automap work for real now
 *
 * Revision 1.4  1998/03/25 22:01:54  tfrieden
 * Inserted a clear_canvas, which doesn`t seem to work :)
 *
 * Revision 1.3  1998/03/22 21:47:43  tfrieden
 * Cleared background on automapper
 *
 * Revision 1.2  1998/03/15 01:22:26  nobody
 * Removed the automap ackground => works now
 *
 * Revision 1.1.1.1  1998/03/03 15:12:13  nobody
 * reimport after crash from backup
 *
 * Revision 1.2  1998/02/28 00:50:04  tfrieden
 * aga support
 *
 * Revision 1.1.1.1  1998/02/13  20:20:39  hfrieden
 * Initial Import
 */


#pragma off (unreferenced)
static char rcsid[] = "$Id: automap.c,v 1.9 1998/04/24 14:26:38 tfrieden Exp $";
...
#define g3_draw_line g3_direct_draw_line
#define g3_draw_sphere g3_direct_draw_sphere
#define gr_bitmapm gr_direct_bitmapm
...
#define MAX_EDGES 15000      // Determined by loading all the levels by John & Mike, Feb 9, 1995
...
extern int Automap_large_screen;    // If set to 1, open automap in 640x480
...
void draw_automap()
{
	...
	#ifdef VIRGIN
	static short automap_color1 = -1;
	static short automap_color2 = -1;
	char namebuf[128];
	extern void virge_repw(UWORD *start __asm("a0"), int num __asm("d0"), UWORD data __asm("d1"));
	UWORD *RenderBuffer = (UWORD *)VirgeGetBuffer();
...
//    current_page ^= 1;
	...
	yy = step;
	while (yy<__height) {
		virge_repw(RenderBuffer+yy*VirgePixelsPerRow, __width, 0x00e0);
		yy+=step;
	}
	#endif
	render_start_frame();
	...
	#ifdef VIRGIN
	if (automap_color1 == -1)
		automap_color1 = BM_XRGB(0,20,0);

	if (automap_color2 == -1)
		automap_color2 = BM_XRGB(0,0,40);

	gr_set_fontcolor(automap_color1, -1);
	sprintf(namebuf, "%s %i: ",TXT_LEVEL, Current_level_num);
	strcat(namebuf, Current_level_name);
	gr_direct_printf( 40, 40,namebuf);
	gr_direct_printf( 40, 22,TXT_AUTOMAP);
	gr_set_fontcolor(automap_color2, -1);
	gr_direct_printf( 40, __height-30,TXT_TURN_SHIP);
	gr_direct_printf( 40, __height-20,TXT_SLIDE_UPDOWN);
	gr_direct_printf( 40, __height-10,TXT_VIEWING_DISTANCE);
	VirgeSwitchBuffer();
	#else
...
#ifdef WARP3D
extern int WARP_automap_active;
#endif
...
void do_automap( int key_code )
{
	...
	if (gr_keep_resolution) {
		mode = VR_screen_mode;
		width = VR_render_width;
		height = VR_render_height;
		#ifdef WARP3D
		WARP_automap_active = 1;
		#endif
	} else {
		if (Automap_large_screen == 0) {
			width = 320;
			height = 400;
			#ifdef VIRGIN
			__width = 320;
	...
	//printf( "Num_vertices=%d, Max_edges=%d, (MAX:%d)\n", Num_vertices, Max_edges, MAX_EDGES );
	//printf( "Allocated %d K for automap edge list\n", (sizeof(Edge_info)+sizeof(short))*Max_edges/1024 );


	//printf("Mode set\n");

	gr_init_sub_canvas(&Pages[0],grd_curcanv,0,0,width,height);                 //printf("1"); fflush(stdout);
//    gr_init_sub_canvas(&Pages[1],grd_curcanv,0,height+1,width,height);          printf("...2"); fflush(stdout);
#ifdef VIRGIN
	gr_init_sub_canvas(&DrawingPages[0],&Pages[0],16,69,width,height);
//    gr_init_sub_canvas(&DrawingPages[1],&Pages[1],16,69,width,height);
#else
	gr_init_sub_canvas(&DrawingPages[0],&Pages[0],16,69,width-32,height-128);   //printf("...3");fflush(stdout);
//    gr_init_sub_canvas(&DrawingPages[1],&Pages[1],16,69,width-32,height-128);   printf("...4");fflush(stdout);
#endif
	Automap_background.bm_data = NULL;
	gr_clear_canvas(0);                                                         //printf("...5\n");fflush(stdout);

/*    if (!Automap_large_screen) {
...
//    for (i=0; i<2; i++ )    {
	{ i = 0;
		gr_set_current_canvas(&Pages[i]);                                       //printf("a..."); fflush(stdout);
		modex_printf( 40, 22,TXT_AUTOMAP,GFONT_BIG_1);                          //printf("b..."); fflush(stdout);
		modex_printf( 70,height-47,TXT_TURN_SHIP,GFONT_SMALL);                  //printf("c..."); fflush(stdout);
		modex_printf( 70,height-31,TXT_SLIDE_UPDOWN,GFONT_SMALL);               //printf("d..."); fflush(stdout);
		modex_printf( 70,height-15,TXT_VIEWING_DISTANCE,GFONT_SMALL);           //printf("e\n");  fflush(stdout);
	}
//    gr_update(NULL);
#endif
	//printf("Stuff painted\n");
...
//    gr_set_mode( SM_320x200x8 );
	gr_set_mode(VR_screen_mode);

	#ifdef WARP3D
	WARP_automap_active = 0;
	#endif
