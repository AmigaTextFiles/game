...
*/
/*
 * $Source: /usr/CVS/descent/main/collide.c,v $
 * $Revision: 1.4 $
 * $Author: nobody $
 * $Date: 1998/08/08 15:42:31 $
 * 
 * $Log: collide.c,v $
 * Revision 1.4  1998/08/08 15:42:31  nobody
 * Activated the Editior
 *
 * Revision 1.3  1998/04/24 14:27:01  tfrieden
 * Bugfixes (alignment problems)
 *
 * Revision 1.2  1998/03/25 22:03:24  tfrieden
 * Removed some more warnings (storage class)
 *
 * Revision 1.1.1.1  1998/03/03 15:12:12  nobody
...
static char rcsid[] = "$Id: collide.c,v 1.4 1998/08/08 15:42:31 nobody Exp $";
...
int check_effect_blowup(segment *seg,int side,vms_vector *pnt)
{
	...
	if ((tm=seg->sides[side].tmap_num2) != 0)
		...
		if ((db=Effects[ec].dest_bm_num)!=-1 && !(Effects[ec].flags&EF_ONE_SHOT)) {
				...
				if (bm->bm_data[y*bm->bm_w+x] != TRANSPARENCY_COLOR) {     //not trans, thus on effect
					int vc,sound_num;
