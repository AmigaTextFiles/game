...
*/
/*
 * $Source: /usr/CVS/descent/main/network.c,v $
 * $Revision: 1.5 $
 * $Author: tfrieden $
 * $Date: 1998/04/24 14:30:20 $
 * 
 * Routines for managing network play.
 * 
 * $Log: network.c,v $
 * Revision 1.5  1998/04/24 14:30:20  tfrieden
 * Multiplayer now really works
 *
 * Revision 1.4  1998/04/14 23:57:05  tfrieden
 * Socket stuff added
 *
 * Revision 1.3  1998/04/13 17:51:31  tfrieden
 * Kali stuff added
 *
 * Revision 1.2  1998/04/03 14:01:44  tfrieden
 * Removed comments
 *
 */

#pragma off (unreferenced)
static char rcsid[] = "$Id: network.c,v 1.5 1998/04/24 14:30:20 tfrieden Exp $";
...
extern int Network_linux;   //  Use kali stuff
...
struct ipx_funcs {
	ubyte * (*ipx_get_my_server_address_func)(void);
	ubyte * (*ipx_get_my_local_address_func)(void);
	int     (*ipx_get_packet_data_func)( ubyte * data );
	void    (*ipx_send_packet_data_func)( ubyte * data, int datasize, ubyte *network, ubyte *address, ubyte *immediate_address );
	void    (*ipx_get_local_target_func)( ubyte * server, ubyte * node, ubyte * local_target );
	void    (*ipx_send_broadcast_packet_data_func)( ubyte * data, int datasize );
	void    (*ipx_send_internetwork_packet_data_func)( ubyte * data, int datasize, ubyte * server, ubyte *address );
	int     (*ipx_change_default_socket_func)( ushort socket_number );
};

struct ipx_funcs ipx_functions = {
	ipx_get_my_server_address,
	ipx_get_my_local_address,
	ipx_get_packet_data,
	ipx_send_packet_data,
	ipx_get_local_target,
	ipx_send_broadcast_packet_data,
	ipx_send_internetwork_packet_data,
	ipx_change_default_socket
};

struct ipx_funcs kali_functions = {
	ipx_get_my_server_address_kali,
	ipx_get_my_local_address_kali,
	ipx_get_packet_data_kali,
	ipx_send_packet_data_kali,
	ipx_get_local_target_kali,
	ipx_send_broadcast_packet_data_kali,
	ipx_send_internetwork_packet_data_kali,
	ipx_change_default_socket_kali
};

struct ipx_funcs *current_funcs;

#define IPX_GET_MY_SERVER_ADDRESS                current_funcs->ipx_get_my_server_address_func
#define IPX_GET_MY_LOCAL_ADDRESS                 current_funcs->ipx_get_my_local_address_func
#define IPX_GET_PACKET_DATA                      current_funcs->ipx_get_packet_data_func
#define IPX_SEND_PACKET_DATA                     current_funcs->ipx_send_packet_data_func
#define IPX_GET_LOCAL_TARGET                     current_funcs->ipx_get_local_target_func
#define IPX_SEND_BROADCAST_PACKET_DATA           current_funcs->ipx_send_broadcast_packet_data_func
#define IPX_SEND_INTERNETWORK_PACKET_DATA        current_funcs->ipx_send_internetwork_packet_data_func
#define IPX_CHANGE_DEFAULT_SOCKET                current_funcs->ipx_change_default_socket_func
...
network_init(void)
{
	...
	if (Network_linux == 0)
		current_funcs = &ipx_functions;
	else
		current_funcs = &kali_functions;
