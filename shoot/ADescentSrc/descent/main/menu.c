...
*/
/*
 * $Source: /usr/CVS/descent/main/menu.c,v $
 * $Revision: 1.7 $
 * $Author: nobody $
 * $Date: 1998/11/09 22:20:36 $
 *
 * Inferno main menu.
 *
 * 
...
static char rcsid[] = "$Id: menu.c,v 1.7 1998/11/09 22:20:36 nobody Exp $";
...
#ifdef VIRGIN
void do_virge_fps_menu(void)
{
	newmenu_item m[4];
	extern int VirgeFPSLimit;
	int item;

	m[0].type = NM_TYPE_CHECK;  m[0].text = "Limit FPS Rate";
	m[1].type = NM_TYPE_TEXT;   m[1].text = "Max FPS Rate:";
	m[2].type = NM_TYPE_NUMBER; m[2].text = "Frames/sec";   m[2].value = VirgeFPSLimit;
		m[2].min_value = 5; m[2].max_value=40;

	if (VirgeFPSLimit > 0) m[0].value = 1;
	else                   m[0].value = 0;

	item = newmenu_do("FPS Blocker", "ESC to cancel", 3, m, NULL);
	...
	if (item>-1) {
		...
		VirgeSetFPSLimit(m[2].value);
	}
}

void do_virge_menu(void)
{
	newmenu_item m[10];
	int nitems;
	int i=0;
	...
	extern int VirgeTMapper;
	extern int VirgeModulateLight;

loop:
	m[0].type = NM_TYPE_CHECK;  m[0].text = "Bilinear Filtering";           m[0].value = VirgeFilter;
	m[1].type = NM_TYPE_CHECK;  m[1].text = "Flashing Light";               m[1].value = VirgeModulateLight;
	m[2].type = NM_TYPE_TEXT;   m[2].text = "Texture Mapper";
	m[3].type = NM_TYPE_RADIO;  m[3].text = "SubPixel Mapper (req. FPU)";   m[3].value = 0; m[3].group = 0;
	m[4].type = NM_TYPE_RADIO;  m[4].text = "FullPixel Mapper";             m[4].value = 0; m[4].group = 0;
	m[5].type = NM_TYPE_RADIO;  m[5].text = "SubPixel Persp. (req. FPU)";   m[5].value = 0; m[5].group = 0;
	m[6].type = NM_TYPE_TEXT;   m[6].text = "Other options...";
	m[7].type = NM_TYPE_MENU;   m[7].text = "FPS Limiter...";

	switch(VirgeTMapper) {
	case 0:
		m[3].value = 1; m[4].value = 0; m[5].value = 0;
		break;
	case 1:
		m[3].value = 0; m[4].value = 1; m[5].value = 0;
		break;
	case 2:
		m[3].value = 0; m[4].value = 0; m[5].value = 1;
		break;
	}

	nitems = 8;
	i = newmenu_do1("ViRGE Menu", "ESC to cancel", nitems, m,NULL ,i);
	...
	if (m[0].value == 1) VirgeBilinearOn();
	else                 VirgeBilinearOff();

	VirgeSetModulate(m[1].value);

	if      (m[3].value == 1) VirgeSetMapper(0);    // SubPixels/FPU
	else if (m[4].value == 1) VirgeSetMapper(1);    // FullPixels/INT
	else if (m[5].value == 1) VirgeSetMapper(2);    // SubPixelsPerp/FPU
	else                      VirgeSetMapper(-1);    // Default

}
#endif

#ifdef WARP3D
void do_warp_filter_menu(void)
{
	newmenu_item m[10];
	int i = 0, j = 0;

	do {
		m[0].type = NM_TYPE_TEXT;  m[0].text = "Texture";
		m[1].type = NM_TYPE_RADIO; m[1].text = "Nearest";   m[1].group = 0;
		m[2].type = NM_TYPE_RADIO; m[2].text = "Linear";    m[2].group = 0;
		m[3].type = NM_TYPE_TEXT;  m[3].text = "Depth";
		m[4].type = NM_TYPE_RADIO; m[4].text = "Nearest";   m[4].group = 1;
		m[5].type = NM_TYPE_RADIO; m[5].text = "Linear";    m[5].group = 1;
		...
		i = newmenu_do1( "Filtering", NULL,  6, m, 0, i );
		if (i == -1) break;

		if (m[1].value == 1) WARP_Filter = 0;
		else                 WARP_Filter = 1;

		if (m[4].value == 1) WARP_Depth = 0;
		else                 WARP_Depth = 1;
...
void do_warp_texture_menu(void)
{
	newmenu_item m[10];
	int i = 0;
	ULONG oldmipmap = WARP_MipMap;

	do {
		m[0].type = NM_TYPE_RADIO; m[0].text = "Perspective";   m[0].group = 0;
		m[1].type = NM_TYPE_RADIO; m[1].text = "Linear";        m[1].group = 0;
		m[2].type = NM_TYPE_CHECK; m[2].text = "Mapping";       m[2].value = WARP_TMap;
		m[3].type = NM_TYPE_CHECK; m[3].text = "MipMapping";    m[3].value = WARP_MipMap;
		m[4].type = NM_TYPE_SLIDER; m[4].text ="LightFX";       m[4].value = (int)WARP_MFactor; m[4].min_value = 0; m[4].max_value = 8;
		...
		i = newmenu_do1( NULL, "Texture Mapping", 5, m, 0, i );
		if (i == -1) break;
		...
		switch (i) {
			...
			case 1: WARP_Persp = 0;
					break;
			case 2: WARP_TMap = m[2].value;
					break;
			case 3: WARP_MipMap = m[3].value;
					break;
			case 4: WARP_MFactor = (W3D_Float)m[4].value;
					break;
		}
	...
	WARP_TMap = m[2].value;
	WARP_MipMap = m[3].value;

	if (WARP_MipMap != oldmipmap) {     //  Need to remake the texture cache
		show_boxed_message("Remaking textures");
		WARP_DeleteTextureCache();
		WARP_InitTextureCache();
		WARP_UpdateTextureCache();
		clear_boxed_message();
	}
	WARP_MFactor = (W3D_Float)m[4].value;
	if (WARP_MFactor == 0.0) WARP_Modulate = 0;
	else                     WARP_Modulate = 1;

	WARP_UpdateState();
}

void do_warp_detail_menu(void)
{
	newmenu_item m[12];
	int i = 0;

	m[0].type = NM_TYPE_SLIDER; m[0].text="Textures";   m[0].value = WARP_HTMap; m[0].min_value = 0; m[0].max_value = 2;
	m[1].type = NM_TYPE_SLIDER; m[1].text="MipMaps";    m[1].value = WARP_HMMap; m[1].min_value = 0; m[1].max_value = 2;
	m[2].type = NM_TYPE_SLIDER; m[2].text="Bi-Filter";  m[2].value = WARP_HBif;  m[2].min_value = 0; m[2].max_value = 2;
	m[3].type = NM_TYPE_SLIDER; m[3].text="D-Filter";   m[3].value = WARP_HDf;   m[3].min_value = 0; m[3].max_value = 2;
	m[4].type = NM_TYPE_SLIDER; m[4].text="Perspective";m[4].value = WARP_HPers; m[4].min_value = 0; m[4].max_value = 2;
	m[5].type = NM_TYPE_SLIDER; m[5].text="Blending";   m[5].value = WARP_HBlend;m[5].min_value = 0; m[5].max_value = 2;
	m[6].type = NM_TYPE_SLIDER; m[6].text="Fogging";    m[6].value = WARP_HFog;  m[6].min_value = 0; m[6].max_value = 2;
	m[7].type = NM_TYPE_SLIDER; m[7].text="Anitalias";  m[7].value = WARP_HAnti; m[7].min_value = 0; m[7].max_value = 2;
	m[8].type = NM_TYPE_SLIDER; m[8].text="Dithering";  m[8].value = WARP_HDith; m[8].min_value = 0; m[8].max_value = 2;
	m[9].type = NM_TYPE_TEXT;   m[9].text="             LO HI";

	i = newmenu_do1( NULL, "Details", 10, m, 0, i );

	WARP_HTMap = m[0].value;
	WARP_HMMap = m[1].value;
	WARP_HBif  = m[2].value;
	WARP_HDf   = m[3].value;
	WARP_HPers = m[4].value;
	WARP_HBlend= m[5].value;
	WARP_HFog  = m[6].value;
	WARP_HAnti = m[7].value;
	WARP_HDith = m[8].value;
...
void do_warp_fog_menu(void)
{
	newmenu_item m[10];
	int i = 0;
	char text[5][30];

	sprintf( text[0], "%d", (int)(WARP_FogParam.fog_start * 100));
	sprintf( text[1], "%d", (int)(WARP_FogParam.fog_end * 100));
	sprintf( text[2], "%d", (int)(WARP_FogParam.fog_color.r * 255));
	sprintf( text[3], "%d", (int)(WARP_FogParam.fog_color.g * 255));
	sprintf( text[4], "%d", (int)(WARP_FogParam.fog_color.b * 255));

	do {
		m[0].type = NM_TYPE_CHECK; m[0].text = "Fog on"; m[0].value = WARP_Fog;
		m[1].type = NM_TYPE_TEXT;  m[1].text = "Fog start/end";
		m[2].type = NM_TYPE_INPUT; m[2].text = text[0];  m[2].text_len = 20;
		m[3].type = NM_TYPE_INPUT; m[3].text = text[1];  m[3].text_len = 20;
		m[4].type = NM_TYPE_TEXT;  m[4].text = " ";
		m[5].type = NM_TYPE_TEXT;  m[5].text = "Fog color";
		m[6].type = NM_TYPE_INPUT; m[6].text = text[2];  m[6].text_len = 20;
		m[7].type = NM_TYPE_INPUT; m[7].text = text[3];  m[7].text_len = 20;
		m[8].type = NM_TYPE_INPUT; m[8].text = text[4];  m[8].text_len = 20;

		i = newmenu_do1( NULL, "Fogging", 9, m, 0, i );
		if (i == -1) break;
	...
	WARP_Fog = m[0].value;
	WARP_FogParam.fog_start = atof(text[0]) / 100.0;
	WARP_FogParam.fog_end = atof(text[1]) / 100.0;
	WARP_FogParam.fog_color.r = atof(text[2]) / 255.0;
	WARP_FogParam.fog_color.g = atof(text[3]) / 255.0;
	WARP_FogParam.fog_color.b = atof(text[4]) / 255.0;

	WARP_UpdateFog();
}

void do_warp_menu(void)
{
	newmenu_item m[10];
	int i = 0;

	do {
		m[0].type = NM_TYPE_MENU; m[0].text = "Filter options...";
		m[1].type = NM_TYPE_MENU; m[1].text = "Texture options...";
		m[2].type = NM_TYPE_MENU; m[2].text = "Fogging options...";
		m[3].type = NM_TYPE_MENU; m[3].text = "Warp detail options...";

		i = newmenu_do1( "Warp3D options", NULL, 4, m, 0, i );

		switch (i) {
			case 0: do_warp_filter_menu();  break;
			case 1: do_warp_texture_menu(); break;
			case 2: do_warp_fog_menu();     break;
			case 3: do_warp_detail_menu();  break;
		}
	} while (i > -1);
}
#endif
void do_options_menu()
{
	...
	do {
	...
	#elif defined(VIRIGN)
		m[9].type = NM_TYPE_MENU; m[9].text="ViRGE options...";
	#else defined(WARP3D)
		m[9].type = NM_TYPE_MENU; m[9].text="WARP3D options...";
	#endif
		...
		switch(i)   {
			...
			#ifdef VIRGIN
			case 9: do_virge_menu();        break;
			#endif
			#ifdef WARP3D
			case 9: do_warp_menu();         break;
			#endif
		}
