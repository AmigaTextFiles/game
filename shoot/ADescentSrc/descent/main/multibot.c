...
*/
/*
 * $Source: /usr/CVS/descent/main/multibot.c,v $
 * $Revision: 1.2 $
 * $Author: tfrieden $
 * $Date: 1998/04/24 14:30:04 $
 * 
...
static char rcsid[] = "$Id: multibot.c,v 1.2 1998/04/24 14:30:04 tfrieden Exp $";
...
//  mprintf((0, "SENDPOS object %d, Gametime %d.\n", objnum, GameTime));
	...
	*(short *)(multibuf+loc) = swapshort((short)objnum_local_to_remote(objnum, (byte *)&multibuf[loc+2]));
	...
	bcopy(multibuf+loc+10, multibuf+loc+9 ,14);     // compensate for aligned struct
																		loc += sizeof(shortpos);

	multi_send_data(multibuf, loc-1, 0);
}
...
multi_send_boss_actions(int bossobjnum, int action, int secondary, int objnum)
{
	...
	*(short *)(multibuf+loc) = swapshort(bossobjnum);       loc += 2; // We won't network map this objnum since its the boss
	...
	*(short *)(multibuf+loc) = swapshort(objnum);           loc += 2; // Objnum of object created by gate-in action
	if (action == 3) {
		*(short *)(multibuf+loc) = swapshort(Objects[objnum].segnum); loc += 2; // Segment number object created in (for gate only)
	}
	else
																		loc += 2; // Dummy
...
multi_do_robot_position(char *buf)
{
	...
	if ((botnum < 0) || (botnum > Highest_object_index)) {
		db_printf(( "Got robot position for object %d which I don't have.\n", botnum));
//      Int3(); // See rob
		return;
	}

	if ((Objects[botnum].type != OBJ_ROBOT) || (Objects[botnum].flags & OF_EXPLODING)) {
		db_printf(( "Ignoring position packet for non-robot or exploding robot.\n"));
		return;
	}
	...
	{   
		if (Objects[botnum].ctype.ai_info.REMOTE_OWNER == -1)
		{
			...
			if (Objects[botnum].ctype.ai_info.REMOTE_SLOT_NUM > 3) {
				db_printf(("Player %d taking control of robot %d WITHOUT claim message.\n", pnum, botnum));
				Objects[botnum].ctype.ai_info.REMOTE_OWNER = pnum;
				Objects[botnum].ctype.ai_info.REMOTE_SLOT_NUM = 0;
			}
			else
		...
		{
			db_printf(( "Ignoring position for robot %d not controlled by player %d.\n", botnum, pnum));
			return;
		}
	}
...
//  Objects[botnum].phys_info.drag = Robot_info[Objects[botnum].id].drag >> 4; // Set drag to low
	...
	bcopy(buf+loc+9, buf+loc+10, 14);           // compensate for aligned struct
...
void multi_robot_request_change(struct object *robot, int player_num) {
