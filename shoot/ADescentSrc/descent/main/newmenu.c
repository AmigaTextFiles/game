...
*/
/*
 * $Source: /usr/CVS/descent/main/newmenu.c,v $
 * $Revision: 1.2 $
 * $Author: nobody $
 * $Date: 1998/03/28 14:48:21 $
 * 
 * Routines for menus.
 * 
 * $Log: newmenu.c,v $
 * Revision 1.2  1998/03/28 14:48:21  nobody
 * Modificatins for mission selection
 *
 * Revision 1.1.1.1  1998/03/03 15:12:29  nobody
 * reimport after crash from backup
 *
 * Revision 1.2  1998/02/28 01:15:45  tfrieden
 * Additional AGA stuff
 *
 * Revision 1.1.1.1  1998/02/13  20:20:57  hfrieden
 * Initial Import
 */


#pragma off (unreferenced)
static char rcsid[] = "$Id: newmenu.c,v 1.2 1998/03/28 14:48:21 nobody Exp $";
...
ReadFileNames:
	...
	glob_ret.gl_offs=0; 
	if (!glob(filespec,0,NULL,&glob_ret)&&glob_ret.gl_pathc) {
		int j;
		for (j=NumFiles;j<glob_ret.gl_pathc+NumFiles;j++) {
			strncpy(&filenames[j*14],glob_ret.gl_pathv[j-NumFiles],13);
			if (player_mode) {
				char *p;
				p=strchr(&filenames[j*14],'.');
				if (p) *p='\0';
			}
		}
		NumFiles=glob_ret.gl_pathc+NumFiles;
	}
...
int newmenu_filelist( char * title, char * filespec, char * filename )
{
	...
	if (!glob(filespec,0,NULL,&glob_ret)&&glob_ret.gl_pathc) {
		int j;
		for (j=NumFiles;j<glob_ret.gl_pathc+NumFiles;j++) {
			strncpy(FilenameText[j],glob_ret.gl_pathv[j-NumFiles],13);
			Filenames[j]=FilenameText[j];
		}
		NumFiles=glob_ret.gl_pathc+NumFiles;
	}
