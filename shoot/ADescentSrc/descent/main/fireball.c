...
*/
/*
 * $Source: /usr/CVS/descent/main/fireball.c,v $
 * $Revision: 1.2 $
 * $Author: tfrieden $
 * $Date: 1998/03/25 22:04:56 $
 * 
 * Code for rendering & otherwise dealing with explosions
 * 
 * $Log: fireball.c,v $
 * Revision 1.2  1998/03/25 22:04:56  tfrieden
 * Removed some more warnings (storage class)
 *
 * Revision 1.1.1.1  1998/03/03 15:12:14  nobody
 * reimport after crash from backup
 *
 * Revision 1.1.1.1  1998/02/13  20:20:51  hfrieden
 * Initial Import
 */

#pragma off (unreferenced)
static char rcsid[] = "$Id: fireball.c,v 1.2 1998/03/25 22:04:56 tfrieden Exp $";
			...
			if ( (obj0p->type == OBJ_CNTRLCEN) || (obj0p->type==OBJ_PLAYER) || ((obj0p->type==OBJ_ROBOT) && ((Objects[parent].type != OBJ_ROBOT) || (Objects[parent].id != obj0p->id)))) {
				...
				if ( dist < maxdistance ) {
					if (object_to_object_visibility(obj, obj0p, FQ_TRANSWALL)) {
						...
						vm_vec_sub( &pos_hit, &obj->pos, &obj0p->pos); // HERE
