...
*/
/*
 * $Source: /usr/CVS/descent/main/multi.c,v $
 * $Revision: 1.4 $
 * $Author: nobody $
 * $Date: 1998/07/14 20:59:26 $
 * 
...
static char rcsid[] = "$Id: multi.c,v 1.4 1998/07/14 20:59:26 nobody Exp $";
...
int objnum_remote_to_local(int remote_objnum, int owner)
{
	...
	if ((owner >= N_players) || (owner < -1)) {
		db_printf(("Warning: Illegal owner: %d (remote: %d)\n", owner, remote_objnum));
//        Int3(); // Illegal!
	...
	if (result < 0)
	{
		db_printf(( "Remote object owner %d number %d mapped to -1!\n", owner, remote_objnum));
		return(-1);
	}
	...
	{
		db_printf(( "Remote object owner %d number %d doesn't match owner %d.\n", owner, remote_objnum, object_owner[result]));
	}
#endif  
...
int objnum_local_to_remote(int local_objnum, byte *owner)
{
	...
	*owner = object_owner[local_objnum];
	...
	result = local_to_remote[local_objnum];
	  ...
	  db_printf(( "Local object %d mapped to owner %d objnum %d.\n", local_objnum,
		*owner, result));
...
{       
	...
	bcopy(&buf[10], &buf[11], 14);           // compensate for aligned struct
...
multi_do_player_explode(char *buf)
{
	...
	{
		db_printf(("Resetting object sync due to player explosion.\n"));
...
//  objp->pos = *(vms_vector *)(buf+28);                // 12 bytes
	...
	db_printf(("I Created %d powerups, remote created %d.\n", Net_create_loc, remote_created));
	...
	for (i = 0; i < remote_created; i++)
	{
		...
		{
			db_printf(( "WARNING: Remote created object has non-valid number %d (player %d)", *(short *)(buf+count), pnum));
		}
		else 
		{
			db_printf(( "WARNING: Could not create all powerups created by player %d.\n", pnum));
		}
//      Assert(*(short *)(buf+count) > 0);
		count += 2;
	}
	for (i = remote_created; i < Net_create_loc; i++) {
		db_printf(( "WARNING: I Created more powerups than player %d, deleting.\n", pnum));
...
multi_send_position(int objnum)
{
	...
	bcopy(multibuf, multibuf+1, 10);     // compensate for aligned struct

	count += sizeof(shortpos) - 1;
	
	multi_send_data(multibuf+1, count, 0);
}
