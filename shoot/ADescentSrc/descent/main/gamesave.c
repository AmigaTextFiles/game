...
*/
/*
 * $Source: /usr/CVS/descent/main/gamesave.c,v $
 * $Revision: 1.3 $
 * $Author: nobody $
 * $Date: 1998/08/08 15:43:59 $
 * 
 * Save game information
 * 
 * $Log: gamesave.c,v $
 * Revision 1.3  1998/08/08 15:43:59  nobody
 * Activated the Editior
 *
 * Revision 1.2  1998/03/13 23:52:01  tfrieden
 * various changes with sound, joystick, shareware, notwork
 *
 * Revision 1.1.1.1  1998/03/03 15:12:20  nobody
 * reimport after crash from backup
 *
 * Revision 1.1.1.1  1998/02/13  20:20:45  hfrieden
 * Initial Import
 */

#pragma off (unreferenced)
static char rcsid[] = "$Id: gamesave.c,v 1.3 1998/08/08 15:43:59 nobody Exp $";
		...
		if (!cfseek( LoadFile, game_fileinfo.walls_offset,SEEK_SET ))   {
			for (i=0;i<game_fileinfo.walls_howmany;i++) {
				...
				if (game_top_fileinfo.fileinfo_version >= 20) {
					...
					Walls[i].pad = read_short(LoadFile);
					...
					Walls[i].segnum         = swapint(w.segnum);
					Walls[i].sidenum        = swapint(w.sidenum);
					Walls[i].linked_wall    = swapint(w.linked_wall);

					Walls[i].type           = w.type;
					Walls[i].flags          = w.flags;
					Walls[i].hps            = (fix)swapint(w.hps);
		...
		if (!cfseek( LoadFile, game_fileinfo.doors_offset,SEEK_SET ))   {
			...
			for (i=0;i<game_fileinfo.doors_howmany;i++) {
				...
				if (game_top_fileinfo.fileinfo_version >= 20) {
					...
					ActiveDoors[i].n_parts = swapint(d.n_parts);
...
int load_level(char * filename_passed)
{
	...
	for (iii = 0; iii < strlen(filename); iii++) {
		filename[iii] = (char)toupper(filename[iii]);
	}
