...
*/
/*
 * $Source: /usr/CVS/descent/mem/mem.c,v $
 * $Revision: 1.1.1.1 $
 * $Author: nobody $
 * $Date: 1998/03/03 15:12:41 $
 * 
...
static char rcsid[] = "$Id: mem.c,v 1.1.1.1 1998/03/03 15:12:41 nobody Exp $";
...
void * mem_malloc( unsigned int size, char * var, char * filename, int line, int fill_zero )
{
	...
	if (ptr==NULL)  {
		fprintf( stderr, "\nMEM_OUT_OF_MEMORY: Malloc returned NULL\n" );
		fprintf( stderr, "\tVar %s, file %s, line %d.\n", var, filename, line );
		fprintf( stderr, "\tAllocated %d+%d=%d RAM\n",size,CHECKSIZE,size+CHECKSIZE);
		Error( "MEM_OUT_OF_MEMORY" );
		exit(1);
	}
