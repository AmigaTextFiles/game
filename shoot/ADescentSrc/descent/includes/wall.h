...
*/
/*
 * $Source: /usr/CVS/descent/includes/wall.h,v $
 * $Revision: 1.1.1.1 $
 * $Author: nobody $
 * $Date: 1998/03/03 15:12:06 $
 * 
 * $Log: wall.h,v $
 * Revision 1.1.1.1  1998/03/03 15:12:06  nobody
 * reimport after crash from backup
 *
 * Revision 1.1.1.1  1998/02/13  20:20:30  hfrieden
 * Initial Import
 *
...
typedef struct {
	fix				play_time	__attribute__ ((packed));
	short				num_frames	__attribute__ ((packed));
	short				frames[MAX_CLIP_FRAMES]	__attribute__ ((packed));
	short				open_sound	__attribute__ ((packed));
	short				close_sound	__attribute__ ((packed));
	short				flags		__attribute__ ((packed));
	char				filename[13]	__attribute__ ((packed));
