...
*/
/*
 * $Source: /usr/CVS/descent/includes/player.h,v $
 * $Revision: 1.1.1.1 $
 * $Author: nobody $
 * $Date: 1998/03/03 15:12:00 $
 * 
 * Structure information for the player
 * 
 * $Log: player.h,v $
 * Revision 1.1.1.1  1998/03/03 15:12:00  nobody
 * reimport after crash from backup
 *
 * Revision 1.1.1.1  1998/02/13  20:20:25  hfrieden
 * Initial Import
 *
...
typedef struct player_ship {
	int 		model_num	__attribute__((packed));
	int		expl_vclip_num	__attribute__((packed));
	fix		mass		__attribute__((packed));
	fix		drag		__attribute__((packed));
	fix		max_thrust	__attribute__((packed));
	fix		reverse_thrust	__attribute__((packed));
	fix		brakes		__attribute__((packed));		//low_thrust
	fix		wiggle		__attribute__((packed));
	fix		max_rotthrust	__attribute__((packed));
	vms_vector gun_points[N_PLAYER_GUNS]	__attribute__((packed));
} player_ship;
