...
*/
/*
 * $Source: /usr/CVS/descent/includes/effects.h,v $
 * $Revision: 1.1.1.1 $
 * $Author: nobody $
 ...
 *
 * Revision 1.1.1.1  1998/02/13  20:20:19  hfrieden
 * Initial Import
 *
...
typedef struct eclip {
	vclip 			vc		__attribute__ ((packed));		//imbedded vclip
	fix			time_left	__attribute__ ((packed));		//for sequencing
	int			frame_count	__attribute__ ((packed));	//for sequencing
	short			changing_wall_texture	__attribute__ ((packed));			//Which element of Textures array to replace.
	short			changing_object_texture	__attribute__ ((packed));		//Which element of ObjBitmapPtrs array to replace.
	int			flags		__attribute__ ((packed));			//see above
	int			crit_clip	__attribute__ ((packed));		//use this clip instead of above one when mine critical
	int			dest_bm_num	__attribute__ ((packed));	//use this bitmap when monitor destroyed
	int			dest_vclip	__attribute__ ((packed));		//what vclip to play when exploding
	int			dest_eclip	__attribute__ ((packed));		//what eclip to play when exploding
	fix			dest_size	__attribute__ ((packed));		//3d size of explosion
	int			sound_num	__attribute__ ((packed));		//what sound this makes
	int			segnum		__attribute__ ((packed));
	int			sidenum		__attribute__ ((packed));	//what seg & side, for one-shot clips
} eclip;
