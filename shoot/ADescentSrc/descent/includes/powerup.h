...
*/
/*
 * $Source: /usr/CVS/descent/includes/powerup.h,v $
 * $Revision: 1.1.1.1 $
 * $Author: nobody $
 * $Date: 1998/03/03 15:12:01 $
 * 
 * Powerup header file.
 * 
 * $Log: powerup.h,v $
 * Revision 1.1.1.1  1998/03/03 15:12:01  nobody
...
typedef struct powerup_type_info {
	int	vclip_num	__attribute__ ((packed));
	int	hit_sound	__attribute__ ((packed));
	fix	size		__attribute__ ((packed));			//3d size of longest dimension
	fix	light		__attribute__ ((packed));		//	amount of light cast by this powerup, set in bitmaps.tbl
} powerup_type_info;
