...
*/
/* $Source: /usr/CVS/descent/2d/font.c,v $
 * $Revision: 1.6 $
 * $Author: nobody $
 * $Date: 1998/09/26 15:00:27 $
 *
 * Graphical routines for drawing fonts.
 *
 * $Log: font.c,v $
 * Revision 1.6  1998/09/26 15:00:27  nobody
 * Added Warp3D support
 *
 * Revision 1.5  1998/08/09 23:01:49  tfrieden
 * Changed underline to inverse
 *
 * Revision 1.4  1998/03/30 18:26:17  hfrieden
 * Additional direct ViRGE render stuff added
 *
 * Revision 1.3  1998/03/28 23:03:38  tfrieden
 * Debugging stuff
 *
 * Revision 1.2  1998/03/22 15:47:31  hfrieden
 * ViRGE direct font support added
 *
 * Revision 1.1.1.1  1998/03/03 15:11:46  nobody
 * reimport after crash from backup
 *
 * Revision 1.2  1998/02/28 00:35:58  tfrieden
 * aga support
 *
 * Revision 1.1.1.1  1998/02/13  20:21:21  hfrieden
 * Initial Import
 */
...
int gr_direct_internal_string0(int x, int y, char *s )
{
	unsigned char * fp;
	ubyte * text_ptr, * next_row, * text_ptr1;
	int r, BitMask, i, bits, width, spacing, letter, underline;
	...
	UWORD FGColor = BitValues[FG_COLOR];
	UWORD BGColor = BitValues[BG_COLOR];

	VideoOffset1 = y * (VirgePixelsPerRow) + x;
	...
	while (next_row != NULL )
	{
		...
		if (x==0x8000) {            //centered
			int xx = get_centered_x(text_ptr1);
			VideoOffset1 = y * (VirgePixelsPerRow) + xx;
		}
		...
		for (r=0; r<FHEIGHT; r++)
		{
			...
			while (*text_ptr)
			{
				...
				if (underline)
					for (i=0; i< width; i++ )
						*(RenderBuffer + VideoOffset++) = FG_COLOR;
				else
				{
					fp += BITS_TO_BYTES(width)*r;
					...
					for (i=0; i< width; i++ )
					{
						...
						if (bits & BitMask)
							*(RenderBuffer+VideoOffset++) = FGColor;
						else
							*(RenderBuffer+VideoOffset++) = BGColor;
						BitMask >>= 1;
					}
				}
			...
			VideoOffset1 += VirgePixelsPerRow; y++;
		}
	}
	return 0;
}

int gr_direct_internal_string0m(int x, int y, char *s )
{
	unsigned char * fp;
	ubyte * text_ptr, * next_row, * text_ptr1;
	int r, BitMask, i, bits, width, spacing, letter, underline;
...
int gr_briefing_internal_string0(int x, int y, char *s )
{
	unsigned char * fp;
	ubyte * text_ptr, * next_row, * text_ptr1;
	int r, BitMask, i, bits, width, spacing, letter, underline;
	...
	UWORD *RenderBuffer = (UWORD *)VirgeFirstBuffer();
#endif
#ifdef WARP3D
	UWORD *RenderBuffer = (UWORD *)WARP_FirstBuffer();
#endif
...
int gr_briefing_internal_string0m(int x, int y, char *s )
{
	unsigned char * fp;
	ubyte * text_ptr, * next_row, * text_ptr1;
	int r, BitMask, i, bits, width, spacing, letter, underline;
...
int gr_string(int x, int y, char *s )
{
	...
	returnme = gr_internal_string_clipped( x, y, s );
	return returnme;
}

#if defined(WARP3D) || defined(VIRGIN)
int gr_direct_string(int x, int y, char *s )
{
	int w, h, aw;
	int clipped=0;
	int returnme;
	...
	if ( !clipped )
		return gr_direct_ustring(x, y, s );
...
int gr_briefing_string(int x, int y, char *s )
{
	int w, h, aw;
	int clipped=0;
	int returnme;
	...
	if ( !clipped )
		return gr_briefing_ustring(x, y, s );
...
int gr_ustring(int x, int y, char *s )
{
	...
	if (FFLAGS & FT_COLOR) {
		returnme =  gr_internal_color_string(x,y,s);
		return returnme;

	}
	else
		if ( BG_COLOR == -1) {
			returnme =  gr_internal_string0m(x,y,s);
			return returnme;
		}
		else {
			returnme = gr_internal_string0(x,y,s);
			return returnme;
		}
...
#if defined(WARP3D) || defined(VIRGIN)
int gr_direct_ustring(int x, int y, char *s )
{
	...
	if (FFLAGS & FT_COLOR) {
		...
		if ( BG_COLOR == -1) {
			returnme =  gr_direct_internal_string0m(x,y,s);
			return returnme;
		}
		else {
			returnme = gr_direct_internal_string0(x,y,s);
			return returnme;
		}
...
int gr_briefing_ustring(int x, int y, char *s )
{
	...
	if (FFLAGS & FT_COLOR) {
		...
		if ( BG_COLOR == -1) {
			returnme =  gr_briefing_internal_string0m(x,y,s);
			return returnme;
		}
		else {
			returnme = gr_briefing_internal_string0(x,y,s);
			return returnme;
		}
...
#if defined(WARP3D) || defined(VIRGIN)
int gr_direct_printf( int x, int y, char * format, ...)
{
	char buffer[1000];
	va_list args;
	...
	va_start(args, format);
	vsprintf(buffer, format, args);
	return gr_direct_string(x,y,buffer);
}

int gr_briefing_printf( int x, int y, char * format, ...)
{
	char buffer[1000];
	va_list args;
	...
	va_start(args, format);
	vsprintf(buffer, format, args);
	return gr_briefing_string(x,y,buffer);
}
...
/*#ifdef SWAP_0_255*/
		colormap[TRANSPARENCY_COLOR] = 0;    // TRANSPARENCY_COLOR;
/*#else
		colormap[255] = 255;
#endif*/
