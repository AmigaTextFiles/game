...
*/
/*
 * $Source: /usr/CVS/descent/2d/disc.c,v $
 * $Revision: 1.3 $
 * $Author: nobody $
 * $Date: 1998/11/09 22:20:07 $
 *
 * Graphical routines for drawing a disk.
 *
 * $Log: disc.c,v $
 * Revision 1.3  1998/11/09 22:20:07  nobody
 * *** empty log message ***
 *
 * Revision 1.2  1998/03/31 17:03:00  hfrieden
 * Added ViRGE direct rendering stuff
 *
...
int gr_direct_disk(fix xc1,fix yc1,fix r1)
{
	int p,x, y, xc, yc, r;
	...
	{
		// Draw the first octant
		gr_direct_scanline( xc-y, xc+y, yc-x );
		gr_direct_scanline( xc-y, xc+y, yc+x );
		...
		if (p<0)
			...
			// Draw the second octant
			gr_direct_scanline( xc-x, xc+x, yc-y );
			gr_direct_scanline( xc-x, xc+x, yc+y );
			p=p+((x-y)<<2)+10;
			y--;
		}
