...
#else
UWORD * scale_dest_ptr;
#endif

#if defined(VIRGIN) || defined(WARP3D)
#define c2px(c) BitValues[c]
#define PSIZE 2
#else
#define c2px(c) (c)
#define PSIZE 1
...
void scale_up_bitmap(grs_bitmap *source_bmp, grs_bitmap *dest_bmp, int x0, int y0, int x1, int y1, fix u0, fix v0,  fix u1, fix v1  )
{
	...
	extern int VirgeBytesPerRow;
#elif defined(WARP3D)
	UWORD *RenderBuffer = (UWORD *)WARP_GetBufferAddress();
#endif
	...
	#else
		scale_dest_ptr = RenderBuffer + (VirgeBytesPerRow/2)*y+x0;
	#endif
...
void rls_stretch_scanline()
{
	...
	for (j=0; j<scale_ydelta_minus_1; j++)  {
		...
		#else
			for (i=0; i<len; i++) *scale_dest_ptr++ = c2px(c);
		#endif
		} else {
			scale_dest_ptr += len;
		}
...
void scale_row_asm_transparent( ubyte * sbits, ubyte * dbits, int width, fix u, fix du )
{
	...
	ubyte c;
	#if defined(VIRGIN) || defined(WARP3D)
	ubyte *dbits_end = &dbits[2*(width-1)];
	#else
...
void scale_bitmap_c_rle(grs_bitmap *source_bmp, grs_bitmap *dest_bmp, int x0, int y0, int x1, int y1, fix u0, fix v0,  fix u1, fix v1  )
{
	...
	for (y=y0; y<=y1; y++ )         {
		...
		scale_row_asm_transparent( scale_rle_data,
		#if defined(VIRGIN) || defined(WARP3D)
			(ubyte *)(RenderBuffer + (VirgeBytesPerRow/2)*y+x0),
		#else
			&dest_bmp->bm_data[dest_bmp->bm_rowsize*y+x0],
		#endif
			x1-x0+1, u0, du );
		v += dv;
	}
