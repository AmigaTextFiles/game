...
*/
/*
 * $Source: /usr/CVS/descent/2d/rle.c,v $
 * $Revision: 1.5 $
 * $Author: nobody $
 * $Date: 1998/09/26 15:05:00 $
 * 
 * Routines to do run length encoding/decoding
 * on bitmaps.
 * 
 * $Log: rle.c,v $
 * Revision 1.5  1998/09/26 15:05:00  nobody
 * Added Warp3D support
 *
 * Revision 1.4  1998/03/30 18:28:47  hfrieden
 * *** empty log message ***
 *
 * Revision 1.3  1998/03/22 15:51:01  hfrieden
 * ViRGE direct RLE stuff added
 *
 * Revision 1.2  1998/03/18 23:19:50  tfrieden
 * Profiling
 *
...
static char rcsid[] = "$Id: rle.c,v 1.5 1998/09/26 15:05:00 nobody Exp $";
...
void rle_direct_stosb(UWORD *dest, int len, int color)
{
	int i;
	for (i=0; i<len; i++ )
		*dest++ = BitValues[color];
}

// Given pointer to start of one scanline of rle data, uncompress it to
// dest, from source pixels x1 to x2.
void gr_direct_rle_expand_scanline_masked( UWORD *dest, ubyte *src, int x1, int x2  )
{
	int i = 0;
	ubyte count;
	ubyte color;
	...
	if ( x1+count > x2 )    {
		count = x2-x1+1;
		if ( color != TRANSPARENCY_COLOR )  rle_direct_stosb( dest, count, color );
		return;
	}
...
#if defined(VIRGIN) || defined(WARP3D)
void gr_direct_rle_expand_scanline( UWORD *dest, ubyte *src, int x1, int x2  )
{
	int i = 0;
	ubyte count;
	ubyte color;
	...
	if ( x1+count > x2 )    {
		count = x2-x1+1;
		rle_direct_stosb( dest, count, color );
		return;
	}
...
void gr_rle_expand_scanline_generic_dead_code( grs_bitmap * dest, int dx, int dy, ubyte *src, int x1, int x2  )
{
	int i = 0, j;
	int count;
	ubyte color;
...
#ifdef VIRGIN
	VirgeInvalidCacheEntry(rle_cache[least_recently_used].expanded_bitmap);
#endif
#ifdef WARP3D
	WARP_InvalidCacheEntry(rle_cache[least_recently_used].expanded_bitmap);
#endif
