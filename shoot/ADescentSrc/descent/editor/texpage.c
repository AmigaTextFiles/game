...
*/
/*
 * $Source: /usr/CVS/descent/editor/texpage.c,v $
 * $Revision: 1.2 $
 * $Author: nobody $
 * $Date: 1998/09/26 15:08:40 $
 * 
 * Routines for displaying texture pages
 * 
 * $Log: texpage.c,v $
 * Revision 1.2  1998/09/26 15:08:40  nobody
 * Added Warp3D support
 *
 * Revision 1.1  1998/08/11 14:33:40  nobody
 * Moved editor directory to root
 *
 * Revision 1.3  1998/08/09 23:05:25  tfrieden
 * Still trying to get the textures
 *
 * Revision 1.2  1998/08/08 16:03:06  nobody
 * Activated Editor
 *
...
static char rcsid[] = "$Id: texpage.c,v 1.2 1998/09/26 15:08:40 nobody Exp $";
...
static void texpage_print_name( char name[13] ) 
{
	...
	for (i=strlen(name);i<12;i++)
		name[i]=' ';
	name[i]=0;
	name[10] = 0;  // Barely enough space for 9 characters....
...
//    editor_status("Number of tmaps: %d/%d, showing page %d", Num_tmaps, NumTextures, TexturePage);

	for (i=0;  i<TMAPS_PER_PAGE; i++ )
		{
		debug(("%d ", i));
		gr_set_current_canvas(TmapBox[i]->canvas);
		if (i+TexturePage*TMAPS_PER_PAGE < NumTextures) {       //Num_tmaps )  {
...
{
	editor_status("Selected %s (%d)", TmapInfo[CurrentTexture].filename, CurrentTexture);
...
void texpage_do()
{
	...
	for (i=0; i<TMAPS_PER_PAGE; i++ ) {
		if (TmapBox[i]->b1_clicked && (i+TexturePage*TMAPS_PER_PAGE < NumTextures)) {
			...
			CurrentTexture = CurrentTmap; //TmapList[CurrentTmap];
			texpage_show_current();

			if (keyd_pressed[KEY_LSHIFT]) {
				debug(("Will replace CurrentTexture (%i) with...(select by pressing Ctrl)\n", CurrentTexture));
				Replacement_list[Num_replacements].old = CurrentTexture;
			}

			if (keyd_pressed[KEY_LCTRL]) {
				debug(( "...Replacement texture for %i is %i\n", Replacement_list[Num_replacements].old, CurrentTexture));
