...
*/
/*
 * $Source: /usr/CVS/descent/editor/objpage.c,v $
 * $Revision: 1.2 $
 * $Author: nobody $
 * $Date: 1998/09/26 15:08:13 $
 * 
 * object selection stuff.
 * 
 * $Log: objpage.c,v $
 * Revision 1.2  1998/09/26 15:08:13  nobody
 * Added Warp3D support
 *
 * Revision 1.1  1998/08/11 14:33:34  nobody
 * Moved editor directory to root
 *
 * Revision 1.2  1998/08/08 16:02:58  nobody
 * Activated Editor
 *
...
static char rcsid[] = "$Id: objpage.c,v 1.2 1998/09/26 15:08:13 nobody Exp $";
...
draw_robot_picture(int id, vms_angvec *orient_angles, int type)
{
	...
	switch (type) {
		...
		case OL_HOSTAGE:
			...
			gr_bitmapm(0,0,&GameBitmaps[Vclip[Hostage_vclip_num[id]].frames[0].index]);
			break;

		case OL_POWERUP:
			if ( Powerup_info[id].vclip_num > -1 )  {
				PIGGY_PAGE_IN(Vclip[Powerup_info[id].vclip_num].frames[0]);
				gr_bitmapm(0,0,&GameBitmaps[Vclip[Powerup_info[id].vclip_num].frames[0].index]);
			}
			break;
