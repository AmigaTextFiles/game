/* AHI SoundIt support V0.2 by Juha Niemimäki
...
/* AHI.device */
#	include <proto/ahi.h>
	#define AHI_CHANNELS 4 /* We could do more but not sure if needed... */
	
    struct Channel {
        struct AHIRequest *audio;
        /* Marks IORequest as "used" so that it can be stopped when needed */
        BOOL mask;
    };

    static struct Channel chans[AHI_CHANNELS] = { 0 };
...
#endif
#elif defined(__MORPHOS__)
  if (buf = (unsigned char *)AllocVecTaskPooled(i))
  {
#else /* M68k */
...
/* free MsgPorts and IOAudios of alls channels, close audio.device */
{
	...
	if ( AHIDevice == 0 && chans[0].audio )
	{
        for (i = 0; i < AHI_CHANNELS; i++)
        {
            stop_channel(i);
        }

        CloseDevice( (struct IORequest*)chans[0].audio );
        AHIDevice = -1;
    }

    for ( i = 0; i < AHI_CHANNELS; i++ )
    {
        DeleteIORequest( (struct IORequest*) chans[i].audio );
        chans[i].audio = NULL;
	}
...
if ( ch <  0 || ch >= num_chans )
    ...
    if ( AHIDevice == 0 )
    {
        if ( chans[ch].audio && chans[ch].mask )
        {
            chans[ch].mask = FALSE;
...
/* The Sample structure must have already been initialized. */
{
    ...
    playback_period = frequency;

    if ( channels > AHI_CHANNELS )
    {
        fprintf(stderr, "%sOnly 4 channels supported\n");
        return( 0 );
    }
    ...
    for ( i = 0; i < AHI_CHANNELS; i++ )
    {
        /* Allocate AHIRequest for each channel */
        /* Fails if ahi_port is NULL */
    	if ( !( chans[i].audio = (struct AHIRequest*) CreateIORequest( ahi_port, sizeof(struct AHIRequest) ) ) )
    	{
            fprintf(stderr,"%sCouldn't create AHIRequest %d\n", i);
            return( 0 );
    	}

    }

	chans[0].audio->ahir_Version = 4;

	if ( ( AHIDevice = OpenDevice(AHINAME, 0, (struct IORequest*)chans[0].audio, 0 ) ) )
	{
     	 fprintf(stderr, "%sCouldn't open AHI.device\n");
     	 return( 0 );
	}
	else
	{
        /* Setup requests... */
	    for ( i = 1; i < AHI_CHANNELS; i++ )
	    {
    	    CopyMem( chans[0].audio, chans[i].audio, sizeof(struct AHIRequest) );
	    }
        ...
        for (i=0; i<num_snd; i++) {
        	...
        	if (!(sa[i].cdata = get_chip_sample(&sa[i]))) {
                fprintf(stderr,"%sCan't alloc RAM for sample %d.\n",i);
                free_chip();
                freechans();
                return 0;
            }
...
int Snd_restore(void)
{
  int i;
#ifdef __USE_AHI__
    for (i=0; i < AHI_CHANNELS; stop_channel(i++));
#else
...
int Snd_effect(int sound_num, int channel)
{
	...
	Sample *s;
  	...
  	if (channel<0 || channel>=AHI_CHANNELS || sound_num < 0 || sound_num >= num_sounds || AHIDevice)
		return (1);

  	if (sounds[sound_num].data == NULL || sounds[sound_num].cdata == NULL) {
    	fprintf(stderr,"Snd_effect: Referencing NULL sound entry in sound #%d.\n",sound_num);
    	return (1);
  	}

    // Mark channel as used...
    if ( chans[channel].mask )
    	stop_channel(channel);

    chans[channel].mask = TRUE;

    ioa = chans[channel].audio;
    s = &sounds[sound_num];

    ioa->ahir_Std.io_Message.mn_Node.ln_Pri = 0;
    ioa->ahir_Std.io_Command = CMD_WRITE;
    ioa->ahir_Std.io_Data = s->cdata;
    ioa->ahir_Std.io_Length = s->len;
    ioa->ahir_Std.io_Offset = 0;
    ioa->ahir_Frequency = playback_period;
    ioa->ahir_Type = AHIST_M8S;
    ioa->ahir_Volume = 0x10000;
    ioa->ahir_Position = 0x8000;
    ioa->ahir_Link = NULL;
