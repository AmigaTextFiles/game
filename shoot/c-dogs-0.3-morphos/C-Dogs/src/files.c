...
-------------------------------------------------------------------------------
 ...
 Rev:    $Revision: 69 $
 URL:    $HeadURL: svn://svn.icculus.org/cdogs-sdl/tags/cdogs-sdl-0.3/src/files.c $
 ID:     $Id: files.c 69 2006-08-15 11:15:35Z lmartinking $
...
char * GetConfigFilePath(const char *name)
{
	...
	strcpy(tmp, homedir);
#else
	tmp = calloc(strlen(CDOGS_CFG_DIR) + strlen(name) + 1, sizeof(char));
#endif
