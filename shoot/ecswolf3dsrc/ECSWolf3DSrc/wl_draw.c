...
static unsigned long colc, c2, f2;
static unsigned int ceiling;
...
static void ClearScreen()
{
	...
	if (viewwidth==320) 
	{
		...
		if(colourDepth == 5) {
			FastFillScreen((byte*)(gfxbuf+51200), colourTable[colf]);
			FastFillScreen((byte*)(gfxbuf+25600), colourTable[colc]);
		}
		return;
	}
...
static void ScaledDraw(byte *gfx, byte count, byte *vid, unsigned int frac, unsigned int delta)
{
	if(colourDepth == 5) {
		while (count--) {
			*vid = colourTable[gfx[frac>>16]];
			vid += 320;
			frac += delta;
		}
	}
...
/*
__inline void __asm ScaledDraw (register __a3 byte *gfx,
		       register __d2 byte count,
		       register __a2 byte *vid,
		       register __d3 unsigned int frac,
		       register __d4 unsigned int delta);
		       
__inline void __asm ScaledDrawTrans (register __a3 byte *gfx,
		       register __d2 byte count,
		       register __a2 byte *vid,
		       register __d3 unsigned int frac,
		       register __d4 unsigned int delta);
...
void ScaleShape(short xcenter, short shapenum, unsigned short height)
{
	unsigned int scaler =frach[((short)height)>>2];
	unsigned int x, p;
...
void SimpleScaleShape(short xcenter, short shapenum, unsigned short height)
{
	unsigned int scaler = frach[height];
	unsigned int x, p;
