...
static struct Mode_Screen *msptr = NULL;
static BOOL video_is_chunkyppc = FALSE;
#endif
...
#ifdef CHUNKYPPC
static unsigned int chunkyppc_time = 0;
#endif
...
/****************************************************************************/
static INLINE unsigned int end_timer (void)
{
...
#ifdef CHUNKYPPC
static void I_InitGraphicsCPPC (void)
{
#ifdef WARPUP
	if ((ChunkyPPCBase = OpenLibrary("chunkyppc.library", 10)) == NULL)
			I_Error("OpenLibrary(""chunkyppc.library"", 10) failed");
#endif

	ms.SCREENWIDTH = SCREENWIDTH;
	ms.SCREENHEIGHT = SCREENHEIGHT; 
	ms.MS_MAXWIDTH = 1600;
	ms.MS_MAXHEIGHT = 1280;
	ms.MINDEPTH = 8;
	ms.MAXDEPTH = 8;
	ms.video_screen = 0;
	ms.video_window = 0;
...
#ifdef WARPUP
	if (!(msptr = OpenGraphics("ADoomWOS", &ms, 2)))
#else
	if (!(msptr = OpenGraphics("ADoomPPC", &ms, 2)))
#endif
		I_Error("OpenGraphics() failed");

	if (!(ChunkyInit(&ms, PIXFMT_LUT8)))
		I_Error("ChunkyInit() failed");

	printf ("Screen Mode $%08x is ", ms.mode);
  printf ("8-BIT");
 	printf (" ChunkyPPC ");
	if (ms.numbuffers == 1)
		printf ("Single Buffered");
	else if (ms.numbuffers == 2)
		printf ("Double Buffered");
	else
		printf ("Triple Buffered");
  printf ("\n");
...
void I_InitGraphics (void)
{
    ...
    if (AslBase == NULL) {
...
#ifdef CHUNKYPPC
		video_is_chunkyppc = M_CheckParm ("-chunkyppc");
		if (video_is_chunkyppc) {
			I_InitGraphicsCPPC();
		} else {
#endif
#ifdef GRAFFITI
  	if (video_graffiti != 0) {
    	...
    	if ((nbytes = GetDisplayInfoData (handle, (UBYTE *)&dispinfo,
                                      	...
                                      	0)) < 40 /*sizeof(struct DisplayInfo)*/) {
    	...
    	if (video_is_native) {
      	...
      	SBuffer[1] = AllocScreenBuffer (video_screen, NULL, 0);
      	SBuffer[2] = AllocScreenBuffer (video_screen, NULL, 0);
  ...
  } else if (M_CheckParm("-nojoy") == 0) {

    if ((gameport_mp = CreateMsgPort ()) == NULL ||
        (gameport_io = (struct IOStdReq *)CreateIORequest (gameport_mp,
  ...
  total_frames++;
	...
	if (video_is_chunkyppc) {
		if (video_palette_changed !=0) {
			LoadColors(&ms,video_colourtable[video_palette_index]);
		...
		if (ms.video_is_native_mode) {
			switch (ms.numbuffers) {
				case 1:
					CallChunkyCopy(&ms, (void *)ms.bitmapa, screens[0], PIXFMT_LUT8, 0, 0);
					break;
				case 2:
					if (ms.bufnum == 0)
						CallChunkyCopy(&ms, (void *)ms.bitmapb, screens[0], PIXFMT_LUT8, 0, 0);
					else
						CallChunkyCopy(&ms, (void *)ms.bitmapa, screens[0], PIXFMT_LUT8, 0, 0);
					DoubleBuffer(&ms);
					break;
				case 3:
					if (ms.bufnum == 0)
						...
						CallChunkyCopy(&ms, (void *)ms.bitmapc, screens[0], PIXFMT_LUT8, 0, 0);
					else
						CallChunkyCopy(&ms, (void *)ms.bitmapa, screens[0], PIXFMT_LUT8, 0, 0);
					DoubleBuffer(&ms);
					break;
			...
			switch (ms.numbuffers) {
				case 1:
					CallChunkyCopy(&ms, ms.screen, screens[0], PIXFMT_LUT8, 0, 0);
					break;
				case 2:
					if (ms.bufnum == 0)
						CallChunkyCopy(&ms, ms.screenb, screens[0], PIXFMT_LUT8, 0, 0);
					else
						CallChunkyCopy(&ms, ms.screen, screens[0], PIXFMT_LUT8, 0, 0);
					DoubleBuffer(&ms);
					break;
				case 3:
					if (ms.bufnum == 0)
						...
						CallChunkyCopy(&ms, ms.screenc, screens[0], PIXFMT_LUT8, 0, 0);
					else
						CallChunkyCopy(&ms, ms.screen, screens[0], PIXFMT_LUT8, 0, 0);
					DoubleBuffer(&ms);
					break;
...
#ifdef CHUNKYPPC
		calc_time (chunkyppc_time, "ChunkyPPC time        ");
#endif
#ifdef PROFILE
    {
      int i;
