...
/* data structure storing extension functions */
...
/*
 * Return a malloc()'ed string containing all the extensions.
 */
...
void *alGetProcAddress(const ALubyte *fname) {
	...
	debug(ALD_EXT, __FILE__, __LINE__,
 ...
 * _alInitExtensions
 *
 * initialize extensions bookkeeping structures.
 *
 * Not much to do here, since the structures are built
 * via the register calls.  So just init the mutex.
 */
...
/*
 * add a function with name "name" and address "addr" to the tree
 * with root at treehead.
 *
...
/*
 *  Destroy the tree with root at treehead, calling ff on each
 *  data item.
 */
static void tree_free(enode_t *treehead, void (*ff)(void *)) {
	enode_t *temp;
...
ALboolean _alLoadDL(const char *fname) {
	...
	/* FIXME: need to add code to call fini_func */
...
ALboolean lal_addFreqFilter(const char *name, freq_filter *addr) {
	...
	fffs = cc->freq_filters;

	for(i = 0; (i < _ALC_MAX_FILTERS) && (fffs->filter != NULL); i++) {
		scmp = strncmp(fffs[i].name, name, _ALF_MAX_NAME);
		if(scmp == 0) {
			/* overwrite existing filter */
			fffs[i].filter = addr;
	...
	/* add new filter */
	strncpy(fffs[i].name, name, _ALF_MAX_NAME);
	fffs[i].filter = addr;
...
/*
 * Save fini func to call on dlclose time
 */
static void _alExtPushFiniFunc(void (*func)(void)) {
...
/**
 * Extension support.
 * Obtain the integer value of an enumeration (usually an extension)
 * with the name ename. 
 *
 * FIXME: kludgey
 */
ALenum alGetEnumValue(const ALubyte *ename) {
	...
	if(ustrcmp("AL_GAIN_LINEAR", ename) == 0) {
		return AL_GAIN_LINEAR;
	}
	...
	if(ustrcmp("AL_PLAY_COUNT", ename) == 0) {
		return AL_PLAY_COUNT;
	}
	...
	if(ustrcmp("AL_BUFFERS_USED", ename) == 0) {
		return AL_BUFFERS_USED;
	}
	...
	if(ustrcmp("AL_DOPPLER_SCALE", ename) == 0) {
		return AL_DOPPLER_SCALE;
	}

	if(ustrcmp("AL_DOPPLER_SHIFT", ename) == 0) {
		return AL_DOPPLER_SHIFT;
	}
	...
	if(ustrcmp("AL_DISTANCE_ATTENUATION", ename) == 0) {
		return AL_DISTANCE_ATTENUATION;
	}

	if(ustrcmp("AL_PROPAGATION_SPEED", ename) == 0) {
		return AL_PROPAGATION_SPEED;
	}
	...
	if(ustrcmp("AL_ENY_ROOM_ROLLOFF_FACTOR_IASIG", ename) == 0) {
		return AL_ENY_ROOM_ROLLOFF_FACTOR_IASIG;
	}
