...
static int write_fd    = -1; /* /dev/dsp file write descriptor */
...
void *grab_write_native(void) {
	...
	int divisor = spot(_ALC_DEF_BUFSIZ) | (1<<16);
...
#if 0
static void EvenPathify(char *path) {
	int last;
...
void capture_nativedevice(UNUSED(void *handle),
	...
	if(can_read == AL_TRUE) {
		if(set_fd(read_fd, AL_TRUE, bufsiz, &linformat, speed, &channels) >= 0) {
			...
			*fmt = LIN2ALFMT(linformat, channels);
			...
			fprintf(stderr, "set_read: (speed %d format 0x%x)\n",
				*speed, *fmt );
