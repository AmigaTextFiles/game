...
// Most ingenious. The old rename ploy. I must congratulate you.
// If there's one thing i enjoy, it's a good rename ploy.
...
byte *Image_LoadPNG (FILE *fin, char *filename, int matchwidth, int matchheight) {
    ...
    //Com_Printf("Image_LoadPNG: %s\n",filename);
#ifndef __AROS__
	if (!png_handle)
    {
        Com_Printf("png_handle == NULL");		
        return NULL;
    }
#endif
    ...
    {
        Com_Printf("Couldn't open file. fin: %d, filename %s",fin,filename);
		return NULL;
    }
