...
//------------------------------------------------------------------------------
#ifdef __MORPHOS__
const char *version_tag = "$VER: OilWar 1.2.1 (19.09.2005)";
#endif
...
int main (int argc, char *argv[])
{
	try
	{
		...
		// Set video mode
#ifdef __MORPHOS__
		pScreen = SDL_SetVideoMode (640, 480, 16, SDL_SWSURFACE);
#else
		...
		while ( Running )
		{
			...
			if ( ShowGameOver )
			{
				...
				{
					// Oil
					ParticleSystems.push_back (cParticleSystem (432, 270, 250, 
						MakeColor (95, 74, 40), MakeColor (0, 0, 0), 2, 40, 1000, true));
					ParticleSystems.push_back (cParticleSystem (541, 275, 250, 
						MakeColor (90, 70, 45), MakeColor (0, 0, 0), 2, 40, 1000, true));
...
//------------------------------------------------------------------------------
bool 
UnitData::Hit (int mouseX, int mouseY, int w, int h)
{
...
//------------------------------------------------------------------------------
bool 
UnitData::ExplosionHit (int explosionX, int explosionY)
{
...
{
	ParticleSystems.push_back (cParticleSystem (x, y, Blood1_Particles,
			Blood1_Start, Blood1_End,
			Blood1_Size, Blood1_Power, Blood1_MaxEnergy));
	ParticleSystems.push_back (cParticleSystem (x, y, Blood2_Particles,
			Blood2_Start, Blood2_End,
			Blood2_Size, Blood2_Power, Blood2_MaxEnergy));
}
//==============================================================================
...
{
	ParticleSystems.push_back (cParticleSystem (x, y, Explosion1_Particles,
			Explosion1_Start, Explosion1_End,
			Explosion1_Size, Explosion1_Power, Explosion1_MaxEnergy));
	
	ParticleSystems.push_back (cParticleSystem (x, y, Explosion2_Particles,
			Explosion2_Start, Explosion2_End,
			Explosion2_Size, Explosion2_Power, Explosion2_MaxEnergy));
	
	ParticleSystems.push_back (cParticleSystem (x, y, Explosion3_Particles,
			Explosion3_Start, Explosion3_End,
...
{
	ParticleSystems.push_back (cParticleSystem (x, y, Sand_Particles,
			Sand_Start, Sand_End,
			Sand_Size, Sand_Power, Sand_MaxEnergy));
}
//==============================================================================
...
void LoadHighscores (void)
{
	...
	if ( pHighscoreFile == NULL )
	{
		...
		if ( pHighscoreFile == NULL )
		{
			cerr << "Unable to open highscore file \'oilwar.scores\' or \'"
