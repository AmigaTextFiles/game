...
//CASULL
void Weapon_Casull_Fire (edict_t *ent)
{
	vec3_t		start;
...
//		gi.sound(ent, CHAN_VOICE, gi.soundindex("weapons/noammo.wav"), 1, ATTN_NORM, 0);
	...
	gi.sound(ent, CHAN_WEAPON, gi.soundindex("slat/weapons/casull_fire.wav"), 1, ATTN_NORM, 0);
	...
	{
		if (ent->client->bleeding)
			fire_bullet (ent, start, forward, damage, kick, DEFAULT_BULLET_HSPREAD*0.7, DEFAULT_BULLET_VSPREAD*0.7, MOD_GLOCK);
		else
			fire_bullet (ent, start, forward, damage, kick, DEFAULT_BULLET_HSPREAD*0.3, DEFAULT_BULLET_VSPREAD*0.3, MOD_GLOCK);
	}
...
void Weapon_Casull (edict_t *ent)
{
	...
	if (ent->client->ps.gunframe == 43)
		gi.sound(ent, CHAN_AUTO, gi.soundindex("slat/weapons/casull_reload1.wav"), 1, ATTN_NORM, 0);//TROND lyd
	if (ent->client->ps.gunframe == 52)
		gi.sound(ent, CHAN_AUTO, gi.soundindex("slat/weapons/casull_reload2.wav"), 1, ATTN_NORM, 0);//TROND lyd

	Weapon_Generic (ent, 3, 11, 32, 36, 40, 60, pause_frames, fire_frames, Weapon_Casull_Fire);
}