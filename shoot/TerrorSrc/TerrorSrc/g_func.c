...
//TROND bug fix tatt vekk
/*		if (other)
			BecomeExplosion1 (other);*/
		if (other->classname == "explosive")
			BecomeExplosion1 (other);
//TROND slutt
		return;
	}
...
{
	if (other->client && other->client->dooropen )
	{
	...
	if (level.time < self->touch_debounce_time)
		return;
	self->touch_debounce_time = 0;//TROND var level.time + 1.0

	//TROND
	//OPNE DØR
	if (other->client)
	{
		//har clienten trykka "open?"
		if (other->client->dooropen == 0)
		{
			//Ja? bra...
			//door_use (self->owner, other, other);
			//other->client->dooropen = 0;
			return;
		}
		else
		{
...
void door_blocked  (edict_t *self, edict_t *other)
{
	...
	{
		// give it a chance to go away on it's own terms (like gibs)
		//TROND tatt vekk skade av dører
		//T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, 100000, 1, 0, MOD_CRUSH);
		// if it's still there, nuke it
//TROND bug fix med eksploderande våpen ol
//		if (other)
//			BecomeExplosion1 (other);
		if (other->classname == "explosive")
			BecomeExplosion1 (other);
