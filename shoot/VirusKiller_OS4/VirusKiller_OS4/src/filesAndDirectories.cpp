...
/* Cyborg 10/25/2005 */
		...
		/* On AmigaOS rootdirs have a trailing colon ':' and not
		   a trailing slash '/' ! So we can't just always add a
		   slash in front of dfile->d_name but only if the last
		   character of 'filename' is NO ':' or '/'.

		   Usually we would just use IDOS->AddPart() and the show
		   could go on, but unfortunately the game source already
		   defines a class 'List' which collides with the 'struct
		   List' definition of the AmigaDOS headers. So we can't
		   include them and therefore can't use DOS safely. Well,
		   then back to the roots and only standard lib functions
		   used to emulate the AddPart() behaviour.

		   !! WARNING: quick&dirty code follows !!
		*/
 
		strcpy(filename, dirName);

		int myTempPos = strlen(filename);

		if((filename[myTempPos-1] == ':') || (filename[myTempPos-1] == '/'))
		...
		sprintf(filename, "%s/%s", dirName, dfile->d_name);
#endif
		debug(("Trying to open '%s' as directory\n", filename));
