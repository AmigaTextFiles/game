...
#if ( ( defined __linux__ ) && ( defined __powerpc__ ) ) || defined(__MORPHOS__) //|| (defined MACOS_X)
...
vm_t *VM_Restart( vm_t *vm ) {
	...
	if ( vm->dllHandle ) {
		...
		if (vm->datasegmentcopy)
		{
			Sys_DeinitDll(vm->dllHandle);

			memcpy(vm->sos.sdata_segment_start, vm->sdatasegmentcopy, vm->sos.sdata_segment_size);
			memcpy(vm->sos.data_segment_start, vm->datasegmentcopy, vm->sos.data_segment_size);
			memset(vm->sos.sbss_segment_start, 0, vm->sos.sbss_segment_size);

			Sys_InitDll(vm->dllHandle, VM_DllSyscall);
...
vm_t *VM_Create( const char *module, int ( *systemCalls )(int *),
	...
	if ( interpret == VMI_NATIVE ) {
		...
		vm->dllHandle = Sys_LoadDll( module, vm->fqpath, &vm->entryPoint, &vm->sos );
		if ( vm->dllHandle ) {
			ok = 1;

			if (vm->sos.sdata_segment_size && vm->sos.data_segment_size)
			{
				vm->sdatasegmentcopy = malloc(vm->sos.sdata_segment_size);
				vm->datasegmentcopy = malloc(vm->sos.data_segment_size);

				if (vm->sdatasegmentcopy && vm->datasegmentcopy)
				{
					memcpy(vm->sdatasegmentcopy, vm->sos.sdata_segment_start, vm->sos.sdata_segment_size);
					memcpy(vm->datasegmentcopy, vm->sos.data_segment_start, vm->sos.data_segment_size);
				}
				else
				{
					ok = 0;
			...
			if (ok && Sys_InitDll(vm->dllHandle, VM_DllSyscall))
				return vm;

			free(vm->sdatasegmentcopy);
			vm->sdatasegmentcopy = 0;

			free(vm->datasegmentcopy);
			vm->datasegmentcopy = 0;
...
int QDECL VM_Call( vm_t *vm, int callnum, ... ) {
	...
	if ( vm->entryPoint ) {
		//rcg010207 -  see dissertation at top of VM_DllSyscall() in this file.
#if ( ( defined __linux__ ) && ( defined __powerpc__ ) ) || ( defined MACOS_X ) || defined(__MORPHOS__)
