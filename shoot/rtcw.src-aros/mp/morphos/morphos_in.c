...
#else
AROS_UFP1(struct InputEvent *, myinputhandler_real,
          AROS_UFHA(struct InputEvent *,      moo,       A0)
         );
#endif
...
#else
	InputHandler.is_Code = (APTR)AROS_ASMSYMNAME(myinputhandler_real);
#endif
	inputreq->io_Data = (void *)&InputHandler;
...
#else
				if (imsgs[i].ie_Code == RAWKEY_NM_WHEEL_UP)
					ev->evValue = K_MWHEELUP;
				else if (imsgs[i].ie_Code == RAWKEY_NM_WHEEL_DOWN)
					ev->evValue = K_MWHEELDOWN;
#endif
...
static LONG lastmouse_x = 0;
static LONG lastmouse_y = 0;
static LONG currmouse_x = 0;
static LONG currmouse_y = 0;

AROS_UFH1(struct InputEvent *, myinputhandler_real,
          AROS_UFHA(struct InputEvent *,      moo,       A0)
         )
{
    AROS_USERFUNC_INIT
	...
	do
	{
		if (coin->ie_Class == IECLASS_RAWMOUSE || coin->ie_Class == IECLASS_RAWKEY || coin->ie_Class == IECLASS_NEWMOUSE)
		{
...
#if defined(__AROS__)
                if ((imsgs[imsghigh].ie_Class == IECLASS_RAWMOUSE) &&
                   !(imsgs[imsghigh].ie_Qualifier & IEQUALIFIER_RELATIVEMOUSE))
                {
                    currmouse_x = imsgs[imsghigh].ie_X;
                    currmouse_y = imsgs[imsghigh].ie_Y;
                    imsgs[imsghigh].ie_X -= lastmouse_x;
                    imsgs[imsghigh].ie_Y -= lastmouse_y;
                    lastmouse_x = currmouse_x;
                    lastmouse_y = currmouse_y;
                }                    
#endif
				imsghigh++;
				imsghigh%= MAXIMSGS;
			...
			{
#if defined(__AROS__)
			        coin->ie_Qualifier |= IEQUALIFIER_RELATIVEMOUSE;
#endif
