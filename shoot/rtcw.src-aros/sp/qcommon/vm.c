...
#if ( ( defined __linux__ ) && ( defined __powerpc__ ) ) || defined(__MORPHOS__)
...
vm_t *VM_Restart( vm_t *vm ) {
	...
	if ( vm->dllHandle ) {
		...
		if (vm->datasegmentcopy)
		{
			...
			memset(vm->sos.bss_segment_start, 0, vm->sos.bss_segment_size);

			Sys_InitDll(vm->dllHandle, VM_DllSyscall); /* Succeed or crash... Sorry :( */
...
//		Com_Printf( "Loading dll file %s.\n", vm->name );
		vm->dllHandle = Sys_LoadDll( module, &vm->entryPoint, &vm->sos );
		if ( vm->dllHandle ) {
			ok = 1;
