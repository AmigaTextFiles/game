...
void *Sys_LoadDll_Wrapped( const char *name, int( **entryPoint ) ( int, ... ), struct SharedObjectSegments *sos)
#else
...
#if defined __i386__
	snprintf( fname, sizeof( fname ), "%s_linux_i386.so", name );
#elif defined __ppc__ || defined __powerpc__   //rcg010207 - PPC support.
	snprintf( fname, sizeof( fname ), "%s_linux_ppc.so", name );
...
#define Q_RTLD    RTLD_NOW
	...
	*entryPoint = dlsym( libHandle, "vmMain" );
	...
	so_get_segment_info = dlsym(libHandle, "rtcw_so_get_segment_info");
...
void *Sys_LoadDll( const char *name, int( **entryPoint ) ( int, ... ), struct SharedObjectSegments *sos) {
	...
	ret = Sys_LoadDll_Wrapped( name, entryPoint, sos);
