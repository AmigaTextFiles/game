...
{
	return va("%s_morphos_ppc.so", name);
}
...
void *Sys_LoadDll(const char *name, int (**entryPoint)(int, ...), struct SharedObjectSegments *sos)
{
	...
	else
	{
		...
		rtcwsp_so_init = dlsym(handle, "rtcwsp_so_init");
		rtcwsp_so_deinit = dlsym(handle, "rtcwsp_so_deinit");
		dllEntry = dlsym(handle, "dllEntry");
		*entryPoint = dlsym(handle, "vmMain");

		if (rtcwsp_so_init == 0 || rtcwsp_so_deinit == 0 || dllEntry == 0 || *entryPoint == 0)
		{
