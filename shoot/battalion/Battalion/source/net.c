...
int amiga_netinit()
{
  if (!(SocketBase = OpenLibrary("bsdsocket.library",SOCKETVERSION))) {
    printf("Can't open bsdsocket.library V%d!\n",SOCKETVERSION);
    return (0);
  }
  if (SocketBaseTags(SBTM_SETVAL(SBTC_ERRNOPTR(sizeof(errno))),&errno,
          		       SBTM_SETVAL(SBTC_LOGTAGPTR),"Battalion",
          		       TAG_END)) {
    printf("Can't set errno-pointer!\n");
    CloseLibrary(SocketBase);
    return(0);
  }
  atexit(amiga_netexit);
  return (-1);
}
...
#else
    IoctlSocket(sockfd,FIONBIO,(char *)&ioctl_true);
#endif
