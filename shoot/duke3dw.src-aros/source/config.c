...
// Added write numvoices etc
// Added ipaddressarg and playernamearg
// Added Botname
// Added LcdMon
// Added (moved) UsePrecache  to Screen Setup
// Fixed null botnames
// Added PlayerColor
// Removed UseMusicDir
// Added weaponhide
...
//int32 MidiPort;
int32 ReverseStereo;
int32 RandomMusic;                                                   // wxrm
int32 UseJoystick = 0, UseMouse = 1;
int32 RunMode = 1;                                                   // wxdf
...
#else
int32 ScreenMode = 1;
int32 ScreenWidth = 1024;                                            // wxdf
int32 ScreenHeight = 768;                                            // wxdf
int32 ScreenBPP = 32;                                                // wxdf
#endif
int32 ForceSetup = 1;
short LcdMon = 1;                                                    // wxga
int   vsync;                                                         // wxvs

char IPAddressArg[8][40];                                            // wxmp
char Botname[8][32];                                                 // wxfp

static char setupfilename[256]={SETUPFILENAME};
static int32 scripthandle = -1;
static int32 setupread = 0;

extern long glusetexcachecompression;  // NB not available in build rev 415+

extern long glanisotropy, glusetexcompr, gltexfiltermode, glusetexcache;
extern long glfovscreen, glwidescreen;
...
void CONFIG_SetDefaults( void )
{
    ...
    FXDevice = 0;
    MusicDevice = 0;
    NumVoices = 32;          // DOS default = 4
    NumChannels = 2;
    NumBits = 16;
#ifdef __AROS__
    MixRate = 22050;
#else	
	MixRate = 44100;		 // DOS default = 11025
#endif    
	SoundToggle = 1;
    MusicToggle = 1;
    VoiceToggle = 1;
    ...
    ReverseStereo = 0;
    RandomMusic = 0;                                                 // wxrm
    myaimmode = ps[0].aim_mode = 1;                                  // wxdf
    glanisotropy = 1;                                                // wxnm
    glwidescreen = 0;                                                // wxnm
    glfovscreen = 8;                                                 // wxvf
    glusetexcache = 0;                                               // wxnm
    usehightile = 1;                                                 // wxnm
    usemodels = 1;                                                   // wxnm
    useprecache = 0;                                                 // wxnm
    ForceSetup = 1;                                                  // wxsm
    ...
    ud.m_ffire = 1;
    ud.levelstats = 0;
    ud.weaponicons = 1;                                              // wxdf
    ud.weaponhide = 0;                                               // wxwh
    ud.autosave = 1;                                                 // wxas
    ud.color = 0;                                                    // wxdm
    ud.fta_on = 1;         // messages
    ShowOpponentWeapons = 0;
    Bstrcpy(ud.rtsname, "DUKE.RTS");
    Bstrcpy(PlayerNameArg, "Player");                                // wxmp
    Bstrcpy(IPAddressArg[0], "127.0.0.0");                           // wxmp
...
int32 CONFIG_ReadSetup( void )
{
    ...
    SCRIPT_GetString( scripthandle, "Comm Setup","PlayerName",&PlayerNameArg[0]);
    SCRIPT_GetNumber( scripthandle, "Comm Setup","PlayerColor",&ud.color);
    if (ud.color == 1)
        ud.color = 9;
    else
    ...
    for (i=0; i<8; i++)                                              // wxmp
    {
        ...
        SCRIPT_GetString( scripthandle, "Comm Setup", buf, &IPAddressArg[i]);
        Bsprintf(buf,"Botname_%d",i+1);
        SCRIPT_GetString( scripthandle, "Comm Setup", buf, &Botname[i]);
        if (strlen(Botname[i]) < 2)                                      // wxfp
        {
            Bsprintf(buf,"Player_%d",i+1);
            strcpy(Botname[i], buf);
        }
    }
    ...
    SCRIPT_GetNumber( scripthandle, "Screen Setup", "GLUsePrecache",&useprecache);        // wxup
    ...
    SCRIPT_GetNumber( scripthandle, "Screen Setup", "GLVSync", &vsync);                   // wxvs
    ...
    SCRIPT_GetNumber( scripthandle, "Misc", "Crosshairs",&dummy);
    if (dummy > 2 || dummy < 0)
        dummy = 1;
    ud.crosshair = dummy;
    ...
    SCRIPT_GetNumber( scripthandle, "Misc", "WeaponIcons", &ud.weaponicons);
    SCRIPT_GetNumber( scripthandle, "Misc", "WeaponHide", &ud.weaponhide);     // wxwh
    SCRIPT_GetNumber( scripthandle, "Misc", "AutoSave", &ud.autosave);         // wxas
    ...
    SCRIPT_GetNumber( scripthandle, "Sound Setup", "RandomMusic",&RandomMusic);
...
void CONFIG_WriteSetup( void )
{
    ...
    SCRIPT_PutNumber( scripthandle, "Screen Setup", "GLUsePrecache",useprecache,false,false);  // wxup
    ...
    SCRIPT_PutNumber( scripthandle, "Screen Setup", "GLFovscreen",glfovscreen,false,false); // wxvf
    SCRIPT_PutNumber( scripthandle, "Screen Setup", "GLVSync",vsync,FALSE,FALSE);              // wxvs
    ...
    SCRIPT_PutNumber( scripthandle, "Sound Setup", "RandomMusic",RandomMusic,false,false);

    SCRIPT_PutNumber( scripthandle, "Sound Setup", "MusicDevice",MusicDevice,false,false);  // wxsm
    SCRIPT_PutNumber( scripthandle, "Sound Setup", "NumVoices",NumVoices,false,false);      // wxsm
    SCRIPT_PutNumber( scripthandle, "Sound Setup", "NumBits",NumBits,false,false);          // wxsm
    SCRIPT_PutNumber( scripthandle, "Sound Setup", "MixRate",MixRate,false,false);          // wxsm
    ...
    SCRIPT_PutNumber( scripthandle, "Misc", "WeaponIcons", ud.weaponicons,false,false);
    SCRIPT_PutNumber( scripthandle, "Misc", "WeaponHide", ud.weaponhide,false,false);    // wxwh
    SCRIPT_PutNumber( scripthandle, "Misc", "AutoSave", ud.autosave,false,false);        // wxas
    ...
    SCRIPT_PutString( scripthandle, "Comm Setup","PlayerName",&PlayerNameArg[0]);
    SCRIPT_PutNumber( scripthandle, "Comm Setup","PlayerColor",ud.color,false,false);

    for (i=0; i<8; i++)
    {
        Bsprintf(buf,"IPAddress_%d",i+1);
        SCRIPT_PutString( scripthandle, "Comm Setup", buf, IPAddressArg[i]);
    }
    for (i=0; i<8; i++)                                              // wxmp
    {
        Bsprintf(buf,"Botname_%d",i+1);
        SCRIPT_PutString( scripthandle, "Comm Setup", buf, Botname[i]);
    }
...
void SetLastPlayed(char *smap, short x)
{
    ...
    if (x == 1)
        SCRIPT_PutString(scripthandle, "Misc", "LastMap", smap);
    else
        SCRIPT_PutString(scripthandle, "Misc", "LastGame", smap);
}
...
void GetLastPlayed(char *smap, int x)
{
    ...
    if (x == 1)
        SCRIPT_GetString(scripthandle, "Misc", "LastMap", &xbuf);
    else
        SCRIPT_GetString(scripthandle, "Misc", "LastGame", &xbuf);
