...
static int osdcmd_vars(const osdfuncparm_t *parm)
{
	...
	if (!Bstrcasecmp(parm->name, "name")) {
		if (showval) { OSD_Printf("Your name is \"%s\"\n", PlayerNameArg); }  // wxmp
		else {
			Bstrncpy(PlayerNameArg, parm->parms[0], sizeof(PlayerNameArg)-1);
			PlayerNameArg[sizeof(PlayerNameArg)-1] = 0;
...
if (VOLUMEONE) {
	...
	OSD_RegisterFunction("name","name: change your multiplayer nickname", osdcmd_vars);
