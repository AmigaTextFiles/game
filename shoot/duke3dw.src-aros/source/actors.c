...
void checkavailweapon( struct player_struct *p )
{
    ...
    if (p->weaponswitch > 1)                                        // wxws
       {
       if (weap == 9 || weap == 6 || weap < 4) // freezer, shrinker, chaingun,shotgun,pistol
          {
          if (p->ammo_amount[3] > 5)           // 1st check chaingun
              weap = 2;
          else
          if (p->ammo_amount[2] > 1)           // next the shotgun
              weap = 1;
          else
              weap = 0;                        // will inc
          }
       ...
       if (weap == 7)
          {
           if (p->ammo_amount[4] > 1)
               weap = 3;
           else
               weap = 0;
          }
       else
       if (weap == 5 || weap == 10)            // 10 = handbomb
          {
           if (p->ammo_amount[8] > 0)
               weap = 7;
           else
               weap = 0;
          }
