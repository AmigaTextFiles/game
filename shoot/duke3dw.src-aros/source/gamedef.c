...
// Added autosave - saveplayer when add health/shield
//
...
//            if(!gotz)
        ...
        case 25:
            ...
            if (ud.god == 0)
            {
                ...
                if (*insptr > 0)
                {
                    if (( j - *insptr ) < (max_player_health>>2) && j >= (max_player_health>>2))
        ...
        case 42:
            ...
            if(ud.multimode < 2)
            {
                if(ud.autodone == 1 || (lastsavedpos >= 0 && ud.recstat != 2))  // wxas
                {
                    ps[g_p].gm = MODE_MENU;
