...
// Added multiplayer name
// Added Spacebar + Open for restart level
// Added if dead with spacebar above
// Added stuff for fakeplayer
// Added clients cannot pause game
// Added weaponhide
// Changed NAM to NAMGAME
//
...
void displayweapon(short snum)
{
    ...
    {
		if(!ud.weaponhide || cw==KNEE_WEAPON || cw == HANDREMOTE_WEAPON) // wxwh
		{
			switch(cw)
			{
                case KNEE_WEAPON:
...
void getinput(short snum)
{
    ...
    int32 momx,momy;
    struct player_struct *p;
    extern int32 bHost;                                              // wxdm

    momx = momy = 0;
    p = &ps[snum];

    if (KB_KeyPressed(sc_Pause) && numplayers > 1 && !bHost)         // wxdm
    {
        KB_ClearKeyDown(sc_Pause);
        adduserquote("Clients cannot pause game.");
        return;
    }
...
if (!VOLUMEONE) {
    ...
    if (KB_KeyPressed(sc_Space) && p->dead_flag)                     // wxsp
        loc.bits |= KB_KeyPressed(sc_Space)<<29;
    else
            ...
            if( ud.multimode > 1 && ( s->pal != 1 || (s->cstat&32768) ) )
            {
                if(p->frag_ps != snum)
                {
                    ...
                    if( ud.user_name[p->frag_ps][0] != 0)
                    {
                        if(snum == screenpeek)
                        {                                            // wxmu
                            sprintf(&fta_quotes[115][0],"%s was KILLED BY %s",&ud.user_name[snum][0], &ud.user_name[p->frag_ps][0]);
                            FTA(115,p);
                        }
                        else
                        {
    ...
    else if((*kb))
    {
        switch( p->curr_weapon )
        {
            ...
            case PISTOL_WEAPON:
                ...
                if((*kb) >= 5)
                {
                    if( p->ammo_amount[PISTOL_WEAPON] <= 0 || (p->ammo_amount[PISTOL_WEAPON]%(NAMGAME?20:12)) )
                    {
                        (*kb)=0;
                        checkavailweapon(p);
                    }
            ...
            case SHOTGUN_WEAPON:
                ...
                if(*kb == 4)
                {
                    ...
                    shoot(pi,SHOTGUN);
                    shoot(pi,SHOTGUN);
                    if (!bFake || (bFake && ud.player_skill > 3))    // wxfp
                    {
                        shoot(pi,SHOTGUN);
                        shoot(pi,SHOTGUN);
                        shoot(pi,SHOTGUN);
...
void computergetinput(long snum, input *syn)
{
         ...
         (cansee(x1,y1,z1-(48<<8),damysect,x2,y2,z2-(48<<8),sprite[ps[goalplayer[snum]].i].sectnum))))
    {
        ...
        if (!bFake)                                                  // wxfp
        {
            fightdist = fdmatrix[p->curr_weapon][ps[goalplayer[snum]].curr_weapon];  // ducks behind you
            if (fightdist < 128)
                fightdist = 128;
        }
        ...
        if (!bFake || (bFake && ud.player_skill > 3)) // keeps his distance  wxfp
        {
            x3 = x2+((x1-x2)*fightdist/dist); // comes in close
        ...
        if (fightdist)
        {
            ...
            i = sintable[(j<<6)&2047];
            if (numplayers > 1 || (numplayers < 2 && ud.player_skill > 2))
            {
            ...
            if (numplayers > 1 || (numplayers < 2 && ud.player_skill > 3))
