...
// Added Spacbar interupts precaching
// Added RandomMusic
// Added MapMusic
// Fixed skill sounds
// Added stuff for fakeplayer
// 4.1.3
// modified waitforeverybody(int x)
// Added player color check
// Added bypass scoreboard on game start
// 4.1.4
...
extern char MapMusicPath[128];
extern char MusicFiles[301][32];
extern int  GetMusicFiles(void);
extern int  GetRandom(short num);
extern int  MusicDir;
...
static void tloadtile(short tilenume, char type)
{
    ...
    {
		if (!(gotpic[tilenume>>3] & pow2char[tilenume&7]))
		    precachecount++;
...
void cacheit(void)
{
	...
	for(i=0;i<MAXTILES;i++)
	{
		...
		{
		   sprintf(tempbuf,"Loading Textures - Spc to skip - %ld%%\n",min(100,100*pc/precachecount));
           if (KB_KeyPressed(sc_Space))                              // wwzz
               break;
...
void resetweapons(short snum)
{
    ...
    if (bFake)                                                       // wxfp
    {
        p->gotweapon[SHOTGUN_WEAPON] = 1;
        p->ammo_amount[SHOTGUN_WEAPON] = 24;
    }
...
void newgame(char vn,char ln,char sk)
{
    ...
    if (globalskillsound >= 0)
      while(Sound[globalskillsound].lock>=200)                     // wxss
        {
            handleevents();
            getpackets();
        }
    ...
    waitforeverybody(5);                                             // wxdm
    ready2send = 0;

//    if (ud.m_recstat != 2 && ud.last_level >= 0 && ud.multimode > 1 && ud.coop != 1) // wxdm
//        dobonus(1);
...
void resetpspritevars(char g)
{
    ...
    while(i >= 0)
    {
        ...
        if (j >= 0)
        {
            ...
            if (p_color[j] == 0 && ud.multimode > 1)                       // wxdm
            {
                if (s->pal == 0)
                {
                    s->pal = p_color[j] = ps[j].palookup = which_palookup;
                    which_palookup++;
                    if (which_palookup >= 17)
                        which_palookup = 9;
                }
                else
                    p_color[j] = ps[j].palookup = s->pal;
            }
            ...
            if (p_color[j] > 0 && ud.multimode > 1)                  // wxdm
                ps[j].palookup = sprite[ps[j].i].pal = p_color[j];
...
void waitforeverybody(int x)                                         // wxdm
{
	long i;
...
void dofrontscreens(char *statustext)
{
    ...
    if (ud.recstat != 2)
    {
       ...
       if (boardfilename[0] != 0 && ud.level_number == 7 && ud.volume_number == 0)
       {
           ...
           gametextpal(160,90+10,boardmapname,14,2);
        }
        else
        {
...
if (!VOLUMEONE)
 {
    ...
    if ( loadboard( level_file_names[ (ud.volume_number*11)+ud.level_number],0,&ps[0].posx, &ps[0].posy, &ps[0].posz, &ps[0].ang,&ps[0].cursectnum ) == -1)
    {
...
playp:
        stopmusic();
        dolower(&tempbuf);                                           // wxdl
        if (strstr(tempbuf, ".mid") || strstr(tempbuf, ".ogg") || strstr(tempbuf, ".wav"))
            playmusic(tempbuf);
        else
#ifdef _WIN32
            ShellExecute(NULL, "Open", tempbuf, NULL, MapMusicPath, SW_HIDE); //SW_SHOWNORMAL);
#else
#warning FIXME: music file opening
		{}
#endif
    }
