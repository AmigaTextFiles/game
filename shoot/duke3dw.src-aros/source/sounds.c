...
void MusicStartup( void )
   ...
   //initprintf("MusicStartup()\n");

   // Music playback is codec based, return if there's no FXDevice
   if (FXDevice < 0)
      return;
   
        sprintf(path, "%stimidity.cfg", "Timidity:");
        res = WildMidi_Init(path, MixRate, WM_MO_ENHANCED_RESAMPLING);
        if (res != 0)
        {
            sprintf(path, "%stimidity.cfg", "PROGDIR:");
            res = WildMidi_Init(path, MixRate, WM_MO_ENHANCED_RESAMPLING);
        }
    //MusicDevice = res;
    MusicDevice = FXDevice;
#endif	
}
