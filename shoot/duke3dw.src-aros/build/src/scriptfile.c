...
#warning FIXME: lowres checking	
#ifndef __AROS__
extern short iLowres;
#endif
...
char *scriptfile_gettoken(scriptfile *sf)
{
    ...
    // This is only necessary when both HRP and LRP are
    // present, but we have the LRP checkbox ticked in
    // the launcher.
#warning FIXME: lowres checking	
#ifndef __AROS__
    if (iLowres && Bstrncasecmp(start, "highres", 7) == 0)
	    sprintf(start, "lowres%s", sf->ltextptr+7);
#endif
