...
// Changed texcache folder to zcache
// Added vsync
//
...
long glfovscreen = 0;                                                // wxws
long glwidescreen = 0;                                               // wxws
long glprojectionhacks = 1;                                          // wxws
...
static char TEXCACHEDIR[] = "wcache";                                // wxwc
typedef struct {
...
float alphahackarray[MAXTILES];                                      // wxed
#include "mdsprite.c"
...
void resizeglcheck()
{
    ...
    if ((glox1 != windowx1) || (gloy1 != windowy1) || (glox2 != windowx2) || (gloy2 != windowy2))
    {
        ...
        ratio = ratio + ((float) glfovscreen / 80);
...
void drawpoly (double *dpx, double *dpy, long n, long method)
{
	...
	if (rendmode >= 3)
	{
		...
		if (!(method&3)) {
            ...
            if (alphahackarray[globalpicnum])                        // wxed
...
void polymost_drawsprite (long snum)
{
	...
	switch((globalorientation>>4)&3)
	{
		...
		case 3: //Voxel sprite
#ifdef USE_OPENGL
			if (rendmode >= 3 && (unsigned)globalpicnum < (unsigned)MAXVOXELS && voxmodels[globalpicnum])   // wxyz
...
static int osdcmd_polymostvars(const osdfuncparm_t *parm)
{
    ...
    //else if (!Bstrcasecmp(parm->name, "r_swapinterval"))             // wxvs
    /*{
...
void polymost_initosdfuncs(void)
{
	...
	//OSD_RegisterFunction("r_swapinterval","r_swapinterval: sets the GL swap interval (VSync)",osdcmd_polymostvars); // wxvs
#endif
