...
static void print_os_version(void)                                   // wxvi
{
    OSVERSIONINFO osv;
    const char *ver = NULL;
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nCmdShow)
{
	...
	startwin_open();                                                 // moved here wxkk ?

	_buildargc = SetCommandLine(argvbuf); //&_buildargv);            // wxsm
...
int SetCommandLine(char *argvbufx)                                   // wxsm
{
...
{
	va_list va;
	char buf[2000]; //,*p=NULL,*q=NULL,workbuf[1024];                // wxin

	va_start(va, f);
	Bvsnprintf(buf, 2000, f, va);                                    // wxin
	va_end(va);
	OSD_Printf(buf);
...
int setvideomode(int x, int y, int c, int fs)
{
	...
	modechange=1;
	videomodereset = 0;
	OSD_ResizeDisplay(xres,yres);                                    // wxef
...
#define ADDMODE(x,y,c,f,n) if (validmodecnt<MAXVALIDMODES) { \
	...
	/*initprintf("  - %dx%d %d-bit %s\n", x, y, c, (f&1)?"fullscreen":"windowed"); // wxvm
	} */
...
typedef void (APIENTRY *PFNWGLEXTSWAPCONTROLPROC) (int);             // wxvs
...
int setpalette(int start, int num, char *dapal)
{
	...
	{
		if (!lpDDPalette) return -1;                     // 0, 0, 256,     (LPPALETTEENTRY)lpal.palPalEntry);
		result = IDirectDrawPalette_SetEntries(lpDDPalette, 0, start, num, (PALETTEENTRY*)&lpal.palPalEntry[start]);
//		result = IDirectDrawPalette_SetEntries(lpDDPalette, 0, start, 256, (LPPALETTEENTRY)&lpal.palPalEntry[start]); // wxeb
		if (result != DD_OK)
		{
...
int setgamma(float ro, float go, float bo)
{
	...
	ro = gr / ro; go = gr / go; bo = gr / bo;                        // wxga  (was all 1.0)
...
static void UninitDirectDraw(void)
{
    ...
    bDDrawInited = FALSE;     // move here to stop crash at exit     // wxef
