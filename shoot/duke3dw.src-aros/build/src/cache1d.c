...
int addsearchpath(const char *p)
{
	...
	if (!strstr(srch->path, "Games") && !strstr(srch->path, "Autoload"))
...
int findfrompath(const char *fn, char **where)
{
    ...
    if (pathsearchmode || ((strchr(fn,'\\') != 0 || strchr(fn,'/') != 0) && strlen(fn) > 12))   // wxps
    {
	  if (access(fn, F_OK) >= 0)
      {
	     *where = strdup(fn);
...
#define MAXGROUPFILES 8     //Warning: Fix groupfil if this is changed wxgf
...
static long groupfil[MAXGROUPFILES] = {-1,-1,-1,-1,-1,-1,-1,-1};     // wxgf
