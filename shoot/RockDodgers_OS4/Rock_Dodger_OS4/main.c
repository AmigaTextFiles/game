...
#define RAND_LIST_SIZE 32767
float rand_list[RAND_LIST_SIZE];
int rand_list_pointer;
/*
...
void initrnd() {/*{{{*/
    int i;
    for (i=0; i<RAND_LIST_SIZE; i++) {
	rand_list[i] = (float)rand()/(float)RAND_MAX;
    }
    rand_list_pointer=0;
}/*}}}*/
float rnd() {/*{{{*/
    ++rand_list_pointer;
    if (rand_list_pointer>=RAND_LIST_SIZE) {
	rand_list_pointer=0;
    }
    return rand_list[rand_list_pointer];
}/*}}}*/
...
int init(int fullscreen) {/*{{{*/
    ...
    flag = SDL_DOUBLEBUF | SDL_SWSURFACE;
    if (fullscreen) flag |= SDL_FULLSCREEN;
    surf_screen = SDL_SetVideoMode(XSIZE,YSIZE,16,flag);

    // Set the title bar text
    SDL_WM_SetCaption("Rock Dodgers OS4 Ported by ToAks", "rockdodgers");
...
int gameloop() {/*{{{*/
	...
	if (!paused) {
		...
		while (countdown<0) {
			...
			rockptr->type_number = rand() % NROCKS;
			rockptr->heat = 0;
			rockptr->image = surf_rock[rockptr->type_number];// [rand()%NROCKS];
			rockptr->active = 1;
