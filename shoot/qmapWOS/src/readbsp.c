...
//
   if (texdatasize)
   {
      ...
      {
        miptex_t *t;
         t=(miptex_t *)((char *)mtl+(mtl->dataofs[i] = LittleLong(mtl->dataofs[i])));
         t->width=LittleLong(t->width);
         t->height=LittleLong(t->height);
         t->offsets[0]=LittleLong(t->offsets[0]);
         t->offsets[1]=LittleLong(t->offsets[1]);
         t->offsets[2]=LittleLong(t->offsets[2]);
         t->offsets[3]=LittleLong(t->offsets[3]);
      }
   }
