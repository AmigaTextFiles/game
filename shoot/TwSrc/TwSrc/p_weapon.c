...
void Rifle_Fire (edict_t *ent)
{
	...
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
        ...
        if (t > 50 || t < -50)
	{
        fire_bullet (ent, start, forward, damage, kick, 2000, 1500, MOD_RIFLE);
        }
        else
        fire_bullet (ent, start, forward, damage, kick, 0, 0, MOD_RIFLE);
	...
	gi.multicast (ent->s.origin, MULTICAST_PVS);
        ...
        gi.sound (ent, CHAN_VOICE, gi.soundindex ("rifle.wav"), 1, ATTN_NORM, 0);
...
static void P_ProjectSource (gclient_t *client, vec3_t point, vec3_t distance, vec3_t forward, vec3_t right, vec3_t result)
{
	...
	VectorCopy (distance, _distance);
        if (client->pers.class == 2) _distance[1] = 0;
...

{
       ...
       // fire away!
       if (ent->client->pers.airstrikevalue == 0) {
       ...
       if (ent->client->pers.airstrikevalue == 1) {
       fire_clustergrenade (ent, start, targetdir , 150, 600, 0.5, 250);
       start[1] = start[1] - 64;
       fire_clustergrenade (ent, start, targetdir , 150, 600, 0.5, 250);
       start[1] = start[1] + 128;
       fire_clustergrenade (ent, start, targetdir , 150, 600, 0.5, 250);
       }
       if (ent->client->pers.airstrikevalue == 2) {
       fire_napalmgrenade (ent, start, targetdir , 150, 300, 4.5, 250);
       start[1] = start[1] - 64;
       fire_clustergrenade (ent, start, targetdir , 150, 300, 3.5, 250);
       start[1] = start[1] + 128;
       fire_clustergrenade (ent, start, targetdir , 150, 300, 2.5, 250);
       }
...
void weapon_grenade_fire (edict_t *ent, qboolean held)
{
	...
	speed = GRENADE_MINSPEED + (GRENADE_TIMER - timer) * ((GRENADE_MAXSPEED - GRENADE_MINSPEED) / GRENADE_TIMER);
        if ((ent->client->pers.class == 4) & (ent->client->pers.grenadevalue == 4)) {
        if (ent->detpack == 0) fire_detpack (ent, start, forward, 1000, speed, timer, 1040, held);
        ent->detpack = 1;
        }
        else
...
void weapon_grenadelauncher_fire (edict_t *ent)
{
	...
	if (! ( (int)dmflags->value & DF_INFINITE_AMMO ) )
  ent->client->pers.inventory[ent->client->ammo_index]--;
  if (ent->client->pers.grenadevalue == 1) ent->client->pers.inventory[ent->client->ammo_index] -= 4;
  if (ent->client->pers.grenadevalue == 2) ent->client->pers.inventory[ent->client->ammo_index] -= 9;
...
void Machinegun_Fire (edict_t *ent)
{
	...
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
        ...
        if (t > 50 || t < -50)
	{
        fire_bullet (ent, start, forward, damage, kick, 600, 300, MOD_MACHINEGUN);
        }
        else
...
void Chaingun_Fire (edict_t *ent)
{
	...
	for (i=0 ; i<shots ; i++)
	{
        ...
        if (t > 50 || t < -50)
	{
        fire_bullet (ent, start, forward, damage, kick, 1500, 1500, MOD_CHAINGUN);
        }
        else
...
void weapon_tranquilizer_fire (edict_t *ent)
{
	vec3_t		start;
	vec3_t		forward, right;
	vec3_t		offset;
	...
	if (is_quad)
	{
       ...
       // fire_shotgun (ent, start, forward, damage, kick, 500, 500, DEFAULT_SHOTGUN_COUNT, MOD_SHOTGUN);
        fire_tranquilizer (ent, start, forward, damage, 600, 0, 0);
...
void Weapon_tranquilizer (edict_t *ent)
{
	...
	static int	fire_frames[]	= {8, 9, 0};
        ...
        Weapon_Generic (ent, 7, 18, 36, 39, pause_frames, fire_frames, weapon_tranquilizer_fire);
}
...
void Weapon_Railgun (edict_t *ent)
{
	...
	static int	fire_frames[]	= {4, 0};
        ...
        Weapon_Generic (ent, 3, 18, 56, 61, pause_frames, fire_frames, Rifle_Fire);
}
