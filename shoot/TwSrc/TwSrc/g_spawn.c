...
spawn_t	spawns[] = {
	...
	{"func_plat", SP_func_plat},
        ...
        {"red_plat", SP_red_plat},
        {"turret", SP_func_rotating},
        {"blue_plat", SP_blue_plat},
	{"func_button", SP_func_button},
	{"func_door", SP_func_door},
        {"red_door", SP_red_door},
        {"blue_door", SP_blue_door},
	...
	{"turret_base", SP_turret_base},
        {"vechile_base", SP_vechile_base},
        {"vechile_seat", SP_vechile_seat},
...
void SP_worldspawn (edict_t *ent)
{
	...
	if (ent->message && ent->message[0])
	{
        ...
        if (ent->t3on) level.team3on = ent->t3on;
        if (ent->team1max) level.team1max = ent->team1max;
        if (ent->team2max) level.team2max = ent->team2max;

        if (ent->shotgun) level.shotgun = ent->shotgun;
        if (ent->rifle) level.rifle = ent->rifle;
        if (ent->phealth) level.phealth = ent->phealth;
        if (ent->noweap) level.noweap = ent->noweap;
