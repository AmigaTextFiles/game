...
void parasite_drain_attack (edict_t *self)
{
	...
	VectorSubtract (start, end, dir);
        T_Damage (self->enemy, self, self, dir, self->enemy->s.origin, vec3_origin, damage, 0, DAMAGE_NO_KNOCKBACK, MOD_DOG);
}
...
// check for gib
	if (self->health <= self->gib_health)
	{
		...
		ThrowHead (self, "models/objects/gibs/head2/tris.md2", damage, GIB_ORGANIC);
               if (self->deadflag != DEAD_DEAD) if (self->creator) self->creator->monstercount = self->creator->monstercount - 1;
...
if (self->creator) self->creator->monstercount = self->creator->monstercount - 1;
}
...
void SP_monster_parasite2 (edict_t *owner)
{
        ...
        if (tr.fraction != 1.0)
	{
        ...
        owner->client->pers.inventory[owner->client->pers.selected_item] = owner->client->pers.inventory[owner->client->pers.selected_item] + 50;
		return;
	}
	...
	if (tr.surface)
      ...
      self->creator = owner;    //for the decoy, this is a pointer to the owner
       if (self->creator) owner->monstercount = owner->monstercount  + 1;
