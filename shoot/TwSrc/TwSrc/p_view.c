...
void SV_CalcViewOffset (edict_t *ent)
{
	...
	vec3_t		v;
        int             bubble;
        int             YDirection,PDirection,RDirection;
...
void SV_CalcBlend (edict_t *ent)
{
    ...
    /*
       ...
       if (ent->RedTime > level.time) SV_AddBlend (1.0, 0.0, 0.0, 0.4, ent->client->ps.blend);

      if (ent->tranquilized == 1)
      {
      /*
        random()*10;
        random()*10;
        random()*10;
        */
        SV_AddBlend (random(), random(), random(), random(), ent->client->ps.blend);
        if (ent->tq_time < level.time) ent->tranquilized = 0;
         }
	// add for powerups
	if (ent->client->quad_framenum > level.framenum)
	{
        ...
        if (ent->client->pers.cloak == 1)
        SV_AddBlend (1, 0.5, 0, 0.20, ent->client->ps.blend);
...
void P_FallingDamage (edict_t *ent)
{
	...
	if (delta < 15)
	{
                if (ent->client->pers.class != 8) ent->s.event = EV_FOOTSTEP;
		return;
	}
  ...
  if ((ent->flags & FL_GODMODE) || (ent->client->kamikaze_mode & 1))
	{
        ...
        if (ent->flags & FL_BLUEFLAG)
        {
                ent->s.effects |= EF_FLAG2 | EF_QUAD;
        }
        if (ent->flags & FL_REDFLAG)
        {
                ent->s.effects |= EF_FLAG1 | EF_PENT;
        }
...
void ClientEndServerFrame (edict_t *ent)
{
	...
	// if the scoreboard is up, update it
        ...
        if (((ent->client->showscores || ent->client->pers.scanner_active || ent->client->pers.classes_active) && deathmatch->value &&
                !(level.framenum & SCANNER_UPDATE_FREQ)) || (ent->client->pers.scanner_active & 2) || (ent->client->pers.classes_active & 2))
	{
