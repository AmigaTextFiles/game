...
void Blue_Touch_Plat_Center (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	if (!other->client)
		return;
...
void Red_Touch_Plat_Center (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	if (!other->client)
		return;
...
void red_plat_spawn_inside_trigger (edict_t *ent)
{
	edict_t	*trigger;
	vec3_t	tmin, tmax;
...
//	
	...
	trigger = G_Spawn();
        trigger->touch = Red_Touch_Plat_Center;
	trigger->movetype = MOVETYPE_NONE;
	trigger->solid = SOLID_TRIGGER;
	trigger->enemy = ent;
...
void blue_plat_spawn_inside_trigger (edict_t *ent)
{
	edict_t	*trigger;
	vec3_t	tmin, tmax;
...
//	
	...
	trigger = G_Spawn();
        trigger->touch = Blue_Touch_Plat_Center;
	trigger->movetype = MOVETYPE_NONE;
	trigger->solid = SOLID_TRIGGER;
	trigger->enemy = ent;
...
void SP_red_plat (edict_t *ent)
{
	...
	ent->use = Use_Plat;
        ...
        red_plat_spawn_inside_trigger (ent);        // the "start moving" trigger 
...
void SP_blue_plat (edict_t *ent)
{
	...
	ent->use = Use_Plat;
        ...
        blue_plat_spawn_inside_trigger (ent);        // the "start moving" trigger 
...
"sounds"
"frags"         how many frags to give to player for activating this?
1) silent
2) steam metal
3) wooden clunk
4) metallic click
...
void button_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	if (!other->client)
        ...
        if (self->tw_team)
        if (other->tw_team != self->tw_team)
        return;


        if (self->buttondelay < level.time)
	{
        if (self->frags)
        if (other->client)
        other->client->resp.score = other->client->resp.score + self->frags;


        if (self->teamfrags)
        if (!level.team3on) {

        self->buttondelay = level.time + self->wait;

        if (other->client)
        if (other->tw_team == 1)
        level.team1_score= level.team1_score + self->teamfrags;
        else
        level.team2_score= level.team2_score + self->teamfrags;
         }
         else
         {
        if (other->client) {
        ...
        if (other->tw_team == 3)
        level.team3_score= level.team3_score + self->teamfrags;
        ...
        if (self->endlevel == 1) {
                        gi.bprintf (PRINT_HIGH, "EndLevel button triggered.\n");

                        if (level.team1_score == level.team2_score)
                        gi.bprintf (PRINT_HIGH, "Result:DRAW!!\n");
                        if (level.team1_score < level.team2_score)
                        gi.bprintf (PRINT_HIGH, "Result:Red Wins!\n");
                        if (level.team1_score > level.team2_score)
                        gi.bprintf (PRINT_HIGH, "Result:Blue Wins!\n");
                        endsound_all ();
 			EndDMLevel ();
			return;
...
{
        if (self->tw_team) {
        if (attacker->tw_team != self->tw_team) return;
        if (self->frags) {
        attacker->client->resp.score += self->frags; 
...
void door_go_down (edict_t *self)
{
	...
	if (strcmp(self->classname, "func_door") == 0)
        ...
        if (strcmp(self->classname, "red_door") == 0)
		Move_Calc (self, self->moveinfo.start_origin, door_hit_bottom);
        if (strcmp(self->classname, "blue_door") == 0)
		Move_Calc (self, self->moveinfo.start_origin, door_hit_bottom);
...
void door_go_up (edict_t *self, edict_t *activator)
{
	...
	if (strcmp(self->classname, "func_door") == 0)
        ...
        else if (strcmp(self->classname, "red_door") == 0)
		Move_Calc (self, self->moveinfo.end_origin, door_hit_top);
        else if (strcmp(self->classname, "blue_door") == 0)
		Move_Calc (self, self->moveinfo.end_origin, door_hit_top);
	else if (strcmp(self->classname, "func_door_rotating") == 0)
...
void Touch_Red_DoorTrigger (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	if (other->health <= 0)
		return;
...
void Touch_Blue_DoorTrigger (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	if (other->health <= 0)
		return;
...
void Think_Red_SpawnDoorTrigger (edict_t *ent)
{
	edict_t		*other;
	vec3_t		mins, maxs;
	...
	other->solid = SOLID_TRIGGER;
	other->movetype = MOVETYPE_NONE;
        other->touch = Touch_Red_DoorTrigger;
	gi.linkentity (other);
...
void Think_Blue_SpawnDoorTrigger (edict_t *ent)
{
	edict_t		*other;
	vec3_t		mins, maxs;
	...
	other->solid = SOLID_TRIGGER;
	other->movetype = MOVETYPE_NONE;
        other->touch = Touch_Blue_DoorTrigger;
	gi.linkentity (other);
...
void SP_red_door (edict_t *ent)
{
	...
	if (ent->health || ent->targetname)
		ent->think = Think_CalcMoveSpeed;
                ...
                ent->think = Think_Red_SpawnDoorTrigger;
}

void SP_blue_door (edict_t *ent)
{
	vec3_t	abs_movedir;
	...
	if (ent->health || ent->targetname)
		ent->think = Think_CalcMoveSpeed;
                ...
                ent->think = Think_Blue_SpawnDoorTrigger;
}
