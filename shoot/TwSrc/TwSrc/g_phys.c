...
*/
            ...
            This is a copy of SV_Physics_Toss() modified to provide more 
            realistic 
            bounce physics 
            ============= 
            */ 
            ...
            ent->velocity[2] -= ent->gravity * sv_gravity->value * FRAMETIME; 
            /* 
            This while() loop is used to work around collision physics modeling 
            inaccuracies 
            introduced by SV_PushEntity(). SV_PushEntity() tries to move an 
            entity from it's 
            current position to it's new position FRAMETIME seconds in the 
            future. It does this 
            by doing a gi.trace() from the current position to the new position. 
            It sets the 
            entity's position to the endpos of the trace. This is fine if the 
            entity didn't hit 
            anything (trace.fraction == 1) however, if trace.fraction < 1 then 
            we wind up only 
            accounting for (1 - trace.fraction) * FRAMETIME seconds of the 
            simulation. This 
            while() loop compensates for this by making sure we cover the full 
            FRAMETIME seconds. 
            */ 
            movetime = FRAMETIME; 
            while (movetime > 0) 
            ...
            VectorScale (trace.plane.normal, DotProduct (trace.plane.normal, 
            ent->velocity), normalVel); 
            ...
            // stop if on ground 
            // only consider stopping if impact plane is less than 45 degrees 
            // arccos(0.7) ~ 45 degrees 
            if (trace.plane.normal[2] > 0.7) 
            { 
            VectorCopy (ent->velocity, temp); 
            temp[2] = 0; 
            if ((VectorLength (temp) < 20) && (ent->velocity[2] < ent->gravity * 
            sv_gravity->value * FRAMETIME * 1.5)) 
            { 
            ent->groundentity = trace.ent; 
            ent->groundentity_linkcount = trace.ent->linkcount; 
            VectorCopy (vec3_origin, ent->velocity); 
