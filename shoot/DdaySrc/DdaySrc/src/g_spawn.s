	.file	"g_spawn.c"
gcc2_compiled.:
	.section	".rodata"
	.align 2
.LC0:
	.string	"weapon_flamethrower"
	.align 2
.LC1:
	.string	"ammo_napalm"
	.globl spawns
	.section	".data"
	.align 2
	.type	 spawns,@object
	.size	 spawns,8192
spawns:
	.long .LC2
	.long SP_item_health
	.long .LC3
	.long SP_item_health_small
	.long .LC4
	.long SP_item_health_large
	.long .LC5
	.long SP_item_health_mega
	.long .LC6
	.long SP_info_player_start
	.long .LC7
	.long SP_info_player_deathmatch
	.long .LC8
	.long SP_info_player_coop
	.long .LC9
	.long SP_info_player_intermission
	.long .LC10
	.long SP_func_plat
	.long .LC11
	.long SP_func_button
	.long .LC12
	.long SP_func_door
	.long .LC13
	.long SP_func_door_secret
	.long .LC14
	.long SP_func_door_rotating
	.long .LC15
	.long SP_func_rotating
	.long .LC16
	.long SP_func_train
	.long .LC17
	.long SP_func_water
	.long .LC18
	.long SP_func_conveyor
	.long .LC19
	.long SP_func_areaportal
	.long .LC20
	.long SP_func_clock
	.long .LC21
	.long SP_func_wall
	.long .LC22
	.long SP_func_object
	.long .LC23
	.long SP_func_timer
	.long .LC24
	.long SP_func_explosive
	.long .LC25
	.long SP_func_killbox
	.long .LC26
	.long SP_trigger_always
	.long .LC27
	.long SP_trigger_once
	.long .LC28
	.long SP_trigger_multiple
	.long .LC29
	.long SP_trigger_relay
	.long .LC30
	.long SP_trigger_push
	.long .LC31
	.long SP_trigger_hurt
	.long .LC32
	.long SP_trigger_key
	.long .LC33
	.long SP_trigger_counter
	.long .LC34
	.long SP_trigger_elevator
	.long .LC35
	.long SP_trigger_gravity
	.long .LC36
	.long SP_trigger_monsterjump
	.long .LC37
	.long SP_info_reinforcement_start
	.long .LC38
	.long SP_info_team_start
	.long .LC39
	.long SP_info_reinforcement_start
	.long .LC40
	.long SP_info_reinforcement_start
	.long .LC41
	.long SP_info_reinforcement_start
	.long .LC42
	.long SP_info_reinforcement_start
	.long .LC43
	.long SP_info_reinforcement_start
	.long .LC44
	.long SP_info_reinforcement_start
	.long .LC45
	.long SP_info_reinforcement_start
	.long .LC46
	.long SP_info_reinforcement_start
	.long .LC47
	.long SP_info_reinforcement_start
	.long .LC48
	.long SP_info_Mission_Results
	.long .LC49
	.long SP_info_Mission_Results
	.long .LC50
	.long SP_info_Mission_Results
	.long .LC51
	.long SP_target_temp_entity
	.long .LC52
	.long SP_target_speaker
	.long .LC53
	.long SP_target_explosion
	.long .LC54
	.long SP_target_changelevel
	.long .LC55
	.long SP_target_secret
	.long .LC56
	.long SP_target_goal
	.long .LC57
	.long SP_target_splash
	.long .LC58
	.long SP_target_spawner
	.long .LC59
	.long SP_target_blaster
	.long .LC60
	.long SP_target_crosslevel_trigger
	.long .LC61
	.long SP_target_crosslevel_target
	.long .LC62
	.long SP_target_laser
	.long .LC63
	.long SP_target_help
	.long .LC64
	.long SP_target_actor
	.long .LC65
	.long SP_target_lightramp
	.long .LC66
	.long SP_target_earthquake
	.long .LC67
	.long SP_target_character
	.long .LC68
	.long SP_target_string
	.long .LC69
	.long SP_worldspawn
	.long .LC70
	.long SP_viewthing
	.long .LC71
	.long SP_light
	.long .LC72
	.long SP_light_mine1
	.long .LC73
	.long SP_light_mine2
	.long .LC74
	.long SP_info_null
	.long .LC75
	.long SP_info_null
	.long .LC76
	.long SP_info_notnull
	.long .LC77
	.long SP_path_corner
	.long .LC78
	.long SP_point_combat
	.long .LC79
	.long SP_misc_explobox
	.long .LC80
	.long SP_misc_banner
	.long .LC81
	.long SP_misc_banner_x
	.long .LC82
	.long SP_misc_banner_1
	.long .LC83
	.long SP_misc_banner_2
	.long .LC84
	.long SP_misc_banner_3
	.long .LC85
	.long SP_misc_banner_4
	.long .LC86
	.long SP_misc_skeleton
	.long .LC87
	.long SP_misc_md2
	.long .LC88
	.long SP_misc_satellite_dish
	.long .LC89
	.long SP_misc_actor
	.long .LC90
	.long SP_misc_gib_arm
	.long .LC91
	.long SP_misc_gib_leg
	.long .LC92
	.long SP_misc_gib_head
	.long .LC93
	.long SP_misc_insane
	.long .LC94
	.long SP_misc_deadsoldier
	.long .LC95
	.long SP_misc_viper
	.long .LC96
	.long SP_misc_viper_bomb
	.long .LC97
	.long SP_misc_bigviper
	.long .LC98
	.long SP_misc_strogg_ship
	.long .LC99
	.long SP_misc_teleporter
	.long .LC100
	.long SP_misc_teleporter_dest
	.long .LC101
	.long SP_misc_blackhole
	.long .LC102
	.long SP_misc_eastertank
	.long .LC103
	.long SP_misc_easterchick
	.long .LC104
	.long SP_misc_easterchick2
	.long .LC105
	.long SP_turret_breach
	.long .LC106
	.long SP_turret_base
	.long .LC107
	.long SP_item_armor_body
	.long .LC108
	.long SP_item_armor_combat
	.long .LC109
	.long SP_item_armor_jacket
	.long .LC110
	.long SP_item_armor_shard
	.long .LC111
	.long SP_item_ammo_grenades
	.long .LC112
	.long SP_item_weapon_mine
	.long .LC0
	.long SP_item_weapon_flamethrower
	.long .LC1
	.long SP_item_ammo_napalm
	.long .LC113
	.long SP_item_powerup_silencer
	.long .LC114
	.long SP_item_powerup_breather
	.long .LC115
	.long SP_item_powerup_enviro
	.long .LC116
	.long SP_item_powerup_adrenaline
	.long .LC117
	.long SP_item_powerup_bandolier
	.long .LC118
	.long SP_item_powerup_pack
	.long .LC119
	.long SP_item_key_blue_key
	.long .LC120
	.long SP_item_key_red_key
	.long .LC121
	.long SP_objective_area
	.long .LC122
	.long SP_objective_touch
	.long .LC123
	.long SP_func_explosive_objective
	.long 0
	.long 0
	.space	7192
	.section	".rodata"
	.align 2
.LC123:
	.string	"func_explosive_objective"
	.align 2
.LC122:
	.string	"objective_touch"
	.align 2
.LC121:
	.string	"objective_area"
	.align 2
.LC120:
	.string	"key_red_key"
	.align 2
.LC119:
	.string	"key_blue_key"
	.align 2
.LC118:
	.string	"item_pack"
	.align 2
.LC117:
	.string	"item_bandolier"
	.align 2
.LC116:
	.string	"item_adrenaline"
	.align 2
.LC115:
	.string	"item_enviro"
	.align 2
.LC114:
	.string	"item_breather"
	.align 2
.LC113:
	.string	"item_silencer"
	.align 2
.LC112:
	.string	"weapon_grenadelauncher"
	.align 2
.LC111:
	.string	"ammo_grenades"
	.align 2
.LC110:
	.string	"item_armor_shard"
	.align 2
.LC109:
	.string	"item_armor_jacket"
	.align 2
.LC108:
	.string	"item_armor_combat"
	.align 2
.LC107:
	.string	"item_armor_body"
	.align 2
.LC106:
	.string	"turret_base"
	.align 2
.LC105:
	.string	"turret_breach"
	.align 2
.LC104:
	.string	"misc_easterchick2"
	.align 2
.LC103:
	.string	"misc_easterchick"
	.align 2
.LC102:
	.string	"misc_eastertank"
	.align 2
.LC101:
	.string	"misc_blackhole"
	.align 2
.LC100:
	.string	"misc_teleporter_dest"
	.align 2
.LC99:
	.string	"misc_teleporter"
	.align 2
.LC98:
	.string	"misc_strogg_ship"
	.align 2
.LC97:
	.string	"misc_bigviper"
	.align 2
.LC96:
	.string	"misc_viper_bomb"
	.align 2
.LC95:
	.string	"misc_viper"
	.align 2
.LC94:
	.string	"misc_deadsoldier"
	.align 2
.LC93:
	.string	"misc_insane"
	.align 2
.LC92:
	.string	"misc_gib_head"
	.align 2
.LC91:
	.string	"misc_gib_leg"
	.align 2
.LC90:
	.string	"misc_gib_arm"
	.align 2
.LC89:
	.string	"misc_actor"
	.align 2
.LC88:
	.string	"misc_satellite_dish"
	.align 2
.LC87:
	.string	"misc_md2"
	.align 2
.LC86:
	.string	"misc_skeleton"
	.align 2
.LC85:
	.string	"misc_banner_4"
	.align 2
.LC84:
	.string	"misc_banner_3"
	.align 2
.LC83:
	.string	"misc_banner_2"
	.align 2
.LC82:
	.string	"misc_banner_1"
	.align 2
.LC81:
	.string	"misc_banner_x"
	.align 2
.LC80:
	.string	"misc_banner"
	.align 2
.LC79:
	.string	"misc_explobox"
	.align 2
.LC78:
	.string	"point_combat"
	.align 2
.LC77:
	.string	"path_corner"
	.align 2
.LC76:
	.string	"info_notnull"
	.align 2
.LC75:
	.string	"func_group"
	.align 2
.LC74:
	.string	"info_null"
	.align 2
.LC73:
	.string	"light_mine2"
	.align 2
.LC72:
	.string	"light_mine1"
	.align 2
.LC71:
	.string	"light"
	.align 2
.LC70:
	.string	"viewthing"
	.align 2
.LC69:
	.string	"worldspawn"
	.align 2
.LC68:
	.string	"target_string"
	.align 2
.LC67:
	.string	"target_character"
	.align 2
.LC66:
	.string	"target_earthquake"
	.align 2
.LC65:
	.string	"target_lightramp"
	.align 2
.LC64:
	.string	"target_actor"
	.align 2
.LC63:
	.string	"target_help"
	.align 2
.LC62:
	.string	"target_laser"
	.align 2
.LC61:
	.string	"target_crosslevel_target"
	.align 2
.LC60:
	.string	"target_crosslevel_trigger"
	.align 2
.LC59:
	.string	"target_blaster"
	.align 2
.LC58:
	.string	"target_spawner"
	.align 2
.LC57:
	.string	"target_splash"
	.align 2
.LC56:
	.string	"target_goal"
	.align 2
.LC55:
	.string	"target_secret"
	.align 2
.LC54:
	.string	"target_changelevel"
	.align 2
.LC53:
	.string	"target_explosion"
	.align 2
.LC52:
	.string	"target_speaker"
	.align 2
.LC51:
	.string	"target_temp_entity"
	.align 2
.LC50:
	.string	"SP_info_Mission_Results"
	.align 2
.LC49:
	.string	"info_Mission_Results"
	.align 2
.LC48:
	.string	"info_mission_results"
	.align 2
.LC47:
	.string	"info_flamethrower_start"
	.align 2
.LC46:
	.string	"info_special_start"
	.align 2
.LC45:
	.string	"info_medic_start"
	.align 2
.LC44:
	.string	"info_engineer_start"
	.align 2
.LC43:
	.string	"info_sniper_start"
	.align 2
.LC42:
	.string	"info_hgunner_start"
	.align 2
.LC41:
	.string	"info_lgunner_start"
	.align 2
.LC40:
	.string	"info_officer_start"
	.align 2
.LC39:
	.string	"info_infantry_start"
	.align 2
.LC38:
	.string	"info_team_start"
	.align 2
.LC37:
	.string	"info_reinforcements_start"
	.align 2
.LC36:
	.string	"trigger_monsterjump"
	.align 2
.LC35:
	.string	"trigger_gravity"
	.align 2
.LC34:
	.string	"trigger_elevator"
	.align 2
.LC33:
	.string	"trigger_counter"
	.align 2
.LC32:
	.string	"trigger_key"
	.align 2
.LC31:
	.string	"trigger_hurt"
	.align 2
.LC30:
	.string	"trigger_push"
	.align 2
.LC29:
	.string	"trigger_relay"
	.align 2
.LC28:
	.string	"trigger_multiple"
	.align 2
.LC27:
	.string	"trigger_once"
	.align 2
.LC26:
	.string	"trigger_always"
	.align 2
.LC25:
	.string	"func_killbox"
	.align 2
.LC24:
	.string	"func_explosive"
	.align 2
.LC23:
	.string	"func_timer"
	.align 2
.LC22:
	.string	"func_object"
	.align 2
.LC21:
	.string	"func_wall"
	.align 2
.LC20:
	.string	"func_clock"
	.align 2
.LC19:
	.string	"func_areaportal"
	.align 2
.LC18:
	.string	"func_conveyor"
	.align 2
.LC17:
	.string	"func_water"
	.align 2
.LC16:
	.string	"func_train"
	.align 2
.LC15:
	.string	"func_rotating"
	.align 2
.LC14:
	.string	"func_door_rotating"
	.align 2
.LC13:
	.string	"func_door_secret"
	.align 2
.LC12:
	.string	"func_door"
	.align 2
.LC11:
	.string	"func_button"
	.align 2
.LC10:
	.string	"func_plat"
	.align 2
.LC9:
	.string	"info_player_intermission"
	.align 2
.LC8:
	.string	"info_player_coop"
	.align 2
.LC7:
	.string	"info_player_deathmatch"
	.align 2
.LC6:
	.string	"info_player_start"
	.align 2
.LC5:
	.string	"item_health_mega"
	.align 2
.LC4:
	.string	"item_health_large"
	.align 2
.LC3:
	.string	"item_health_small"
	.align 2
.LC2:
	.string	"item_health"
	.align 2
.LC124:
	.string	"ED_CallSpawn: NULL classname\n"
	.align 2
.LC125:
	.string	"%s doesn't have a spawn function\n"
	.align 2
.LC126:
	.string	"%f %f %f"
	.align 2
.LC127:
	.string	"%s is not a field\n"
	.section	".text"
	.align 2
	.globl ED_ParseField
	.type	 ED_ParseField,@function
ED_ParseField:
	stwu 1,-48(1)
	mflr 0
	stmw 27,28(1)
	stw 0,52(1)
	lis 9,fields@ha
	mr 31,3
	lwz 0,fields@l(9)
	mr 30,4
	mr 28,5
	la 29,fields@l(9)
	cmpwi 0,0,0
	bc 12,2,.L35
.L37:
	lwz 0,12(29)
	andi. 9,0,2
	bc 4,2,.L36
	lwz 3,0(29)
	mr 4,31
	bl Q_stricmp
	cmpwi 0,3,0
	bc 4,2,.L36
	lwz 0,12(29)
	andi. 9,0,1
	bc 12,2,.L39
	lis 9,st@ha
	la 27,st@l(9)
	b .L40
.L39:
	mr 27,28
.L40:
	lwz 10,8(29)
	cmplwi 0,10,11
	bc 12,1,.L33
	lis 11,.L58@ha
	slwi 10,10,2
	la 11,.L58@l(11)
	lis 9,.L58@ha
	lwzx 0,10,11
	la 9,.L58@l(9)
	add 0,0,9
	mtctr 0
	bctr
	.align 2
	.align 2
.L58:
	.long .L54-.L58
	.long .L55-.L58
	.long .L42-.L58
	.long .L33-.L58
	.long .L53-.L58
	.long .L56-.L58
	.long .L33-.L58
	.long .L33-.L58
	.long .L33-.L58
	.long .L33-.L58
	.long .L33-.L58
	.long .L33-.L58
.L42:
	mr 3,30
	bl strlen
	lis 9,gi+132@ha
	mr 31,3
	lwz 0,gi+132@l(9)
	addi 28,31,1
	li 4,766
	mr 3,28
	mtlr 0
	blrl
	li 10,0
	mr 7,3
	cmpw 0,10,28
	mr 9,7
	bc 4,0,.L51
	mr 3,31
	li 6,10
.L45:
	lbzx 0,30,10
	rlwinm 11,0,0,0xff
	mr 8,0
	cmpwi 0,11,92
	bc 4,2,.L46
	cmpw 0,10,3
	bc 4,0,.L46
	addi 10,10,1
	lbzx 0,30,10
	cmpwi 0,0,110
	bc 4,2,.L47
	stb 6,0(9)
	b .L61
.L47:
	stb 11,0(9)
	b .L61
.L46:
	stb 8,0(9)
.L61:
	addi 9,9,1
	addi 10,10,1
	cmpw 0,10,28
	bc 12,0,.L45
.L51:
	lwz 0,4(29)
	stwx 7,27,0
	b .L33
.L53:
	lis 4,.LC126@ha
	mr 3,30
	la 4,.LC126@l(4)
	addi 5,1,8
	addi 6,1,12
	addi 7,1,16
	crxor 6,6,6
	bl sscanf
	lfs 0,8(1)
	lwz 0,4(29)
	stfsx 0,27,0
	lwz 9,4(29)
	lfs 0,12(1)
	add 9,27,9
	stfs 0,4(9)
	lwz 11,4(29)
	lfs 0,16(1)
	add 11,27,11
	stfs 0,8(11)
	b .L33
.L54:
	mr 3,30
	bl atoi
	lwz 0,4(29)
	stwx 3,27,0
	b .L33
.L55:
	mr 3,30
	bl atof
	frsp 1,1
	lwz 0,4(29)
	stfsx 1,27,0
	b .L33
.L56:
	mr 3,30
	bl atof
	lwz 0,4(29)
	li 10,0
	frsp 1,1
	stwx 10,27,0
	lwz 9,4(29)
	add 9,27,9
	stfs 1,4(9)
	lwz 11,4(29)
	add 11,27,11
	stw 10,8(11)
	b .L33
.L36:
	lwzu 0,16(29)
	cmpwi 0,0,0
	bc 4,2,.L37
.L35:
	lis 9,gi+4@ha
	lis 3,.LC127@ha
	lwz 0,gi+4@l(9)
	la 3,.LC127@l(3)
	mr 4,31
	mtlr 0
	crxor 6,6,6
	blrl
.L33:
	lwz 0,52(1)
	mtlr 0
	lmw 27,28(1)
	la 1,48(1)
	blr
.Lfe1:
	.size	 ED_ParseField,.Lfe1-ED_ParseField
	.section	".rodata"
	.align 2
.LC128:
	.string	"ED_ParseEntity: EOF without closing brace"
	.align 2
.LC129:
	.string	"ED_ParseEntity: closing brace without data"
	.section	".text"
	.align 2
	.globl ED_ParseEdict
	.type	 ED_ParseEdict,@function
ED_ParseEdict:
	stwu 1,-304(1)
	mflr 0
	stmw 28,288(1)
	stw 0,308(1)
	stw 3,264(1)
	mr 28,4
	li 5,72
	lis 3,st@ha
	li 4,0
	la 3,st@l(3)
	li 29,0
	crxor 6,6,6
	bl memset
	addi 30,1,264
	b .L65
.L66:
	lwz 0,264(1)
	cmpwi 0,0,0
	bc 4,2,.L67
	lis 9,gi+28@ha
	lis 3,.LC128@ha
	lwz 0,gi+28@l(9)
	la 3,.LC128@l(3)
	mtlr 0
	crxor 6,6,6
	blrl
.L67:
	mr 4,31
	li 5,255
	addi 3,1,8
	bl strncpy
	mr 3,30
	bl COM_Parse
	lwz 0,264(1)
	mr 31,3
	cmpwi 0,0,0
	bc 4,2,.L68
	lis 9,gi+28@ha
	lis 3,.LC128@ha
	lwz 0,gi+28@l(9)
	la 3,.LC128@l(3)
	mtlr 0
	crxor 6,6,6
	blrl
.L68:
	lbz 0,0(31)
	cmpwi 0,0,125
	bc 4,2,.L69
	lis 9,gi+28@ha
	lis 3,.LC129@ha
	lwz 0,gi+28@l(9)
	la 3,.LC129@l(3)
	mtlr 0
	crxor 6,6,6
	blrl
.L69:
	lbz 0,8(1)
	li 29,1
	cmpwi 0,0,95
	bc 12,2,.L65
	mr 4,31
	addi 3,1,8
	mr 5,28
	bl ED_ParseField
.L65:
	mr 3,30
	bl COM_Parse
	mr 31,3
	lbz 0,0(31)
	cmpwi 0,0,125
	bc 4,2,.L66
	cmpwi 0,29,0
	bc 4,2,.L72
	mr 3,28
	li 4,0
	li 5,1016
	crxor 6,6,6
	bl memset
.L72:
	lwz 3,264(1)
	lwz 0,308(1)
	mtlr 0
	lmw 28,288(1)
	la 1,304(1)
	blr
.Lfe2:
	.size	 ED_ParseEdict,.Lfe2-ED_ParseEdict
	.section	".rodata"
	.align 2
.LC130:
	.string	"%i teams with %i entities\n"
	.section	".text"
	.align 2
	.globl G_FindTeams
	.type	 G_FindTeams,@function
G_FindTeams:
	stwu 1,-64(1)
	mflr 0
	stmw 21,20(1)
	stw 0,68(1)
	lis 9,globals@ha
	li 10,1
	la 8,globals@l(9)
	lis 11,g_edicts@ha
	lwz 0,72(8)
	li 22,0
	li 26,0
	lwz 9,g_edicts@l(11)
	cmpw 0,10,0
	addi 30,9,1016
	bc 4,0,.L75
	mr 23,8
	lis 21,globals@ha
.L77:
	lwz 0,88(30)
	addi 27,10,1
	addi 25,30,1016
	cmpwi 0,0,0
	bc 12,2,.L76
	lwz 0,312(30)
	cmpwi 0,0,0
	bc 12,2,.L76
	lwz 0,268(30)
	andi. 9,0,1024
	bc 4,2,.L76
	stw 30,572(30)
	mr 28,30
	addi 22,22,1
	lwz 0,72(23)
	addi 26,26,1
	mr 29,27
	mr 31,25
	cmpw 0,27,0
	bc 4,0,.L76
	la 24,globals@l(21)
.L84:
	lwz 0,88(31)
	cmpwi 0,0,0
	bc 12,2,.L83
	lwz 4,312(31)
	cmpwi 0,4,0
	bc 12,2,.L83
	lwz 0,268(31)
	andi. 9,0,1024
	bc 4,2,.L83
	lwz 3,312(30)
	bl strcmp
	cmpwi 0,3,0
	bc 4,2,.L83
	stw 31,568(28)
	addi 26,26,1
	lwz 0,268(31)
	mr 28,31
	stw 30,572(31)
	ori 0,0,1024
	stw 0,268(31)
.L83:
	lwz 0,72(24)
	addi 29,29,1
	addi 31,31,1016
	cmpw 0,29,0
	bc 12,0,.L84
.L76:
	lwz 0,72(23)
	mr 10,27
	mr 30,25
	cmpw 0,10,0
	bc 12,0,.L77
.L75:
	lis 9,gi+4@ha
	lis 3,.LC130@ha
	lwz 0,gi+4@l(9)
	la 3,.LC130@l(3)
	mr 4,22
	mr 5,26
	mtlr 0
	crxor 6,6,6
	blrl
	lwz 0,68(1)
	mtlr 0
	lmw 21,20(1)
	la 1,64(1)
	blr
.Lfe3:
	.size	 G_FindTeams,.Lfe3-G_FindTeams
	.section	".rodata"
	.align 2
.LC131:
	.string	"skill"
	.align 2
.LC132:
	.string	"%f"
	.align 2
.LC133:
	.string	"ED_LoadFromFile: found %s when expecting {"
	.align 2
.LC134:
	.string	"command"
	.align 2
.LC135:
	.string	"*27"
	.align 2
.LC136:
	.string	"%i entities inhibited\n"
	.align 2
.LC137:
	.string	"server map: %s\n"
	.align 2
.LC138:
	.long 0x0
	.align 2
.LC139:
	.long 0x40400000
	.align 2
.LC140:
	.long 0x3f800000
	.align 2
.LC141:
	.long 0x40000000
	.section	".text"
	.align 2
	.globl SpawnEntities
	.type	 SpawnEntities,@function
SpawnEntities:
	stwu 1,-80(1)
	mflr 0
	stmw 21,36(1)
	stw 0,84(1)
	lis 9,skill@ha
	stw 4,8(1)
	mr 22,3
	lwz 11,skill@l(9)
	mr 28,5
	lis 24,skill@ha
	lfs 1,20(11)
	bl floor
	lis 9,.LC138@ha
	frsp 1,1
	la 9,.LC138@l(9)
	lfs 0,0(9)
	fcmpu 0,1,0
	bc 4,0,.L92
	lis 11,.LC138@ha
	la 11,.LC138@l(11)
	lfs 1,0(11)
.L92:
	lis 9,.LC139@ha
	la 9,.LC139@l(9)
	lfs 0,0(9)
	fcmpu 0,1,0
	bc 4,1,.L93
	lis 11,.LC139@ha
	la 11,.LC139@l(11)
	lfs 1,0(11)
.L93:
	lwz 9,skill@l(24)
	lfs 0,20(9)
	fcmpu 0,0,1
	bc 12,2,.L94
	lis 3,.LC132@ha
	lis 29,gi@ha
	la 29,gi@l(29)
	la 3,.LC132@l(3)
	creqv 6,6,6
	bl va
	mr 4,3
	lwz 0,152(29)
	lis 3,.LC131@ha
	la 3,.LC131@l(3)
	mtlr 0
	blrl
.L94:
	bl SaveClientData
	lis 21,game@ha
	lis 25,g_edicts@ha
	lis 9,gi+140@ha
	li 3,766
	lwz 0,gi+140@l(9)
	addi 23,1,8
	mtlr 0
	blrl
	lis 29,level@ha
	li 5,308
	la 29,level@l(29)
	li 4,0
	mr 3,29
	crxor 6,6,6
	bl memset
	lis 9,game@ha
	lis 11,g_edicts@ha
	la 30,game@l(9)
	lwz 3,g_edicts@l(11)
	li 4,0
	lwz 5,1548(30)
	mulli 5,5,1016
	bl memset
	mr 4,22
	li 5,63
	addi 3,29,72
	bl strncpy
	mr 4,28
	addi 3,30,1032
	li 5,511
	bl strncpy
	lwz 0,1544(30)
	li 11,0
	cmpw 0,11,0
	bc 4,0,.L96
	lwz 9,g_edicts@l(25)
	mr 3,30
	li 10,0
	addi 9,9,1100
.L98:
	lwz 0,1028(3)
	addi 11,11,1
	add 0,0,10
	stw 0,0(9)
	addi 10,10,4732
	lwz 0,1544(3)
	addi 9,9,1016
	cmpw 0,11,0
	bc 12,0,.L98
.L96:
	lis 9,gi@ha
	li 30,0
	la 26,gi@l(9)
	li 27,0
	b .L102
.L103:
	lbz 0,0(4)
	cmpwi 0,0,123
	bc 12,2,.L104
	lwz 9,28(26)
	lis 3,.LC133@ha
	la 3,.LC133@l(3)
	mtlr 9
	crxor 6,6,6
	blrl
.L104:
	cmpwi 0,30,0
	bc 4,2,.L105
	lwz 30,g_edicts@l(25)
	b .L106
.L105:
	bl G_Spawn
	mr 30,3
.L106:
	lwz 3,8(1)
	mr 4,30
	bl ED_ParseEdict
	stw 3,8(1)
	lis 4,.LC134@ha
	lis 3,level+72@ha
	la 4,.LC134@l(4)
	la 3,level+72@l(3)
	bl Q_stricmp
	cmpwi 0,3,0
	bc 4,2,.L107
	lwz 3,284(30)
	lis 4,.LC27@ha
	la 4,.LC27@l(4)
	bl Q_stricmp
	cmpwi 0,3,0
	bc 4,2,.L107
	lwz 3,272(30)
	lis 4,.LC135@ha
	la 4,.LC135@l(4)
	bl Q_stricmp
	cmpwi 0,3,0
	bc 4,2,.L107
	lwz 0,288(30)
	rlwinm 0,0,0,22,20
	stw 0,288(30)
.L107:
	lwz 0,g_edicts@l(25)
	cmpw 0,30,0
	bc 12,2,.L108
	lis 11,.LC138@ha
	lis 9,deathmatch@ha
	la 11,.LC138@l(11)
	lfs 12,0(11)
	lwz 11,deathmatch@l(9)
	lfs 0,20(11)
	fcmpu 0,0,12
	bc 12,2,.L109
	lwz 0,288(30)
	andi. 9,0,2048
	b .L135
.L109:
	lwz 9,skill@l(24)
	lwz 0,288(30)
	lfs 13,20(9)
	fcmpu 0,13,12
	bc 4,2,.L114
	andi. 9,0,256
	bc 4,2,.L113
.L114:
	lis 11,.LC140@ha
	la 11,.LC140@l(11)
	lfs 0,0(11)
	fcmpu 0,13,0
	bc 4,2,.L115
	andi. 9,0,512
	bc 4,2,.L113
.L115:
	lis 11,.LC141@ha
	la 11,.LC141@l(11)
	lfs 0,0(11)
	fcmpu 0,13,0
	bc 12,2,.L116
	lis 9,.LC139@ha
	la 9,.LC139@l(9)
	lfs 0,0(9)
	fcmpu 0,13,0
	bc 4,2,.L111
.L116:
	andi. 11,0,1024
.L135:
	bc 12,2,.L111
.L113:
	mr 3,30
	addi 27,27,1
	bl G_FreeEdict
	b .L102
.L111:
	rlwinm 0,0,0,24,18
	stw 0,288(30)
.L108:
	lwz 0,284(30)
	cmpwi 0,0,0
	mr 4,0
	bc 4,2,.L117
	lwz 9,4(26)
	lis 3,.LC124@ha
	la 3,.LC124@l(3)
	mtlr 9
	crxor 6,6,6
	blrl
	b .L102
.L133:
	mr 4,29
	mr 3,30
	bl SpawnItem
	b .L102
.L134:
	lwz 0,4(29)
	mr 3,30
	mtlr 0
	blrl
	b .L102
.L117:
	la 11,game@l(21)
	li 31,0
	lwz 0,1556(11)
	lis 9,itemlist@ha
	la 29,itemlist@l(9)
	cmpw 0,31,0
	bc 4,0,.L125
	mr 28,11
.L121:
	lwz 3,0(29)
	cmpwi 0,3,0
	bc 12,2,.L123
	bl strcmp
	cmpwi 0,3,0
	bc 12,2,.L133
.L123:
	lwz 0,1556(28)
	addi 31,31,1
	addi 29,29,104
	lwz 4,284(30)
	cmpw 0,31,0
	bc 12,0,.L121
.L125:
	lis 9,spawns@ha
	lwz 0,spawns@l(9)
	la 29,spawns@l(9)
	cmpwi 0,0,0
	bc 12,2,.L131
.L128:
	lwz 3,0(29)
	bl strcmp
	cmpwi 0,3,0
	bc 12,2,.L134
	lwzu 0,8(29)
	lwz 4,284(30)
	cmpwi 0,0,0
	bc 4,2,.L128
.L131:
	lwz 9,4(26)
	lis 3,.LC125@ha
	la 3,.LC125@l(3)
	mtlr 9
	crxor 6,6,6
	blrl
.L102:
	mr 3,23
	bl COM_Parse
	lwz 0,8(1)
	mr 4,3
	cmpwi 0,0,0
	bc 4,2,.L103
	lis 29,gi@ha
	lis 3,.LC136@ha
	la 29,gi@l(29)
	la 3,.LC136@l(3)
	lwz 9,4(29)
	mr 4,27
	mtlr 9
	crxor 6,6,6
	blrl
	bl G_FindTeams
	bl PlayerTrail_Init
	lwz 0,4(29)
	lis 3,.LC137@ha
	mr 4,22
	la 3,.LC137@l(3)
	mtlr 0
	crxor 6,6,6
	blrl
	lwz 0,84(1)
	mtlr 0
	lmw 21,36(1)
	la 1,80(1)
	blr
.Lfe4:
	.size	 SpawnEntities,.Lfe4-SpawnEntities
	.globl dday_statusbar
	.section	".rodata"
	.align 2
.LC142:
	.ascii	"yb\t-24 if 6    xv\t0    hnum    xv\t50    pic 0 endif  if 4"
	.ascii	" \txv\t100 \tanum \txv\t200 \tpic 4  endif if 6 \txv\t150 \t"
	.ascii	"rnum endif if 6 \txv\t250 \tpic 6 endif yb\t-50 if 7 \txv\t0"
	.ascii	" \tpic 7 \tyb\t-68 \tstat_string 8 \tyb\t-50 endif if 10 \tx"
	.ascii	"v\t246 \tnum\t2\t10 \txv\t296 \tpic\t9 endif if 11 \txv\t148"
	.ascii	" \tpic\t11 endif if 6    xl 16    yb -60    pic 28 endif if "
	.ascii	"16    xl 0    yt 0    pic 16 endif if 17    xv 0    yb -200 "
	.ascii	"   stat_string 18    if 19       yb -190       stat_string 1"
	.ascii	"9    endif    if 20       xv -50        yb -200       pic 20"
	.ascii	"     endif endif  if 21    xr 1 0    yb  0    xv  0    yv  0"
	.ascii	"    pic 21 endif  if 29"
	.string	"    xr 1 0    yb  0    xv  156    yv  119    pic 29 endif yt 17 xr -75  string2 \"KILLS\" xr -140  string2 \"POINTS\" yt 32 if 22 xr -26 pic 22 endif yt 34 xr -98 num 3 23 xr -160 num 3 24 yt 64 if 25 xr -26 pic 25 endif yt 68 xr -98 num 3 26 xr -160 num 3 27 "
	.section	".sdata","aw"
	.align 2
	.type	 dday_statusbar,@object
	.size	 dday_statusbar,4
dday_statusbar:
	.long .LC142
	.section	".rodata"
	.align 2
.LC143:
	.string	"unit1_"
	.align 2
.LC144:
	.string	"%i"
	.align 2
.LC145:
	.string	"i_help"
	.align 2
.LC146:
	.string	"i_health"
	.align 2
.LC147:
	.string	"i_medic"
	.align 2
.LC148:
	.string	"help"
	.align 2
.LC149:
	.string	"field_3"
	.align 2
.LC150:
	.string	"i_respcount"
	.align 2
.LC151:
	.string	"i_dday"
	.align 2
.LC152:
	.string	"crosshair"
	.align 2
.LC153:
	.string	"scorehead"
	.align 2
.LC154:
	.string	"scoreleft"
	.align 2
.LC155:
	.string	"scoreright"
	.align 2
.LC156:
	.string	"inventory"
	.align 2
.LC157:
	.string	"s_00"
	.align 2
.LC158:
	.string	"s_10"
	.align 2
.LC159:
	.string	"s_20"
	.align 2
.LC160:
	.string	"s_30"
	.align 2
.LC161:
	.string	"s_40"
	.align 2
.LC162:
	.string	"s_50"
	.align 2
.LC163:
	.string	"s_60"
	.align 2
.LC164:
	.string	"s_70"
	.align 2
.LC165:
	.string	"s_80"
	.align 2
.LC166:
	.string	"s_90"
	.align 2
.LC167:
	.string	"s_100"
	.align 2
.LC168:
	.string	"sv_gravity"
	.align 2
.LC169:
	.string	"800"
	.align 2
.LC170:
	.string	"players/tear.wav"
	.align 2
.LC171:
	.string	"players/cloth.wav"
	.align 2
.LC172:
	.string	"player/lava1.wav"
	.align 2
.LC173:
	.string	"player/lava2.wav"
	.align 2
.LC174:
	.string	"misc/pc_up.wav"
	.align 2
.LC175:
	.string	"misc/talk1.wav"
	.align 2
.LC176:
	.string	"misc/udeath.wav"
	.align 2
.LC177:
	.string	"items/respawn1.wav"
	.align 2
.LC178:
	.string	"*death1.wav"
	.align 2
.LC179:
	.string	"*death2.wav"
	.align 2
.LC180:
	.string	"*death3.wav"
	.align 2
.LC181:
	.string	"*death4.wav"
	.align 2
.LC182:
	.string	"*fall1.wav"
	.align 2
.LC183:
	.string	"*fall2.wav"
	.align 2
.LC184:
	.string	"*gurp1.wav"
	.align 2
.LC185:
	.string	"*gurp2.wav"
	.align 2
.LC186:
	.string	"*jump1.wav"
	.align 2
.LC187:
	.string	"*pain25_1.wav"
	.align 2
.LC188:
	.string	"*pain25_2.wav"
	.align 2
.LC189:
	.string	"*pain50_1.wav"
	.align 2
.LC190:
	.string	"*pain50_2.wav"
	.align 2
.LC191:
	.string	"*pain75_1.wav"
	.align 2
.LC192:
	.string	"*pain75_2.wav"
	.align 2
.LC193:
	.string	"*pain100_1.wav"
	.align 2
.LC194:
	.string	"*pain100_2.wav"
	.align 2
.LC195:
	.string	"foot/grass1.wav"
	.align 2
.LC196:
	.string	"foot/grass2.wav"
	.align 2
.LC197:
	.string	"foot/wood1.wav"
	.align 2
.LC198:
	.string	"foot/wood2.wav"
	.align 2
.LC199:
	.string	"foot/metal1.wav"
	.align 2
.LC200:
	.string	"foot/metal2.wav"
	.align 2
.LC201:
	.string	"foot/sand1.wav"
	.align 2
.LC202:
	.string	"foot/sand2.wav"
	.align 2
.LC203:
	.string	"player/step1.wav"
	.align 2
.LC204:
	.string	"player/step2.wav"
	.align 2
.LC205:
	.string	"player/step3.wav"
	.align 2
.LC206:
	.string	"player/step4.wav"
	.align 2
.LC207:
	.string	"misc/rumble.wav"
	.align 2
.LC208:
	.string	"misc/ring.wav"
	.align 2
.LC209:
	.string	"#w_p38.md2"
	.align 2
.LC210:
	.string	"#w_m98k.md2"
	.align 2
.LC211:
	.string	"#w_mp40.md2"
	.align 2
.LC212:
	.string	"#w_mp43.md2"
	.align 2
.LC213:
	.string	"#w_mg42.md2"
	.align 2
.LC214:
	.string	"#w_panzer.md2"
	.align 2
.LC215:
	.string	"#w_m98ks.md2"
	.align 2
.LC216:
	.string	"#a_masher.md2"
	.align 2
.LC217:
	.string	"#w_colt45.md2"
	.align 2
.LC218:
	.string	"#w_m1.md2"
	.align 2
.LC219:
	.string	"#w_thompson.md2"
	.align 2
.LC220:
	.string	"#w_bar.md2"
	.align 2
.LC221:
	.string	"#w_bhmg.md2"
	.align 2
.LC222:
	.string	"#w_bazooka.md2"
	.align 2
.LC223:
	.string	"#w_m1903.md2"
	.align 2
.LC224:
	.string	"#a_grenade.md2"
	.align 2
.LC225:
	.string	"#w_flame.md2"
	.align 2
.LC226:
	.string	"#w_morphine.md2"
	.align 2
.LC227:
	.string	"#w_knife.md2"
	.align 2
.LC228:
	.string	"#w_binoc.md2"
	.align 2
.LC229:
	.string	"#w_tnt.md2"
	.align 2
.LC230:
	.string	"player/gasp1.wav"
	.align 2
.LC231:
	.string	"player/gasp2.wav"
	.align 2
.LC232:
	.string	"player/watr_in.wav"
	.align 2
.LC233:
	.string	"player/watr_out.wav"
	.align 2
.LC234:
	.string	"player/watr_un.wav"
	.align 2
.LC235:
	.string	"player/u_breath1.wav"
	.align 2
.LC236:
	.string	"player/u_breath2.wav"
	.align 2
.LC237:
	.string	"items/pkup.wav"
	.align 2
.LC238:
	.string	"world/land.wav"
	.align 2
.LC239:
	.string	"misc/h2ohit1.wav"
	.align 2
.LC240:
	.string	"items/damage.wav"
	.align 2
.LC241:
	.string	"items/protect.wav"
	.align 2
.LC242:
	.string	"items/protect4.wav"
	.align 2
.LC243:
	.string	"weapons/noammo.wav"
	.align 2
.LC244:
	.string	"infantry/inflies1.wav"
	.align 2
.LC245:
	.string	"models/objects/gibs/sm_meat/tris.md2"
	.align 2
.LC246:
	.string	"models/objects/gibs/arm/tris.md2"
	.align 2
.LC247:
	.string	"models/objects/gibs/bone/tris.md2"
	.align 2
.LC248:
	.string	"models/objects/gibs/bone2/tris.md2"
	.align 2
.LC249:
	.string	"models/objects/gibs/chest/tris.md2"
	.align 2
.LC250:
	.string	"models/objects/gibs/skull/tris.md2"
	.align 2
.LC251:
	.string	"models/objects/gibs/head2/tris.md2"
	.align 2
.LC252:
	.string	"misc/hitleg.wav"
	.align 2
.LC253:
	.string	"misc/hittorso.wav"
	.align 2
.LC254:
	.string	"misc/hithelm.wav"
	.align 2
.LC255:
	.string	"misc/hithead.wav"
	.align 2
.LC256:
	.string	"Morphine"
	.align 2
.LC257:
	.string	"Helmet"
	.align 2
.LC258:
	.string	"Fists"
	.align 2
.LC259:
	.string	"Binoculars"
	.align 2
.LC260:
	.string	"Knife"
	.align 2
.LC261:
	.string	"Flamethrower"
	.align 2
.LC262:
	.string	"flame_mag"
	.align 2
.LC263:
	.string	"m"
	.align 2
.LC264:
	.string	"mmnmmommommnonmmonqnmmo"
	.align 2
.LC265:
	.string	"abcdefghijklmnopqrstuvwxyzyxwvutsrqponmlkjihgfedcba"
	.align 2
.LC266:
	.string	"mmmmmaaaaammmmmaaaaaabcdefgabcdefg"
	.align 2
.LC267:
	.string	"mamamamamama"
	.align 2
.LC268:
	.string	"jklmnopqrstuvwxyzyxwvutsrqponmlkj"
	.align 2
.LC269:
	.string	"nmonqnmomnmomomno"
	.align 2
.LC270:
	.string	"mmmaaaabcdefgmmmmaaaammmaamm"
	.align 2
.LC271:
	.string	"mmmaaammmaaammmabcdefaaaammmmabcdefmmmaaaa"
	.align 2
.LC272:
	.string	"aaaaaaaazzzzzzzz"
	.align 2
.LC273:
	.string	"mmamammmmammamamaaamammma"
	.align 2
.LC274:
	.string	"abcdefghijklmnopqrrqponmlkjihgfedcba"
	.align 2
.LC275:
	.string	"a"
	.section	".text"
	.align 2
	.globl SP_worldspawn
	.type	 SP_worldspawn,@function
SP_worldspawn:
	stwu 1,-32(1)
	mflr 0
	stmw 28,16(1)
	stw 0,36(1)
	mr 30,3
	li 11,1
	li 9,3
	li 0,2
	stw 11,40(30)
	stw 9,248(30)
	stw 0,264(30)
	stw 11,88(30)
	bl InitBodyQue
	bl SetItemNames
	lis 9,st+20@ha
	lwz 4,st+20@l(9)
	cmpwi 0,4,0
	bc 12,2,.L137
	lis 3,level+136@ha
	la 3,level+136@l(3)
	bl strcpy
.L137:
	lwz 4,280(30)
	cmpwi 0,4,0
	bc 12,2,.L138
	lbz 0,0(4)
	cmpwi 0,0,0
	bc 12,2,.L138
	lis 9,gi+24@ha
	li 3,0
	lwz 0,gi+24@l(9)
	mtlr 0
	blrl
	lis 3,level+8@ha
	lwz 4,280(30)
	li 5,64
	la 3,level+8@l(3)
	bl strncpy
	b .L139
.L138:
	lis 3,level+8@ha
	li 5,64
	la 3,level+8@l(3)
	addi 4,3,64
	bl strncpy
.L139:
	lis 9,st@ha
	lwz 4,st@l(9)
	cmpwi 0,4,0
	bc 12,2,.L140
	lbz 0,0(4)
	cmpwi 0,0,0
	bc 12,2,.L140
	lis 9,gi+24@ha
	li 3,2
	lwz 0,gi+24@l(9)
	mtlr 0
	blrl
	b .L141
.L140:
	lis 9,gi+24@ha
	lis 4,.LC143@ha
	lwz 0,gi+24@l(9)
	la 4,.LC143@l(4)
	li 3,2
	mtlr 0
	blrl
.L141:
	lis 29,st@ha
	lis 9,gi@ha
	la 29,st@l(29)
	lis 3,.LC132@ha
	lfs 1,4(29)
	la 31,gi@l(9)
	la 3,.LC132@l(3)
	lis 28,.LC144@ha
	creqv 6,6,6
	bl va
	lwz 9,24(31)
	mr 4,3
	li 3,4
	mtlr 9
	blrl
	lfs 1,8(29)
	lis 3,.LC126@ha
	lfs 2,12(29)
	la 3,.LC126@l(3)
	lfs 3,16(29)
	creqv 6,6,6
	bl va
	lwz 9,24(31)
	mr 4,3
	li 3,3
	mtlr 9
	blrl
	lwz 4,532(30)
	la 3,.LC144@l(28)
	crxor 6,6,6
	bl va
	lwz 9,24(31)
	mr 4,3
	li 3,1
	mtlr 9
	blrl
	lis 11,maxclients@ha
	lwz 9,maxclients@l(11)
	la 3,.LC144@l(28)
	lfs 0,20(9)
	fctiwz 13,0
	stfd 13,8(1)
	lwz 4,12(1)
	crxor 6,6,6
	bl va
	lwz 9,24(31)
	mr 4,3
	li 3,30
	mtlr 9
	blrl
	lwz 11,24(31)
	lis 9,dday_statusbar@ha
	li 3,5
	lwz 4,dday_statusbar@l(9)
	mtlr 11
	blrl
	lwz 9,40(31)
	lis 3,.LC145@ha
	la 3,.LC145@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC146@ha
	la 3,.LC146@l(3)
	mtlr 9
	blrl
	lwz 10,40(31)
	lis 9,level+268@ha
	lis 11,.LC147@ha
	stw 3,level+268@l(9)
	mtlr 10
	la 3,.LC147@l(11)
	blrl
	lwz 9,40(31)
	lis 3,.LC148@ha
	la 3,.LC148@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC149@ha
	la 3,.LC149@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC150@ha
	la 3,.LC150@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC151@ha
	la 3,.LC151@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC152@ha
	la 3,.LC152@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC153@ha
	la 3,.LC153@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC154@ha
	la 3,.LC154@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC155@ha
	la 3,.LC155@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC156@ha
	la 3,.LC156@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC157@ha
	la 3,.LC157@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC158@ha
	la 3,.LC158@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC159@ha
	la 3,.LC159@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC160@ha
	la 3,.LC160@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC161@ha
	la 3,.LC161@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC162@ha
	la 3,.LC162@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC163@ha
	la 3,.LC163@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC164@ha
	la 3,.LC164@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC165@ha
	la 3,.LC165@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC166@ha
	la 3,.LC166@l(3)
	mtlr 9
	blrl
	lwz 9,40(31)
	lis 3,.LC167@ha
	la 3,.LC167@l(3)
	mtlr 9
	blrl
	lwz 4,48(29)
	cmpwi 0,4,0
	bc 4,2,.L142
	lwz 0,148(31)
	lis 3,.LC168@ha
	lis 4,.LC169@ha
	la 3,.LC168@l(3)
	la 4,.LC169@l(4)
	mtlr 0
	blrl
	b .L143
.L142:
	lwz 0,148(31)
	lis 3,.LC168@ha
	la 3,.LC168@l(3)
	mtlr 0
	blrl
.L143:
	lis 29,gi@ha
	lis 3,.LC170@ha
	la 29,gi@l(29)
	la 3,.LC170@l(3)
	lwz 9,36(29)
	mtlr 9
	blrl
	lwz 10,36(29)
	lis 9,snd_fry@ha
	lis 11,.LC171@ha
	stw 3,snd_fry@l(9)
	mtlr 10
	la 3,.LC171@l(11)
	blrl
	lwz 9,36(29)
	lis 3,.LC172@ha
	la 3,.LC172@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC173@ha
	la 3,.LC173@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC174@ha
	la 3,.LC174@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC175@ha
	la 3,.LC175@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC176@ha
	la 3,.LC176@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC177@ha
	la 3,.LC177@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC178@ha
	la 3,.LC178@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC179@ha
	la 3,.LC179@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC180@ha
	la 3,.LC180@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC181@ha
	la 3,.LC181@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC182@ha
	la 3,.LC182@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC183@ha
	la 3,.LC183@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC184@ha
	la 3,.LC184@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC185@ha
	la 3,.LC185@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC186@ha
	la 3,.LC186@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC187@ha
	la 3,.LC187@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC188@ha
	la 3,.LC188@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC189@ha
	la 3,.LC189@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC190@ha
	la 3,.LC190@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC191@ha
	la 3,.LC191@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC192@ha
	la 3,.LC192@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC193@ha
	la 3,.LC193@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC194@ha
	la 3,.LC194@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC195@ha
	la 3,.LC195@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC196@ha
	la 3,.LC196@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC197@ha
	la 3,.LC197@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC198@ha
	la 3,.LC198@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC199@ha
	la 3,.LC199@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC200@ha
	la 3,.LC200@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC201@ha
	la 3,.LC201@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC202@ha
	la 3,.LC202@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC203@ha
	la 3,.LC203@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC204@ha
	la 3,.LC204@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC205@ha
	la 3,.LC205@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC206@ha
	la 3,.LC206@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC207@ha
	la 3,.LC207@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC208@ha
	la 3,.LC208@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC209@ha
	la 3,.LC209@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC210@ha
	la 3,.LC210@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC211@ha
	la 3,.LC211@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC212@ha
	la 3,.LC212@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC213@ha
	la 3,.LC213@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC214@ha
	la 3,.LC214@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC215@ha
	la 3,.LC215@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC216@ha
	la 3,.LC216@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC217@ha
	la 3,.LC217@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC218@ha
	la 3,.LC218@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC219@ha
	la 3,.LC219@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC220@ha
	la 3,.LC220@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC221@ha
	la 3,.LC221@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC222@ha
	la 3,.LC222@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC223@ha
	la 3,.LC223@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC224@ha
	la 3,.LC224@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC225@ha
	la 3,.LC225@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC226@ha
	la 3,.LC226@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC227@ha
	la 3,.LC227@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC228@ha
	la 3,.LC228@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC229@ha
	la 3,.LC229@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC230@ha
	la 3,.LC230@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC231@ha
	la 3,.LC231@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC232@ha
	la 3,.LC232@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC233@ha
	la 3,.LC233@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC234@ha
	la 3,.LC234@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC235@ha
	la 3,.LC235@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC236@ha
	la 3,.LC236@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC237@ha
	la 3,.LC237@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC238@ha
	la 3,.LC238@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC239@ha
	la 3,.LC239@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC240@ha
	la 3,.LC240@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC241@ha
	la 3,.LC241@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC242@ha
	la 3,.LC242@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC243@ha
	la 3,.LC243@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC244@ha
	la 3,.LC244@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC245@ha
	la 3,.LC245@l(3)
	mtlr 9
	blrl
	lwz 10,32(29)
	lis 9,sm_meat_index@ha
	lis 11,.LC246@ha
	stw 3,sm_meat_index@l(9)
	mtlr 10
	la 3,.LC246@l(11)
	blrl
	lwz 9,32(29)
	lis 3,.LC247@ha
	la 3,.LC247@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC248@ha
	la 3,.LC248@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC249@ha
	la 3,.LC249@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC250@ha
	la 3,.LC250@l(3)
	mtlr 9
	blrl
	lwz 9,32(29)
	lis 3,.LC251@ha
	la 3,.LC251@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC252@ha
	la 3,.LC252@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC253@ha
	la 3,.LC253@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC254@ha
	la 3,.LC254@l(3)
	mtlr 9
	blrl
	lwz 9,36(29)
	lis 3,.LC255@ha
	la 3,.LC255@l(3)
	mtlr 9
	blrl
	lis 3,.LC256@ha
	la 3,.LC256@l(3)
	bl FindItem
	bl PrecacheItem
	lis 3,.LC257@ha
	la 3,.LC257@l(3)
	bl FindItem
	bl PrecacheItem
	lis 3,.LC258@ha
	la 3,.LC258@l(3)
	bl FindItem
	bl PrecacheItem
	lis 3,.LC259@ha
	la 3,.LC259@l(3)
	bl FindItem
	bl PrecacheItem
	lis 3,.LC260@ha
	la 3,.LC260@l(3)
	bl FindItem
	bl PrecacheItem
	lis 3,.LC261@ha
	la 3,.LC261@l(3)
	bl FindItem
	bl PrecacheItem
	lis 3,.LC262@ha
	la 3,.LC262@l(3)
	bl FindItem
	bl PrecacheItem
	lwz 9,24(29)
	lis 4,.LC263@ha
	li 3,800
	la 4,.LC263@l(4)
	mtlr 9
	blrl
	lwz 9,24(29)
	lis 4,.LC264@ha
	li 3,801
	la 4,.LC264@l(4)
	mtlr 9
	blrl
	lwz 9,24(29)
	lis 4,.LC265@ha
	li 3,802
	la 4,.LC265@l(4)
	mtlr 9
	blrl
	lwz 9,24(29)
	lis 4,.LC266@ha
	li 3,803
	la 4,.LC266@l(4)
	mtlr 9
	blrl
	lwz 9,24(29)
	lis 4,.LC267@ha
	li 3,804
	la 4,.LC267@l(4)
	mtlr 9
	blrl
	lwz 9,24(29)
	lis 4,.LC268@ha
	li 3,805
	la 4,.LC268@l(4)
	mtlr 9
	blrl
	lwz 9,24(29)
	lis 4,.LC269@ha
	li 3,806
	la 4,.LC269@l(4)
	mtlr 9
	blrl
	lwz 9,24(29)
	lis 4,.LC270@ha
	li 3,807
	la 4,.LC270@l(4)
	mtlr 9
	blrl
	lwz 9,24(29)
	lis 4,.LC271@ha
	li 3,808
	la 4,.LC271@l(4)
	mtlr 9
	blrl
	lwz 9,24(29)
	lis 4,.LC272@ha
	li 3,809
	la 4,.LC272@l(4)
	mtlr 9
	blrl
	lwz 9,24(29)
	lis 4,.LC273@ha
	li 3,810
	la 4,.LC273@l(4)
	mtlr 9
	blrl
	lwz 9,24(29)
	lis 4,.LC274@ha
	li 3,811
	la 4,.LC274@l(4)
	mtlr 9
	blrl
	lwz 0,24(29)
	lis 4,.LC275@ha
	li 3,863
	la 4,.LC275@l(4)
	mtlr 0
	blrl
	bl GetMapObjective
	lwz 0,36(1)
	mtlr 0
	lmw 28,16(1)
	la 1,32(1)
	blr
.Lfe5:
	.size	 SP_worldspawn,.Lfe5-SP_worldspawn
	.comm	is_silenced,1,1
	.comm	maplist,1060,4
	.comm	team_list,8,4
	.align 2
	.globl SP_item_weapon_flamethrower
	.type	 SP_item_weapon_flamethrower,@function
SP_item_weapon_flamethrower:
	stwu 1,-32(1)
	mflr 0
	stmw 29,20(1)
	stw 0,36(1)
	mr 29,3
	lis 3,.LC0@ha
	la 3,.LC0@l(3)
	bl FindItemByClassname
	mr 4,3
	mr 3,29
	bl SpawnItem
	lwz 0,36(1)
	mtlr 0
	lmw 29,20(1)
	la 1,32(1)
	blr
.Lfe6:
	.size	 SP_item_weapon_flamethrower,.Lfe6-SP_item_weapon_flamethrower
	.align 2
	.globl SP_item_ammo_napalm
	.type	 SP_item_ammo_napalm,@function
SP_item_ammo_napalm:
	stwu 1,-32(1)
	mflr 0
	stmw 29,20(1)
	stw 0,36(1)
	mr 29,3
	lis 3,.LC1@ha
	la 3,.LC1@l(3)
	bl FindItemByClassname
	mr 4,3
	mr 3,29
	bl SpawnItem
	lwz 0,36(1)
	mtlr 0
	lmw 29,20(1)
	la 1,32(1)
	blr
.Lfe7:
	.size	 SP_item_ammo_napalm,.Lfe7-SP_item_ammo_napalm
	.align 2
	.globl ED_CallSpawn
	.type	 ED_CallSpawn,@function
ED_CallSpawn:
	stwu 1,-32(1)
	mflr 0
	stmw 28,16(1)
	stw 0,36(1)
	mr 30,3
	lwz 0,284(30)
	cmpwi 0,0,0
	bc 4,2,.L9
	lis 9,gi+4@ha
	lis 3,.LC124@ha
	lwz 0,gi+4@l(9)
	la 3,.LC124@l(3)
	mtlr 0
	crxor 6,6,6
	blrl
	b .L8
.L144:
	mr 3,30
	mr 4,31
	bl SpawnItem
	b .L8
.L145:
	lwz 0,4(31)
	mr 3,30
	mtlr 0
	blrl
	b .L8
.L9:
	lis 9,game@ha
	li 29,0
	la 9,game@l(9)
	lis 11,itemlist@ha
	lwz 0,1556(9)
	la 31,itemlist@l(11)
	cmpw 0,29,0
	bc 4,0,.L11
	mr 28,9
.L13:
	lwz 3,0(31)
	cmpwi 0,3,0
	bc 12,2,.L12
	lwz 4,284(30)
	bl strcmp
	cmpwi 0,3,0
	bc 12,2,.L144
.L12:
	lwz 0,1556(28)
	addi 29,29,1
	addi 31,31,104
	cmpw 0,29,0
	bc 12,0,.L13
.L11:
	lis 9,spawns@ha
	lwz 0,spawns@l(9)
	la 31,spawns@l(9)
	cmpwi 0,0,0
	bc 12,2,.L18
.L20:
	lwz 3,0(31)
	lwz 4,284(30)
	bl strcmp
	cmpwi 0,3,0
	bc 12,2,.L145
	lwzu 0,8(31)
	cmpwi 0,0,0
	bc 4,2,.L20
.L18:
	lis 9,gi+4@ha
	lis 3,.LC125@ha
	lwz 4,284(30)
	lwz 0,gi+4@l(9)
	la 3,.LC125@l(3)
	mtlr 0
	crxor 6,6,6
	blrl
.L8:
	lwz 0,36(1)
	mtlr 0
	lmw 28,16(1)
	la 1,32(1)
	blr
.Lfe8:
	.size	 ED_CallSpawn,.Lfe8-ED_CallSpawn
	.align 2
	.globl ED_NewString
	.type	 ED_NewString,@function
ED_NewString:
	stwu 1,-32(1)
	mflr 0
	stmw 29,20(1)
	stw 0,36(1)
	mr 31,3
	bl strlen
	lis 9,gi+132@ha
	mr 29,3
	lwz 0,gi+132@l(9)
	addi 30,29,1
	li 4,766
	mr 3,30
	mtlr 0
	blrl
	li 11,0
	mr 7,3
	cmpw 0,11,30
	mr 9,7
	bc 4,0,.L25
	mr 3,29
	li 6,10
.L27:
	lbzx 0,31,11
	rlwinm 10,0,0,0xff
	mr 8,0
	cmpwi 0,10,92
	bc 4,2,.L28
	cmpw 0,11,3
	bc 4,0,.L28
	addi 11,11,1
	lbzx 0,31,11
	cmpwi 0,0,110
	bc 4,2,.L29
	stb 6,0(9)
	b .L146
.L29:
	stb 10,0(9)
	b .L146
.L28:
	stb 8,0(9)
.L146:
	addi 9,9,1
	addi 11,11,1
	cmpw 0,11,30
	bc 12,0,.L27
.L25:
	mr 3,7
	lwz 0,36(1)
	mtlr 0
	lmw 29,20(1)
	la 1,32(1)
	blr
.Lfe9:
	.size	 ED_NewString,.Lfe9-ED_NewString
	.ident	"GCC: (GNU) 2.95.3 20010315 (release)"
