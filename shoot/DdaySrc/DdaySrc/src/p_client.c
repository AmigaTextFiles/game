...
*/
	...
	ent->stance_view=22;				//in each clientthink.
...
*/
		...
		if (ucmd->upmove && ent->client->speedmax[2] > 0)  {	

			ucmd->upmove      = ent->client->speedmax[2];

			if (client->jump_push == false) 
			{
				if (client->jump_stamina >= JUMP_TH) {
					if (ent->waterlevel != 3)
 						client->jump_stamina -= JUMP_TAKE;
				}
			}			
...
*/
		// pbowens: new grenade system
		if (client->grenade) // disable the grenade from tiggering self, but still leave timers on
		{
