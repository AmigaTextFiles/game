...
//this function exits observer mode, presumebly after they have chosen mos. They must have
...
void M_ChooseMOS(edict_t *ent)
{
	...
	client_menu( ent, 0, "*D-DAY: NORMANDY",				PMENU_ALIGN_CENTER, NULL, NULL );
...
void M_Team_Join(edict_t *ent, pmenu_t *p, int choice)
{
	...
	for(i=0;i<MAX_TEAM_MATES;i++)
	{
		...
		if (!team_list[choice]->units[i])
		{
			if (ent->client->resp.team_on)
			{
				team_list[ent->client->resp.team_on->index]->total--;
...
void ChooseTeam(edict_t *ent) {
	...
	// rezmoth - must wait until end of lobby time
...
//	client_menu(ent, 4, NULL,						PMENU_ALIGN_CENTER, NULL, NULL );
	...
	for(i=0;i<MAX_TEAMS;i++)
	{
		...
		theText = gi.TagMalloc(sizeof("123456789012 [00/00]"), TAG_GAME);
		strcat(theText, va("%12s [%i/%i]", team_list[i]->teamname, team_list[i]->total, max_clients));
