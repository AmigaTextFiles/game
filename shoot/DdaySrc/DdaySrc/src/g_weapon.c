...
{
	int speed=1500;//850;  //bullet speed
...
//			grenade->s.modelindex =  gi.modelindex (va("models/objects/%s/tris.md2", (team) ? "masher" : "grenade2"));
			//faf: commented out above line for team dll support, replaced with below
			...
			//        "[name of dll]grenade.md2" to work.  usa & grm will keep old names.
...
void Weapon_HMG_Fire (edict_t *ent)
{
	...
	{
		gi.cprintf(ent, PRINT_HIGH, "You can't shoot that thing while standing up!\n");
		return;
	}
...
/*
	...
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);*/
        ...
        if (ent->client->aim)
			VectorSet(offset, 0, 0, ent->viewheight + 3);
		else
			VectorSet(offset, 0, 0, ent->viewheight + 3);
