...
void CL_R_Explode_SP (vec3_t origin)
{
	...
	CL_Explosion_Particle (origin, true, true);

	for (i=-1; i<2; i+=2)
		for (j=-1; j<2; j+=2)
			for (k=-1; k<2; k+=2)
			{
				org[0]=origin[0]+i*EXP_DIST;
				org[1]=origin[1]+j*EXP_DIST;
				org[2]=origin[2]+k*EXP_DIST;
				CL_Explosion_Particle(org, false, true);
			}
}
...
void CL_G_Explode_SP (vec3_t origin)
{
	...
	CL_Explosion_Particle (origin, true, false);
	...
	for (i=-1; i<2; i+=2)
		for (j=-1; j<2; j+=2)
			for (k=-1; k<2; k+=2)
			{
				...
				org[1]=origin[1]+j*EXP_DIST;
				org[2]=origin[2]+k*EXP_DIST;
				CL_Explosion_Particle(org, false, false);
			}
}
#endif
...
void CL_GunSmokeEffect (vec3_t org, vec3_t dir)
{
	...
	CL_ParticleSmokeEffect (origin, velocity, 20);
...
#else
			CL_BlasterParticles(pos,dir,10);
#endif
...
#else
		CL_ParticleSmokeEffect (pos, dir, 5);
#endif
		break;
