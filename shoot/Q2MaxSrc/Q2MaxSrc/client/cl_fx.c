...
void CL_Blood_SP (vec3_t origin, vec3_t velocity, int frame);
void CL_Tracer_SP (vec3_t origin, vec3_t velocity, float alphadelta);
void CL_Bubble_SP (vec3_t origin, vec3_t velocity);
void CL_RailTrail_SP(vec3_t origin);
void CL_RailSpiral_SP(vec3_t origin,vec3_t velocity);
void CL_Laser_SP (vec3_t origin);
#endif
...
//PGM
#ifdef Q2MAX
	if ((vidref_val == VIDREF_GL)||(vidref_val == VIDREF_SOFT))
#else
...
void pExplosionThink (cparticle_t *p, vec3_t org, vec3_t angle, float *alpha, float *size, int *image)
{
	if (*alpha>.85)
...
void CL_Explosion_Particle (vec3_t org, qboolean large, qboolean rocket)
{
	cparticle_t *p;
...
void pSplashThink (cparticle_t *p, vec3_t org, vec3_t angle, float *alpha, float *size, int *image)
{
	vec3_t len;
	...
	*size *= (float)(SplashSize/VectorLength(len)) * 0.5/((4-*size));
	...
	VectorCopy(p->vel, angle);
	VectorNormalize(angle);
	VectorScale(angle, 5, angle);
...
void CL_ParticleEffectSplash (vec3_t org, vec3_t dir, int color8, int count)
{
	...
	for (i=0 ; i<count ; i++)
	{
		...
		setupParticle (
			org[0],	org[1],	org[2],
			org[0]+((rand()&7)-4)+d*dir[0],	org[1]+((rand()&7)-4)+d*dir[1],	org[2]+((rand()&7)-4)+d*dir[2],
			dir[0]*75 + crand()*20,	dir[1]*75 + crand()*20,	dir[2]*75 + crand()*20,
			0,		0,		0,
			color[0],		color[1],		color[2],
			0,	0,	0,
			1,		-1.0 / (0.5 + frand()*0.3),
...
void CL_ExplosionParticles (vec3_t org)
{
	...
	for (i=0 ; i<256 ; i++)
	{
		setupParticle (
			...
			(rand()%200)-100,			(rand()%200)-100,			(rand()%200)-100,
			0,		0,		0,
			255,	255,		200,
			0,	0,	0,
			1,		-0.8 / (0.5 + frand()*0.3),
...
#else
#define PARTICLETIME ((cl.time - p->time)*0.001)
#define pBlasterMinSize 5
#define pBlasterMaxSize 5
void pBlasterThink (cparticle_t *p, vec3_t org, vec3_t angle, float *alpha, float *size, int *image)
{
	vec3_t len;
	VectorSubtract(p->angle, org, len);
	...
	while (len > 0)
	{
		...
		if (flags & EF_ROCKET)
		{
			...
			setupParticle (
				...
				crand()*5,	crand()*5,	crand()*5,
				0,		0,		5,
				100 + randy,	100 + randy,	100 + randy,
				0,	0,	0,
				1,		-0.75 / (0.8+frand()*0.2),
				5,			20,
				particle_smoke,
				0,
				NULL,0);
	...
	while (len > 0)
	{
		...
		if (left)
		{
			...
			setupParticle (
				0,	0,	0,
				move[0],	move[1],	move[2],
				leftdir[0]*10,	leftdir[1]*10,	leftdir[2]*10,
				0,		0,		0,
				255,	50+rand()*25,	0,
				0,	0,	0,
...
#ifdef Q2MAX
void pBFGThink (cparticle_t *p, vec3_t org, vec3_t angle, float *alpha, float *size, int *image)
{
	vec3_t len;
...
void CL_AddParticles (void)
{
	...
	for (p=active_particles ; p ; p=next)
	{
		...
		{
			p->thinknext=false;
			p->think(p, org, angle, &alpha, &size, &image);
		}
