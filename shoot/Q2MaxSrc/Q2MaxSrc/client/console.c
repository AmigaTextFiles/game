...
#else
  re.DrawStretchPic (0, -viddef.height+lines, viddef.width, viddef.height, "conback", alpha);
#endif
	SCR_AddDirtyPoint (0,0);
...
#else
	Com_sprintf (version, sizeof(version), "Quake2maX v%4.2f", VERSION);

	for (x=0 ; x<16 ; x++)
		re.DrawChar (viddef.width-125+x*8, lines-12, 128 + version[x] );
#endif
