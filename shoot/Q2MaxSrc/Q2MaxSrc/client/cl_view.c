...
{
	vec3_t end, up, oldorg;
	float alphacalc = cl_3dcam_dist->value, dist_up, dist_back, angle;

	if (cl_3dcam_dist->value<0)
		Cvar_SetValue( "cl_3dcam_dist", 0 );
	else if (cl_3dcam_dist->value>cl_3dcam_distmax->value)
		Cvar_SetValue( "cl_3dcam_dist", cl_3dcam_distmax->value );

	//and who said trig was pointless?
	angle = cl_3dcam_angle->value/90.0;
	dist_up = cl_3dcam_dist->value * sin( angle );
	dist_back =  cl_3dcam_dist->value * cos ( angle );
	//finish polar vector

	VectorCopy(cl.refdef.vieworg, oldorg);
	VectorScale (cl.v_forward, -dist_back, end);
	VectorAdd(cl.refdef.vieworg, end, end);


	VectorScale(cl.v_up, dist_up, up);
	...
	viewermodelalpha = VectorLength(end)/alphacalc;
...
{
#ifdef Q2MAX
	if (ent->flags&RF_VIEWERMODEL) //stick shadows on player nicely :)
	{	int i; for (i=0;i<2;i++)
			ent->oldorigin[i] = ent->origin[i] = cl.refdef.vieworg[i];
  		if (cl_3dcam->value)
		{
			AddViewerEntAlpha(ent);
			ent->flags&=~RF_VIEWERMODEL;
			ent->flags|=RF_FORCE_SHADOW;
		}
	}
#endif
...
void V_RenderView( float stereo_separation )
{
	...
	if ( cl.frame.valid && (cl.force_refdef || !cl_paused->value) )
	{
		...
		if (cl_3dcam->value) //3D Cam -- psychospaz
