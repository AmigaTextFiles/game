...
// =================
	...
	if ( ( currententity->flags&RF_WEAPONMODEL || currententity->flags&RF_FORCE_SHADOW) && ( r_lefthand->value == 1.0F ) )
	{
...
#if 0
	if (!(currententity->flags & (RF_VIEWERMODEL | RF_WEAPONMODEL)))
	{
		qglColor4f (1,1,1,0);
...
#if 1
	if ((currententity->flags&RF_FORCE_SHADOW) || !(currententity->flags & (RF_TRANSLUCENT | RF_WEAPONMODEL)))
	{
		switch ((int)(gl_shadows->value))
		{
		case 0:
			break;
		case 2: //old quake2 version - alpha less on trans
			{
				if (currententity->flags&RF_TRANSLUCENT)
					qglColor4f (0,0,0,0.5*currententity->alpha);
				else
			...
			break;
		case 3: //MrG's stencil buffered version with upgrade from yours truly
			{
				if (currententity->flags&RF_TRANSLUCENT)
			...
			break;
		default: //psychospaz's super shadows, baby :)
			{
