...
image_t		*r_particlesmoke;
image_t		*r_particleblood;
image_t		*r_particlebubble;
image_t		*r_particleblaster;
image_t		*r_particleexplosion[7];
...
#ifndef AMIGA
		qglColor4ub(255, 255, 255, alpha*255);
#else
...
void R_SortParticlesOnList (int num_particles, const particle_t particles[])
{
	...
	for (i=0 ; i<num_particles ; i++)
		theparts[i] = NewSortPart(&particles[i]);

	qsort((void *) theparts, num_particles, sizeof(sortedpart_t), transCompare);
}

int texParticle (particle_type type)
{
	image_t		*part_img;
	...
	{
	case particle_smoke:
		part_img = r_particlesmoke;
		break;
	case particle_blood:
		part_img = r_particleblood;
		break;
	case particle_bubble:
		part_img = r_particlebubble;
		break;
	case particle_blaster:
		part_img = r_particleblaster;
		break;	
	case particle_rexplosion1:
		part_img = r_particleexplosion[0];
		break;	
	case particle_rexplosion2:
		part_img = r_particleexplosion[1];
		break;	
	case particle_rexplosion3:
		part_img = r_particleexplosion[2];
		break;	
	case particle_rexplosion4:
		part_img = r_particleexplosion[3];
		break;	
	case particle_rexplosion5:
		part_img = r_particleexplosion[4];
		break;	
	case particle_rexplosion6:
		part_img = r_particleexplosion[5];
		break;	
	case particle_rexplosion7:
		part_img = r_particleexplosion[6];
		break;		
	default:
		part_img = r_particletexture;
		break;
	}
...
void GL_DrawParticles( int num_particles, qboolean inWater )
{
	...
	numBatches = num_particles / MAX_PARTICLE_BATCH;
	numLeft = num_particles % MAX_PARTICLE_BATCH;
  ...
  while (numBatches)
  {
		...
		for ( i=0 ; i < MAX_PARTICLE_BATCH ; i++)
		{
			if (gl_transrendersort->value)
			{
				p = theparts[partNum+i].p;

				if ( (inWater && !theparts[partNum+i].inwater) || (!inWater && theparts[partNum+i].inwater) )
					continue;
			}
			else if (inWater)
				p=&r_newrefdef.particles[partNum+i];
			else
				continue;

			oldrender=rendertype;
			rendertype = (p->flags&PART_TRANS)? 2: (p->flags&PART_BLACK)? 1: 0;
  	...
  	numBatches--;
  	partNum=partNum+MAX_PARTICLE_BATCH;
  }
  for (i=0;i<numLeft;i++)
  {
		{
...
#ifdef AMIGA
		qglColor4f(((float)p->red)/256,((float)p->green)/256,((float)p->blue)/256,((float)alpha)/256);
#else
...
void R_Register( void )
{
	...
	gl_shellstencil = ri.Cvar_Get ("gl_shellstencil", "1", 0 );
