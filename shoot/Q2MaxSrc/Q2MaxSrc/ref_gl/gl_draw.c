...
image_t	*Draw_FindPic (char *name)
{
	...
	{
		char	superfullname[MAX_QPATH];

		Com_sprintf(fullname, sizeof(fullname), "%s", name+1);
		name[strlen(name)-4] = 0;

	//	ri.Sys_Error( ERR_FATAL, "%s", name );

		Com_sprintf (superfullname, sizeof(superfullname), "pics/%s.tga", name);
		gl = GL_FindImage (superfullname, it_pic);
		if (!gl)
		{
			Com_sprintf (superfullname, sizeof(superfullname), "pics/%s.jpg", name);
			gl = GL_FindImage (superfullname, it_pic);
		}
