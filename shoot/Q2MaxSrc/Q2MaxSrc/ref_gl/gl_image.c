...
#ifdef IMAGECACHE
	if (!strstr(image->name,"***")) HARD_AccessMember(TextureCache, ORDNUM(image));
#endif
...
{
	int			samples;
	static	unsigned	scaled[IMAGERESMAX*IMAGERESMAX];	// was 256*256
	int			scaled_width, scaled_height;
	int			i, c;
	byte		*scan;
	int comp;
		
		// ri.Con_Printf( PRINT_ALL, "x= %i y= %i\n", width,height );
	...
	if (scaled_width > IMAGERESMAX) //changed to match new values
		scaled_width = IMAGERESMAX;
	if (scaled_height > IMAGERESMAX)
		scaled_height = IMAGERESMAX;
	...
	//here all code for paletted textures is ripped out
	//gives it a nicer and cleaner look
...
nonscrap:
  ...
  if (!strstr(image->name,"***")) HARD_AddMember(TextureCache,ORDNUM(image),image->width*image->height,(void *)image);
#endif
...
#ifdef JPEG_IMPLEMENTED
	else if (!strcmp(name+len-4, ".jpg")) //heffo hax0r
	{
...
void GL_FreeUnusedImages (void)
{
	...
	r_particlesmoke->registration_sequence = registration_sequence;
	r_particleblood->registration_sequence = registration_sequence;
	r_particlebubble->registration_sequence = registration_sequence;
	r_particleblaster->registration_sequence = registration_sequence;
	for (i=0; i<7; i++)
		r_particleexplosion[i]->registration_sequence = registration_sequence;
