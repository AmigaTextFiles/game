...
public:
    ...
    IL_Screen(bool telaCheia, int resolucaoWidth = SCREEN_WIDTH, int resolucaoHeight = SCREEN_HEIGHT, int bits = 32) {
        ...
        if (telaCheia == true) {
            ...
            #ifdef psp
            surface = SDL_SetVideoMode(resolucaoWidth, resolucaoHeight, bits, SDL_SWSURFACE | SDL_FULLSCREEN);
            #else
            surface = SDL_SetVideoMode(resolucaoWidth, resolucaoHeight, bits, SDL_DOUBLEBUF | SDL_HWSURFACE | SDL_FULLSCREEN);
            #endif
        }
...
int main(int argc, char *argv[]) {
    ...
    musica = Mix_LoadMUS ( "res/MOD.The_Roque_Mallet" );
    ...
    pressF11Rect.x = 10;
    pressF11Rect.y = 580;
    pressF11Surface = TTF_RenderText_Solid( fontPequena, "Ported by Porting for Profit", corBranca);
