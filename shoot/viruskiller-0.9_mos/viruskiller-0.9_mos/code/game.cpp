...
void doRoundClear()
{
	...
	while (true)
	{
		...
		{
			info = 1;
			if ((gameData.activeDirs < 4) && (addedDir == false))
			{
				if ((gameData.level % 3) == 0)
				{
					setupActiveDirectories(1);
					addedDir = true;
				}
			}
		}
		...
		if (info == 1)
		{
			...
			if (addedDir)
			{
				...
				graphics.drawString(400, 520, TXT_CENTERED, graphics.screen, "New Directory Added!");
				graphics.setFontColor(0xff, 0xff, 0xff, 0x00, 0x00, 0x00);
			}
		...
		else if (info == 3)
		{
			...
			graphics.drawString(400, 320, TXT_CENTERED, graphics.screen, "READY!", gameData.level);
		}
...
int doGame()
{
	...
	int virusSpawn = 100 * Math::rrand(2, 4);
	int itemSpawnTime = 100 * Math::rrand(ITEM_MINWAIT, ITEM_MAXWAIT);
	...
	unsigned int lastTime = time + (roundTime * 100);
	unsigned int now;
	...
	if (gameData.skill < 3)
	{
		graphics.drawString(400, 270, TXT_CENTERED, graphics.screen, "Round #1");
		graphics.drawString(400, 320, TXT_CENTERED, graphics.screen, "READY!", gameData.level);
	}
	...
	while (true)
	{
		...
		{
			gameData.threadStopTimer -= (int)(1 * engine.getTimeDifference());
		...
		if (engine.mouseLeft)
		{
			...
			{
				if (gameData.threadStopTimer == 0)
					gameData.kernelPower -= (.5 * engine.getTimeDifference());
		...
		if (addMoreViruses)
		{
			virusSpawn -= (int)(1 * engine.getTimeDifference());
		...
		if (gameData.threadStopTimer == 0)
			time += (int)(1 * engine.getTimeDifference());

		itemSpawnTime -= (int)(1 * engine.getTimeDifference());
		...
		if (time >= lastTime)
		{
			...
			else
			{
				...
				if ((gameData.level % 2) != 0)
					maxViruses += 2 + (1 * gameData.skill);

				lastTime = time;

				if ((gameData.level % 5) == 0)
					minViruses += 2 + (1 * gameData.skill);
