...
int E_FILTER(const SDL_Event *event) {
	...
	case SDL_KEYDOWN:
		...
		if (Alt&&((event->key.keysym.sym==SDLK_RETURN)||(event->key.keysym.sym==SDLK_f))){
			...
			UC=event->key.keysym.unicode; //wawa *.unicode
		break;
...
SDL_Rect Src;
  ...
  chdir(SHARE_DIR);
	...
	TitleS=SDL_DisplayFormat(IMG_Load("./img/title.jpg")); //edit wawa
	/*wawa
	TitleBit = SDL_DisplayFormat(TitleS);
	...
	Music = Mix_LoadMUS("./snd/smashem.wav");
}
...
Uint32 OldTicks;
	...
	while ((Return==-1)){
		...
		if (SDL_GetTicks()-OldTicks>10000){
            ...
            StartTime=SDL_GetTicks(); //wawa
			Src.y+=10*M; //edit wawa (multiplied by 10)
			SDL_BlitSurface(TitleS,&Src,screen,NULL);
			if (Passwd[0]!='\0'){
				boxRGBA(screen,0,390,399,399,0,255,0,64);
				stringRGBA(screen,200-strlen(Passwd)*4,391,Passwd,10,250,10,128);
			}
			EndTime=SDL_GetTicks(); //wawa
			if (EndTime-StartTime < FrameTime) SDL_Delay(FrameTime-(EndTime-StartTime));

            //SDL_Delay(15); //edit wawa, whole delay line
			SDL_UpdateRect(screen, 0, 0, 0, 0);
			if (Src.y==400*(M>0)){
				OldTicks=SDL_GetTicks();
				M*=-1;
...
// load config
  ...
  if (Sound)
    if ( Mix_OpenAudio(11025, AUDIO_S8, 1, 1024) < 0 ){
      fprintf(stderr,"Warning: Couldn't set 11025Hz 8-bit audio\n- Reason: %s\n",SDL_GetError());
      Sound=0;
    }
  if (joystick)
