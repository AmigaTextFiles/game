  ...
  boolean drawcursor )
{
  ...
  if (drawcursor && x + SHORT_SWAP(l->f['_' - l->sc]->width) <= SCREENWIDTH)
  {
    // killough 1/18/98 -- support multiple lines
  ...
  boolean*  on )
{
  ...
  for (i=0;i<h;i++)
    ...
    (
      &s->l[i],
      x,
      y - i*(SHORT_SWAP(font[0]->height)+1),
      font,
      startchar,
      cr
    );
...
static void HUlib_eraseMBg(hu_mtext_t* m)
{
  ...
  {
    lh = SHORT_SWAP(m->l[0].f[0]->height) + 1;
