...
void P_LoadVertexes (int lump)
{
  ...
  for (i=0; i<numvertexes; i++)
    {
      vertexes[i].x = SHORT_SWAP(((mapvertex_t *) data)[i].x)<<FRACBITS;
      vertexes[i].y = SHORT_SWAP(((mapvertex_t *) data)[i].y)<<FRACBITS;
    }
  ...
  for (i=0; i<numsegs; i++)
    {
      ...
      li->v1 = &vertexes[SHORT_SWAP(ml->v1)];
      li->v2 = &vertexes[SHORT_SWAP(ml->v2)];

      li->angle = (SHORT_SWAP(ml->angle))<<16;
...
// errors _can_ cause firelines.
      ...
      li->offset = (SHORT_SWAP(ml->offset))<<16;
      linedef = SHORT_SWAP(ml->linedef);
      ldef = &lines[linedef];
      li->linedef = ldef;
      side = SHORT_SWAP(ml->side);
...
void P_LoadSubsectors (int lump)
{
  ...
  for (i=0; i<numsubsectors; i++)
    {
      subsectors[i].numlines  = SHORT_SWAP(((mapsubsector_t *) data)[i].numsegs );
      subsectors[i].firstline = SHORT_SWAP(((mapsubsector_t *) data)[i].firstseg);
    }
  ...
  for (i=0; i<numsectors; i++)
    {
      ...
      ss->floorheight = SHORT_SWAP(ms->floorheight)<<FRACBITS;
      ss->ceilingheight = SHORT_SWAP(ms->ceilingheight)<<FRACBITS;
      ss->floorpic = R_FlatNumForName(ms->floorpic);
      ss->ceilingpic = R_FlatNumForName(ms->ceilingpic);
      ss->lightlevel = SHORT_SWAP(ms->lightlevel);
      ss->special = SHORT_SWAP(ms->special);
      ss->oldspecial = SHORT_SWAP(ms->special);
      ss->tag = SHORT_SWAP(ms->tag);
      ss->thinglist = NULL;
  ...
  for (i=0; i<numnodes; i++)
    {
      ...
      no->x = SHORT_SWAP(mn->x)<<FRACBITS;
      no->y = SHORT_SWAP(mn->y)<<FRACBITS;
      no->dx = SHORT_SWAP(mn->dx)<<FRACBITS;
      no->dy = SHORT_SWAP(mn->dy)<<FRACBITS;

      for (j=0 ; j<2 ; j++)
        {
          int k;
          no->children[j] = SHORT_SWAP(mn->children[j]);
          for (k=0 ; k<4 ; k++)
            no->bbox[j][k] = SHORT_SWAP(mn->bbox[j][k])<<FRACBITS;
        }
    }
  ...
  for (i=0; i<numthings; i++)
    {
      ...
      mt->x = SHORT_SWAP(mt->x);
      mt->y = SHORT_SWAP(mt->y);
      mt->angle = SHORT_SWAP(mt->angle);
      mt->type = SHORT_SWAP(mt->type);
      mt->options = SHORT_SWAP(mt->options);
  ...
  for (i=0; i<numlines; i++)
    {
      ...
      ld->flags = SHORT_SWAP(mld->flags);
      ld->special = SHORT_SWAP(mld->special);
      ld->tag = SHORT_SWAP(mld->tag);
      v1 = ld->v1 = &vertexes[SHORT_SWAP(mld->v1)];
      v2 = ld->v2 = &vertexes[SHORT_SWAP(mld->v2)];
      ld->dx = v2->x - v1->x;
      ld->dy = v2->y - v1->y;
      ...
      ld->sidenum[0] = SHORT_SWAP(mld->sidenum[0]);
      ld->sidenum[1] = SHORT_SWAP(mld->sidenum[1]);
  ...
  for (i=0; i<numsides; i++)
    {
      ...
      sd->textureoffset = SHORT_SWAP(msd->textureoffset)<<FRACBITS;
      sd->rowoffset = SHORT_SWAP(msd->rowoffset)<<FRACBITS;

      // killough 4/4/98: allow sidedef texture names to be overloaded
      // killough 4/11/98: refined to allow colormaps to work as wall
      // textures if invalid as colormaps but valid as textures.

      sd->sector = sec = &sectors[SHORT_SWAP(msd->sector)];
      switch (sd->special)
        {
...
void P_LoadBlockMap (int lump)
{
  ...
  if (M_CheckParm("-blockmap") || (count = W_LumpLength(lump)/2) >= 0x10000)
  ...
  else
    {
      ...
      blockmaplump[0] = SHORT_SWAP(wadblockmaplump[0]);
      blockmaplump[1] = SHORT_SWAP(wadblockmaplump[1]);
      blockmaplump[2] = (long)(SHORT_SWAP(wadblockmaplump[2])) & 0xffff;
      blockmaplump[3] = (long)(SHORT_SWAP(wadblockmaplump[3])) & 0xffff;

      for (i=4 ; i<count ; i++)
        {
          short t = SHORT_SWAP(wadblockmaplump[i]);          // killough 3/1/98
