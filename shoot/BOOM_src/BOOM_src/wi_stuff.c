...
// NET GAME STUFF
#define NG_STATSY     50
#define NG_STATSX     (32 + SHORT_SWAP(star->width)/2 + 32*!dofrags)
...
void WI_drawLF(void)
{
  ...
  // draw <LevelName>
  V_DrawPatch((SCREENWIDTH - SHORT_SWAP(lnames[wbs->last]->width))/2,
  y, FB, lnames[wbs->last]);

  // draw "Finished!"
  y += (5*SHORT_SWAP(lnames[wbs->last]->height))/4;

  V_DrawPatch((SCREENWIDTH - SHORT_SWAP(finished->width))/2,
  y, FB, finished);
}
...
void WI_drawEL(void)
{
  ...
  // draw "Entering"
  V_DrawPatch((SCREENWIDTH - SHORT_SWAP(entering->width))/2,
  y, FB, entering);

  // draw level
  y += (5*SHORT_SWAP(lnames[wbs->next]->height))/4;

  V_DrawPatch((SCREENWIDTH - SHORT_SWAP(lnames[wbs->next]->width))/2,
  ...
  patch_t*  c[] )
{
  ...
  do
  {
    left = lnodes[wbs->epsd][n].x - SHORT_SWAP(c[i]->leftoffset);
    top = lnodes[wbs->epsd][n].y - SHORT_SWAP(c[i]->topoffset);
    right = left + SHORT_SWAP(c[i]->width);
    bottom = top + SHORT_SWAP(c[i]->height);
...
( int   x,
  ...
  int   digits )
{
  int   fontwidth = SHORT_SWAP(num[0]->width);
  int   neg;
  int   temp;
  ...
  int   t )
{
  ...
  if (t <= 61*59)  // otherwise known as 60*60 -1 == 3599
  {
    ...
    {
      n = (t / div) % 60;
      x = WI_drawNum(x, y, n, 2) - SHORT_SWAP(colon->width);
      div *= 60;
  ...
  {
    // "sucks"
    V_DrawPatch(x - SHORT_SWAP(sucks->width), y, FB, sucks);
  }
}
...
void WI_drawDeathmatchStats(void)
{
  ...
  V_DrawPatch(DM_TOTALSX-SHORT_SWAP(total->width)/2,
  ...
  for (i=0 ; i<MAXPLAYERS ; i++)
  {
    if (playeringame[i])
    {
      V_DrawPatch(x-SHORT_SWAP(p[i]->width)/2,
        DM_MATRIXY - WI_SPACINGY,
        FB,
        p[i]);

      V_DrawPatch(DM_MATRIXX-SHORT_SWAP(p[i]->width)/2,
        y,
        FB,
        p[i]);
    ...
    {
      // V_DrawPatch(x-SHORT_SWAP(bp[i]->width)/2,
      //   DM_MATRIXY - WI_SPACINGY, FB, bp[i]);
      // V_DrawPatch(DM_MATRIXX-SHORT_SWAP(bp[i]->width)/2,
      //   y, FB, bp[i]);
    }
    x += DM_SPACINGX;
    y += WI_SPACINGY;
...
void WI_drawNetgameStats(void)
{
  ...
  int   x;
  int   y;
  int   pwidth = SHORT_SWAP(percent->width);
  ...
  // draw stat titles (top line)
  V_DrawPatch(NG_STATSX+NG_SPACINGX-SHORT_SWAP(kills->width),
  NG_STATSY, FB, kills);

  V_DrawPatch(NG_STATSX+2*NG_SPACINGX-SHORT_SWAP(items->width),
  NG_STATSY, FB, items);

  V_DrawPatch(NG_STATSX+3*NG_SPACINGX-SHORT_SWAP(secret->width),
  NG_STATSY, FB, secret);

  if (dofrags)
    V_DrawPatch(NG_STATSX+4*NG_SPACINGX-SHORT_SWAP(frags->width),
      NG_STATSY, FB, frags);

  // draw stats
  y = NG_STATSY + SHORT_SWAP(kills->height);
  ...
  for (i=0 ; i<MAXPLAYERS ; i++)
  {
    ...
    x = NG_STATSX;
    V_DrawPatch(x-SHORT_SWAP(p[i]->width), y, FB, p[i]);

    if (i == me)
      V_DrawPatch(x-SHORT_SWAP(p[i]->width), y, FB, star);
...
void WI_drawStats(void)
{
  ...
  lh = (3*SHORT_SWAP(num[0]->height))/2;
  ...
  V_DrawPatch(SP_TIMEX, SP_TIMEY, FB, timeP);
...
void WI_loadData(void)
{
  ...
  // "time"
  timeP = W_CacheLumpName("WITIME", PU_STATIC);
