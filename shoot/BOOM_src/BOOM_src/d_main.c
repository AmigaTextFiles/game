...
//  calls I_GetTime, I_StartFrame, and I_GetEvent
//

extern boolean demorecording;

static void D_DoomLoop(DisplayType_t displayType)
{
  if (demorecording)
    G_BeginRecording ();
...
void D_setDoomExeDir(char* path) {
  ...
  if ((p = M_CheckParm("-playdemo")) && p < myargc-1)
    {
      ...
      D_DoomLoop(displayType);  // never returns
    }
