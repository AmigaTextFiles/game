  ...
  if (strlen(filename)<=4 || strcasecmp(filename+strlen(filename)-4, ".wad" ))
    {
      ...
      singleinfo.size = LONG_SWAP(filelength(handle));
  ...
  else
    {
      ...
      header.numlumps = LONG_SWAP(header.numlumps);
      header.infotableofs = LONG_SWAP(header.infotableofs);
    ...
    for (i=startlump ; i<numlumps ; i++,lump_p++, fileinfo++)
      {
        ...
        lump_p->position = LONG_SWAP(fileinfo->filepos);
        lump_p->size = LONG_SWAP(fileinfo->size);
...
void WritePredefinedLumpWad(const char *filename)
{
  ...
  if ( (handle = open (filenam, O_RDWR | O_CREAT | O_BINARY, S_IWUSR|S_IRUSR)) != -1)
  {
    ...
    header.numlumps = LONG_SWAP(num_predefined_lumps);
    header.infotableofs = LONG_SWAP(sizeof(header));
    ...
    for (i=0;i<num_predefined_lumps;i++)
    {
      ...
      fileinfo.filepos = LONG_SWAP(filepos);
      fileinfo.size = LONG_SWAP(predefined_lumps[i].size);
