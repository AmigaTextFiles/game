  ...
  boolean refresh )
{
  ...
  int   w = SHORT_SWAP(n->p[0]->width);
  int   h = SHORT_SWAP(n->p[0]->height);
  int   x = n->x;
  ...
  boolean   refresh )
{
  ...
  if (*mi->on && (mi->oldinum != *mi->inum || refresh))
  {
    if (mi->oldinum != -1)
    {
      x = mi->x - SHORT_SWAP(mi->p[mi->oldinum]->leftoffset);
      y = mi->y - SHORT_SWAP(mi->p[mi->oldinum]->topoffset);
      w = SHORT_SWAP(mi->p[mi->oldinum]->width);
      h = SHORT_SWAP(mi->p[mi->oldinum]->height);
  ...
  boolean   refresh )
{
  ...
  {
    x = bi->x - SHORT_SWAP(bi->p->leftoffset);
    y = bi->y - SHORT_SWAP(bi->p->topoffset);
    w = SHORT_SWAP(bi->p->width);
    h = SHORT_SWAP(bi->p->height);
