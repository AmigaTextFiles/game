...
//   0,255,0,ss_keys,   "[0-255(59)] key to view help screen"},
    ...
    {"snd_channels",&numChannels, 16,
     1,UL,0,ss_none,   "[1-?(16)] number of audio events simultaneously"},  // killough
...
void WritePCXfile(char* filename,byte* data,int width,int height,byte* palette)
  ...
  pcx->xmin = 0;
  pcx->ymin = 0;
  pcx->xmax = SHORT_SWAP(width-1);
  pcx->ymax = SHORT_SWAP(height-1);
  pcx->hres = SHORT_SWAP(width);
  pcx->vres = SHORT_SWAP(height);
  memset (pcx->palette,0,sizeof(pcx->palette));
  pcx->color_planes = 1;        // chunky image
  pcx->bytes_per_line = SHORT_SWAP(width);
  pcx->palette_type = SHORT_SWAP(2); // not a grey scale
...
void WriteBMPfile(char* filename,byte* data,int width,int height,byte* palette)
  ...
  bmfh.bfType = SHORT_SWAP(19778);
  bmfh.bfSize = LONG_SWAP(fhsiz+ihsiz+256L*4+width*height);
  bmfh.bfReserved1 = SHORT_SWAP(0);
  bmfh.bfReserved2 = SHORT_SWAP(0);
  bmfh.bfOffBits = LONG_SWAP(fhsiz+ihsiz+256L*4);

  bmih.biSize = LONG_SWAP(ihsiz);
  bmih.biWidth = LONG_SWAP(width);
  bmih.biHeight = LONG_SWAP(height);
  bmih.biPlanes = SHORT_SWAP(1);
  bmih.biBitCount = SHORT_SWAP(8);
  bmih.biCompression = LONG_SWAP(BI_RGB);
  bmih.biSizeImage = LONG_SWAP(wid*height);
  bmih.biXPelsPerMeter = LONG_SWAP(0);
  bmih.biYPelsPerMeter = LONG_SWAP(0);
  bmih.biClrUsed = LONG_SWAP(256);
  bmih.biClrImportant = LONG_SWAP(256);
...
// Fix #includes, remove external decls duplicated elsewhere, fix LONG_SWAP() conflict
//
