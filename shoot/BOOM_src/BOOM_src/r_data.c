  ...
  for (; --i >=0; patch++)
    {
      ...
      int x1 = patch->originx, x2 = x1 + SHORT_SWAP(realpatch->width);
      ...
      for (; x1<x2 ; x1++)
        if (collump[x1] == -1)      // Column has multiple patches?
          ...
          R_DrawColumnInCache((column_t*)((byte*)realpatch+LONG_SWAP(cofs[x1])),
    ...
    while (--i >= 0)
      {
        ...
        int x1 = patch++->originx, x2 = x1 + SHORT_SWAP(realpatch->width), x = x1;
        const int *cofs = realpatch->columnofs-x1;
        ...
        for ( ; x<x2 ; x++)
          {
            ...
            const column_t *col = (column_t*)((byte*)realpatch+LONG_SWAP(cofs[x]));
            for (;col->topdelta != 0xff; count[x].posts++)
              col = (column_t *)((byte *) col + col->length + 4);
            count[x].patches++;
            collump[x] = pat;
            colofs[x] = LONG_SWAP(cofs[x])+3;
          }
      }
  }
...
void R_InitTextures (void)
{
  ...
  nummappatches = LONG_SWAP(*((int *)names));
  name_p = names+4;
  ...
  if (W_CheckNumForName("TEXTURE2") != -1)
    {
      ...
      numtextures2 = LONG_SWAP(*maptex2);
  ...
  for (i=0 ; i<numtextures ; i++, directory++)
    {
      ...
      texture = textures[i] =
        Z_Malloc(sizeof(texture_t) +
                 sizeof(texpatch_t)*(SHORT_SWAP(mtexture->patchcount)-1),
                 PU_STATIC, 0);

      texture->width = SHORT_SWAP(mtexture->width);
      texture->height = SHORT_SWAP(mtexture->height);
      texture->patchcount = SHORT_SWAP(mtexture->patchcount);
      ...
      for (j=0 ; j<texture->patchcount ; j++, mpatch++, patch++)
        {
          patch->originx = SHORT_SWAP(mpatch->originx);
          patch->originy = SHORT_SWAP(mpatch->originy);
          patch->patch = patchlookup[SHORT_SWAP(mpatch->patch)];
          if (patch->patch == -1)
            {
              //jff 8/3/98 use logical output routine
              lprintf(LO_ERROR,"\nR_InitTextures: Missing patch %d in texture %.8s",
                     SHORT_SWAP(mpatch->patch), texture->name); // killough 4/17/98
              ++errors;
            }
        }
  ...
  for (i=0 ; i< numspritelumps ; i++)
    {
      ...
      spritewidth[i] = SHORT_SWAP(patch->width)<<FRACBITS;
      spriteoffset[i] = SHORT_SWAP(patch->leftoffset)<<FRACBITS;
      spritetopoffset[i] = SHORT_SWAP(patch->topoffset)<<FRACBITS;
    }
}
