/*
	...
	$Id: sys_unix.c 4371 2011-12-12 16:01:57Z sezero $
*/
...
#if defined(__AROS__)
    char cdir[256];
    //Con_Printf ("%s: path: %s, pattern: %s\n", __thisfunc__, path, pattern); 
    if (apath)
        Sys_Error ("Sys_FindFirst without FindClose");
    else
        apath = AllocMem (sizeof(struct AnchorPath) + PATH_SIZE, MEMF_CLEAR);
                        
    //Con_Printf ("%s: AnchorPath allocated\n", __thisfunc__); 
    if (apath)
    {
        ...
        //Con_Printf ("%s: Changing directory...\n", __thisfunc__);         
        oldcurrentdir = CurrentDir(Lock(path,SHARED_LOCK));

        if (MatchFirst(pattern, apath) == 0)
        {
            fib = &(apath->ap_Info);
            //Con_Printf ("%s: Found match: %s\n", __thisfunc__,fib->fib_FileName); 
            return fib->fib_FileName;
        }
    //Con_Printf ("%s: No match.\n", __thisfunc__);
    }
    return NULL;
#else
	if (finddir)
...
void Sys_FindClose (void)
{
    ...
    if (apath != NULL)
    {
        //Con_Printf ("%s: MatchEnd deallocating stuff.\n", __thisfunc__); 
        MatchEnd(apath);
        FreeMem(apath, sizeof(struct AnchorPath) + PATH_SIZE);
        //Con_Printf ("%s: Changing back directory...\n", __thisfunc__); 
...
#if defined(__AROS__)
	while (WaitForChar(stdin,timeout.tv_usec))
#else
