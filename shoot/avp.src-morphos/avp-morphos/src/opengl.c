...
static unsigned int maxTextureSize = 0;
static unsigned char* tempTextureBuffer = 0;
static GLuint	texMinFilter = GL_LINEAR_MIPMAP_LINEAR;


int	highQualityTextures = 1;
int	enableMipmaps = 1;


#define TA_MAXVERTICES		(2*1024)
#define TA_MAXTRIANGLES		(2*1024)

#define OPT_DRAWELEMENTS
...
void InitOpenGL()
{
	...
	if(tempTextureBuffer==0 && maxTextureSize>0)
		tempTextureBuffer = malloc(maxTextureSize*maxTextureSize*4);
		
		
#ifdef __MORPHOS__		
	pglHint(TGL_LOWQUALITY_TEXTURES_HINT, (highQualityTextures>0) ? GL_FALSE : GL_TRUE);
#endif		
	...
	if (tex->filter != CurrentFilteringMode) {
		switch(CurrentFilteringMode) {
			...
			case FILTERING_BILINEAR_ON:
				...
				pglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, texMinFilter);
				break;
			default:
				break;
		}
...
unsigned char* LimitTextureSize(unsigned char* p, unsigned int* w, unsigned int* h)
{
	// todo: fix non-power-of-two textures? There are none in avp but perhaps some mods might use them?
	unsigned int   newW = *w;
	unsigned int   newH = *h;
	unsigned char* newP =  p;
	
	int largestSide = (newW >= newH) ? newW : newH;
	if((maxTextureSize>0) && (largestSide > maxTextureSize))
	{
		...
		{
			float s = ((float)maxTextureSize / (float)largestSide);
			newW = (int) ((float)newW * s);
			newH = (int) ((float)newH * s);
			newP = tempTextureBuffer;
			unsigned char* dstPtr = newP;
			for(int y=0; y<newH; y++)
			{
				int syi = (int) (((float)y / (float)newW) * (float)newH);
				for(int x=0; x<newW; x++)
				{
					int sxi = (int) (((float)x / (float)newW) * (float)newH);
					unsigned char* srcPtr = &p[(syi*newW*4) + (sxi*4)];
					*dstPtr++ = *srcPtr++;
					*dstPtr++ = *srcPtr++;
					*dstPtr++ = *srcPtr++;
					*dstPtr++ = *srcPtr++;
...
unsigned char* HalveTexture(unsigned char* p, unsigned int* w, unsigned int* h, unsigned int c)
{
	if(!tempTextureBuffer)
		return NULL;
		
	unsigned int newW = *w >> 1;
	unsigned int newH = *h >> 1;
	if(newW < 1 || newH < 1)
		return NULL;
	
	
	unsigned int pitch = (*w)*c;
	unsigned char* dstPtr = tempTextureBuffer;

	if(c==4)
	{
		for(int y=0; y<newH; y++)
		{
			unsigned char* line1 = &p[y*pitch*2];
			unsigned char* line2 = line1 + pitch;
			for(int x=0; x<newW; x++)
			{
				*dstPtr++ = ((line1[0] + line1[4] + line2[0] + line2[4])>>2) & 0xFF;
				*dstPtr++ = ((line1[1] + line1[5] + line2[1] + line2[5])>>2) & 0xFF;
				*dstPtr++ = ((line1[2] + line1[6] + line2[2] + line2[6])>>2) & 0xFF;
				*dstPtr++ = ((line1[3] + line1[7] + line2[3] + line2[7])>>2) & 0xFF;
				line1 += 8; line2 += 8;
			}
		}
	}
	else if(c==3)
	{
		for(int y=0; y<newH; y++)
		{
			...
			for(int x=0; x<newW; x++)
			{
				*dstPtr++ = ((line1[0] + line1[3] + line2[0] + line2[3])>>2) & 0xFF;
				*dstPtr++ = ((line1[1] + line1[4] + line2[1] + line2[4])>>2) & 0xFF;
				*dstPtr++ = ((line1[2] + line1[5] + line2[2] + line2[5])>>2) & 0xFF;
				line1 += 6; line2 += 6;
			}
		}
	}
	...
	*w = newW;
	*h = newH;
	return tempTextureBuffer;
}


void UpdateOGLTexture(D3DTexture* tex, unsigned char* buf, GLuint format)
{
	FlushTriangleBuffers(1);
	pglBindTexture(GL_TEXTURE_2D, tex->id);

	unsigned int texW = tex->w;
	unsigned int texH = tex->h;
	unsigned char* texBuf = LimitTextureSize(buf, &texW, &texH);
	...
	if(enableMipmaps)
	{
		...
		while(texBuf)
		{
			pglTexSubImage2D(GL_TEXTURE_2D, mip, 0, 0, texW, texH, format, GL_UNSIGNED_BYTE, texBuf);
			texBuf = HalveTexture(texBuf, &texW, &texH, (format == GL_RGB) ? 3 : 4);
			mip++;
		}
	}
	else
	{
		pglTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, texW, texH, format, GL_UNSIGNED_BYTE, texBuf);
	}
...
GLuint CreateOGLTexture(D3DTexture *tex, unsigned char *buf)
{
	...
	if(enableMipmaps)
	{
		...
		while(texBuf)
		{
			pglTexImage2D(GL_TEXTURE_2D, mip, GL_RGBA, texW, texH, 0, GL_RGBA, GL_UNSIGNED_BYTE, texBuf);
			texBuf = HalveTexture(texBuf, &texW, &texH, 4);
			mip++;
		}
	}
	else
	{
		pglTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texW, texH, 0, GL_RGBA, GL_UNSIGNED_BYTE, texBuf);
	}
