...
const char* window_title = "Aliens versus Predator";
...
extern int enableMipmaps;
extern int highQualityTextures;
...
void LoadDeviceAndVideoModePreferences()
{
	...
	CurrentVideoMode = -1;
	fp = OpenGameFile("AvP_Video.cfg", FILEMODE_READONLY, FILETYPE_CONFIG);
	if (fp != NULL)
	{
		int magic;
		if(fread(&magic, 4, 1, fp) && magic == 0xDEADBEEF)
		{
			fread(&CurrentVideoMode, 	4, 1, fp);
			fread(&highQualityTextures, 4, 1, fp);
			fread(&enableMipmaps, 		4, 1, fp);
			fread(&FullscreenMode,		4, 1, fp);
		}
	}
	fclose(fp);
	
	if(CurrentVideoMode >= 0 && CurrentVideoMode < TotalVideoModes && VideoModeList[CurrentVideoMode].available)
		return;
...
void SaveDeviceAndVideoModePreferences()
{
	...
	fp = OpenGameFile("AvP_Video.cfg", FILEMODE_WRITEONLY, FILETYPE_CONFIG);
	if (fp != NULL)
	{
		int magic = 0xDEADBEEF;
		fwrite(&magic, 4, 1, fp);
		fwrite(&CurrentVideoMode, 		4, 1, fp);
		fwrite(&highQualityTextures, 	4, 1, fp);
		fwrite(&enableMipmaps, 			4, 1, fp);
		fwrite(&FullscreenMode,			4, 1, fp);
		fclose(fp);
	}
}

void ApplyDeviceAndVideoModePreferences()
{
	if((FullscreenMode  && !(surface->flags & SDL_FULLSCREEN)) || (!FullscreenMode && surface->flags & SDL_FULLSCREEN))
	{
...
int InitSDL()
{
	...
	if (SDL_Init(SDL_INIT_VIDEO) < 0)
		Amiga_FatalError("SDL Init failed: %s", SDL_GetError());
...
int SetSoftVideoMode(int Width, int Height, int Depth)
{
	...
	{
		if((WantFullscreen || FullscreenMode) && !WantWindowed)
			flags |= SDL_FULLSCREEN;
		isgrab = SDL_GRAB_OFF;
	}
	...
	if ((surface = SDL_SetVideoMode(Width, Height, Depth, flags)) == NULL)
		Amiga_FatalError("(Software) SDL SetVideoMode failed: %s\n", SDL_GetError());
...
int SetOGLVideoMode(int Width, int Height)
{
	...
	if ((surface = SDL_SetVideoMode(Width, Height, 0, flags)) == NULL) {
		Amiga_FatalError("(OpenGL) SDL SetVideoMode failed: %s\n", SDL_GetError());
	}
...
"      [-f | --fullscreen]     Play the game fullscreen\n"
"      [-w | --windowed]       Play the game in a window\n"
"      [-s | --nosound]        Do not access the soundcard\n"
"      [-m | --nomusic]        Do not play ingame music\n"
...
{			
	...
	if (argc>1)
	{
		while ((c = getopt_long(argc, argv, "hvfwsmd:", getopt_long_options, NULL)) != -1)
		{
			switch(c)
			{
				case 'h':
					printf("%s\n", GetAvpVersionString());
					printf("%s\n", GetAvpExtraVersionString());
	...
	if(!InitGameDirectories(argv[0]))
	{
		Amiga_GameDataInstaller();
	
		if(!InitGameDirectories(argv[0]))
			Amiga_FatalError("Unable to find AvP gamedata. Make sure the game is correctly installed.");
	}
	...
	if (gm == SDL_GRAB_ON || surface->flags & SDL_FULLSCREEN)
		SDL_ShowCursor(0);
