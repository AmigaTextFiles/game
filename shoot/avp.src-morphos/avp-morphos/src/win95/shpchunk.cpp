...
{ 
	...
	while ((data-buffer_ptr)< (signed)size)
	{
		int s = *(int*)(data+8);
		BYTESWAP4(s);
		if (s + (data-buffer_ptr) > (signed)size)
		{
...
,num_verts (vtsize / 12) // 12 bytes per vertex
{
	...
	int num_verts_TEMP = vtsize;
	BYTESWAP4(num_verts_TEMP);
	num_verts_TEMP /= 12;
	...
	*((ChunkVectorInt**) &vert_data) = parent->shape_data_store->v_list;
	...
	for (i=0;i<num_verts;i++)
	{
		...
		parent->shape_data_store->v_list[i].ByteSwap();
...
{
chunk_implement("Shape_Vertex_Chunk::output_chunk");
return TRUE;
/*
	unsigned long junk;
	BOOL ok;
...
{
chunk_implement("Shape_Vertex_Chunk::fill_data_block");
/*
...
,num_verts (vtsize / sizeof(ChunkVectorInt)) 
{
	...
	*((ChunkVectorFloat**) &vert_norm_data) = parent->shape_data_store->v_normal_list;
	...
	for (i=0;i<num_verts;i++)
	{
		...
		parent->shape_data_store->v_normal_list[i].ByteSwap();
...
{
chunk_implement("Shape_Vertex_Normal_Chunk::output_chunk");
return TRUE;
/*
	unsigned long junk;
	BOOL ok;
...
{
chunk_implement("Shape_Vertex_Normal_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
,num_polys (pnsize / sizeof(ChunkVectorFloat)) 
{
	...
	*((ChunkVectorFloat**) &poly_norm_data) = parent->shape_data_store->p_normal_list;
	...
	for (i=0;i<num_polys;i++)
	{
		...
		parent->shape_data_store->p_normal_list[i].ByteSwap();
...
{
chunk_implement("Shape_Polygon_Normal_Chunk::output_chunk");
return TRUE;
/*
	unsigned long junk;
	BOOL ok;
...
{
chunk_implement("Shape_Polygon_Normal_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
,poly_data (parent->shape_data_store->poly_list)
,num_polys (psize / 36) // 9 * 4 bytes per polygon
{
	int i, j;
	...
	*((ChunkPoly **) &poly_data) = parent->shape_data_store->poly_list;
	...
	for (i=0; i<num_polys; i++)
	{
		...
		BYTESWAP4(parent->shape_data_store->poly_list[i].engine_type);
		BYTESWAP4(parent->shape_data_store->poly_list[i].normal_index);
		BYTESWAP4(parent->shape_data_store->poly_list[i].flags);
		BYTESWAP4(parent->shape_data_store->poly_list[i].colour);

		parent->shape_data_store->poly_list[i].num_verts = 0;

		int j=0;
		int val = *((int *) (pdata + (36*i) + 16 + (j*4)));
		BYTESWAP4(val);
		
		while(val != -1)
		{
			parent->shape_data_store->poly_list[i].vert_ind[j] = val;
			parent->shape_data_store->poly_list[i].num_verts++;
			j++;
			val = *((int *) (pdata + (36*i) + 16 + (j*4)));
			BYTESWAP4(val);
		}
		/*
...
,num_polys (psize / 36) // 9 * 4 bytes per polygon
{
	...
	*((ChunkPoly **) &poly_data) =poly_list;
	...
	for (i=0; i<num_polys; i++)
	{
		...
		BYTESWAP4(poly_list[i].engine_type);
		BYTESWAP4(poly_list[i].normal_index);
		BYTESWAP4(poly_list[i].flags);
		BYTESWAP4(poly_list[i].colour);
...
{
chunk_implement("Shape_Polygon_Chunk::output_chunk");
/*
	unsigned long junk;
	BOOL ok;
	char * data_block;
...
{
chunk_implement("Shape_Polygon_Chunk::fill_data_block");
/*
	int i, j;
	strncpy (data_start, identifier, 8);
	data_start += 8;
...
:Chunk (parent,"SHPCENTR")
{
	...
	BYTESWAP4(cs->radius_about_centre);
}

void Shape_Centre_Chunk::fill_data_block(char * data_start)
{
chunk_implement("Shape_Centre_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Shape_UV_Coord_Chunk::output_chunk");
return TRUE;
/*
	unsigned long junk;
	BOOL ok;
...
{
chunk_implement("Shape_UV_Coord_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
,tex_fns ()
{
	...
	num_tex_fns = *((int *)tfndata);
	BYTESWAP4(num_tex_fns);
	tfndata += 4;
...
{
chunk_implement("Shape_Texture_Filenames_Chunk::output_chunk");
return TRUE;
/*
	unsigned long junk;
	BOOL ok;
...
{
chunk_implement("Shape_Texture_Filenames_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
	...
	shape_data (parent->shape_data_store)
{
	...
	BYTESWAP4(file_id_num);
	hdata+=4;

	
	parent->shape_data_store->num_verts = *((int *) hdata );
	BYTESWAP4(parent->shape_data_store->num_verts);
	hdata+=4;
	

	parent->shape_data_store->num_polys = *((int *) hdata );
	BYTESWAP4(parent->shape_data_store->num_polys);
	hdata+=4;

	parent->shape_data_store->radius = *((float *) hdata);
	BYTESWAP4(parent->shape_data_store->radius);
	hdata+=4;
	
	parent->shape_data_store->max.x = *((int *) hdata);
	BYTESWAP4(parent->shape_data_store->max.x);
	hdata+=4;
	
	parent->shape_data_store->min.x = *((int *) hdata);
	BYTESWAP4(parent->shape_data_store->min.x);
	hdata+=4;

	parent->shape_data_store->max.y = *((int *) hdata);
	BYTESWAP4(parent->shape_data_store->max.y);
	hdata+=4;
	
	parent->shape_data_store->min.y = *((int *) hdata);
	BYTESWAP4(parent->shape_data_store->min.y);
	hdata+=4;

	parent->shape_data_store->max.z = *((int *) hdata);
	BYTESWAP4(parent->shape_data_store->max.z);
	hdata+=4;
	
	parent->shape_data_store->min.z = *((int *) hdata);
	BYTESWAP4(parent->shape_data_store->min.z);
	hdata+=4;

	version_no = *((int *) hdata);
	BYTESWAP4(version_no);
	hdata+=4;

	num_as_obj = *((int *) hdata);
	BYTESWAP4(num_as_obj);
	hdata+=4;
...
{
chunk_implement("Shape_Header_Chunk::output_chunk");
return TRUE;
/*
	unsigned long junk;
	BOOL ok;
...
{
chunk_implement("Shape_Header_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Shape_Merge_Data_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Shape_External_Filename_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Shape_External_Object_Name_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
: Chunk (parent, "FRMMORPH")
{
	...
	for (int i=0; i<num_frames; i++)
	{
		...
		BYTESWAP4(frame_store[i*3]);
		data +=4;
		
		frame_store[i*3+1] = *((int *)data);
		BYTESWAP4(frame_store[i*3+1]);
		data +=4;
		
		frame_store[i*3+2] = *((int *)data);
		BYTESWAP4(frame_store[i*3+2]);
		data +=4;
	}
}	
...
{
chunk_implement("Shape_Morphing_Frame_Data_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Shape_Sub_Shape_Header_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Shape_Poly_Change_Info_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
: Chunk (parent, "SHPPCINF")
{
	...
	for (int i=0; i<n_entries; i++)
	{
		...
		BYTESWAP4(pci.poly_num);
		data += 4;
		
		pci.vert_num_before = *((int *) data);
		BYTESWAP4(pci.vert_num_before);
		data += 4;
		
		pci.vert_num_after = *((int *) data);
		BYTESWAP4(pci.vert_num_after);
		data += 4;
...
{
chunk_implement("Shape_Name_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Shape_Fragments_Data_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Shape_Fragment_Location_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Shape_Fragment_Type_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Anim_Shape_Sequence_Data_Chunk::fill_data_block");
/*
...
{
chunk_implement("Anim_Shape_Frame_Data_Chunk::fill_data_block");
/*
	strncpy (datastart, identifier, 8);
	datastart += 8;
	*((int *) datastart) = chunk_size;
...
{
chunk_implement("Poly_Not_In_Bounding_Box_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Anim_Shape_Centre_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Console_Type_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
	block_size=*(int*)data;
	BYTESWAP4(block_size);
	data+=4;
	first_pointer=*(int*)data;
	BYTESWAP4(first_pointer);
	data+=4;
	...
	if(block_size)
	{
		...
		{
			memory_block[i] = *(int*)data;
			BYTESWAP4(memory_block[i]);
			data+=4;
		}
	}
	else
...
{
chunk_implement("Shape_Preprocessed_Data_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
