...
#define chunk_implement(x)	{ printf("*** %s:%i NOT IMPLEMENTED %s\n\0", __FILE__, __LINE__, x); }
...
#define CHUNK_WITH_CHILDREN_LOADER_INIT_PT2 { \
	...
	while (__data - __buffer_ptr < (signed)__size){ \
		int s = *(int*)(__data+8); \
		BYTESWAP4(s); \
		if (s + (__data-__buffer_ptr) > (signed)__size){ \
...
#define CHUNK_WITH_CHILDREN_LOADER_FOR(id,chunkclass) \
			...
			new chunkclass(this,__data+12,s-12); \
			__data += s;}

#define CHUNK_WITH_CHILDREN_LOADER_END \
		else { \
			new Miscellaneous_Chunk (this, __data, (__data + 12), s-12 ); \
			__data += s;}}}
...
static Chunk * RifCreateClassObject ## tokenClassName ##_## nLine (Chunk_With_Children* parent,const char* data) { \
	int s = *(int*)(data+8); \
	BYTESWAP4(s); \
	Chunk * pChunk = new tokenClassName(parent,data+12,s-12); \
	return pChunk; \
} \
...
static Chunk * RifCreateClassObject ## tokenClassName ##_## nLine (Chunk_With_Children* parent,const char* data) { \
	int s = *(int*)(data+8); \
	BYTESWAP4(s) \
	Chunk * pChunk = new tokenClassName((parentClassName*)parent,data+12,s-12); \
	return pChunk; \
} \
 class RegisterRifChunkClass ## tokenClassName ##_## nLine { \
	public: RegisterRifChunkClass ## tokenClassName ##_## nLine () { \
