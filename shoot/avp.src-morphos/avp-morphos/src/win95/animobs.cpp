...
{
	chunk_implement("Object_Animation_Sequence_Frame_Chunk::fill_data_block");
/*
...
: Chunk (parent, "OBASEQFR")
{
	...
	data_start += 4;
	BYTESWAP4(at_frame_no);
	
	frame_ref_no = *((int *) data_start);
	data_start += 4;
	BYTESWAP4(frame_ref_no);
...
{
	chunk_implement("Object_Animation_Sequence_Header_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
...
: Chunk (parent, "OBASEQHD"), sequence_name (0)
{
	...
	BYTESWAP4(sequence_number);
	data_start += 4;

	sub_sequence_number = *((int *) data_start);
	BYTESWAP4(sub_sequence_number);
	data_start += 4;
...
: Chunk (parent, "OBASEQTM")
{
	...
	BYTESWAP4(sequence_time);	
}

void Object_Animation_Sequence_Time_Chunk::fill_data_block(char* data_start)
{
	chunk_implement("Object_Animation_Sequence_Time_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
	chunk_implement("Object_Animation_Sequence_Speed_Chunk::fill_data_block");
/*
	CHUNK_FILL_START
...
{
	chunk_implement("Object_Animation_Sequence_Flags_Chunk::fill_data_block");
/*
	CHUNK_FILL_START
...
:Chunk(parent,"OBANALLS")
{
	...
	for(int i=0;i<num_sequences;i++)
	{
		...
		BYTESWAP4(seq->num_frames);
		data+=4;
		seq->sequence_number=*(int*)data;
		BYTESWAP4(seq->sequence_number);
		data+=4;
		seq->sub_sequence_number=*(int*)data;
		BYTESWAP4(seq->sub_sequence_number);
		data+=4;
		seq->sequence_time=*(int*)data;
		BYTESWAP4(seq->sequence_time);
		data+=4;
		...
		for(unsigned j=0;j<seq->num_frames;j++)
		{
			...
			seq->frames[j].orientation.ByteSwap();
			seq->frames[j].transform.ByteSwap();
			BYTESWAP4(seq->frames[j].at_frame_no);
			BYTESWAP4(seq->frames[j].flags);
...
{
	chunk_implement("Object_Animation_All_Sequence_Chunk::fill_data_block");
/*
