...
: Chunk(parent, "TEXTANIM")
{
	...
	if(NumPolys)
	{
		...
		for(int i=0;i<NumPolys;i++)
		{
			...
			BYTESWAP4(ta->NumVerts);
			data+=4;
			
			ta->AnimFlags=*((int*)data);
			BYTESWAP4(ta->AnimFlags);
			data+=4;
			
			ta->Identifier=*((int*)data);
			BYTESWAP4(ta->Identifier);
			data+=4;
...
{
chunk_implement("Animation_Chunk::output_chunk");
return TRUE;
/*
	unsigned long junk;
	BOOL ok;
...
{
chunk_implement("Animation_Chunk::fill_data_block");
/*
