...
{
chunk_implement("Chunk_With_BMPs::fill_data_block");
/*
...
{
chunk_implement("BMP_Names_Version_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("BMP_Names_ExtraData_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
...
{
chunk_implement("External_Shape_BMPs_Store_Chunk::fill_data_block");
/*
...
: Chunk_With_BMPs (parent, "SHBMPNAM", sdata, ssize)
{
	...
	int val = *(int*)(sdata);
	BYTESWAP4(val);
	flags = (GlobalBMPFlags)(val & GBF_MASK);
	sdata+=4;
...
ImageDescriptor::ImageDescriptor(char const * datablock)
{
	...
	int val = *(int*)(datablock);
	BYTESWAP4(val);
	flags = (IDscFlags)(val & IDSCF_MASK);
...
{
	*(int *)datablock = flags;
	BYTESWAP4(datablock[0]);
	*(int *)(datablock+4) = spares[0];
	BYTESWAP4(datablock[4]);
	*(int *)(datablock+8) = spares[1];
	BYTESWAP4(datablock[8]);
	*(int *)(datablock+12) = spares[2];
	BYTESWAP4(datablock[12]);
	datablock+=16;
...
: Chunk(parent,"MATCHIMG")
{
	...
	int val = *(int*)(datablock+8);
	BYTESWAP4(val);
	flags = (MICFlags)(val & MICF_MASK);
...
{
chunk_implement("Matching_Images_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*(int *) data_start = chunk_size;
...
: Chunk(parent,"BMPMD5ID")
,flags(BMD5F_0)
,version_num(rcbmp.version_num)
,spare(0)
{
...
: Chunk(parent,"BMPMD5ID")
{
	...
	int val = *(int*)(datablock+4);
	BYTESWAP4(val);
	flags = (BMPMD5_Flags)(val & BMD5F_MASK);
	
	version_num = *(int*)(datablock+8);
	BYTESWAP4(version_num);
...
{
chunk_implement("Bitmap_MD5_Chunk::fill_data_block");
/*
