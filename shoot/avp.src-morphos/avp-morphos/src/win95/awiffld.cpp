				...
				case IFF::IlbmBmhdChunk::MASK_TRANSPARENTCOL:
					...
					if (pixelFormat.palettizedB)
						...
						AwIffConvTransp::rawTranspCol = 
							 ((((static_cast<unsigned>(m_pPalette[AwIffConvTransp::iTranspCol].r))&0xFF)>>pixelFormat.redRightShift)<<pixelFormat.redLeftShift) |
							 ((((static_cast<unsigned>(m_pPalette[AwIffConvTransp::iTranspCol].g))&0xFF)>>pixelFormat.greenRightShift)<<pixelFormat.greenLeftShift) |
							 ((((static_cast<unsigned>(m_pPalette[AwIffConvTransp::iTranspCol].b))&0xFF)>>pixelFormat.blueRightShift)<<pixelFormat.blueLeftShift);
					break;
				default:
