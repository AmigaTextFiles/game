...
{
chunk_implement("Object_Header_Chunk::output_chunk\n\0");
return TRUE;
/*
	unsigned long junk;
	BOOL ok;
...
{
chunk_implement("Object_Header_Chunk::fill_data_block\n\0");
/*
...
{
chunk_implement("Object_Notes_Chunk::fill_data_block\n\0");
	/*
	strncpy (data_start, identifier, 8);
...
{
chunk_implement("Object_Notes_Chunk::output_chunk\n\0");
return TRUE;
/*
	unsigned long junk;
	BOOL ok;
...
: Chunk_With_Children (parent, "MODULEDT")
{
	...
	while ((mdata-buffer_ptr)< (signed) msize)
	{
		int s = *(int*)(mdata+8);
		BYTESWAP4(s);
		if ((s) + (mdata-buffer_ptr) > (signed) msize)
		{
			Parent_File->error_code = CHUNK_FAILED_ON_LOAD_NOT_RECOGNISED;
			break;
		}
...
{
chunk_implement("VModule_Array_Chunk::fill_data_block\n\0");
/*
	strncpy (data_start, identifier, 8);
...
: Chunk (parent, "VMDARRAY")
{
	...
	for (int i=0; i<num_array_items; i++)
	{
		...
		BYTESWAP4(vmod_array[i].object_index);
		vmdata += 4;

		vmod_array[i].branch_no = *((int *) vmdata);
		BYTESWAP4(vmod_array[i].branch_no);
		vmdata += 4;
		
		vmod_array[i].flags = *((int *) vmdata);
		BYTESWAP4(vmod_array[i].flags);
		vmdata += 4;

		vmod_array[i].spare = *((int *) vmdata);
		BYTESWAP4(vmod_array[i].spare);
		vmdata += 4;
...
{
chunk_implement("Adjacent_Modules_Chunk::fill_data_block\n\0");
/*
	strncpy (data_start, identifier, 8);
...
{
chunk_implement("Adjacent_Module_Entry_Points_Chunk::fill_data_block\n\0");
/*
	strncpy (data_start, identifier, 8);
...
{
chunk_implement("Module_Flag_Chunk::fill_data_block\n\0");
/*
	strncpy (data_start, identifier, 8);
...
{
chunk_implement("Module_Zone_Chunk::fill_data_block\n\0");
/*
	strncpy (data_start, identifier, 8);
...
{
chunk_implement("Module_Acoustics_Chunk::fill_data_block\n\0");
/*
	strncpy (data_start, identifier, 8);
...
{
chunk_implement("Object_Track_Chunk2::fill_data_block\n\0");
/*
	strncpy (data_start, identifier, 8);
...
: Chunk (parent, "OBJTRAK2")
{
	...
	for(i=0;i<num_sections;i++)
	{
		...
		sections[i].quat_start.ByteSwap();
		sections[i].quat_end.ByteSwap();
		sections[i].pivot_start.ByteSwap();
		sections[i].pivot_end.ByteSwap();
		sections[i].object_offset.ByteSwap();
		BYTESWAP4(sections[i].time_for_section);
		BYTESWAP4(sections[i].spare);
...
:Chunk(parent,"TRAKSOUN")
{
	...
	BYTESWAP4(inner_range);
	data+=4;
	
	outer_range=*(unsigned long*)data;
	BYTESWAP4(outer_range);
	data+=4;

	max_volume=*(int*)data;
	BYTESWAP4(max_volume);
	data+=4;
...
{
chunk_implement("Object_Track_Sound_Chunk::fill_data_block\n\0");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
:Chunk(parent,"ALTLOCAT")
{
	...
	for(int i=0;i<num_locations;i++)
	{
		...
		locations[i].position.ByteSwap();
		locations[i].orientation.ByteSwap();
		BYTESWAP4(locations[i].spare1);
		BYTESWAP4(locations[i].spare2);
...
{
chunk_implement("Object_Alternate_Locations_Chunk::fill_data_block\n\0");
/*
