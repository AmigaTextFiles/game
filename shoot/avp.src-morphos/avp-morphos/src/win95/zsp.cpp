...
ZSP_Data::ZSP_Data (const char * zdata, size_t /*zsize*/)
{
	...
	num_x_cubes = *((int *) ptr);
	BYTESWAP4(num_x_cubes);
	ptr += 4;

	num_y_cubes = *((int *) ptr);
	BYTESWAP4(num_y_cubes);
	ptr += 4;

	num_z_cubes = *((int *) ptr);
	BYTESWAP4(num_z_cubes);
	ptr += 4;
	...
	for (i=0; i<numzones; i++)
	{
		...
		BYTESWAP4(tmpzone.num_z_polys);
		ptr += 4;

		tmpzone.num_z_verts = *((int *) ptr);
		BYTESWAP4(tmpzone.num_z_verts);
		ptr += 4;
		...
		for (j=0; j<tmpzone.num_z_polys; j++)
		{
			...
			BYTESWAP4(tmpzone.z_poly_list[j]);
			ptr += 4;
		}
		...
		for (j=0; j<tmpzone.num_z_verts; j++)
		{
			...
			BYTESWAP4(tmpzone.z_vert_list[j]);
			ptr += 4;
		}
...
{
chunk_implement("Shape_ZSP_Data_Chunk::fill_data_block");
/*
