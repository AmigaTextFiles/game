		...
		for(int j=0;j<NumPitch;j++)
		{
			...
			for(int k=0;k<NumFrames;k++)
			{
				...
				for(int l=0;l<4;l++)
				{
					...
					BYTESWAP4(f->UVCoords[l][0]);
					data+=4;
					
					f->UVCoords[l][1]=*((int*)data);		
					BYTESWAP4(f->UVCoords[l][1]);
					data+=4;
				}
			}
		}
...
{
chunk_implement("Sprite_Action_Chunk::fill_data_block");
/*
...
:Chunk_With_Children(parent,"SPRITEPC")
{
	...
	while ((data-buffer_ptr)< (signed) datasize)
	{
		int s = *(int*)(data+8);
		BYTESWAP4(s);
		if (s + (data-buffer_ptr) > (signed) datasize)
		{
		...
		{
			new Sprite_Action_Chunk (this, (data + 12), s-12);
			data += s;
		}
		else
		{
			new Miscellaneous_Chunk (this, data, (data + 12), s-12 );
			data += s;
		}
...
{
chunk_implement("Sprite_Size_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Sprite_Version_Number_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Sprite_Bitmap_Scale_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Sprite_Bitmap_Centre_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Sprite_Extent_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
