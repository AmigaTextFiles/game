	...
	if(NumWaypoints)
	{
		...
		for(int i=0;i<NumWaypoints;i++)
		{
			...
			for(j=0;j<cw->NumWPLinks;j++)
			{
				...
				BYTESWAP4(cw->WayLinks[j].index);
				BYTESWAP4(cw->WayLinks[j].flags);
				data+=sizeof(WaypointLink);
			}
				
			cw->NumModLinks=*(int*)data;
			BYTESWAP4(cw->NumModLinks);
			data+=4;
...
{
chunk_implement("Module_Waypoint_Chunk::fill_data_block");
/*
...
{
chunk_implement("AI_Module_Master_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("AI_Module_Slave_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
