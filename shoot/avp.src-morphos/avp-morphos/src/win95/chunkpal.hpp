...
extern unsigned char * grab_pixel_data(int ssize, unsigned char* sdata);
...
public:
	...
	Environment_Palette_Chunk (Chunk_With_Children * const parent, const char * sdata, size_t const /*ssize*/)
		: Chunk (parent, "ENVPALET")
		{
			...
			flags = *(int *)(sdata+4);
			BYTESWAP4(flags);
			pixel_data = grab_pixel_data(width, (unsigned char *)(sdata+8));
	...
	Preset_Palette (char const * sdata)
		{
			...
			BYTESWAP4(startpos);
			pixel_data = grab_pixel_data(size, (unsigned char *)(sdata+20));
			sdata += 20+size*3;
...
public:
	...
	TLTConfigBlock(char const * datablock)
		{
			...
			BYTESWAP8(input_intensity_start);
			output_intensity_start = *(double *)(datablock+8);
			BYTESWAP8(output_intensity_start);
			input_intensity_end = *(double *)(datablock+16);
			BYTESWAP8(input_intensity_end);
			output_intensity_end = *(double *)(datablock+24);
			BYTESWAP8(output_intensity_end);
	...
	inline void WriteData(char * datablock) const
		{
		chunk_implement("TLTConfigBlock::WriteData");
		/*
