...
void ObjectID::ByteSwap()
{
	int ids[2] = { id1, id2 };
	char* s = (char*) ids;
	if(s[7] != 0)
	{
		printf("******* should have byteswapped ************\n");
	}
}


char chnktype_temp_idstring[256];
char* ObjectID::GetDebugString()
{
	int ids[2] = { id1, id2 };
	char* s = (char*)ids;
	sprintf(chnktype_temp_idstring, " %02x,%02x,%02x,%02x %02x,%02x,%02x,%02x ", s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7]);
	return chnktype_temp_idstring;
}
