...
struct ChunkVector
{
	...
	double z;
  	...
  	void ByteSwap() { BYTESWAP8(x); BYTESWAP8(y); BYTESWAP8(z);	}
...
struct ChunkVectorInt
{
	...
	int z;
  	...
  	void ByteSwap() { BYTESWAP4(x); BYTESWAP4(y); BYTESWAP4(z); }
...
struct ChunkUV
{
	...
	float v;
  	...
  	void ByteSwap() { BYTESWAP4(u); BYTESWAP4(v); }
};
...
struct ChunkMatrix
{
	...
	int mat33;
  	...
  	void ByteSwap()
  	{
		BYTESWAP4(mat11); BYTESWAP4(mat12); BYTESWAP4(mat13);
		BYTESWAP4(mat21); BYTESWAP4(mat22); BYTESWAP4(mat23);
		BYTESWAP4(mat31); BYTESWAP4(mat32); BYTESWAP4(mat33);
	}
};
...
public:
	...
	void ByteSwap()
	{
		BYTESWAP4(engine_type);
		BYTESWAP4(normal_index);
		BYTESWAP4(flags);
		BYTESWAP4(colour);
		BYTESWAP4(num_verts);
		BYTESWAP4(vert_ind[0]);
		BYTESWAP4(vert_ind[1]);
		BYTESWAP4(vert_ind[2]);
		BYTESWAP4(vert_ind[3]);
	}
};
...
struct ChunkQuat
{
	...
	void ByteSwap() { BYTESWAP4(x); BYTESWAP4(y); BYTESWAP4(z); BYTESWAP4(w); }
};
...
struct ChunkObject
{
	...
	void ByteSwap()
	{
		...
		is_base_object = (is_base_object != 0) ? TRUE : FALSE;
		BYTESWAP4(index_num);
		ID.ByteSwap();
	}
};
...
struct obinfile
{
	...
	void ByteSwap() { BYTESWAP4(filepos); BYTESWAP4(length); }
};
...
struct shpinfile
{
	...
	void ByteSwap() { BYTESWAP4(filepos); BYTESWAP4(id); BYTESWAP4(length); }
};
...
struct poly_change_info
{
	...
	void ByteSwap() { BYTESWAP4(poly_num); BYTESWAP4(vert_num_before); BYTESWAP4(vert_num_after); }
};
