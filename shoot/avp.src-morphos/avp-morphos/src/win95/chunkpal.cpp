...
unsigned char * grab_pixel_data(int ssize, unsigned char* sdata)
{
	if (!ssize) return 0;
	ssize *= 3;
	...
	for (int cnt = ssize; cnt; --cnt, ++ddata, ++sdata)
	{
		*ddata = *sdata;
	}
	return pixels;
...
{
chunk_implement("Environment_Palette_Chunk::fill_data_block");
/*
...
{
chunk_implement("Preset_Palette::fill_data_block");
/*
	*(int*)data_start = size;
	*(int*)(data_start+4) = flags;
...
{
	version_num = *(int *)(sdata+0);
	BYTESWAP4(version_num);
	flags = *(int *)(sdata+4);
	BYTESWAP4(flags);
...
{
chunk_implement("Preset_Palette_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*(int *) data_start = chunk_size;
...
, table (0)
{
	...
	num_levels = *(int*)(sdata+4);
	BYTESWAP4(num_levels);
	
	flags = *(int*)(sdata+28);
	BYTESWAP4(flags);
	
	int i;
	for (i=0; i<ChunkTLT_NumReserved; ++i)
	{
		reserved[i] = *(int *)(sdata+8+(i<<2));
		BYTESWAP4(reserved[i]);
	}
...
{
chunk_implement("Environment_TLT_Chunk::fill_data_block");
/*
	int i;
	strncpy (data_start, identifier, 8);
	data_start += 8;
...
{
chunk_implement("TLT_Config_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("Environment_Game_Mode_Header_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("RIF_Child_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
: Chunk(parent,"RIFCHILD")
, egm_parent((Environment_Game_Mode_Chunk * const)parent)
{
	int i;
	...
	for (i = listsize; i; --i)
	{
		...
		BYTESWAP4(temp.flags);
		sdata += 4;
		
		unsigned int v1;
		v1 = *(unsigned int *)sdata;
		BYTESWAP4(v1);
		temp.enum_id = (int)(v1 >> BMPFLAGS_ENUMID_SHIFT);
		int v2 = *(int *)sdata;
		BYTESWAP4(v2);
		temp.version_num = v2 & BMPFLAGS_VERSION_NUM_MASK;
		sdata += 4;
...
{
chunk_implement("Preset_Palette_Store_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*(int *) data_start = chunk_size;
...
, table (0)
{
	...
	for (i=0; i<ChunkCPLU_NumReserved; ++i)
	{
		reserved[i] = *(int *)(sdata+4+(i<<2));
		BYTESWAP4(reserved[i]);
	}
...
{
chunk_implement("Coloured_Polygons_Lookup_Chunk::fill_data_block");
/*
	int i;
	strncpy (data_start, identifier, 8);
	data_start += 8;
