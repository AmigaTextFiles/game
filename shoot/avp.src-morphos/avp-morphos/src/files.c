...
static char* RemoveTrailingDirSeparator(char* rfilename)
{
	if(!rfilename)
		return rfilename;
...
static char *FixFilename(const char *filename, const char *prefix, int force)
{
	...
	strcpy(f, prefix);
	if ((plen>1) && (prefix[plen - 2] != '/') && (prefix[plen - 2] != ':')) // don't add another dir separator
...
int CreateGameDirectory(const char *dirname)
{
	...
	RemoveTrailingDirSeparator(rfilename);
...
/*
  Game-specific helper function.
 */
int CheckGameDirectory(char *dir)
{
	if (!dir || !*dir) {
		return 0;
	}
	...
	if (!try_game_directory(dir, "avp_huds/marine.rif")) {
		return 0;
	}
	...
	if (!try_game_directory(dir, "avp_rifs/invasion.rif")) {
		return 0;
	}
	...
	if (!try_game_directory(dir, "fastfile/mffinfo.txt")) {
		return 0;
	}
#elif PREDATOR_DEMO
	if (!try_game_directory(dir, "avp_huds")) {
	...
	if (!try_game_directory(dir, "avp_huds/predator.rif")) {
		return 0;
	}
	...
	if (!try_game_directory(dir, "avp_rifs/invasion_p.rif")) {
		return 0;
	}
	if (!try_game_directory(dir, "fastfile")) {
		return 0;
...
#elif ALIEN_DEMO
	if (!try_game_directory(dir, "alienavp_huds")) {
		return 0;
	}
	
	if (!try_game_directory(dir, "alienavp_huds/alien.rif")) {
		return 0;
	}
	...
	if (!try_game_directory(dir, "avp_rifs/invasion_a.rif")) {
		return 0;
	}

	if (!try_game_directory(dir, "alienfastfile")) {
		return 0;
	}
	
	if (!try_game_directory(dir, "alienfastfile/ffinfo.txt")) {
		return 0;
	}
#else		
	if (!try_game_directory(dir, "avp_huds")) {
	...
	if (!try_game_directory(dir, "language.txt")) {
		return 0;
	}
 ...
 */
int InitGameDirectories(char *argv0)
{
