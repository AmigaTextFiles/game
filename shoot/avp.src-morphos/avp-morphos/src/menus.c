...
#define PACKCOLOR(r,g,b)			((((r)>>3)<<11) | (((g)>>2)<<5 ) | (((b)>>3)))
#define UNPACKR(c)					(((c) & 0xF800)>>8)
#define UNPACKG(c)					(((c) & 0x07E0)>>3)
#define UNPACKB(c)					(((c) & 0x001F)<<3)
#define UNPACKCOLOR(c, r, g, b) 	{ r = UNPACKR(c); g = UNPACKG(c); b = UNPACKB(c); }
...
static void DrawAvPMenuGlowyBar(int topleftX, int topleftY, int alpha, int length)
{
	...
	if (alpha>ONE_FIXED) {
		...
		for (y=topleftY; y<gfxPtr->Height+topleftY; y++) {
			...
			for (x=0; x<length; x++) {
				*destPtr =	PACKCOLOR(srcPtr[0], srcPtr[1], srcPtr[2]);
				destPtr++;
			}
		...
		for (y=topleftY; y<gfxPtr->Height+topleftY; y++) {
			...
			for (x=0; x<length; x++)
			{
				if (srcPtr[0] || srcPtr[1] || srcPtr[2])
				{
					...
					UNPACKCOLOR(*destPtr, destR, destG, destB);
					...
					*destPtr = PACKCOLOR(destR, destG, destB);
				}
...
void RenderKeyConfigRectangle(int alpha)
{
	...
	unsigned short val = MUL_FIXED(0xFF, alpha);
	if(val > 0x00FF) val = 0x00FF;
	c = PACKCOLOR(val, val, val);
	/*
...
void DrawAvPMenuGfxEx(AVPMENUGFX* gfxPtr, int topleftX, int topleftY, int alpha,enum AVPMENUFORMAT_ID format)
{
	D3DTexture *image;
	unsigned char *srcPtr;
	unsigned short *destPtr;
...
{
	GLOBALASSERT(menuGfxID < MAX_NO_OF_AVPMENUGFXS);
	AVPMENUGFX* gfxPtr = &AvPMenuGfxStorage[menuGfxID];
	DrawAvPMenuGfxEx(gfxPtr, topleftX, topleftY, alpha, format);
}
	...
	if (alpha == ONE_FIXED) {
		...
		for (y=0; y<480; y++) {
			...
			for (x=0; x<640; x++) {
				...
				*destPtr = PACKCOLOR(srcR2, srcG2, srcB2);
				srcPtr += 4;
				srcPtr2 += 4;
				destPtr++;
			}
		...
		for (y=topleftY; y<gfxPtr->Height+topleftY; y++) {
			...
			for (x=0; x<length; x++) {
				...
				if (srcPtr[0] || srcPtr[1] || srcPtr[2]) {
					...
					*destPtr = PACKCOLOR(srcR, srcG, srcB);
				}
