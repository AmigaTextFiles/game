...
void GenerateSoundBuffer(int* handle, void* data, int len, ALushort fmt, ALushort freq)
{
	alGenBuffers(1, handle);
	if(fmt==AL_FORMAT_MONO16 || fmt==AL_FORMAT_STEREO16)
	{
		unsigned short* convdata = (unsigned short*) malloc(len);
		unsigned short* pdata = (unsigned short*) data;
		memcpy(convdata, data, len);
		for(int i=0; i<len / 2; i++)
		{
			convdata[i] = pdata[i];
			BYTESWAP2(convdata[i]);
		}
		alBufferData(*handle, fmt, convdata, len, freq);
		free(convdata);
	}
	else
	{
		alBufferData(*handle, fmt, data, len, freq);
	}
}
...
int LoadWavFile(int soundNum, char * wavFileName)
{
	...
	GenerateSoundBuffer(&GameSounds[soundNum].dsBufferP, bufferPtr, len, format, freq);
...
fprintf(stderr, "OPENAL: Loaded %s\n", GameSounds[soundIndex].wavName);
	...
	if( LoadWAV( bufferPtr, &udata, &rfmt, &rfreq, &len, &seclen ) ) {
		...
		GenerateSoundBuffer(&GameSounds[soundIndex].dsBufferP, udata, len, rfmt, rfreq);
	...
	int chunklen = ((bufferPtr[4] <<  0) | (bufferPtr[5] << 8) |  (bufferPtr[6] << 16) | (bufferPtr[7] << 24));
	return bufferPtr + 8 + chunklen;
}
