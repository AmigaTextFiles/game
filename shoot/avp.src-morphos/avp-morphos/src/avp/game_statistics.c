...
void ByteSwap_AvP_GameStats_Stored(AvP_GameStats_Stored* p)
{
#if defined(__BIG_ENDIAN__)
	if(p)
	{
		for(int i=0; i<STATS_VICTIM_MAXIMUM; i++)
		{
			BYTESWAP4(p->Killed[i]);
			BYTESWAP4(p->Decapitated[i]);
			BYTESWAP4(p->TrophiesOrLiveHeadBites[i]);
			BYTESWAP4(p->DeadHeadBites[i]);
		}
		BYTESWAP4(p->ShotsFired);
		BYTESWAP4(p->Accuracy);
		BYTESWAP4(p->Spotted);
		BYTESWAP4(p->Total_ElapsedSeconds);
		BYTESWAP4(p->Total_ElapsedMinutes);
		BYTESWAP4(p->Total_ElapsedHours);
		BYTESWAP4(p->Cloak_ElapsedSeconds);
		BYTESWAP4(p->Cloak_ElapsedMinutes);
		BYTESWAP4(p->Cloak_ElapsedHours);
		BYTESWAP4(p->HealthDamage);
		BYTESWAP4(p->ArmourDamage);
		BYTESWAP4(p->Speed);
		BYTESWAP4(p->FieldChargeUsed);
		BYTESWAP4(p->HeadShotPercentage);
	}
#endif	
}