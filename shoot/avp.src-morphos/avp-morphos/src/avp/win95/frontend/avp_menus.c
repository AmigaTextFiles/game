...
extern void ApplyDeviceAndVideoModePreferences();
	...
	switch(AvP.PlayerType)
	{
		...
		case I_Marine:
		{
			if (MarineEpisodeToPlay==MAX_NO_OF_BASIC_MARINE_EPISODES-1)
			{
				...
				FlipBuffers();
				ClearScreenToBlack();
				PlayBinkedFMV("FMVs/marineoutro.bik", volume);
			}
			break;
		}
		case I_Alien:
		{
			if (AlienEpisodeToPlay==MAX_NO_OF_BASIC_ALIEN_EPISODES-1)
			{
				...
				FlipBuffers();
				ClearScreenToBlack();
				PlayBinkedFMV("FMVs/alienoutro.bik", volume);
			}
			break;
		}
		case I_Predator:
		{
			if (PredatorEpisodeToPlay==MAX_NO_OF_BASIC_PREDATOR_EPISODES-1)
			{
				...
				FlipBuffers();
				ClearScreenToBlack();
				PlayBinkedFMV("FMVs/predatoroutro.bik", volume);
			}
			break;
		}
	}
...
void HandlePreGameFMVs(void)
{
	...
	if (AvPMenus.MenusState == MENUSSTATE_STARTGAME && LoadGameRequest == SAVELOAD_REQUEST_NONE)
	{
		...
		if (!stricmp("derelict", LevelName))
		{
			...
			FlipBuffers();
			ClearScreenToBlack();
			PlayBinkedFMV("FMVs/marineintro.bik", volume);
		}
		else if (!stricmp("temple", LevelName))
		{
			...
			FlipBuffers();
			ClearScreenToBlack();
			PlayBinkedFMV("FMVs/alienintro.bik", volume);
		}
		else if (!stricmp("fall", LevelName))
		{
			...
			FlipBuffers();
			ClearScreenToBlack();
			PlayBinkedFMV("FMVs/predatorintro.bik", volume);
		}
	}
}
...
//					!(key >= KEY_F1 && key <= KEY_F12) &&
	...
	{	
		...
		{
			if( (AvPMenus.CurrentMenu == AVPMENU_MAINMENUAVOPTIONS && 
					(AvPMenus.CurrentlySelectedElement == 1 ||
					 AvPMenus.CurrentlySelectedElement == 2 ||
					 AvPMenus.CurrentlySelectedElement == 3)) ||
				(AvPMenus.CurrentMenu == AVPMENU_INGAMEAVOPTIONS &&
					(AvPMenus.CurrentlySelectedElement == 0 ||
					 AvPMenus.CurrentlySelectedElement == 1 ||
					 AvPMenus.CurrentlySelectedElement == 2 ||
					 AvPMenus.CurrentlySelectedElement == 3)))
...
//				netGameData.myGameState = NGS_Playing;
		...
		case AVPMENU_ELEMENT_VIDEOMODEOK:
		{
			if (interactionID == AVPMENU_ELEMENT_INTERACTION_SELECT)
			{
				...
				ApplyDeviceAndVideoModePreferences();
				SetupNewMenu(elementPtr->b.MenuToGoTo);
			}
			break;
		}
