...
void ByteSwapUserProfile(AVP_USER_PROFILE *profilePtr)
{
#if defined(__BIG_ENDIAN__)
	if(profilePtr)
	{
		BYTESWAP4(profilePtr->FileTime);
		for(int i=0; i<6; i++)
		{
			BYTESWAP4(profilePtr->unused[i]);
		}  
		BYTESWAP4(profilePtr->CDPlayerVolume);
		BYTESWAP4(profilePtr->SmackerSoundVolume);
		BYTESWAP4(profilePtr->EffectsSoundVolume);
		BYTESWAP4(profilePtr->MoviesAreActive);
		BYTESWAP4(profilePtr->IntroOutroMoviesAreActive);
		
		ByteSwap_MENU_DETAIL_LEVEL_OPTIONS(&(profilePtr->DetailLevelSettings));

		ByteSwap_PLAYER_INPUT_CONFIGURATION(&(profilePtr->MarineInputPrimaryConfig));
		ByteSwap_PLAYER_INPUT_CONFIGURATION(&(profilePtr->MarineInputSecondaryConfig));
		ByteSwap_PLAYER_INPUT_CONFIGURATION(&(profilePtr->AlienInputPrimaryConfig));
		ByteSwap_PLAYER_INPUT_CONFIGURATION(&(profilePtr->AlienInputSecondaryConfig));
		ByteSwap_PLAYER_INPUT_CONFIGURATION(&(profilePtr->PredatorInputPrimaryConfig));
		ByteSwap_PLAYER_INPUT_CONFIGURATION(&(profilePtr->PredatorInputSecondaryConfig));
	
		ByteSwap_CONTROL_METHODS(&(profilePtr->ControlMethods));
		ByteSwap_JOYSTICK_CONTROL_METHODS(&(profilePtr->JoystickControlMethods));
		
		for(int i=0; i<I_MaxDifficulties; i++)
			for(int j=0; j<AVP_ENVIRONMENT_END_OF_LIST; j++)
				ByteSwap_AvP_GameStats_Stored(&(profilePtr->PersonalBests[i][j]));

		BYTESWAP4(profilePtr->ext_Magic);
		BYTESWAP4(profilePtr->ext_EnableMusic);
	}
#endif	
}
...
{
	char *filename = new char [strlen(USER_PROFILES_PATH)+strlen(profilePtr->Name)+strlen(USER_PROFILES_SUFFIX)+16];
	...
	AVP_USER_PROFILE* tempProfile = new AVP_USER_PROFILE;
	*tempProfile = *profilePtr;
	
	ByteSwapUserProfile(tempProfile);
	
	fwrite(tempProfile,sizeof(AVP_USER_PROFILE),1,file);
...
static int LoadUserProfiles(void)
{
	...
	while ((gdf = ScanGameDirectory(gd)) != NULL)
	{
		...
		int sizeread = fread(profilePtr, 1, sizeof(AVP_USER_PROFILE), rif_file);
		
		if (sizeread != sizeof(AVP_USER_PROFILE))
		{
	       	fclose(rif_file);
			delete[] pszFullPath;
			delete profilePtr;
		...
		ByteSwapUserProfile(profilePtr);
...
extern void GetSettingsFromUserProfile(void)
{
	...
	if(UserProfilePtr->ext_Magic == 0xDEADFACE)
	{
		MusicIsEnabled = UserProfilePtr->ext_EnableMusic;
	}
}
...
extern void SaveSettingsToUserProfile(AVP_USER_PROFILE *profilePtr)
{
	...
	profilePtr->AutoWeaponChangeDisabled=   !AutoWeaponChangeOn;
   	...
   	profilePtr->ext_Magic = 					0xDEADFACE;
   	profilePtr->ext_EnableMusic = 				MusicIsEnabled;
