...
#define SWAP4(x) x = (((x&0x000000FF)<<24) | ((x&0x0000FF00)<<8) | ((x&0x00FF0000)>>8) | ((x&0xFF000000)>>24))
...
FFError FFHeaderI::Read(char const *_filename)
{
	...
	for (unsigned int i=0; i<num_files; ++i)
	{
		...
		size_t offs = *(size_t *)((size_t)headerP + 0);
		SWAP4(offs);

		size_t leng = *(size_t *)((size_t)headerP + 4);
		SWAP4(leng);

		void * dataP = (void *)((size_t)data + offs);
