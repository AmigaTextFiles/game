...
:Chunk(parent,"AVPPATH2")
{
	...
	for(int i=0;i<PathLength;i++)
	{
		...
		Path[i].location.ByteSwap();
		BYTESWAP4(Path[i].module_index);
		BYTESWAP4(Path[i].flags);
		BYTESWAP4(Path[i].spare2);
...
{
chunk_implement("AVP_Path_Chunk::fill_data_block");
/*
