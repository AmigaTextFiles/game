...
void create_strategies_from_list ()
{
	for(LIF<BehaviourBlockData*> blif(&Behav_List);!blif.done();blif.next())
	{
		...
		ObjectID* idPtr = (ObjectID*)&sbPtr->SBname[0];
		*idPtr=bbd->id;
...
/*
	amiga temp debugging...
	
	char tempID[8];
	memcpy((char*)tempID, &bbd->id, 8);
	if(tempID[7]==0)
	{
		BYTESWAP4(tempID[0]);
		BYTESWAP4(tempID[4]);
		memcpy((char*)&bbd->id, (char*)tempID, 8);
	}
*/	
...
// 	printf("# Adding to behavior [%02d] (%s) [%s]\n", sb_type, bbd->id.GetDebugString(), name);
