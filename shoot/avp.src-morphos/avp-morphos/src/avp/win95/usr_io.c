...
PLAYER_INPUT_CONFIGURATION DefaultMarineInputPrimaryConfig =
{
    ...
    {KEY_MMOUSE},		// FlashbackWeapon;

    {KEY_Q},		  	// ImageIntensifier;
    {KEY_F}, 		   	// ThrowFlare;
    {KEY_LEFTALT},  	// Jetpack;
    {KEY_SEMICOLON},	// Taunt
    {KEY_F1},			// Message history
    {KEY_T},			// Say
    {KEY_Y},			// Species say
    {KEY_TAB}			// scores
};
...
void SaveDefaultPrimaryConfigs(void)
{
	...
	PLAYER_INPUT_CONFIGURATION tempCfgMarine 	= DefaultMarineInputPrimaryConfig;
	PLAYER_INPUT_CONFIGURATION tempCfgPredator	= DefaultPredatorInputPrimaryConfig;
	PLAYER_INPUT_CONFIGURATION tempCfgAlien		= DefaultAlienInputPrimaryConfig;

	ByteSwap_PLAYER_INPUT_CONFIGURATION(&tempCfgMarine);
	ByteSwap_PLAYER_INPUT_CONFIGURATION(&tempCfgPredator);
	ByteSwap_PLAYER_INPUT_CONFIGURATION(&tempCfgAlien);

	fwrite(&tempCfgMarine,sizeof(PLAYER_INPUT_CONFIGURATION),1,file);
	fwrite(&tempCfgPredator,sizeof(PLAYER_INPUT_CONFIGURATION),1,file);
	fwrite(&tempCfgAlien,sizeof(PLAYER_INPUT_CONFIGURATION),1,file);
...
void LoadDefaultPrimaryConfigs(void)
{
	...
	ByteSwap_PLAYER_INPUT_CONFIGURATION(&DefaultMarineInputPrimaryConfig);
	ByteSwap_PLAYER_INPUT_CONFIGURATION(&DefaultPredatorInputPrimaryConfig);
	ByteSwap_PLAYER_INPUT_CONFIGURATION(&DefaultAlienInputPrimaryConfig);
...
void ByteSwap_PLAYER_INPUT_CONFIGURATION(PLAYER_INPUT_CONFIGURATION* p)
{
#if defined(__BIG_ENDIAN__)
#endif
}

void ByteSwap_CONTROL_METHODS(CONTROL_METHODS* p)
{
#if defined(__BIG_ENDIAN__)
	if(p)
	{
		BYTESWAP4(p->MouseXSensitivity);
		BYTESWAP4(p->MouseYSensitivity);
		BYTESWAP4(p->VAxisIsMovement);
		BYTESWAP4(p->HAxisIsTurning);
		BYTESWAP4(p->FlipVerticalAxis);
		BYTESWAP4(p->AutoCentreOnMovement);
	}
#endif	
}

void ByteSwap_JOYSTICK_CONTROL_METHODS(JOYSTICK_CONTROL_METHODS* p)
{
#if defined(__BIG_ENDIAN__)
	if(p)
	{
		BYTESWAP4(p->JoystickEnabled);
		BYTESWAP4(p->JoystickVAxisIsMovement);
		BYTESWAP4(p->JoystickHAxisIsTurning);
		BYTESWAP4(p->JoystickFlipVerticalAxis);
		BYTESWAP4(p->JoystickPOVVAxisIsMovement);
		BYTESWAP4(p->JoystickPOVHAxisIsTurning);
		BYTESWAP4(p->JoystickPOVFlipVerticalAxis);
		BYTESWAP4(p->JoystickRudderEnabled);		  
		BYTESWAP4(p->JoystickRudderAxisIsTurning);
		BYTESWAP4(p->JoystickTrackerBallEnabled);
		BYTESWAP4(p->JoystickTrackerBallFlipVerticalAxis);
		BYTESWAP4(p->JoystickTrackerBallHorizontalSensitivity);
		BYTESWAP4(p->JoystickTrackerBallVerticalSensitivity);
	}
#endif	
}
