...
{
chunk_implement("AVP_Generator_Chunk::fill_data_block");
/*
...
{
chunk_implement("AVP_Generator_Extra_Name_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
:Chunk(parent,"GENEXSET")
{
	...
	BYTESWAP4(GenerationRate);
	
	CHUNK_EXTRACT(GenRateIncrease,int)
	BYTESWAP4(GenRateIncrease);
...
{
chunk_implement("AVP_Generator_Extended_Settings_Chunk::fill_data_block");
/*
	CHUNK_FILL_START
...
:Chunk(parent,"GLOGENDC")
{
	...
	BYTESWAP4(EnemyGenerated);
	data+=sizeof(int);
	
	MaxNPCSOnLevel=*(int*)data;
	BYTESWAP4(MaxNPCSOnLevel);
	data+=sizeof(int);
	
	NPCSPerMinute=*(int*)data;
	BYTESWAP4(NPCSPerMinute);
	data+=sizeof(int);
	
	NPCAcceleration=*(int*)data;
	BYTESWAP4(NPCAcceleration);
	data+=sizeof(int);
	
	HiveStateChangeTime=*(int*)data;
	BYTESWAP4(HiveStateChangeTime);
	data+=sizeof(int);
...
{
chunk_implement("Global_Generator_Data_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
chunk_implement("AVP_Player_Start_Chunk::fill_data_block");
/*
	CHUNK_FILL_START
...
:Chunk (parent,"AVPCABLE")
{
	...
	max_charge=*(int*)data;
	BYTESWAP4(max_charge);
	data+=4;

	initial_charge=*(int*)data;
	BYTESWAP4(initial_charge);
	data+=4;

	recharge_rate=*(int*)data;
	BYTESWAP4(recharge_rate);
	data+=4;
...
{
chunk_implement("AVP_Power_Cable_Chunk::fill_data_block");
/*
...
:Chunk(parent,"AVPENVIR")
{
	...
	for(int i=0; i<sizeof(AVP_Environment_Settings)/4; i++)
	{
		BYTESWAP4(settingsPtr[i]);
	}
...
{
chunk_implement("AVP_Environment_Settings_Chunk::fill_data_block");
/*
	strncpy (data_start, identifier, 8);
	data_start += 8;
	*((int *) data_start) = chunk_size;
...
{
	num_decals=*(int*)data;
	BYTESWAP4(num_decals);
	data+=4;

	int loaded_decal_size=*(int*)data;
	BYTESWAP4(loaded_decal_size);
	data+=4;
...
{
chunk_implement("AVP_Decal_Chunk::fill_data_block");
/*
	strncpy (data, identifier, 8);
	data += 8;
	*((int *) data) = chunk_size;
...
{
chunk_implement("AVP_Particle_Generator_Data_Chunk::fill_data_block");
/*
	CHUNK_FILL_START
	CHUNK_FILL(type,int)
...
{
chunk_implement("Rainbow6_Generator_Extra_Data_Chunk::fill_data_block");
/*
	strncpy (data, identifier, 8);
	data += 8;
	*(int *) data = chunk_size;
