...
{
chunk_implement("Placed_Hierarchy_Data_Chunk::fill_data_block");
/*
...
:Chunk(parent,"PLHISEQU")
{
	...
	if(sound_list_size)
	{
		...
		for(int i=0;i<sound_list_size;i++)
		{
			...
			BYTESWAP4(sound_list[i].sound_index);
			BYTESWAP4(sound_list[i].start_time);
			BYTESWAP4(sound_list[i].end_time);
...
{
chunk_implement("Placed_Hierarchy_Sequence_Chunk::fill_data_block");
/*
	strncpy (data, identifier, 8);
	data += 8;
	*((int *) data) = chunk_size;
