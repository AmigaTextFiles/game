...
void SaveGame()
{
	...
	int magic = 0xBEEFFACE;
	fwrite(&magic, 4, 1, file);
	fwrite(SaveInfo.BufferStart+headerLength, SaveInfo.BufferSpaceUsed-headerLength, 1, file);
	fclose(file);
...
void LoadSavedGame()
{
	...
	if (!terminal_error)
	{
		if(*((int*)LoadInfo.BufferPos) == 0xBEEFFACE)
		{
			LoadInfo.BufferSpaceLeft -= 4;
			LoadInfo.BufferPos += 4;
			LoadInfo.BufferStart = LoadInfo.BufferPos;
		}
		else
		{
			terminal_error = TRUE;
