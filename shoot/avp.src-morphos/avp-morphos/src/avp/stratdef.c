...
{	
	...
	/*
	char swappedName[8];
	memcpy((char*)swappedName, id_name, 8);
	BYTESWAP4(swappedName[0]);
	BYTESWAP4(swappedName[4]);
	*/
	//------------------------------
			...
			if(sbptr->SBname)
				{
					...
					// amiga debug:
					/*
					if(NAME_ISEQUAL(sbptr->SBname, (char*)swappedName))
					{
						printf("---- Mathing with swapped name\n");
						return((sbptr));
					}
					*/
					//-----------------------------------------------------
