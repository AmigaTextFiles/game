...
void ByteSwap_MENU_DETAIL_LEVEL_OPTIONS(MENU_DETAIL_LEVEL_OPTIONS* p)
{
#if defined(__BIG_ENDIAN__)
	if(p)
	{
		BYTESWAP4(p->DecalNumber);
		BYTESWAP4(p->LightCoronas);
		BYTESWAP4(p->DecalsOnCharacters);
		BYTESWAP4(p->DeformableExplosions);
		BYTESWAP4(p->CharacterComplexity);
		BYTESWAP4(p->ParticleComplexity);
	}
#endif	
}
