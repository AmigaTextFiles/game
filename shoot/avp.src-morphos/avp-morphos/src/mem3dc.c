...
void* avpAllocMem(size_t __size)
{
	AllocMem(size, MEMF_ANY);
}

void avpDeallocMem(void* __ptr)
{
	FreeMem(__ptr, MEMF_ANY);
}
...
{
  #ifdef OVERRUN_SIZE
  void *ptr = (void *)AvpAllocMem((size_t) size + OVERRUN_SIZE(size));
  #else
  void *ptr = (void *)AvpAllocMem((size_t) size);
  #endif
  if(ptr==NULL) 
  {
...
#endif
{
  if(AdjustMallocRecord((long)ptr,FREEING_MEMORY, string, lineno))
          AvpDeallocMem((void *)ptr);      /* free previously malloced ptr */
  return;
}
