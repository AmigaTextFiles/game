...
//#define AHI_TASK_PRIORITY	10
#define AHI_TASK_PRIORITY	0


static char *ahi_device;
struct SignalSemaphore ahi_task_sema;
...
static int AHIProc(void)
{
	...
	data->mp->mp_SigBit = -1;
	ReleaseSemaphore(&ahi_task_sema);
	return 0;
}
...
static ALCboolean ahi_open_playback(ALCdevice *device, const ALCchar *deviceName)
{
	...
	if (OpenDevice((CONST_STRPTR)AHINAME, 0, (struct IORequest*)data->io, 0))	{
	...
	data->buf = AllocVec(data->bsize, MEMF_ANY);
	data->buf2 = AllocVec(data->bsize, MEMF_ANY);
	if (data->buf == NULL || data->buf2 == NULL) {
		return ALC_FALSE;
	}
	
	InitSemaphore(&ahi_task_sema);
