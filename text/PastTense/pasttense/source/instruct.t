...
instructionsVerb: Soloverb
    verb = 'instructions'
    soloaction(actor) =
    {
	"The instructions are long.  If you want to capture this
	information to a file (so that you can print it out, for
	example), answer NO below, then use the SCRIPT command (just
	type SCRIPT, then answer the question asking you for a
	filename), then enter the INSTRUCTIONS command again.";

	for (;;)
	{
	    local resp;

	    "\bDo you want to see the instructions now? (Y is affirmative) >";
	    resp := yorn();
	    if (resp = 1) break;
	    if (resp = 0) return;
	}
	...
	/*
	 *   Note - the instructions below are almost generic, so they
	 *   could easily be lifted and used in almost any other game.
	 *   The only game-specific information is in the SAVING AND
	 *   RESTORING section, where the text mentions the game's
	 *   name specifically, and describes some of the game's
	 *   design in general.
	 */
...
TRAVEL
\b
At any time during the game, you are in a location.  The game describes your
...
you can abbreviate to X).  You can PUT an object IN, ON, UNDER, or
BEHIND another object when appropriate.  The game recognizes many
other verbs as well.  We tried to make all of the verbs obvious; if
you find a knob, you will be able to TURN it, and if you find a
button, you will be able to PUSH it.  By the same token, you probably
won't need to turn the button or push the knob.
\b
...
\n\t	GET THE DUST BUNNY OUT FROM BEHIND THE COUCH
\n\t    CLIMB UP THE LADDER
\n\t    TURN THE KNOB
\n\t    JUMP
...
certain ways with these characters.  For example, you can GIVE or SHOW
things to them, and you could try to attack them.  In addition, you
can ask and tell characters about things:
\b
\t    ASK WIZARD ABOUT WAND\n
\t    TELL WIZARD ABOUT WAND
\b
You can also tell a character to do something.  To do this, type the
character's name, then a comma, then a command that you want the
character to perform.  You can type several commands for the character
all on the same line by separating the commands with periods.  For
example:
\b
...
are to finishing the game.  At certain points in the game, you will be
...
the game, and increases as you get further in the game.  Sometimes you
will lose points.  When this happens, it usually means that your
action has made the game impossible to win, and that you should
UNDO or RESTORE a saved game.";
...
they're in the descriptions simply to make the story more interesting.
For those objects that are important, the game recognizes many
synonyms; if the game doesn't understand a word you use, or any of its
common synonyms, you are probably trying something that is not
...
\b
NOTIFY:  Toggles whether or not you wish to be notifed when your score changes.
\b
TENSE:  Alternates between telling the story in the past and present tense.
\b
INDENT:  Toggles paragraph indentation.
\b
SPACE:  Toggles paragraph spacing.
\b
FOOTNOTE or NOTE: Prints the text of a footnote.  Footnotes are
numbers enclosed in square brackets; for example: [1].  Type NOTE
followed by the footnote number (e.g.: NOTE 1) to get the text for a
footnote.
\b
HINT:  Gives you a (hopefully) helpful hint.  This does not affect your
score, and you are encouraged to get hints whenever you need to.
\b
...
undo over a hundred commands.  You can also type, for example, UNDO 12
to undo the last 12 commands.  If you try to undo a lot of commands
at once, the game will ask you to confirm, since there is no way to
automatically redo undone commands.
\b
