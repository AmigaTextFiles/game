    ...
    glui32 rock)
{
    ...
    */
    ...
    sprintf(buf2, "%s%s", buf, suffix); /*AMIGA*/
    ...
    switch (usage & fileusage_TypeMask) {
        case fileusage_SavedGame:
            if (fmode != filemode_Read)
                prompt = "Save the game";
            else
                prompt = "Load a saved game";
            break;
        case fileusage_Transcript:
            if (fmode != filemode_Read)
                prompt = "Save a transcript";
            else
                prompt = "Load a saved transcript";
            break;
        case fileusage_InputRecord:
            if (fmode != filemode_Read)
                prompt = "Save an input file";
            else
                prompt = "Load a saved input file";
            break;
        case fileusage_Data:
        default:
            if (fmode != filemode_Read)
                prompt = "Save a data file";
            else
                prompt = "Load a saved data file";
            break;
    }

    if (!amiga_filereq(buf, BUFLEN,
        ASLFR_TitleText, prompt,
        ASLFR_DoSaveMode, (fmode != filemode_Read),
