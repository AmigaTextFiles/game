...
void gli_initialize_windows()
{
    ...
    for (ix=0; ix<style_NUMSTYLES; ix++) {
        ...
        if (ix == style_Emphasized || ix == style_Note)
            val |= A_UNDERLINE;
        /*AMIGA if (ix == style_Alert)
            val |= A_REVERSE;*/
        if (ix == style_Header || ix == style_Subheader/*AMIGA || ix == style_Input*/)
            val |= A_BOLD;
    ...
    compute_content_box();
        ...
        just_resumed = FALSE;
        just_killed = FALSE;
        signal(SIGCONT, gli_sig_resume); /*AMIGA*/
        signal(SIGHUP, gli_sig_interrupt); /*AMIGA*/
        signal(SIGINT, gli_sig_interrupt); /*AMIGA*/

#ifdef OPT_WINCHANGED_SIGNAL
            screen_size_changed = FALSE;
            signal(SIGWINCH, gli_sig_winsize); /*AMIGA*/
#endif /* OPT_WINCHANGED_SIGNAL */
...
/*AMIGA static*/ void gli_sig_winsize(int val)
{
    endwin();
