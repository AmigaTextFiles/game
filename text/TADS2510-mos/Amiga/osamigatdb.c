/*
 * OS-dependent module for Amiga debugger.
 *
 * David Kinder
 */
...
int InputRow,InputCol,InputDRow,InputDCol;
...
struct Window *Window, *DbgWnd, *Current;
...
void amiga_init(void)
{
	...
	SetMenuStrip(DbgWnd,Menus);

	SetDrMd(Window->RPort,JAM2);
	SetDrMd(DbgWnd->RPort,JAM2);
...
static int special = 0;
	...
	amiga_flush();
	if (Current == DbgWnd)
		amiga_move(InputDCol,InputDRow);
	else
	...
	amiga_cursor();
	port_signals = PORTSIG(Current->UserPort);
	while(1)
	{
		while(imsg = (struct IntuiMessage *)GetMsg(Current->UserPort))
		{
			class = imsg->Class;
			code = imsg->Code;
...
static struct EasyStruct requester =
	{ sizeof(struct EasyStruct),0,"TADS",0,0 };
...
void amiga_move(int x,int y)
{
	amiga_flush();
	Move(RastPort,
		Window->BorderLeft+(x*RastPort->TxWidth),
...
{
	*maxline = G_oss_screen_height-1;
	*maxcol = G_oss_screen_width-1;
}
...
int old;
	...
	amiga_flush();
	old = (Current == DbgWnd) ? 1 : 0;

	WindowToBack(pg != 0 ? Window : DbgWnd);
	Current = pg == 1 ? DbgWnd : Window;
	RastPort = Current->RPort;
...
va_list argptr;
	...
	for (p = buf; *p; )
	{
		...
		if ((win->oswflg & OSWFMORE) && win->oswx == win->oswx1 && win->oswmore+1 >= win->oswy2 - win->oswy1)
		{
		char c;
	  ...
	  case ' ':
		...
		int c = *p1;
			...
			*p1 = '\0';
			...
			if (win->oswx + strlen(p) > win->oswx2 && (win->oswflg & OSWFCLIP))
	...
	do
	{
	  ossdsp(win->oswy, win->oswx, win->oswcolor, " ");
	  ++(win->oswx);
	  if (win->oswx > win->oswx2 && (win->oswflg & OSWFCLIP)) break;
	}
	while ((win->oswx - 2) & 7);
			}
			p = p1 + 1;
...
void PropFontOn(void)
{
}

void PropFontOff(void)
{
}
...
{
	int err;
	extern int tddmain(int argc, char **argv, appctxdef *appctx, char *save_ext);
