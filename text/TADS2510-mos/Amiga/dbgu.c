...
#define DBGUHLIN '-'                                    /* horizontal line */
#define DBGARROW '>'                               /* right-pointing arrow */
#define DBGBPCHAR '*'                    /* breakpoint indicator character */
...
void dbguini(dbgcxdef *ctx, const char *game_filename)
{
		...
		ossvpg(1);
		ossclr(0, 0, maxline, maxcol, COLOUR_NORMAL);

		dbgcurcolor = COLOUR_BOLD;
		dbgbpcolor = COLOUR_REVERSE;
		watch_color = COLOUR_NORMAL;
		ldesc_color = COLOUR_REVERSE;

		/* initialize windows */
		divline = maxline * 2 / 3;
		osdbgwop(&dbgsrcwin, 0, 0, maxcol, divline-1, COLOUR_NORMAL);
