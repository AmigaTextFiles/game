...
/*AMIGA*/
    ...
    switch (hint) {
        case stylehint_Weight:
            if (val)
                styleattrs[styl] |= A_BOLD;
            else
                styleattrs[styl] &= ~A_BOLD;
            break;
        case stylehint_Oblique:
            if (val)
                styleattrs[styl] |= A_UNDERLINE;
            else
                styleattrs[styl] &= ~A_UNDERLINE;
            break;
        case stylehint_ReverseColor:
            if (val)
                styleattrs[styl] |= A_REVERSE;
            else
                styleattrs[styl] &= ~A_REVERSE;
            break;
    }
...
/*AMIGA*/
    ...
    switch (hint) {
        case stylehint_Weight:
            if (styl == style_Header || styl == style_Subheader)
                styleattrs[styl] |= A_BOLD;
            else
                styleattrs[styl] &= ~A_BOLD;
            break;
        case stylehint_Oblique:
            if (styl == style_Emphasized || styl == style_Note)
                styleattrs[styl] |= A_UNDERLINE;
            else
                styleattrs[styl] &= ~A_UNDERLINE;
            break;
    ...
    switch (hint) {
        ...
        case stylehint_Oblique:
            *result = ((styleattrs[styl] & A_UNDERLINE) != 0);
            return TRUE;
        case stylehint_ReverseColor: /*AMIGA*/
            *result = ((styleattrs[styl] & A_REVERSE) != 0); /*AMIGA*/
            return TRUE; /*AMIGA*/
