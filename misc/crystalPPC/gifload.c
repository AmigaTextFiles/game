/*
 * gifload.c: based strongly on gifload.c from wt which
 * in turn was based on gif2ras.c.
 */
...
//vonmir
extern long myfread(char *fname,char *ptr,long filesize);
//eingefügt
...
{
     Graphic_file *gf = (Graphic_file*)malloc(sizeof(Graphic_file));
...
//    fatal_error("GIF data read failed");
   if(myfread(fname,ptr,filesize)!=0) fatal_error("GIF data read failed");
//
...
/* Read in global colormap. */
  ...
  if (HasColormap) {
    if (Verbose)
      fprintf(stderr, "%s is %dx%d, %d bits/pixel, (%d colors).\n",
...
/* Now read in values from the image descriptor */
  ...
  gfile->palette = (RGBcolor*)malloc(sizeof(RGBcolor) * ColorMapSize);
  for (i = 0; i < ColorMapSize; i++) {
       gfile->palette[i].red = Red[i];
       gfile->palette[i].green = Green[i];
       gfile->palette[i].blue = Blue[i];
  }
  gfile->bitmap = (unsigned char*)malloc(Width * Height);
  gfile->type = gfPaletted;
  gfile->width = Width;
  gfile->height = Height;
  gfile->transparent_entry = transparency;
  gfile->palette_entries = ColorMapSize;
