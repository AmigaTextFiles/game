...
//// amiga raw keys http://whdload.de/docs/en/rawkey.html
u16 system_alt_map[] = {32, 53, 51, 34, 18, 35, 36, 37, 23, 38, 39, 40, 55,
            54, 24, 25, 16, 19, 33, 20, 22, 52, 17, 50, 21, 49};
...
AGI_EVENT *key_parse(SDL_keysym *keysym)
{
    ...
    else
    {
        ...
        // printf("event.c: keysym->sym: %c  - SDLK_a: %c\n", keysym->sym , SDLK_a);
        // printf("event.c: return value: %d\n", (system_alt_map[keysym->sym - SDLK_a] << 8));
        // printf("event.c: system_keymap(keysym): %d\n", system_keymap(keysym));
    }
    return agi_event;
}
