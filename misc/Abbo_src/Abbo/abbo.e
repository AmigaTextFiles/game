...
DEF step,steplen,
	...
	cannonS,cannonSlen,
	showS,showSlen
DEF abbo[2]:ARRAY OF LONG,klucze,diamenty=4,
	teleport[2]:ARRAY OF LONG,
	kratka1[3]:ARRAY OF LONG,
...
DEF win=NIL:PTR TO window,
	winb=NIL:PTR TO window
DEF	picTlo=NIL:PTR TO super_picture,
...
setup()
step,steplen:=loadRaw('PROGDIR:sfx/step')
diam,diamlen:=loadRaw('PROGDIR:sfx/diamond')
door,doorlen:=loadRaw('PROGDIR:sfx/door')
keyF,keyFlen:=loadRaw('PROGDIR:sfx/key')
box,boxlen:=loadRaw('PROGDIR:sfx/box')
portalS,portalSlen:=loadRaw('PROGDIR:sfx/teleport')
cannonS,cannonSlen:=loadRaw('PROGDIR:sfx/cannon')
showS,showSlen:=loadRaw('PROGDIR:sfx/show')
...
picTlo.clear()
picTlo.dtload('PROGDIR:gfx/bkg1.iff')
settext(gui,textgad,'||                    ')
picTlo.paltoscr(scr)
picWall.load('PROGDIR:gfx/blok1.iff')
settext(gui,textgad,'||                    ')
		picAbboD1.load('PROGDIR:gfx/abbo/dol1',FALSE,FALSE)
settext(gui,textgad,'|||                   ')
		picAbboD2.load('PROGDIR:gfx/abbo/dol2',FALSE,FALSE)
settext(gui,textgad,'||||                  ')
		picAbboP.load('PROGDIR:gfx/abbo/prawo',FALSE,FALSE)
settext(gui,textgad,'|||||                 ')
		picAbboL.load('PROGDIR:gfx/abbo/lewo',FALSE,FALSE)
settext(gui,textgad,'||||||                ')
		picAbboG.load('PROGDIR:gfx/abbo/gora',FALSE,FALSE)
settext(gui,textgad,'|||||||               ')
picKra.load('PROGDIR:gfx/blok3.iff',FALSE,FALSE)
settext(gui,textgad,'||||||||              ')
picTele.load('PROGDIR:gfx/blok5.iff',FALSE,FALSE)
settext(gui,textgad,'|||||||||             ')
picMag.load('PROGDIR:gfx/blok2.iff',FALSE,FALSE)
settext(gui,textgad,'||||||||||            ')
picKrak.load('PROGDIR:gfx/blok4.iff',FALSE,FALSE)
settext(gui,textgad,'|||||||||||           ')
picPort.load('PROGDIR:gfx/blok6.iff',FALSE,FALSE)
settext(gui,textgad,'||||||||||||          ')
picChm.load('PROGDIR:gfx/blok7.iff',FALSE,FALSE)
settext(gui,textgad,'|||||||||||||         ')
picDzP.load('PROGDIR:gfx/cannonP.iff',FALSE,FALSE)
settext(gui,textgad,'||||||||||||||        ')
picDzL.load('PROGDIR:gfx/cannonL.iff',FALSE,FALSE)
settext(gui,textgad,'|||||||||||||||       ')
picF1.load('PROGDIR:gfx/cannonF1.iff',FALSE,FALSE)
settext(gui,textgad,'||||||||||||||||      ')
picF2.load('PROGDIR:gfx/cannonF2.iff',FALSE,FALSE)
settext(gui,textgad,'|||||||||||||||||     ')
picM1.load('PROGDIR:gfx/monst1.iff',FALSE,FALSE)
settext(gui,textgad,'||||||||||||||||||    ')
picM2.load('PROGDIR:gfx/monst2.iff',FALSE,FALSE)
settext(gui,textgad,'|||||||||||||||||||   ')
picKey.load('PROGDIR:gfx/blok8.iff',FALSE,FALSE)
settext(gui,textgad,'||||||||||||||||||||  ')
picDoor.load('PROGDIR:gfx/blok9.iff',FALSE,FALSE)
settext(gui,textgad,'||||||||||||||||||||| ')
picDia.load('PROGDIR:gfx/blok10.iff',FALSE,FALSE)
settext(gui,textgad,'||||||||||||||||||||||')
picEkr.load('PROGDIR:gfx/ekran.iff',FALSE,FALSE)

cleangui(gui)

winb:=OpenWindowTagList(0,[WA_WIDTH,640,WA_INNERHEIGHT,469,
								WA_CUSTOMSCREEN,scr,
								WA_BORDERLESS,TRUE,
								WA_TOP,11,
								WA_BACKDROP,TRUE,
                                WA_SCREENTITLE,'Game paused !',0])
        BltBitMapRastPort(picEkr.bitmap(),0,0,winb.rport,
                                        0,0,
                                        640,469,
                                        $c0)
win:=OpenWindowTagList(0,[WA_WIDTH,600,WA_INNERHEIGHT,360,
								...
								WA_TOP,20,
								WA_BACKDROP,TRUE,
                                WA_SCREENTITLE,'Abbo by Jakub Madej        UNREGISTERED',0])
...
PROC mainproc()
  ...
  LOOP
		...
		IF ((key<>77) AND (key<>76) AND (key<>78) AND (key<>79) AND (key<>16)) THEN	key:=GetKey()
		IF key=16
			level:=0
			start()
		ENDIF
...
PROC select_screen()
 ...
 fr:=AllocAslRequest(ASL_SCREENMODEREQUEST,[ASLSM_TITLETEXT,'Select 640x480x8 ScreenMode:',
											NIL])
 IF AslRequest(fr,0)
   scr:=OpenScreenTagList(NIL, [SA_DISPLAYID, fr.displayid,
				...
				IF class=IDCMP_INTUITICKS
					...
					ENDFOR
						...
						->ENDIF
							...
							ENDIF
	                        IF key=76
								IF ((abbo[0]=23) AND (abbo[1]=15)) THEN SetWindowTitles(win,0,'Abbo by Jakub Madej        UNREGISTERED')
								IF ((abbo[0]=6) AND (abbo[1]=15)) THEN SetWindowTitles(win,0,'Abbo by Jakub Madej        UNREGISTERED')
								IF ((abbo[0]=38) AND (abbo[1]=15)) THEN SetWindowTitles(win,0,'Abbo by Jakub Madej        UNREGISTERED')
								gora()
							ENDIF
							IF key=79
								lewo()
								...
								stopChannels(CHAN_LEFT1+CHAN_RIGHT1+CHAN_LEFT2+CHAN_RIGHT2)
								NEW rt.reqtooller()
								rt.setattrs([RT_TITLE, 'Abbo request',
								 RT_WIN,winb,
    	    			         NIL, NIL])

								liczba:=rt.req(RTREQ_NUMBER, [RT_MINVAL, 1, RT_MAXVAL, 50, RT_DEFVAL, 1,RT_TEXT,'Select start level:',
									 RT_GADS,'Cancel',
                                     NIL, NIL])
								IF liczba<>0
									level:=rt.get(RT_NUM)
							...
							IF key=78
								...
								IF rt.req(RTREQ_EASY, [RT_TEXT,'Do you really want to quit Abbo ?',
                                     RT_GADS,    'Not today|Sure',
                                     NIL, NIL])=0 THEN quit()
								END rt
								abboBlt(4)
								ENDIF
...
PROC loadlevel()
 ...
 IF level=0 
  levels:='PROGDIR:levels/main'
 ENDIF
 IF level=1
  levels:='PROGDIR:levels/level1'
  IF oldlevel>9
   picTlo.clear()
   picTlo.dtload('PROGDIR:gfx/bkg1.iff')
  ENDIF
 ...
 ENDIF
 IF level=10
  levels:='PROGDIR:levels/level10'
  IF ((oldlevel>19) OR (oldlevel<10))
   picTlo.clear()
   picTlo.dtload('PROGDIR:gfx/bkg2.iff')
  ENDIF
 ENDIF
 IF level=11
  levels:='PROGDIR:levels/level11'
  IF ((oldlevel>19) OR (oldlevel<10))
   picTlo.clear()
   picTlo.dtload('PROGDIR:gfx/bkg2.iff')
  ENDIF
 ...
 ENDIF
 IF level=20
  levels:='PROGDIR:levels/level20'
  IF ((oldlevel>29) OR (oldlevel<20))
   picTlo.clear()
   picTlo.dtload('PROGDIR:gfx/bkg3.iff')
  ENDIF
 ENDIF
 IF level=21
  levels:='PROGDIR:levels/level21'
  IF ((oldlevel>29) OR (oldlevel<20))
   picTlo.clear()
   picTlo.dtload('PROGDIR:gfx/bkg3.iff')
  ENDIF
 ...
 ENDIF
 IF level=30
  levels:='PROGDIR:levels/level30'
  IF ((oldlevel>39) OR (oldlevel<30))
   picTlo.clear()
   picTlo.dtload('PROGDIR:gfx/bkg4.iff')
  ENDIF
 ENDIF
 IF level=31
  levels:='PROGDIR:levels/level31'
  IF ((oldlevel>39) OR (oldlevel<30))
   picTlo.clear()
   picTlo.dtload('PROGDIR:gfx/bkg4.iff')
  ENDIF
 ...
 ENDIF
 IF level=40
  levels:='PROGDIR:levels/level40'
  IF oldlevel<40
   picTlo.clear()
   picTlo.dtload('PROGDIR:gfx/bkg5.iff')
  ENDIF
 ENDIF
 IF level=41
  levels:='PROGDIR:levels/level41'
  IF oldlevel<40
   picTlo.clear()
   picTlo.dtload('PROGDIR:gfx/bkg5.iff')
  ENDIF
 ...
 IF ((level>0) AND (level<51))
  ...
  RETURN
 ENDIF
 SetWindowTitles(win,0,'Abbo by Jakub Madej        UNREGISTERED')
ENDPROC
