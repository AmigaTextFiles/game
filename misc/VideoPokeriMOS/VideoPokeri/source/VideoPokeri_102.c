...
if ( ( (id==12) || (id==51) ) && (pelitilanne==1) )
    ...
    if (voitot>tulos[14])                       /* HIGHSCORE-LISTA   */
        ...
        pelitilanne = 7;
        GadgetitKondixeen();
        ShadowText(RP, 22, 47, 5, 4, (STRPTR)SekaTextit[KIELI]);
        i=0;
        ...
        ShadowText(RP, 12, 215, 9, 4, (STRPTR)SekaTextit[3+KIELI]);
        ...
        if (tyhjennys!=4)
            {
            ShadowText(RP, 22, 47, 28, 26, (STRPTR)SekaTextit[6+KIELI]);
            for (i=0; i<15; i++)
                {
                sprintf(stringi, "%s%d.  %s", (i<9) ? " " : "", i+1, lista[i]);
                ShadowText(RP, 22, (SHORT)(63+i*9), 16, 5, stringi);
...
if ( (id==15 || id==14) && pelitilanne == 6 )       /* PIENI / SUURI */
    ...
    if ( ( (id==15) && kortit[67]<7 ) || ( (id==14) && kortit[67]>7 ) || kortit[67]==15 )
        ...
        RectFill(RP, 0, 199, 319, 223);         /* ALAPALKKI PUNAISEKSI */
        ColorSquare(RP, 70, 202, 36, 18, 8);        /* ALALOOTA */
        ShadowText(RP,  16, 215, 1, 4, (STRPTR)SekaTextit[15+KIELI] );
...
if (id==20)
    ...
    sprintf(lista[sijoitus-1], "%s %5d", stringi, (int)voitot);
...
ShadowText(RP, 21, 19, 1, 4, (STRPTR)Peitot[KIELI*2]);      /* PELIT */
SetAPen(RP, 9);
RectFill(RP, 198, 10, 255, 20);
ShadowText(RP, 200, 19, 1, 4, (STRPTR)Peitot[KIELI*2+1]);   /* VOITOT */
SetAPen(RP, oldapen);
}
...
for(i=panokset[nytpanos]; i<(tuplavoitto+1); i+=panokset[nytpanos])
    ...
    TextText(RP, 260, 19, 4, stringi); 
    sprintf(stringi, "%3d", tuplavoitto-i);
    TextText(RP, 77, 215, 4, stringi); 
    Sound(3, 100, 0);
...
#else
textAttr.ta_Name  = "XCourier.font";
#endif
...
SetPointer(window, (APTR)XPointer, 15, 16, -8, -9);   /* OMA POINTTERI */
...
if ( infile == 0)
    ...
    {
    mypointer = (APTR)AllocMem(51202, MEMF_ANY);
    if (!mypointer)
        {
        Close((BPTR)infile);
        cleanExit(9);
