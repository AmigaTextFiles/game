...
/* Version 3.3 (NB broken version using SDL_LoadBMP) */
...
AWSprite::AWSprite(char *imageFileName, Uint32 hmf) {
	initialize_sprite();

	// Load in the image
	SDL_Surface* temp = SDL_LoadBMP(imageFileName);
...
//  divided into
AWSprite::AWSprite(char *imageFileName, Uint32 cols, Uint32 rows) {
	initialize_sprite();
...
void AWSprite::create_frames(Uint32 cols, Uint32 rows) {
	frames = new SDL_Surface*[cols*rows];
